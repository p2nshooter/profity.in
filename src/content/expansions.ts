import type { Article, Section } from './articles';

/**
 * Additive expansions for the Profity.in bilingual library.
 *
 * Every entry here APPENDS sections to an existing article. Nothing is
 * removed, nothing is rewritten, and two articles are never merged into one
 * URL. Multiple entries may target the same slug — they apply in order, which
 * is how later top-off passes work.
 *
 * Both languages are written by hand. Hindi is the native language of the
 * site; the English is a genuine translation rather than a summary, so both
 * columns carry the same argument at the same depth.
 *
 * The editorial rules of articles.ts apply here unchanged: educational only,
 * no guaranteed returns, no stock tips, no schemes.
 */
export interface Expansion {
  slug: string;
  sections: Section[];
}

export const EXPANSIONS: Expansion[] = [
  {
    slug: 'sip-mutual-fund-guide',
    sections: [
      {
        hHi: 'SIP की राशि कैसे तय करें',
        hEn: 'How to decide the SIP amount',
        pHi: [
          'सबसे आम सवाल यह है कि हर महीने कितना डालना चाहिए, और सबसे आम ग़लती यह है कि लोग इसका उत्तर किसी सूत्र से निकालने की कोशिश करते हैं। असल में सही राशि वह है जो आप बुरे महीनों में भी बिना रोके चला सकें।',
          'इसे तय करने का एक व्यावहारिक तरीक़ा है। पिछले बारह महीनों में जो आपका सबसे तंग महीना रहा, उसमें आपके पास महीने के अंत में कितना बचा था — SIP की राशि उससे कुछ कम रखिए, ऊपर नहीं।',
          'लोग उल्टा करते हैं: सबसे अच्छे महीने को देखकर राशि तय करते हैं, फिर तीन महीने बाद उसे रोकना पड़ता है। रुकी हुई SIP का नुक़सान केवल छूटी हुई किश्तों का नहीं होता; वह आदत को भी तोड़ देती है, और आदत ही इस पूरी व्यवस्था की असली पूँजी है।',
          'यदि बाद में आय बढ़े, तो राशि बढ़ाई जा सकती है — और यह बढ़ाना रोकने से कहीं आसान है। इसलिए कम से शुरू करना कमज़ोरी नहीं, रणनीति है।',
        ],
        pEn: [
          'The most common question is how much to put in each month, and the most common mistake is trying to answer it with a formula. The right amount is really the one you can keep running through your worst months without stopping.',
          'There is a practical way to arrive at it. Look at your tightest month over the past year and see what was actually left at the end of it — set the SIP a little below that figure rather than above it.',
          'People tend to do the opposite: they size the instalment against their best month and then have to stop it three months later. A stopped SIP costs more than the missed instalments; it breaks the habit, and the habit is the real capital in this whole arrangement.',
          'If income rises later, the amount can be raised — and raising is far easier than restarting. So beginning small is a strategy rather than a weakness, whatever the round numbers in an advertisement suggest.',
        ],
      },
      {
        hHi: 'तारीख़ का चुनाव: वेतन के तुरंत बाद',
        hEn: 'Choosing the date: right after payday',
        pHi: [
          'यह छोटा-सा विवरण नतीजे पर बड़ा असर डालता है, और अधिकांश लोग इसे बिना सोचे छोड़ देते हैं। SIP की तारीख़ महीने के अंत में नहीं, वेतन आने के अगले या उससे अगले दिन होनी चाहिए।',
          'कारण मनोवैज्ञानिक है, गणितीय नहीं। महीने के अंत तक खाते में जो बचता है वह अनिश्चित होता है, और अनिश्चितता ही वह चीज़ है जिससे किश्तें फ़ेल होती हैं। वेतन के तुरंत बाद कटने वाली राशि पर कोई प्रतिस्पर्धा नहीं होती।',
          'लोग अक्सर पूछते हैं कि क्या किसी ख़ास तारीख़ पर बाज़ार सस्ता मिलता है। लंबी अवधि में इसका कोई भरोसेमंद पैटर्न नहीं है — जो अंतर आता है वह इतना छोटा है कि उसकी तुलना में किश्त का न चूकना कहीं अधिक मायने रखता है।',
        ],
        pEn: [
          'This is a small detail with a large effect on outcomes, and most people leave it to chance. The SIP date should not sit at the end of the month; it should sit on the day after payday, or the day after that.',
          'The reason is psychological rather than mathematical. Whatever is left in the account at month-end is uncertain, and uncertainty is exactly what makes instalments fail. Money that leaves immediately after salary arrives faces no competition from anything else.',
          'People often ask whether some particular date buys the market cheaper. Over long periods there is no reliable pattern in this — the difference it makes is small enough that simply never missing an instalment matters considerably more.',
        ],
      },
      {
        hHi: 'बाज़ार गिरने पर क्या करें',
        hEn: 'What to do when the market falls',
        pHi: [
          'यह वह क्षण है जिस पर पूरी SIP की सफलता टिकी होती है, और यही वह क्षण है जब सबसे अधिक लोग ग़लत क़दम उठाते हैं। जब बाज़ार गिरता है, तो किश्त रोकना सबसे स्वाभाविक प्रतिक्रिया लगती है — और वह सबसे महँगी भी है।',
          'गिरावट में आपकी वही राशि अधिक यूनिट ख़रीदती है। यही रुपया-लागत औसत का पूरा तंत्र है, और वह केवल तभी काम करता है जब आप गिरावट के दौरान ख़रीदते रहें। जो व्यक्ति गिरावट में रुक जाता है, उसने इस व्यवस्था का सबसे उपयोगी हिस्सा छोड़ दिया।',
          'यह कहना आसान है और करना कठिन, इसलिए एक व्यावहारिक सहारा उपयोगी है: गिरावट के दिनों में अपना खाता मत देखिए। किश्त स्वचालित है, उसे आपकी उपस्थिति की ज़रूरत नहीं। जो चीज़ नुक़सान करती है वह गिरावट नहीं, गिरावट देखकर लिया गया निर्णय है।',
          'और यह भी याद रखिए कि यह लेख किसी विशेष फंड की सिफ़ारिश नहीं करता और न ही किसी प्रतिफल का वादा। बाज़ार जोखिम वास्तविक है; यहाँ केवल व्यवहार की बात हो रही है।',
        ],
        pEn: [
          'This is the moment the whole SIP depends on, and it is also the moment at which most people take the wrong step. When the market falls, stopping the instalment feels like the natural response — and it is the most expensive one available.',
          'In a fall, the same amount of money buys more units. That is the entire mechanism of rupee-cost averaging, and it only works if you keep buying while prices are down. Somebody who stops during a decline has abandoned the most useful part of the arrangement.',
          'That is easy to say and hard to do, so a practical crutch helps: do not look at the account on falling days. The instalment is automatic and does not need you present. What causes damage is not the fall itself but the decision taken while watching it.',
          'And remember that none of this recommends any particular fund or promises any return. Market risk is real; what is being discussed here is only behaviour, which is the part actually within your control.',
        ],
      },
      {
        hHi: 'SIP से पैसा निकालना: एकमुश्त नहीं',
        hEn: 'Taking money out: not all at once',
        pHi: [
          'निवेश की बात बहुत होती है और निकासी की बहुत कम, जबकि नतीजा अक्सर निकासी के तरीक़े से तय होता है। जिस अनुशासन से पैसा डाला जाता है, उसी अनुशासन से निकालना भी चाहिए।',
          'यदि किसी लक्ष्य के लिए पैसा चाहिए — मान लीजिए तीन साल बाद — तो लक्ष्य से कुछ साल पहले ही धीरे-धीरे उसे कम जोखिम वाले विकल्प में ले जाना समझदारी है। लक्ष्य की तारीख़ पर बाज़ार कहाँ होगा, यह किसी को नहीं पता।',
          'इसका उल्टा भी उतना ही सच है: यदि पैसे की कोई तय ज़रूरत नहीं है, तो केवल इसलिए निकाल लेना कि "अच्छा फ़ायदा हो गया" प्रायः महँगा पड़ता है, क्योंकि उसके बाद दोबारा प्रवेश का सही समय ढूँढ़ना लगभग असंभव होता है।',
        ],
        pEn: [
          'A great deal is said about investing and very little about withdrawing, even though the outcome is often decided by how the exit is handled. The discipline that puts money in should apply equally to taking it out.',
          'If the money is meant for a goal — say three years away — it is sensible to begin shifting it gradually into lower-risk options some years before that date arrives. Nobody knows where the market will be on the particular day you need the money.',
          'The reverse is just as true: if there is no defined need for the money, withdrawing merely because "it has done well" usually turns out expensive, because finding the right moment to re-enter afterwards is very nearly impossible.',
        ],
      },
      {
        hHi: 'कर का पक्ष, संक्षेप में',
        hEn: 'The tax side, briefly',
        pHi: [
          'SIP के हर किश्त को कर की दृष्टि से एक अलग ख़रीद माना जाता है, और यही वह बात है जो अधिकांश लोगों को चौंकाती है। इसका अर्थ यह है कि हर किश्त की धारण-अवधि अलग-अलग गिनी जाती है।',
          'व्यावहारिक असर यह होता है कि यदि आप निवेश के तुरंत बाद पूरा पैसा निकालें, तो शुरुआती किश्तें लंबी अवधि में आ सकती हैं जबकि हाल की किश्तें छोटी अवधि में — और दोनों पर कर का व्यवहार अलग होता है।',
          'यहाँ दरें और सीमाएँ जानबूझकर नहीं लिखी जा रही हैं, क्योंकि ये बजट के साथ बदलती रहती हैं और पुरानी संख्या नई ग़लती बन जाती है। निकासी से पहले उस वर्ष के नियम देख लेना या किसी कर सलाहकार से पूछ लेना ही सही तरीक़ा है।',
        ],
        pEn: [
          'For tax purposes each SIP instalment is treated as a separate purchase, and this is the point that surprises most people when they first meet it. It means the holding period is counted separately for every instalment.',
          'The practical effect is that if you redeem everything shortly after investing, the earliest instalments may qualify as long-term while the most recent ones do not — and the two are treated differently.',
          'Rates and thresholds are deliberately not quoted here, because they change with each budget and an outdated number simply becomes a new mistake. Checking the rules for the relevant year, or asking a tax adviser before redeeming, is the correct approach.',
        ],
      },
    ],
  },

  {
    slug: 'chaiwale-ki-kahani-compounding',
    sections: [
      {
        hHi: 'पहले दस साल सबसे धीमे क्यों लगते हैं',
        hEn: 'Why the first ten years feel the slowest',
        pHi: [
          'इस कहानी का सबसे कठिन हिस्सा शुरुआत नहीं, बीच का दशक है। पहले कुछ वर्षों में जो रक़म जमा होती है वह लगभग पूरी तरह आपकी अपनी डाली हुई होती है — बढ़त का हिस्सा इतना छोटा रहता है कि दिखता ही नहीं।',
          'यही वह जगह है जहाँ अधिकांश लोग हार मान लेते हैं। पाँच साल बाद खाता देखकर लगता है कि इतनी मेहनत का नतीजा बहुत मामूली है, और उसी क्षण मन कहता है कि यह तरीक़ा काम नहीं कर रहा।',
          'पर गणित की बनावट ही ऐसी है। वृद्धि पहले धीमी होती है क्योंकि वह जिस राशि पर लगती है वह अभी छोटी है। जैसे-जैसे मूल राशि बड़ी होती है, वही प्रतिशत बड़ी संख्या बनाने लगता है — और यही मोड़ बीस साल के आसपास आता है।',
          'चायवाले की कहानी का असली सबक़ यही है: वह इसलिए सफल नहीं हुआ कि उसने कुछ चतुर किया, बल्कि इसलिए कि उसने उस दशक में कुछ नहीं किया जब कुछ न करना सबसे कठिन था।',
        ],
        pEn: [
          'The hardest part of this story is not the beginning but the middle decade. In the first few years almost everything in the account is money you put there yourself — the growth component stays so small that it is barely visible.',
          'This is where most people give up. Looking at the balance after five years, the result of all that effort seems trivial, and at exactly that moment the mind says the method is not working.',
          'But the mathematics is built this way. Growth is slow at first because the amount it acts on is still small. As the base grows larger, the same percentage begins to produce large numbers — and that turn usually arrives somewhere around the twenty-year mark.',
          'That is the real lesson of the chaiwala\'s story: he did not succeed because he did something clever, but because he did nothing during the decade when doing nothing was hardest.',
        ],
      },
      {
        hHi: 'वह अंतर जो आय से नहीं, आदत से बनता है',
        hEn: 'The gap that comes from habit rather than income',
        pHi: [
          'लोग यह मान लेते हैं कि ऐसी कहानियाँ केवल तभी संभव हैं जब आय अच्छी हो। असल में जो चीज़ काम करती है वह आय और ख़र्च के बीच का अंतर है, और वह अंतर किसी भी आय-स्तर पर बनाया जा सकता है।',
          'दो लोगों की तुलना कीजिए। एक की आय दूसरे से दोगुनी है पर वह जो कमाता है लगभग सब ख़र्च कर देता है। दूसरा कम कमाता है पर हर महीने एक तय हिस्सा अलग रख देता है। बीस साल बाद दूसरे के पास कुछ होता है और पहले के पास कुछ नहीं।',
          'यह कोई नैतिक शिक्षा नहीं है; यह केवल अंकगणित है। संपत्ति आय से नहीं बनती, बचे हुए हिस्से से बनती है — और बचा हुआ हिस्सा एक निर्णय है, कोई परिस्थिति नहीं।',
          'यही कारण है कि आय बढ़ने पर भी बहुत से लोगों की स्थिति नहीं बदलती। जब ख़र्च आय के साथ-साथ बढ़ता है, तो अंतर वहीं का वहीं रहता है, और अंतर ही वह इकलौती चीज़ है जो काम करती है।',
        ],
        pEn: [
          'People assume that stories like this are only possible on a good income. What actually does the work is the gap between income and spending, and that gap can be created at any level of income at all.',
          'Compare two people. One earns twice as much as the other and spends very nearly all of it. The other earns less but sets a fixed share aside every month. Twenty years later the second has something and the first has nothing.',
          'This is not a moral lesson; it is only arithmetic. Wealth is not built out of income but out of what remains — and what remains is a decision rather than a circumstance handed to you.',
          'That is why many people\'s position does not change even as their income rises. When spending grows alongside earnings, the gap stays exactly where it was, and the gap is the only thing that does any work.',
        ],
      },
      {
        hHi: 'जिन तीन चीज़ों ने इस कहानी को बचाया',
        hEn: 'The three things that protected this story',
        pHi: [
          'तीस साल लंबे किसी भी प्रयास में कुछ न कुछ ग़लत होता ही है, और यह कहानी भी अपवाद नहीं थी। जो चीज़ इसे बचाए रखी, वह तीन साधारण सुरक्षा-उपाय थे।',
          'पहला आपातकालीन कोष था। जब दुकान कुछ महीने बंद रही, तो उस कोष ने काम चलाया और निवेश को छूने की नौबत नहीं आई। यही वह बिंदु है जहाँ अधिकांश लंबी योजनाएँ टूटती हैं।',
          'दूसरा स्वास्थ्य बीमा था। एक बड़ा अस्पताल-ख़र्च वर्षों की बचत को कुछ हफ़्तों में समाप्त कर सकता है, और भारत में यही सबसे आम कारण है जिससे परिवार अपनी जमा-पूँजी खो देते हैं।',
          'तीसरा और सबसे कम चर्चित उपाय था — कोई क़र्ज़ न लेना जो आय से बड़ा हो। ऊँची ब्याज दर पर लिया गया क़र्ज़ चक्रवृद्धि को उल्टी दिशा में चलाता है, और उस दिशा में वह और भी तेज़ है।',
        ],
        pEn: [
          'Over any effort lasting thirty years something is bound to go wrong, and this story was no exception. What kept it intact were three ordinary protections rather than anything clever.',
          'The first was an emergency fund. When the stall stayed shut for a few months, that fund carried the household and the investments were never touched. This is precisely the point at which most long plans break.',
          'The second was health insurance. A single large hospital bill can erase years of saving within weeks, and in India that is the most common single reason families lose their accumulated money.',
          'The third and least discussed protection was taking on no debt larger than the income could carry. Borrowing at a high rate runs compounding in reverse, and in that direction it moves considerably faster.',
        ],
      },
      {
        hHi: 'यदि आपने देर से शुरू किया है',
        hEn: 'If you are starting late',
        pHi: [
          'ऐसी कहानियाँ पढ़कर सबसे आम प्रतिक्रिया यह होती है कि "मेरे पास तीस साल नहीं हैं"। यह सच हो सकता है, और फिर भी निष्कर्ष वह नहीं है जो लगता है।',
          'देर से शुरू करने पर तीन चीज़ें बदलती हैं। पहली, राशि बड़ी रखनी पड़ती है, क्योंकि समय कम है। दूसरी, लक्ष्य अधिक स्पष्ट होने चाहिए, क्योंकि सुधार का मौक़ा कम है। तीसरी, जोखिम का स्तर अधिक सावधानी से चुनना पड़ता है।',
          'जो नहीं बदलता वह यह है कि शुरू करना ही एकमात्र क़दम है जो काम करता है। पैंतालीस की उम्र में शुरू किया गया निवेश बीस साल में भी सार्थक हो जाता है, और साठ की उम्र में शुरू किया गया दस साल में भी कुछ न होने से बेहतर है।',
          'सबसे बुरा विकल्प यह मान लेना है कि अब देर हो चुकी है। यह मान्यता अपने आप को सच कर लेती है, और यही इस कहानी से सीखने लायक़ आख़िरी बात है।',
        ],
        pEn: [
          'The most common reaction to a story like this is "I don\'t have thirty years." That may well be true, and the conclusion that follows is not the one it appears to be.',
          'Starting late changes three things. The amount has to be larger, because there is less time. The goals need to be more specific, because there is less room to correct course. And the level of risk has to be chosen more carefully.',
          'What does not change is that starting is the only step that does anything. Money invested at forty-five still becomes meaningful across twenty years, and money invested at sixty across ten years is better than nothing at all.',
          'The worst available option is to decide that it is now too late. That belief makes itself true, and it is the last thing worth taking away from this story.',
        ],
      },
      {
        hHi: 'कहानी का वह हिस्सा जो कोई नहीं दोहराता',
        hEn: 'The part of the story nobody repeats',
        pHi: [
          'ऐसी कहानियाँ सुनाते समय अंत पर ध्यान रहता है — आज़ादी, आराम, सुरक्षा। पर जिन तीस वर्षों में यह बना, वे साधारण थे, और यही बात दोहराने लायक़ है।',
          'उन तीस वर्षों में कोई नाटकीय क्षण नहीं था। कोई सही समय पर लिया गया चतुर निर्णय नहीं, कोई ऐसा साल नहीं जिसने सब बदल दिया। केवल हर महीने वही छोटी-सी क्रिया, दोहराई हुई।',
          'यही कारण है कि यह तरीक़ा किसी को उत्साहित नहीं करता और यही कारण है कि यह काम करता है। जो चीज़ रोमांचक होती है, वह प्रायः जोखिम भरी होती है; जो उबाऊ होती है, वह प्रायः टिकाऊ।',
          'इसलिए यदि इस पूरे लेख से केवल एक वाक्य याद रखना हो, तो वह यह है: संपत्ति किसी एक अच्छे निर्णय से नहीं बनती, बल्कि एक साधारण निर्णय को बहुत बार दोहराने से बनती है।',
        ],
        pEn: [
          'When stories like this are told, attention settles on the ending — the freedom, the ease, the security. But the thirty years that built it were ordinary, and that is the part worth repeating.',
          'Across those thirty years there was no dramatic moment. No clever decision taken at exactly the right time, no single year that changed everything. Only the same small action every month, repeated.',
          'This is why the method excites nobody, and it is also why it works. What is thrilling is usually risky; what is boring is usually durable, and over three decades durability is the only quality that matters.',
          'So if only one sentence from all of this is worth keeping, it is this: wealth is not built from one good decision but from repeating an ordinary decision a very large number of times.',
        ],
      },
    ],
  },

  {
    slug: 'bacchon-ko-paise-ki-samajh',
    sections: [
      {
        hHi: 'उम्र के हिसाब से क्या सिखाया जाए',
        hEn: 'What to teach at which age',
        pHi: [
          'बच्चों को पैसे की समझ देने में सबसे आम ग़लती यह है कि सब कुछ एक साथ समझाने की कोशिश की जाती है। उम्र के हिसाब से बाँट देने पर यह काम कहीं आसान हो जाता है।',
          'पाँच से सात साल की उम्र में केवल एक चीज़ सिखाई जा सकती है और वही पर्याप्त है: चीज़ों की क़ीमत होती है, और पैसा सीमित है। इस उम्र में गुल्लक और दुकान पर ख़ुद पैसे देने का अनुभव ही पूरा पाठ है।',
          'आठ से बारह की उम्र में इंतज़ार जोड़ा जा सकता है। इस उम्र में बच्चा यह समझ सकता है कि आज की छोटी चीज़ छोड़कर कुछ हफ़्तों बाद बड़ी चीज़ ली जा सकती है — और यही वह क्षमता है जिस पर बाद का पूरा वित्तीय जीवन टिकता है।',
          'तेरह से अठारह में तीसरी परत आती है: आय, कर, और उधार का विचार। यही वह उम्र है जब बच्चे को घर के असली बजट का कुछ हिस्सा दिखाया जा सकता है, और उस पारदर्शिता का असर किसी उपदेश से कहीं अधिक होता है।',
        ],
        pEn: [
          'The most common mistake in teaching children about money is trying to explain everything at once. Splitting it by age makes the task far easier and considerably more effective.',
          'Between five and seven only one thing can be taught, and it is enough: things cost money, and money runs out. At this age a piggy bank and handing over the coins in a shop themselves is the entire lesson.',
          'Between eight and twelve, waiting can be added. A child of this age can understand that giving up something small today buys something larger in a few weeks — and that capacity is what the whole of later financial life rests on.',
          'From thirteen to eighteen a third layer arrives: income, tax, and the idea of borrowing. This is the age at which some part of the real household budget can be shown, and that transparency does more than any amount of advice.',
        ],
      },
      {
        hHi: 'जेब ख़र्च की राशि और नियम',
        hEn: 'The pocket money amount, and the rules around it',
        pHi: [
          'जेब ख़र्च देने का उद्देश्य बच्चे को ख़ुश करना नहीं, उसे अभ्यास देना है। इसलिए राशि से अधिक महत्वपूर्ण यह है कि नियम स्पष्ट और स्थिर हों।',
          'तीन नियम पर्याप्त हैं। पहला, राशि तय हो और तय दिन आए — अनियमित पैसा कोई अभ्यास नहीं देता। दूसरा, उसका उपयोग बच्चे का अपना निर्णय हो, चाहे वह निर्णय आपको बेवक़ूफ़ी लगे। तीसरा, ख़त्म हो जाने पर अगली तारीख़ से पहले और न मिले।',
          'तीसरा नियम सबसे कठिन है और सबसे ज़रूरी। जब बच्चा तीन दिन में सब ख़र्च कर देता है और बाक़ी महीना ख़ाली रहता है, तो वह वह सबक़ सीखता है जो कोई व्याख्यान नहीं दे सकता — और वह सबक़ इस उम्र में बहुत सस्ता पड़ता है।',
          'यह भी ध्यान रखिए कि जेब ख़र्च को घर के कामों से जोड़ना ठीक है या नहीं, इस पर राय बँटी हुई है। एक व्यावहारिक बीच का रास्ता यह है कि रोज़मर्रा के काम बिना पैसे के हों, और कोई अतिरिक्त, वैकल्पिक काम कमाई का अवसर बने।',
        ],
        pEn: [
          'The purpose of pocket money is not to please a child but to give them practice. So the rules around it matter more than the amount, and clarity matters more than generosity.',
          'Three rules are enough. First, the amount is fixed and arrives on a fixed day — irregular money gives no practice at all. Second, how it is used is the child\'s own decision, even when that decision looks foolish to you. Third, when it runs out, no more arrives before the next date.',
          'The third rule is the hardest and the most important. When a child spends everything in three days and faces an empty rest of the month, they learn something no lecture can deliver — and at that age the lesson is very cheap.',
          'Opinion is divided on whether pocket money should be tied to chores. A workable middle path is that ordinary daily chores carry no payment, while some additional, optional task becomes an opportunity to earn.',
        ],
      },
      {
        hHi: 'पहला बैंक खाता और डिजिटल पैसा',
        hEn: 'The first bank account and digital money',
        pHi: [
          'गुल्लक की एक बड़ी ताक़त यह है कि पैसा दिखता है। डिजिटल पैसे में यही ताक़त ग़ायब हो जाती है, और यही आज के बच्चों के लिए सबसे बड़ी नई चुनौती है।',
          'इसलिए बैंक खाते की ओर बढ़ते समय एक क़दम बीच में रखना उपयोगी है: पासबुक। जब बच्चा ख़ुद जाकर प्रविष्टि करवाता है और संख्या बढ़ती हुई देखता है, तो अदृश्य पैसा फिर से दिखने लगता है।',
          'UPI के साथ नियम और सख़्त होने चाहिए, और यहाँ बात सुरक्षा की भी है। पहले कुछ महीनों में लेन-देन की सीमा कम रखिए, हर लेन-देन साथ बैठकर कीजिए, और यह स्पष्ट कर दीजिए कि PIN किसी को भी नहीं बताया जाता — किसी को भी नहीं, चाहे वह कोई भी हो।',
          'सबसे उपयोगी अभ्यास यह है कि महीने के अंत में बच्चे के साथ बैठकर उसका अपना विवरण देखा जाए। कहाँ गया पैसा, यह सवाल बचपन में पूछना सीख लेने पर जीवन भर काम आता है।',
        ],
        pEn: [
          'One of the great strengths of a piggy bank is that the money is visible. Digital money removes that strength entirely, and it is the largest new difficulty facing children today.',
          'So when moving towards a bank account it helps to keep one step in between: the passbook. When a child goes and gets an entry made and watches the number rise, invisible money becomes visible again.',
          'With UPI the rules should be stricter, and here safety matters as well. Keep transaction limits low for the first few months, do every transaction sitting together, and make it absolutely clear that the PIN is never told to anybody — anybody at all, whoever they claim to be.',
          'The most useful habit is to sit with the child at month-end and look through their own statement. Learning to ask where the money went is a question that keeps paying for the rest of a life.',
        ],
      },
      {
        hHi: 'माता-पिता जो कहते हैं और जो करते हैं',
        hEn: 'What parents say against what they do',
        pHi: [
          'बच्चे पैसे के बारे में जो सीखते हैं, उसका अधिकांश हिस्सा बताया नहीं जाता — वह देखा जाता है। यही कारण है कि सबसे अच्छी शिक्षा भी तब बेअसर हो जाती है जब व्यवहार उसके उलट हो।',
          'यदि घर में हर तनाव के बाद ख़रीदारी होती है, तो बच्चा यही सीखता है कि पैसा भावना का इलाज है। यदि क़ीमत के बारे में कभी बात नहीं होती, तो वह सीखता है कि यह विषय गुप्त है।',
          'इसका उल्टा भी उतना ही सच है। जब बच्चा देखता है कि माता-पिता किसी बड़ी ख़रीद से पहले तुलना करते हैं, कुछ महीने रुकते हैं, और फिर निर्णय लेते हैं — तो वह पूरा तरीक़ा बिना किसी उपदेश के सीख जाता है।',
          'एक और चीज़ जो असामान्य रूप से असरदार है: बच्चों के सामने अपनी वित्तीय ग़लती स्वीकार करना। यह उन्हें बताता है कि ग़लतियाँ सामान्य हैं और सुधारी जा सकती हैं, जो शायद इस पूरे विषय का सबसे उपयोगी संदेश है।',
        ],
        pEn: [
          'Most of what children learn about money is never told to them — it is watched. That is why even the best instruction stops working when behaviour runs the other way.',
          'If every tense evening at home ends in a purchase, the child learns that money is a treatment for feelings. If prices are never discussed, they learn that the subject is a secret one.',
          'The reverse holds just as strongly. When a child sees parents compare options before a large purchase, wait a few months, and then decide — the whole method transfers without a word of advice being given.',
          'One other thing is unusually effective: admitting a financial mistake of your own in front of them. It tells them that mistakes are ordinary and can be corrected, which may be the single most useful message in this entire subject.',
        ],
      },
      {
        hHi: 'देने वाला डिब्बा: वह हिस्सा जिसे लोग छोड़ देते हैं',
        hEn: 'The giving jar: the part people leave out',
        pHi: [
          'तीन डिब्बों में से जिसे सबसे पहले छोड़ दिया जाता है वह देने वाला डिब्बा है, और यही शायद सबसे अधिक काम करने वाला है।',
          'इसका कारण यह है कि देना पैसे को उसके सही स्थान पर रख देता है — एक साधन के रूप में, लक्ष्य के रूप में नहीं। जो बच्चा हर महीने कुछ हिस्सा किसी और के लिए अलग रखता है, वह पैसे को अपनी पहचान से जोड़ना नहीं सीखता।',
          'व्यावहारिक तरीक़ा यह है कि बच्चे को ख़ुद चुनने दिया जाए कि वह हिस्सा कहाँ जाए। पड़ोस का कोई काम, कोई संस्था, या कोई एक बार की मदद — चुनाव उसका हो, राशि छोटी हो, और वह ख़ुद देने जाए।',
          'इस अभ्यास का असर तुरंत नहीं दिखता, और शायद यही इसकी सबसे बड़ी ख़ूबी है। यह उन कुछ चीज़ों में है जो बचपन में सिखाई जाती हैं और जिनका असर तीस साल बाद दिखता है।',
        ],
        pEn: [
          'Of the three jars, the one dropped first is almost always the giving jar — and it may well be the one that does the most work over a lifetime.',
          'The reason is that giving puts money in its correct place, as a tool rather than as a goal. A child who sets aside a share for somebody else every month does not learn to attach money to their own identity.',
          'The practical method is to let the child choose where that share goes. Something in the neighbourhood, an organisation, or a single act of help — the choice is theirs, the amount is small, and they hand it over themselves.',
          'The effect of this habit is not visible immediately, and that is perhaps its greatest quality. It belongs to the small set of things taught in childhood whose results show up thirty years later.',
        ],
      },
    ],
  },

  {
    slug: 'emergency-fund-kitna-chahiye',
    sections: [
      {
        hHi: 'नौकरी की क़िस्म से संख्या कैसे बदलती है',
        hEn: 'How the number changes with the kind of work you do',
        pHi: [
          'तीन से छह महीने का नियम एक औसत है, और औसत किसी पर भी ठीक नहीं बैठता। असली संख्या इस बात से निकलती है कि आपकी आय कितनी स्थिर है और नई आय मिलने में कितना समय लगेगा।',
          'सरकारी नौकरी या लंबे अनुबंध वाली स्थिर नौकरी में तीन महीने पर्याप्त हो सकते हैं, क्योंकि आय अचानक रुकने की संभावना कम है और अगर रुके तो सूचना पहले मिल जाती है।',
          'निजी क्षेत्र की सामान्य नौकरी में चार से छह महीने अधिक उपयुक्त हैं। यहाँ नोटिस अवधि छोटी हो सकती है और नई नौकरी खोजने में कई महीने लग सकते हैं, ख़ासकर वरिष्ठ पदों पर जहाँ अवसर कम होते हैं।',
          'स्वरोज़गार, दुकान, फ्रीलांस या कमीशन पर चलने वाली आय में नौ से बारह महीने भी अधिक नहीं हैं। यहाँ आय हर महीने बदलती है, और मंदी का असर एक साथ पूरे क्षेत्र पर पड़ता है।',
        ],
        pEn: [
          'The three-to-six-month rule is an average, and an average fits nobody exactly. Your real number comes from how stable your income is and how long replacing it would take.',
          'In a government job or a stable long-contract position, three months may well be enough, because income is unlikely to stop suddenly and when it does there is usually warning beforehand.',
          'In an ordinary private-sector job, four to six months fits better. Notice periods can be short and finding the next role can take several months, particularly at senior levels where fewer positions exist.',
          'For self-employment, a shop, freelance work or commission-based income, nine to twelve months is not excessive. Income varies every month here, and a downturn tends to hit an entire sector at the same time.',
        ],
      },
      {
        hHi: 'घर में कितने लोग उस आय पर निर्भर हैं',
        hEn: 'How many people depend on that income',
        pHi: [
          'दूसरा गुणक ज़िम्मेदारी का है, और यह पहले से भी अधिक असर डालता है। अकेले रहने वाले व्यक्ति और चार लोगों के परिवार के इकलौते कमाने वाले के लिए एक ही संख्या नहीं हो सकती।',
          'यदि घर में दो लोग कमाते हैं और दोनों की नौकरियाँ अलग-अलग क्षेत्रों में हैं, तो जोखिम बँट जाता है और कोष थोड़ा छोटा रखा जा सकता है। पर यदि दोनों एक ही उद्योग में हैं, तो यह बँटवारा भ्रम है — मंदी दोनों को एक साथ छूती है।',
          'तीसरा विचार आश्रितों का है, ख़ासकर बुज़ुर्ग माता-पिता या पढ़ाई कर रहे बच्चे। ये ख़र्च आय रुकने पर भी नहीं रुकते, और यही वह श्रेणी है जिसे लोग गणना करते समय सबसे अधिक कम आँकते हैं।',
          'एक सरल तरीक़ा यह है: उन सभी ख़र्चों को गिनिए जो आपकी आय बंद होने पर भी अगले महीने आएँगे। उसी संख्या को महीनों से गुणा कीजिए, वेतन को नहीं।',
        ],
        pEn: [
          'The second multiplier is responsibility, and it moves the number even more than the first one does. A person living alone and the sole earner of a family of four cannot use the same figure.',
          'If two people in the household earn and their jobs sit in different sectors, the risk is genuinely spread and the fund can be a little smaller. But if both work in the same industry, that spreading is an illusion — a downturn reaches both at once.',
          'The third consideration is dependants, particularly elderly parents or children in education. Those costs do not stop when income does, and this is the category people most consistently underestimate when doing the calculation.',
          'A simple approach: count every expense that will still arrive next month if your income stops today. Multiply that figure by the months, not your salary.',
        ],
      },
      {
        hHi: 'कोष का इस्तेमाल कब करना है और कब नहीं',
        hEn: 'When to use the fund and when not to',
        pHi: [
          'आपातकालीन कोष बनाना आधा काम है; उसे बचाए रखना दूसरा आधा है। और इसके लिए यह तय होना चाहिए कि "आपात" का अर्थ क्या है।',
          'तीन प्रश्नों की जाँच उपयोगी है। क्या यह अप्रत्याशित है? क्या यह ज़रूरी है? क्या यह अभी है? यदि तीनों का उत्तर हाँ है, तो कोष का उपयोग सही है।',
          'नौकरी जाना, अचानक चिकित्सा ख़र्च, घर की कोई ऐसी मरम्मत जिसके बिना रहना संभव न हो — ये तीनों जाँच पार करते हैं। छुट्टी, त्योहार, कोई अच्छा सौदा, या किसी निवेश का "मौक़ा" — इनमें से कोई नहीं।',
          'सबसे कठिन श्रेणी बीच की है: वह ख़र्च जो ज़रूरी तो है पर अप्रत्याशित नहीं। बच्चे की फ़ीस, बीमा का प्रीमियम, वाहन की सर्विस — इनके लिए अलग "सिंकिंग फंड" बनाना चाहिए, आपातकालीन कोष नहीं छूना चाहिए।',
        ],
        pEn: [
          'Building an emergency fund is half the work; keeping it intact is the other half. For that, it has to be settled in advance what "emergency" actually means.',
          'A three-question test helps. Is it unexpected? Is it necessary? Is it now? If all three answers are yes, using the fund is correct.',
          'Losing a job, a sudden medical cost, a home repair you cannot live without — all three clear the test. A holiday, a festival, a good deal, or an investment "opportunity" — none of them do.',
          'The hardest category is the middle one: expenses that are necessary but not unexpected. School fees, an insurance premium, a vehicle service — those deserve a separate sinking fund, and the emergency fund should be left alone.',
        ],
      },
      {
        hHi: 'ख़र्च करने के बाद उसे दोबारा भरना',
        hEn: 'Refilling it after it has been used',
        pHi: [
          'जिस दिन कोष का उपयोग होता है, उसी दिन एक दूसरी योजना शुरू होनी चाहिए — उसे दोबारा भरने की। यह क़दम लगभग हमेशा छूट जाता है, और इसी चूक से लोग दूसरी बार असुरक्षित पकड़े जाते हैं।',
          'व्यावहारिक तरीक़ा यह है कि कोष का उपयोग करते ही उसी दिन एक स्वचालित मासिक हस्तांतरण फिर से शुरू कर दिया जाए, भले वह पहले से छोटा हो। छोटी राशि से दोबारा शुरू करना बड़ी राशि की प्रतीक्षा करने से हमेशा बेहतर है।',
          'दूसरा उपयोगी नियम यह है कि जब तक कोष अपने स्तर पर वापस न आ जाए, तब तक कोई नया बड़ा ख़र्च न लिया जाए। यह असुविधाजनक लगता है, और यही असुविधा कोष को दोबारा बनने देती है।',
          'और यदि कोष का उपयोग करना पड़ा, तो उसे विफलता मत समझिए। कोष का पूरा उद्देश्य यही था। जो व्यक्ति उसे इस्तेमाल करके फिर भर लेता है, उसकी व्यवस्था काम कर रही है — यही सफलता की परिभाषा है।',
        ],
        pEn: [
          'On the day the fund is used, a second plan should begin — refilling it. This step is almost always skipped, and skipping it is how people get caught unprotected a second time.',
          'The practical method is to restart an automatic monthly transfer the same day the fund is drawn on, even at a smaller amount than before. Restarting small always beats waiting until a large amount is available.',
          'A second useful rule is to take on no new large expense until the fund is back at its level. That feels inconvenient, and the inconvenience is exactly what allows the fund to rebuild.',
          'And if the fund had to be used, do not treat that as a failure. Using it was its entire purpose. Somebody who draws on it and then refills it has a system that is working — that is what success looks like here.',
        ],
      },
      {
        hHi: 'यह कोष निवेश नहीं है',
        hEn: 'This fund is not an investment',
        pHi: [
          'यह बात बार-बार कहनी पड़ती है क्योंकि प्रलोभन लगातार बना रहता है। आपातकालीन कोष का काम बढ़ना नहीं, उपलब्ध रहना है, और ये दोनों उद्देश्य एक साथ नहीं सधते।',
          'जो पैसा ऐसी जगह रखा है जहाँ से निकालने में तीन दिन लगते हैं या जिसका मूल्य निकालते समय गिरा हुआ हो सकता है, वह आपात के लिए नहीं है — चाहे उस पर प्रतिफल कितना भी अच्छा दिखे।',
          'इस कोष पर मिलने वाला ब्याज मुद्रास्फीति से कम होगा, और यह स्वीकार करने लायक़ है। आप उस अंतर को प्रतिफल के रूप में नहीं, बीमा के प्रीमियम के रूप में देखिए — क्योंकि व्यावहारिक रूप से वह यही है।',
          'जब कोष अपने लक्ष्य तक पहुँच जाए, तब उससे ऊपर की हर राशि निवेश में जानी चाहिए। यही सही क्रम है: पहले सुरक्षा, फिर वृद्धि — और इस क्रम को उलटने से ही अधिकांश योजनाएँ बीच में टूटती हैं।',
        ],
        pEn: [
          'This has to be repeated because the temptation is constant. The job of an emergency fund is not to grow but to be available, and those two objectives cannot be served at the same time.',
          'Money kept somewhere that takes three days to reach, or whose value may be down on the day you need it, is not emergency money — however good the return on it looks in a comparison table.',
          'The interest this fund earns will be below inflation, and that is worth accepting. Treat the difference not as a lost return but as an insurance premium, because in practical terms that is exactly what it is.',
          'Once the fund reaches its target, everything above that level should go into investments. That is the correct order: protection first, then growth — and reversing the order is how most plans break down halfway through.',
        ],
      },
      {
        hHi: 'हर साल संख्या दोबारा देखिए',
        hEn: 'Revisit the number every year',
        pHi: [
          'एक बार तय की गई संख्या हमेशा के लिए सही नहीं रहती, क्योंकि जिन चीज़ों से वह निकली थी वे बदलती रहती हैं।',
          'तीन घटनाएँ ऐसी हैं जिनके बाद गणना दोबारा करनी चाहिए: घर में कोई नया आश्रित आना, किराया या EMI बदलना, और नौकरी या व्यवसाय की प्रकृति बदलना। इनमें से कोई भी संख्या को काफ़ी हिला देता है।',
          'यदि ऐसी कोई घटना न भी हो, तो साल में एक बार दस मिनट का पुनरावलोकन पर्याप्त है। पिछले बारह महीनों का औसत मासिक ख़र्च निकालिए और उसी से नई संख्या बना लीजिए।',
          'यह छोटा-सा अभ्यास इस पूरे विषय का सबसे कम मेहनत वाला और सबसे अधिक असर वाला हिस्सा है — और इसीलिए यह सबसे अधिक छोड़ा जाता है।',
        ],
        pEn: [
          'A number decided once does not stay correct forever, because the things it was derived from keep changing underneath it.',
          'Three events should trigger a recalculation: a new dependant in the household, a change in rent or EMI, and a change in the nature of the job or business. Any one of them moves the figure substantially.',
          'Even if none of those happens, a ten-minute review once a year is enough. Work out the average monthly expense across the past twelve months and build the new number from that.',
          'This small exercise is the lowest-effort and highest-impact part of the whole subject — which is precisely why it is the part most often skipped.',
        ],
      },
    ],
  },

  {
    slug: 'upi-fraud-se-bachav',
    sections: [
      {
        hHi: 'ठगी की बुनियादी बनावट: जल्दबाज़ी और डर',
        hEn: 'The basic architecture of a scam: hurry and fear',
        pHi: [
          'नौ अलग तरकीबें याद रखना कठिन है, पर उनके पीछे की बनावट एक ही है, और उसे पहचान लेना नौ तरकीबों से अधिक काम आता है।',
          'हर UPI ठगी दो में से एक भावना पर चलती है। पहली जल्दबाज़ी है — "अभी करना होगा, वरना मौक़ा चला जाएगा"। दूसरी डर है — "आपका खाता बंद हो जाएगा", "आपके नाम पर मामला दर्ज है"।',
          'दोनों का उद्देश्य एक ही है: आपको सोचने का समय न देना। कोई भी असली बैंक, कोई भी असली सरकारी विभाग, और कोई भी असली कंपनी आपको तीन मिनट में निर्णय लेने के लिए मजबूर नहीं करती।',
          'इसलिए सबसे मज़बूत बचाव कोई तकनीकी सेटिंग नहीं बल्कि एक वाक्य है जिसे आप हर बार दोहरा सकें: "मैं दस मिनट बाद वापस कॉल करूँगा।" लगभग हर ठगी इसी वाक्य पर टूट जाती है।',
        ],
        pEn: [
          'Remembering nine separate tricks is hard, but the structure behind all of them is the same, and recognising that structure serves better than memorising the list.',
          'Every UPI scam runs on one of two emotions. The first is hurry — "it has to be done now or the chance is gone". The second is fear — "your account will be frozen", "a case has been registered in your name".',
          'Both exist for the same purpose: to deny you time to think. No genuine bank, no genuine government department and no genuine company forces a decision within three minutes.',
          'So the strongest defence is not a technical setting but a sentence you can repeat every time: "I will call you back in ten minutes." Very nearly every scam collapses against that one line.',
        ],
      },
      {
        hHi: 'परिवार के बुज़ुर्गों की रक्षा कैसे करें',
        hEn: 'Protecting the older people in the family',
        pHi: [
          'ठगी का सबसे बड़ा हिस्सा उन लोगों तक पहुँचता है जिन्होंने UPI हाल ही में सीखा है, और भारत में इसका अर्थ प्रायः घर के बुज़ुर्ग होते हैं।',
          'यहाँ सबसे उपयोगी क़दम सीमा तय करना है। दैनिक लेन-देन की सीमा कम रखिए — जितनी उनकी वास्तविक ज़रूरत है, उससे थोड़ी ऊपर। इससे किसी भी एक ठगी में जाने वाली अधिकतम राशि सीमित हो जाती है।',
          'दूसरा क़दम एक अलग खाता है। मुख्य बचत खाते को UPI से मत जोड़िए; एक छोटे खाते में महीने भर का ख़र्च रखिए और UPI उसी से चले। यह एक ही व्यवस्था अधिकांश बड़े नुक़सान रोक देती है।',
          'तीसरा क़दम बातचीत का है, और वह सबसे महत्वपूर्ण है। उन्हें बताइए कि यदि कभी कुछ ग़लत हो जाए तो डाँट नहीं पड़ेगी। जो लोग शर्म के डर से बताने में देर करते हैं, उन्हीं का नुक़सान सबसे बड़ा होता है, क्योंकि पहला घंटा ही सबसे क़ीमती होता है।',
        ],
        pEn: [
          'The largest share of fraud reaches people who learned UPI recently, and in India that usually means the older members of a household.',
          'The most useful step here is limits. Keep the daily transaction limit low — a little above what they actually need. That caps the maximum any single scam can take, regardless of how convincing it was.',
          'The second step is a separate account. Do not link the main savings account to UPI; keep a month\'s spending in a small account and let UPI run from that one. This single arrangement prevents most large losses.',
          'The third step is a conversation, and it matters most of all. Tell them that if something goes wrong there will be no scolding. People who delay reporting out of embarrassment suffer the biggest losses, because the first hour is the most valuable one.',
        ],
      },
      {
        hHi: 'फ़ोन पर वे सेटिंग जो अभी बदल देनी चाहिए',
        hEn: 'The phone settings worth changing today',
        pHi: [
          'कुछ सेटिंग ऐसी हैं जिन्हें एक बार ठीक कर देने पर वे लगातार सुरक्षा देती रहती हैं, और उनमें कुल दस मिनट लगते हैं।',
          'पहली, स्क्रीन-शेयरिंग और रिमोट-कंट्रोल वाले ऐप हटा दीजिए, जब तक कि किसी काम के लिए वे सचमुच ज़रूरी न हों। भारत में बड़ी संख्या में ठगी इन्हीं ऐप के माध्यम से हुई है।',
          'दूसरी, अनजान स्रोत से ऐप इंस्टॉल करने की अनुमति बंद रखिए। कोई भी असली बैंक आपसे कभी किसी लिंक से APK डाउनलोड करने को नहीं कहेगा।',
          'तीसरी, बैंक और UPI ऐप पर स्क्रीन लॉक अलग रखिए, और सूचनाएँ लॉक स्क्रीन पर न दिखें ऐसा कर दीजिए। OTP का लॉक स्क्रीन पर दिखना उसे लगभग बेकार बना देता है।',
          'चौथी, हर UPI ऐप में लेन-देन की सीमा ख़ुद तय कर दीजिए। डिफ़ॉल्ट सीमा प्रायः आपकी ज़रूरत से बहुत ऊँची होती है, और उसे घटाने में कोई नुक़सान नहीं है।',
        ],
        pEn: [
          'A few settings, fixed once, keep protecting you continuously afterwards, and together they take about ten minutes to change.',
          'First, remove screen-sharing and remote-control apps unless some job genuinely requires them. A very large number of frauds in India have run through exactly these applications.',
          'Second, keep installation from unknown sources switched off. No genuine bank will ever ask you to download an APK from a link sent to you.',
          'Third, set a separate screen lock on banking and UPI apps, and stop notifications from appearing on the lock screen. An OTP visible on a locked phone is close to useless as a protection.',
          'Fourth, set your own transaction limits inside every UPI app. The default limits are usually far above what you need, and there is no cost at all to lowering them.',
        ],
      },
      {
        hHi: 'ठगी के बाद का पहला घंटा',
        hEn: 'The first hour after being defrauded',
        pHi: [
          'यदि पैसा चला गया है, तो अगला घंटा बाक़ी सब कुछ से अधिक मायने रखता है, क्योंकि उसी दौरान रक़म को रोका जा सकता है।',
          'पहला क़दम: बैंक के आधिकारिक नंबर पर तुरंत कॉल कीजिए और लेन-देन की सूचना दीजिए। नंबर बैंक की वेबसाइट या पासबुक से लीजिए — किसी खोज परिणाम से नहीं, क्योंकि नक़ली हेल्पलाइन नंबर इसी जगह डाले जाते हैं।',
          'दूसरा क़दम: राष्ट्रीय साइबर अपराध हेल्पलाइन पर शिकायत दर्ज कीजिए। जितनी जल्दी शिकायत दर्ज होती है, उतनी अधिक संभावना होती है कि प्राप्तकर्ता खाते को रोका जा सके।',
          'तीसरा क़दम: सब कुछ सुरक्षित रखिए — संदेश, कॉल का समय, लेन-देन आईडी, और वह नंबर या UPI पता जिस पर पैसा गया। यह विवरण बाद में माँगा जाता है और स्मृति से नहीं दिया जा सकता।',
          'और चौथा: उसी नंबर पर वापस बात मत कीजिए। ठगी के बाद अक्सर दूसरा कॉल आता है, जिसमें कोई "मदद" करने की पेशकश करता है — वह उसी गिरोह का दूसरा चरण होता है।',
        ],
        pEn: [
          'If money has gone, the next hour matters more than everything else, because that is the window in which the amount can still be stopped.',
          'First: call your bank\'s official number immediately and report the transaction. Take the number from the bank\'s website or your passbook — never from a search result, because fake helpline numbers are planted in exactly that place.',
          'Second: file a complaint on the national cybercrime helpline. The sooner a complaint is registered, the better the chance that the receiving account can be frozen before the money moves on.',
          'Third: preserve everything — the messages, the times of calls, the transaction ID, and the number or UPI address the money went to. These details are asked for later and cannot be supplied from memory.',
          'And fourth: do not talk to that number again. A second call often follows a fraud, offering to "help" recover the money — that call is the same group running its second stage.',
        ],
      },
      {
        hHi: 'वे तीन बातें जो कभी नहीं बदलतीं',
        hEn: 'The three things that never change',
        pHi: [
          'तरकीबें हर साल नई आती हैं, इसलिए सूची याद रखना दीर्घकालिक बचाव नहीं है। तीन नियम ऐसे हैं जो हर नई तरकीब पर भी लागू रहते हैं।',
          'पहला: पैसा प्राप्त करने के लिए PIN कभी नहीं लगता। यदि कोई आपसे पैसा भेजने की बात कहकर PIN डलवा रहा है, तो वह पैसा आपसे निकल रहा है, आ नहीं रहा।',
          'दूसरा: कोई भी संस्था कभी OTP, PIN या पासवर्ड नहीं माँगती — बैंक नहीं, पुलिस नहीं, कंपनी नहीं, कोई नहीं। यह नियम अपवाद-रहित है, और इसी वजह से यह इतना उपयोगी है।',
          'तीसरा: जो कोई आपको सोचने का समय नहीं दे रहा, वही सबसे संदिग्ध है। सच्ची बात इंतज़ार कर सकती है; झूठी बात नहीं कर सकती, क्योंकि इंतज़ार उसे उजागर कर देता है।',
          'इन तीन वाक्यों को घर में किसी दिखने वाली जगह पर लिख देना — फ़्रिज पर, या फ़ोन के पीछे — किसी भी तकनीकी उपाय से अधिक असरदार साबित होता है।',
        ],
        pEn: [
          'New tricks appear every year, so memorising a list is not a long-term defence. Three rules hold against every new variation as well as every old one.',
          'First: a PIN is never required to receive money. If somebody is having you enter a PIN while talking about sending you money, that money is leaving your account rather than arriving in it.',
          'Second: no institution ever asks for an OTP, PIN or password — not a bank, not the police, not a company, nobody. The rule has no exceptions, and that is exactly what makes it so useful.',
          'Third: whoever is denying you time to think is the most suspicious person in the conversation. A true statement can wait; a false one cannot, because waiting is what exposes it.',
          'Writing those three sentences somewhere visible at home — on the fridge, or on the back of a phone — turns out to be more effective than any technical measure.',
        ],
      },
    ],
  },

  // ---------------------------------------------------------------------
  // Batch two.
  // ---------------------------------------------------------------------

  {
    slug: 'ppf-account-complete-guide',
    sections: [
      {
        hHi: 'जमा की तारीख़ का असर',
        hEn: 'Why the deposit date matters',
        pHi: [
          'PPF का एक विवरण ऐसा है जो नियमों में लिखा है पर बहुत कम लोग उसका उपयोग करते हैं: ब्याज की गणना महीने की पाँचवीं तारीख़ और महीने के अंत के बीच की सबसे कम शेष राशि पर होती है।',
          'इसका सीधा अर्थ यह है कि पाँच तारीख़ के बाद जमा किया गया पैसा उस पूरे महीने का ब्याज नहीं कमाता। बारह किश्तों में जमा करने वाले के लिए यह हर साल कुछ महीनों का ब्याज छोड़ देने जैसा है।',
          'इसलिए यदि आप मासिक जमा करते हैं, तो तारीख़ पाँच से पहले रखिए। और यदि आप साल में एक बार जमा करते हैं, तो अप्रैल की शुरुआत सबसे अच्छी है, क्योंकि तब वह राशि पूरे वर्ष ब्याज कमाती है।',
          'यह कोई चतुराई नहीं है, केवल नियम का सही उपयोग है — और पंद्रह साल में इसका असर अपने आप बड़ा हो जाता है।',
        ],
        pEn: [
          'One detail of PPF is written into the rules and used by very few people: interest is calculated on the lowest balance between the fifth of the month and the end of that month.',
          'The direct consequence is that money deposited after the fifth earns no interest for that whole month. For somebody depositing in twelve instalments, that amounts to giving away several months of interest every single year.',
          'So if you deposit monthly, keep the date before the fifth. And if you deposit once a year, early April is the best choice, because the amount then earns interest across the full year rather than part of it.',
          'This is not a clever trick, only correct use of the rule as written — and across fifteen years its effect grows large on its own.',
        ],
      },
      {
        hHi: 'यह खाता किसके लिए सबसे उपयुक्त है',
        hEn: 'Who this account suits best',
        pHi: [
          'PPF हर किसी के लिए एक जैसा उपयोगी नहीं है, और यह तय कर लेना कि आप किस श्रेणी में हैं, बाद की उलझन बचा देता है।',
          'यह उन लोगों के लिए सबसे उपयुक्त है जिनकी आय पर कर लगता है और जो लंबी अवधि के लिए एक ऐसा हिस्सा चाहते हैं जिसमें बाज़ार का उतार-चढ़ाव न हो। ऐसे व्यक्ति के लिए यह पोर्टफ़ोलियो का स्थिर सिरा बन जाता है।',
          'यह उन लोगों के लिए कम उपयुक्त है जिनकी आय कर के दायरे में नहीं आती, क्योंकि तब इसका सबसे बड़ा लाभ — कर से छूट — लागू ही नहीं होता, और तरलता की कमी बिना किसी बदले के रह जाती है।',
          'और यह उन लोगों के लिए अनुपयुक्त है जिन्हें अगले कुछ वर्षों में पैसे की ज़रूरत पड़ सकती है। पंद्रह साल की अवधि कोई औपचारिकता नहीं है; इसे हल्के में लेने पर बाद में असुविधा होती है।',
        ],
        pEn: [
          'PPF is not equally useful to everybody, and deciding which category you fall into saves a good deal of confusion later on.',
          'It suits people whose income is taxed and who want a long-term portion that carries no market movement. For such a person it becomes the stable end of a portfolio, and it plays that role well.',
          'It suits people below the tax threshold much less, because its largest benefit — the tax treatment — does not apply to them, and the lack of liquidity remains without anything given in exchange for it.',
          'And it is unsuitable for anybody who may need the money within the next few years. The fifteen-year term is not a formality, and treating it lightly leads to inconvenience later.',
        ],
      },
      {
        hHi: 'बच्चों और जीवनसाथी के नाम खाते',
        hEn: 'Accounts in the name of children and a spouse',
        pHi: [
          'एक सवाल जो बार-बार आता है वह यह है कि परिवार के हर सदस्य के नाम PPF खाता खोलकर अधिक जमा किया जा सकता है या नहीं।',
          'नाबालिग बच्चे के नाम खाता खोला जा सकता है और अभिभावक उसका संचालन करते हैं। पर ध्यान रखने की बात यह है कि एक वयस्क द्वारा अपने और अपने नाबालिग बच्चों के खातों में मिलाकर जमा की जाने वाली राशि पर संयुक्त सीमा लागू होती है।',
          'जीवनसाथी का खाता अलग होता है और उसकी अपनी सीमा होती है, बशर्ते जमा उसकी अपनी आय से हो। यदि जमा आपकी आय से हो रहा है, तो आय-क्लबिंग के नियम विचारणीय हो जाते हैं।',
          'ये नियम विस्तार में जाते हैं और समय-समय पर स्पष्टीकरण भी आते रहते हैं, इसलिए बड़ी राशि की योजना बनाने से पहले वर्तमान नियम देख लेना या किसी कर सलाहकार से पूछ लेना ही सही तरीक़ा है।',
        ],
        pEn: [
          'A question that keeps recurring is whether more can be deposited by opening a PPF account in the name of every family member.',
          'An account can be opened for a minor child with a guardian operating it. What has to be kept in mind is that a combined limit applies across an adult\'s own account and those of their minor children taken together.',
          'A spouse\'s account is separate and carries its own limit, provided the deposits come from their own income. If the deposits come from yours, the income-clubbing provisions become relevant and need to be considered.',
          'These rules run into detail and clarifications appear from time to time, so before planning around large amounts the correct approach is to check the current position or ask a tax adviser.',
        ],
      },
      {
        hHi: 'PPF की तुलना किससे की जानी चाहिए',
        hEn: 'What PPF should actually be compared with',
        pHi: [
          'सबसे आम ग़लत तुलना PPF और इक्विटी के बीच की जाती है, और उसका निष्कर्ष हमेशा एक जैसा और हमेशा भ्रामक होता है।',
          'ये दोनों एक ही काम नहीं करते। इक्विटी वृद्धि के लिए है और उसमें अनिश्चितता है; PPF निश्चितता के लिए है और उसमें वृद्धि सीमित है। इन्हें एक-दूसरे का विकल्प मानना दोनों को ग़लत समझना है।',
          'सही तुलना PPF की उन्हीं साधनों से है जो वही काम करते हैं — निश्चित प्रतिफल, लंबी अवधि, कम जोखिम। इस पंक्ति में रखकर देखने पर PPF की स्थिति स्पष्ट दिखती है, ख़ासकर कर-व्यवहार के कारण।',
          'एक संतुलित पोर्टफ़ोलियो में दोनों की जगह होती है, और अनुपात उम्र, लक्ष्य और आराम के स्तर से तय होता है। यह लेख किसी अनुपात की सिफ़ारिश नहीं करता; यह केवल यह कह रहा है कि प्रश्न "कौन बेहतर" नहीं बल्कि "कितना-कितना" है।',
        ],
        pEn: [
          'The most common bad comparison sets PPF against equity, and the conclusion drawn from it is always the same and always misleading.',
          'The two do not do the same job. Equity is for growth and carries uncertainty; PPF is for certainty and carries limited growth. Treating them as alternatives to one another misunderstands both of them.',
          'The correct comparison is against instruments doing the same job — fixed return, long horizon, low risk. Placed in that row, PPF\'s position becomes clear, particularly because of how it is treated for tax.',
          'A balanced portfolio has room for both, and the proportion depends on age, goals and how much movement you can live with. This article recommends no particular proportion; it only points out that the question is not "which is better" but "how much of each".',
        ],
      },
      {
        hHi: 'खाता खुला रखने की छोटी-सी शर्त',
        hEn: 'The small condition for keeping the account alive',
        pHi: [
          'PPF खाते के साथ एक ऐसी शर्त जुड़ी है जिसे भूल जाना आसान है और जिसका परिणाम असुविधाजनक होता है: हर वित्तीय वर्ष में एक न्यूनतम राशि जमा करनी होती है।',
          'यदि किसी वर्ष यह न्यूनतम जमा नहीं होता, तो खाता निष्क्रिय हो जाता है। निष्क्रिय खाते से ऋण या आंशिक निकासी की सुविधा नहीं मिलती, और उसे दोबारा चालू कराने के लिए छूटे हुए वर्षों का भुगतान और एक छोटा शुल्क देना पड़ता है।',
          'यह समस्या प्रायः उन लोगों के साथ होती है जो किसी वर्ष अधिक जमा कर देते हैं और अगले वर्ष यह मानकर छोड़ देते हैं कि पर्याप्त हो चुका है। सीमा वार्षिक है, और पिछले वर्ष का अतिरिक्त अगले वर्ष नहीं गिना जाता।',
          'इससे बचने का सबसे आसान तरीक़ा एक स्वचालित मासिक निर्देश है, चाहे राशि छोटी हो। छोटी राशि नियमित रूप से जाना, बड़ी राशि कभी-कभी जाने से बेहतर व्यवस्था है।',
        ],
        pEn: [
          'One condition attached to a PPF account is easy to forget and inconvenient when it is: a minimum amount has to be deposited in every financial year.',
          'If that minimum is missed in any year, the account becomes dormant. A dormant account cannot be used for loans or partial withdrawals, and reviving it requires paying for the missed years along with a small penalty.',
          'This usually happens to people who deposit a large amount in one year and then skip the next, assuming they have already put in enough. The limit is annual, and an excess in one year does not carry forward into the next.',
          'The easiest way to avoid it is a standing monthly instruction, however small the amount. A small sum going in regularly is a better arrangement than a large sum going in occasionally.',
        ],
      },
      {
        hHi: 'पंद्रह साल बाद का सबसे कम चर्चित विकल्प',
        hEn: 'The least discussed option after fifteen years',
        pHi: [
          'परिपक्वता के बाद तीन रास्तों की चर्चा होती है, पर उनमें से एक पर सबसे कम ध्यान जाता है: बिना नई जमा किए खाता चलाते रहना।',
          'इस विकल्प में आपको आगे कुछ भी डालना नहीं पड़ता, और शेष राशि पर ब्याज मिलता रहता है। साथ ही हर वर्ष एक निकासी की सुविधा भी बनी रहती है।',
          'यह उन लोगों के लिए असामान्य रूप से उपयोगी है जो सेवानिवृत्ति के आसपास हैं और जिन्हें एक ऐसे साधन की ज़रूरत है जो सुरक्षित हो, कर के लिहाज़ से अनुकूल हो, और जिसमें से ज़रूरत पड़ने पर हर साल कुछ निकाला जा सके।',
          'बहुत से लोग परिपक्वता पर पूरी राशि निकाल लेते हैं और फिर उसे कहीं और लगाने में महीनों लगाते हैं। इस विकल्प को जानना उस जल्दबाज़ी को टाल देता है, और अक्सर यही सबसे शांत निर्णय होता है।',
        ],
        pEn: [
          'Three routes are discussed after maturity, and one of them gets the least attention of all: keeping the account running without any fresh deposits.',
          'Under this option nothing further has to be paid in, and the balance continues to earn interest. Alongside that, one withdrawal a year remains available whenever it is needed.',
          'It is unusually useful for people near retirement who want something safe, favourably treated for tax, and from which a portion can be drawn each year if required.',
          'A great many people withdraw the entire amount at maturity and then spend months deciding where to put it. Knowing this option removes that hurry, and it is very often the calmest decision available.',
        ],
      },
    ],
  },

  {
    slug: 'health-insurance-family-guide',
    sections: [
      {
        hHi: 'प्रतीक्षा अवधि: वह घड़ी जो पहले दिन से चलती है',
        hEn: 'Waiting periods: the clock that starts on day one',
        pHi: [
          'स्वास्थ्य बीमा में सबसे अधिक निराशा प्रतीक्षा अवधि से आती है, और इसका कारण यह है कि लोग पॉलिसी लेते समय इसे पढ़ते नहीं और दावे के समय पहली बार सामना करते हैं।',
          'तीन अलग घड़ियाँ एक साथ चलती हैं। पहली आरंभिक प्रतीक्षा है — पॉलिसी शुरू होने के कुछ दिनों तक दुर्घटना के अलावा कुछ भी कवर नहीं होता।',
          'दूसरी विशिष्ट बीमारियों की प्रतीक्षा है। कुछ सामान्य स्थितियाँ — जैसे कुछ प्रकार की सर्जरी — पॉलिसी शुरू होने के एक या दो वर्ष बाद ही कवर होती हैं, भले ही वे पहले से मौजूद न रही हों।',
          'तीसरी और सबसे लंबी पूर्व-मौजूद बीमारियों की प्रतीक्षा है। यही वह श्रेणी है जिस पर सबसे अधिक विवाद होते हैं, और यही वह कारण है जिससे स्वास्थ्य बीमा जल्दी लेना — यानी स्वस्थ रहते हुए लेना — इतना महत्वपूर्ण है। घड़ी अभी शुरू होगी, बीमारी के बाद नहीं।',
        ],
        pEn: [
          'Most of the disappointment in health insurance comes from waiting periods, and the reason is that people do not read them while buying and meet them for the first time at claim.',
          'Three separate clocks run at once. The first is the initial waiting period — for some days after the policy begins, nothing except accidents is covered at all.',
          'The second is the waiting period for specified illnesses. Certain common conditions, including some kinds of surgery, are covered only after a year or two of the policy, even where they did not exist beforehand.',
          'The third and longest is the waiting period for pre-existing conditions. This is the category that generates the most disputes, and it is exactly why buying health cover early — that is, while healthy — matters so much. The clock starts now, not after a diagnosis.',
        ],
      },
      {
        hHi: 'रूम रेंट की सीमा और उसका छिपा हुआ गणित',
        hEn: 'The room rent limit and its hidden arithmetic',
        pHi: [
          'यह पॉलिसी का सबसे कम समझा जाने वाला खंड है और दावे के समय सबसे महँगा। कई पॉलिसियों में कमरे का किराया बीमा राशि के किसी प्रतिशत तक सीमित होता है।',
          'लोग मान लेते हैं कि यदि वे महँगे कमरे में रहें तो केवल किराए का अंतर देना होगा। असल में कई पॉलिसियों में ऐसा नहीं होता — अनुपातिक कटौती लागू होती है, और तब पूरे बिल का एक हिस्सा अस्वीकृत हो जाता है, केवल किराए का नहीं।',
          'यानी यदि आपने अनुमत सीमा से दोगुने किराए वाला कमरा लिया, तो सर्जन की फ़ीस, जाँच और दवाइयों तक में अनुपातिक कटौती लग सकती है। यही वह जगह है जहाँ "पूरा कवर था फिर भी आधा बिल देना पड़ा" जैसी शिकायतें जन्म लेती हैं।',
          'इसलिए पॉलिसी चुनते समय यह देखिए कि रूम रेंट पर कोई सीमा है या नहीं। बिना सीमा वाली पॉलिसी का प्रीमियम थोड़ा अधिक होता है, और वह अंतर प्रायः एक ही अस्पताल-भर्ती में वसूल हो जाता है।',
        ],
        pEn: [
          'This is the least understood clause in a policy and the most expensive one at claim time. Many policies cap the room rent at some percentage of the sum insured.',
          'People assume that taking a costlier room means paying only the difference in rent. In many policies that is not what happens — a proportionate deduction applies, and then a share of the entire bill is disallowed rather than just the rent.',
          'So if you took a room at twice the permitted rent, proportionate cuts can reach the surgeon\'s fee, the investigations and even the medicines. This is where complaints of the "I had full cover and still paid half the bill" kind come from.',
          'So when choosing a policy, check whether any room rent limit exists. A policy without one costs a little more in premium, and that difference is often recovered in a single hospital admission.',
        ],
      },
      {
        hHi: 'नियोक्ता की पॉलिसी काफ़ी क्यों नहीं है',
        hEn: 'Why the employer policy is not enough',
        pHi: [
          'बहुत से लोग यह मानकर अपनी पॉलिसी नहीं लेते कि दफ़्तर से कवर मिला हुआ है। यह भरोसा तीन कारणों से जोखिम भरा है।',
          'पहला, वह कवर नौकरी से जुड़ा है। जिस दिन नौकरी छूटेगी — इस्तीफ़ा, छँटनी या सेवानिवृत्ति — उसी दिन कवर भी चला जाएगा, और वह प्रायः वही समय होता है जब आय भी नहीं होती।',
          'दूसरा, राशि आमतौर पर छोटी होती है। नियोक्ता की पॉलिसियाँ प्रायः एक बुनियादी स्तर पर होती हैं, जो एक बड़ी बीमारी के लिए पर्याप्त नहीं है।',
          'तीसरा और सबसे महत्वपूर्ण: प्रतीक्षा अवधि की घड़ी आपकी अपनी पॉलिसी में ही चलती है। यदि आप पैंतालीस की उम्र में अपनी पहली पॉलिसी लेंगे, तो उसकी घड़ी तभी शुरू होगी — और तब तक जो स्थितियाँ बन चुकी होंगी वे पूर्व-मौजूद मानी जाएँगी।',
          'इसलिए व्यावहारिक सलाह यह है कि नियोक्ता के कवर को अतिरिक्त परत मानिए, बुनियाद नहीं। एक छोटी-सी अपनी पॉलिसी साथ चलती रहे, ताकि घड़ी चलती रहे।',
        ],
        pEn: [
          'Many people skip buying their own policy on the strength of cover provided at work. That reliance is risky for three reasons.',
          'First, the cover is tied to the job. On the day the job ends — resignation, redundancy or retirement — the cover ends with it, and that is very often the same period in which there is no income either.',
          'Second, the sum is usually small. Employer policies typically sit at a basic level, which is not enough for a serious illness.',
          'Third and most important: the waiting-period clock runs inside your own policy. If you buy your first policy at forty-five, the clock starts then — and whatever conditions have developed by that point will count as pre-existing.',
          'The practical advice is therefore to treat employer cover as an additional layer rather than a foundation. Keep a small policy of your own running alongside it, so the clock keeps running too.',
        ],
      },
      {
        hHi: 'नकदी-रहित और प्रतिपूर्ति — दोनों के लिए तैयारी',
        hEn: 'Cashless and reimbursement — preparing for both',
        pHi: [
          'नकदी-रहित सुविधा तभी काम करती है जब अस्पताल बीमा कंपनी के नेटवर्क में हो। आपात स्थिति में आप हमेशा अस्पताल नहीं चुन सकते, इसलिए दोनों रास्तों के लिए तैयार रहना ज़रूरी है।',
          'नकदी-रहित के लिए दो चीज़ें साथ रखिए: पॉलिसी नंबर और बीमा कंपनी का हेल्पलाइन नंबर, फ़ोन में भी और घर पर काग़ज़ पर भी। भर्ती के समय यही सबसे पहले माँगा जाता है।',
          'प्रतिपूर्ति के लिए काग़ज़ात का अनुशासन चाहिए। हर बिल मूल रूप में, डिस्चार्ज सारांश, सभी जाँच रिपोर्ट, और डॉक्टर का पर्चा — इनमें से कोई एक भी छूटने पर दावा अटक जाता है।',
          'एक व्यावहारिक आदत यह है कि अस्पताल में रहते हुए ही हर काग़ज़ की फ़ोटो ले ली जाए। डिस्चार्ज के बाद मूल काग़ज़ जमा करने पड़ते हैं, और तब आपके पास अपनी प्रति होना बहुत काम आता है।',
        ],
        pEn: [
          'Cashless treatment works only when the hospital is inside the insurer\'s network. In an emergency you cannot always choose the hospital, so being ready for both routes matters.',
          'For cashless, keep two things together: the policy number and the insurer\'s helpline number, both on the phone and on paper at home. Those are the first things asked for at admission.',
          'For reimbursement, what is needed is discipline about paperwork. Every bill in original, the discharge summary, all investigation reports, and the doctor\'s prescription — a claim stalls if any one of them is missing.',
          'A practical habit is to photograph every document while still in the hospital. Originals have to be submitted after discharge, and having your own copy at that point turns out to be very useful.',
        ],
      },
      {
        hHi: 'हर साल नवीनीकरण के समय क्या देखें',
        hEn: 'What to check at renewal each year',
        pHi: [
          'स्वास्थ्य बीमा एक बार का निर्णय नहीं है; उसका सबसे महत्वपूर्ण हिस्सा हर साल का नवीनीकरण है, और वहीं सबसे बड़ी ग़लती भी होती है — तारीख़ चूक जाना।',
          'नवीनीकरण चूकने का परिणाम केवल कुछ दिनों का अंतराल नहीं है। कई मामलों में पूरी प्रतीक्षा अवधि दोबारा शुरू हो सकती है, यानी वर्षों की निरंतरता एक भूली हुई तारीख़ से समाप्त हो जाती है।',
          'दूसरा काम राशि की समीक्षा है। चिकित्सा ख़र्च सामान्य मुद्रास्फीति से तेज़ी से बढ़ते हैं, इसलिए जो राशि पाँच साल पहले पर्याप्त थी वह आज नहीं है। हर कुछ वर्षों में कवर बढ़ाना इस योजना का हिस्सा होना चाहिए।',
          'तीसरा, संचित बोनस देखिए। कई पॉलिसियों में दावा-मुक्त वर्ष के बाद कवर अपने आप बढ़ता है, और यह बढ़ोतरी बिना अतिरिक्त प्रीमियम के मिलती है — इसे जानना और इसका हिसाब रखना उपयोगी है।',
        ],
        pEn: [
          'Health insurance is not a one-time decision; its most important part is the renewal each year, and that is also where the biggest mistake happens — missing the date.',
          'Missing a renewal does not merely create a gap of a few days. In many cases the whole waiting period can restart, meaning years of continuity end because of one forgotten date.',
          'The second task is reviewing the sum insured. Medical costs rise faster than general inflation, so an amount that was adequate five years ago is not adequate now. Raising cover every few years should be part of the plan.',
          'Third, look at the accumulated bonus. Many policies increase the cover automatically after a claim-free year, and that increase comes without extra premium — it is worth knowing about and keeping track of.',
        ],
      },
      {
        hHi: 'पहले से मौजूद बीमारी छिपाने का परिणाम',
        hEn: 'What happens when a pre-existing condition is hidden',
        pHi: [
          'यह सबसे महँगी ग़लती है जो कोई स्वास्थ्य बीमा में कर सकता है, और यह प्रायः डर से की जाती है, बेईमानी से नहीं — लोगों को लगता है कि बताने पर पॉलिसी नहीं मिलेगी।',
          'वास्तविकता इसके उलट है। अधिकांश स्थितियों में पॉलिसी मिल जाती है; कभी प्रीमियम थोड़ा अधिक होता है, कभी उस विशेष स्थिति के लिए प्रतीक्षा अवधि लंबी होती है। यह असुविधा है, अस्वीकृति नहीं।',
          'जबकि छिपाने का परिणाम यह होता है कि जिस दिन दावा आता है, उसी दिन पूरी पॉलिसी रद्द हो सकती है और दावा अस्वीकार हो सकता है। वर्षों का प्रीमियम भी गया और सुरक्षा भी।',
          'इसलिए प्रस्ताव-पत्र भरते समय हर स्थिति लिख देना ही एकमात्र सुरक्षित रास्ता है — और यदि कोई एजेंट कहे कि "यह लिखने की ज़रूरत नहीं", तो वही वाक्य उस एजेंट को बदल देने का पर्याप्त कारण है।',
        ],
        pEn: [
          'This is the most expensive mistake available in health insurance, and it is usually made out of fear rather than dishonesty — people believe that disclosing will mean being refused.',
          'The reality runs the other way. In most situations a policy is still issued; sometimes the premium is a little higher, sometimes the waiting period for that particular condition is longer. That is inconvenience, not rejection.',
          'Whereas the consequence of hiding it is that on the day a claim arrives, the entire policy can be cancelled and the claim declined. Years of premium are lost and so is the protection.',
          'So writing down every condition on the proposal form is the only safe route — and if an agent says "there is no need to mention that", the sentence itself is sufficient reason to change agents.',
        ],
      },
    ],
  },

  {
    slug: 'old-vs-new-tax-regime',
    sections: [
      {
        hHi: 'चुनाव कब और कितनी बार किया जा सकता है',
        hEn: 'When the choice can be made, and how often',
        pHi: [
          'तुलना का गणित समझ लेने के बाद अगला व्यावहारिक प्रश्न यह है कि यह चुनाव कब करना होता है और क्या उसे बदला जा सकता है।',
          'वेतनभोगी व्यक्ति के लिए यह चुनाव आमतौर पर वर्ष की शुरुआत में नियोक्ता को बताना होता है, ताकि TDS सही कटे। पर यह घोषणा अंतिम नहीं होती — रिटर्न भरते समय स्थिति दोबारा चुनी जा सकती है।',
          'इसका व्यावहारिक अर्थ यह है कि यदि वर्ष के बीच आपकी परिस्थिति बदल जाए — नया गृह ऋण, नया बीमा, या कोई बड़ा निवेश — तो रिटर्न के समय आप वह विकल्प चुन सकते हैं जो अंततः बेहतर बैठता है।',
          'व्यवसाय या पेशे से आय वाले लोगों के लिए नियम अलग और अधिक कड़े हैं, और वहाँ बार-बार बदलने की स्वतंत्रता सीमित होती है। यह वह बिंदु है जहाँ किसी कर सलाहकार से पूछ लेना सबसे अधिक उपयोगी होता है।',
        ],
        pEn: [
          'Once the comparison arithmetic is understood, the next practical question is when this choice has to be made and whether it can be changed.',
          'For a salaried person the choice is usually declared to the employer at the start of the year so that TDS is deducted correctly. But that declaration is not final — the position can be chosen again while filing the return.',
          'The practical effect is that if circumstances change mid-year — a new home loan, new insurance, or a large investment — you can pick whichever option finally works out better at the time of filing.',
          'For people with business or professional income the rules are different and considerably tighter, and the freedom to switch back and forth is limited. This is the point at which asking a tax adviser is most useful.',
        ],
      },
      {
        hHi: 'वे कटौतियाँ जिन्हें लोग गिनना भूल जाते हैं',
        hEn: 'The deductions people forget to count',
        pHi: [
          'तुलना करते समय सबसे आम ग़लती यह है कि केवल बड़ी और परिचित कटौतियाँ गिनी जाती हैं, और छोटी छूट जाती हैं। मिलकर वे छोटी कटौतियाँ नतीजा पलट सकती हैं।',
          'सबसे अधिक भूली जाने वाली श्रेणी वह है जो अपने आप हो रही होती है — भविष्य निधि में नियोक्ता के माध्यम से जाने वाला योगदान, बच्चों की स्कूल फ़ीस का ट्यूशन हिस्सा, और गृह ऋण की मूल राशि की अदायगी।',
          'दूसरी भूली जाने वाली श्रेणी स्वास्थ्य से जुड़ी है — अपने और माता-पिता के स्वास्थ्य बीमा का प्रीमियम, और कुछ स्थितियों में निवारक जाँच का ख़र्च।',
          'तीसरी वह है जो केवल कुछ लोगों पर लागू होती है — शिक्षा ऋण का ब्याज, दान, और किराए से जुड़ी छूट। ये सार्वभौमिक नहीं हैं, पर जिन पर लागू होती हैं उनके लिए बड़ी होती हैं।',
          'इसलिए तुलना से पहले एक पूरी सूची बना लीजिए। जो कटौतियाँ सूची में नहीं आतीं, वे गणना में भी नहीं आतीं — और तब निष्कर्ष ग़लत निकलता है।',
        ],
        pEn: [
          'The most common error while comparing is counting only the large, familiar deductions and letting the small ones slip. Taken together, those small ones can reverse the result.',
          'The most frequently forgotten category is the one happening automatically — the provident fund contribution routed through the employer, the tuition component of children\'s school fees, and the principal repayment on a home loan.',
          'The second forgotten category relates to health — premiums for your own and your parents\' health insurance, and in some situations the cost of preventive check-ups.',
          'The third applies only to some people — interest on an education loan, donations, and rent-related relief. These are not universal, but for those they do apply to they are substantial.',
          'So build a complete list before comparing. Deductions that do not make it onto the list do not make it into the calculation either — and then the conclusion comes out wrong.',
        ],
      },
      {
        hHi: 'तीस मिनट की तुलना को दस मिनट कैसे बनाएँ',
        hEn: 'Turning the thirty-minute comparison into ten',
        pHi: [
          'यदि आप यह तुलना हर साल करते हैं, तो एक बार एक सरल ढाँचा बना लेने से हर बार का काम बहुत छोटा हो जाता है।',
          'एक स्प्रेडशीट में तीन स्तंभ बनाइए: मद, पुरानी व्यवस्था में मान्य राशि, नई व्यवस्था में मान्य राशि। पंक्तियों में अपनी सारी आय और सारी कटौतियाँ लिख दीजिए, एक बार।',
          'हर वर्ष केवल संख्याएँ बदलनी होती हैं, ढाँचा नहीं। और चूँकि ढाँचा आपका अपना है, इसलिए उसमें वही मदें होती हैं जो आप पर सचमुच लागू होती हैं — किसी सामान्य कैलकुलेटर की तुलना में यह कहीं अधिक भरोसेमंद है।',
          'एक और उपयोगी अभ्यास यह है कि पिछले वर्ष की गणना सुरक्षित रखी जाए। दो वर्षों की तुलना अगल-बग़ल देखने पर यह भी दिख जाता है कि आपकी स्थिति किस दिशा में जा रही है।',
        ],
        pEn: [
          'If you do this comparison every year, building a simple structure once makes the work far smaller on every subsequent occasion.',
          'Set up three columns in a spreadsheet: the item, the amount allowed under the old regime, and the amount allowed under the new one. Write all your income and all your deductions into the rows, once.',
          'Each year only the numbers change, not the structure. And because the structure is your own, it contains exactly the items that actually apply to you — which makes it considerably more reliable than a generic calculator.',
          'Another useful habit is to keep last year\'s calculation. Seeing two years side by side also shows which direction your own position is moving in.',
        ],
      },
      {
        hHi: 'गृह ऋण वाले लोगों के लिए विशेष स्थिति',
        hEn: 'The particular position of people with a home loan',
        pHi: [
          'गृह ऋण इस तुलना को सबसे अधिक हिलाने वाला अकेला कारक है, इसलिए यह अलग विचार का हक़दार है।',
          'ऋण के शुरुआती वर्षों में EMI का बड़ा हिस्सा ब्याज होता है, और यही वह हिस्सा है जिस पर पुरानी व्यवस्था में सबसे बड़ी राहत मिलती है। इसलिए ऋण के पहले वर्षों में तुलना अक्सर एक ओर झुकती है।',
          'जैसे-जैसे वर्ष बीतते हैं, ब्याज का हिस्सा घटता है और मूल का बढ़ता है। इसका अर्थ यह है कि वही व्यक्ति जिसके लिए आज एक व्यवस्था बेहतर है, कुछ वर्षों बाद दूसरी व्यवस्था में बेहतर हो सकता है।',
          'इसीलिए यह तुलना एक बार करके भूल जाने वाली चीज़ नहीं है। गृह ऋण वाले व्यक्ति के लिए हर वर्ष दोबारा देखना सबसे अधिक मायने रखता है, क्योंकि उसकी स्थिति हर वर्ष अपने आप बदल रही होती है।',
        ],
        pEn: [
          'A home loan is the single factor that moves this comparison the most, which is why it deserves separate consideration.',
          'In the early years of a loan the larger part of each EMI is interest, and interest is where the biggest relief sits under the old regime. So during those first years the comparison often leans one way.',
          'As the years pass the interest component shrinks and the principal component grows. This means the same person for whom one regime is better today may be better off under the other a few years from now.',
          'That is why this is not a comparison to do once and forget. For somebody with a home loan, redoing it every year matters most, because their position is changing on its own each year.',
        ],
      },
      {
        hHi: 'कर बचाने के लिए ग़लत निवेश करना',
        hEn: 'Making the wrong investment in order to save tax',
        pHi: [
          'यह चेतावनी लेख में पहले भी है, पर इसे दोहराना ज़रूरी है क्योंकि यह हर वर्ष मार्च में सबसे अधिक होने वाली ग़लती है।',
          'जब कर बचाने की समय-सीमा पास आती है, तो लोग जल्दबाज़ी में ऐसे उत्पाद ख़रीद लेते हैं जो कर तो बचा देते हैं पर बाक़ी सभी दृष्टियों से अनुपयुक्त होते हैं — लंबी अवधि तक फँसा देने वाले, कम प्रतिफल वाले, या ऐसे जिनमें बीमा और निवेश मिला दिया गया हो।',
          'सही क्रम यह है: पहले तय कीजिए कि आपके लक्ष्यों के लिए कौन-सा साधन उपयुक्त है, फिर देखिए कि उनमें से कौन-सा कर लाभ भी देता है। कर लाभ चुनाव का कारण नहीं, उसका बोनस होना चाहिए।',
          'और यदि किसी वर्ष कोई उपयुक्त विकल्प न मिले, तो कर देना बुरा नहीं है। एक ग़लत उत्पाद पंद्रह साल तक साथ रहता है; एक वर्ष का अतिरिक्त कर एक बार में समाप्त हो जाता है।',
        ],
        pEn: [
          'This warning already appears earlier in the article, and it deserves repeating because it is the mistake made most often every March.',
          'As the deadline for tax-saving approaches, people hurry into products that do save tax but are unsuitable in every other respect — locking money up for long periods, returning little, or mixing insurance and investment into one thing.',
          'The correct order is: first decide which instrument suits your goals, then see which of those also carries a tax benefit. The tax benefit should be a bonus on the choice, not the reason for it.',
          'And if no suitable option presents itself in a particular year, paying the tax is not a disaster. A wrong product stays with you for fifteen years; one year of extra tax is over in a single payment.',
        ],
      },
      {
        hHi: 'दोनों व्यवस्थाओं में जो एक जैसा रहता है',
        hEn: 'What stays the same under both regimes',
        pHi: [
          'इस बहस में इतना ध्यान अंतरों पर जाता है कि समानताएँ छूट जाती हैं, जबकि व्यावहारिक जीवन में वही समानताएँ अधिक काम आती हैं।',
          'दोनों व्यवस्थाओं में रिटर्न भरना उतना ही ज़रूरी है, समय-सीमा वही है, और देर से भरने पर परिणाम वही हैं। व्यवस्था का चुनाव इस दायित्व को नहीं बदलता।',
          'दोनों में ही आपका फ़ॉर्म 26AS और वार्षिक सूचना विवरण वही जानकारी दिखाते हैं, और दोनों में ही उस जानकारी का आपके रिटर्न से मेल खाना ज़रूरी है।',
          'और सबसे महत्वपूर्ण: दोनों में ही रिकॉर्ड रखना उतना ही आवश्यक है। किस व्यवस्था में आप हैं, इससे यह नहीं बदलता कि प्रमाण माँगे जाने पर आपके पास होने चाहिए।',
        ],
        pEn: [
          'So much attention goes to the differences in this debate that the similarities get missed, even though in daily practice the similarities matter more.',
          'Filing a return is equally necessary under both, the deadline is the same, and the consequences of filing late are the same. Choosing a regime does not change that obligation at all.',
          'Under both, your Form 26AS and Annual Information Statement show the same information, and under both that information has to match what your return says.',
          'And most importantly: keeping records is equally necessary under either. Which regime you are in does not change the fact that if proof is asked for, you need to have it.',
        ],
      },
    ],
  },

  {
    slug: 'credit-score-improve-guide',
    sections: [
      {
        hHi: 'स्कोर बनने में कितना समय लगता है',
        hEn: 'How long the score actually takes to build',
        pHi: [
          'सबसे आम अपेक्षा यह होती है कि सुधार कुछ हफ़्तों में दिखेगा, और यही अपेक्षा सबसे अधिक निराशा पैदा करती है। स्कोर एक धीमी व्यवस्था है, और उसका धीमापन जानबूझकर है।',
          'सामान्य तौर पर किसी भी बदलाव का असर दिखने में एक से तीन महीने लगते हैं, क्योंकि ऋणदाता अपनी रिपोर्ट मासिक भेजते हैं। इसलिए आज किया गया भुगतान अगले महीने से पहले रिपोर्ट में नहीं आएगा।',
          'बड़े सुधार — जैसे बहुत कम स्कोर से 750 के ऊपर जाना — में छह महीने से दो साल तक लग सकते हैं। यहाँ कोई शॉर्टकट नहीं है, और जो कोई शॉर्टकट बेच रहा है वह कुछ और बेच रहा है।',
          'इसका उपयोगी पक्ष यह है कि धीमापन दोनों दिशाओं में काम करता है। जिस तरह सुधार धीरे आता है, उसी तरह एक चूक से पूरा स्कोर तुरंत नष्ट नहीं होता — यदि उसके बाद व्यवहार सुधर जाए।',
        ],
        pEn: [
          'The most common expectation is that improvement will show within a few weeks, and that expectation causes most of the disappointment. The score is a slow system, and its slowness is deliberate.',
          'In general any change takes one to three months to appear, because lenders send their reports monthly. So a payment made today will not show in the report before next month at the earliest.',
          'Large improvements — moving from a very low score to above 750 — can take anywhere from six months to two years. There is no shortcut here, and anybody selling one is selling something else.',
          'The useful side of this is that the slowness works in both directions. Just as improvement arrives gradually, one slip does not destroy the whole score instantly, provided behaviour improves afterwards.',
        ],
      },
      {
        hHi: 'क्रेडिट उपयोग अनुपात का सही प्रबंधन',
        hEn: 'Managing the credit utilisation ratio properly',
        pHi: [
          'यह स्कोर का दूसरा सबसे बड़ा घटक है और शायद वह जिस पर सबसे तेज़ी से काम किया जा सकता है, क्योंकि इसमें पुराना इतिहास बदलने की ज़रूरत नहीं।',
          'अनुपात का अर्थ है: आपने अपनी उपलब्ध सीमा का कितना हिस्सा इस्तेमाल किया। यदि सीमा एक लाख है और बकाया साठ हज़ार है, तो उपयोग साठ प्रतिशत है — और यह ऊँचा माना जाता है, भले ही आप पूरा भुगतान समय पर करते हों।',
          'यहाँ एक विवरण महत्वपूर्ण है जो अधिकांश लोग नहीं जानते: रिपोर्ट में वह राशि जाती है जो विवरण-तिथि पर बकाया थी, न कि वह जो आपने अंततः चुकाई। इसलिए यदि आप हर महीने पूरा भुगतान करते हैं पर विवरण-तिथि पर बकाया ऊँचा रहता है, तो रिपोर्ट में ऊँचा उपयोग ही जाएगा।',
          'इसका सरल समाधान यह है कि विवरण-तिथि से कुछ दिन पहले आंशिक भुगतान कर दिया जाए। यह छोटा-सा समय-प्रबंधन बिना कुछ और बदले उपयोग अनुपात को नीचे ले आता है।',
          'दूसरा समाधान सीमा बढ़वाना है, बशर्ते ख़र्च न बढ़े। वही ख़र्च बड़ी सीमा पर कम प्रतिशत बनता है — पर यह तभी काम करता है जब बढ़ी हुई सीमा एक प्रलोभन न बन जाए।',
        ],
        pEn: [
          'This is the second largest component of the score and probably the one that can be improved fastest, because it does not require changing any past history.',
          'The ratio means how much of your available limit you have used. If the limit is one lakh and the outstanding is sixty thousand, utilisation is sixty percent — and that counts as high even if you pay in full and on time.',
          'One detail matters here that most people do not know: what goes into the report is the amount outstanding on the statement date, not what you eventually paid. So if you clear the bill in full every month but the balance is high on the statement date, the report still shows high utilisation.',
          'The simple fix is to make a part payment a few days before the statement date. That small piece of timing brings the ratio down without changing anything else about your spending.',
          'The second fix is asking for a higher limit, provided spending does not rise with it. The same spend becomes a smaller percentage of a larger limit — but this only works if the higher limit does not turn into a temptation.',
        ],
      },
      {
        hHi: 'रिपोर्ट में ग़लती मिलने पर क्या करें',
        hEn: 'What to do when the report contains an error',
        pHi: [
          'क्रेडिट रिपोर्ट में ग़लतियाँ असामान्य नहीं हैं, और वे प्रायः तभी पकड़ में आती हैं जब कोई ऋण अस्वीकार हो जाता है। इसलिए साल में एक बार अपनी रिपोर्ट देख लेना सबसे सस्ता निवारक उपाय है।',
          'सबसे आम ग़लतियाँ चार प्रकार की होती हैं: कोई ऐसा खाता जो आपका है ही नहीं, बंद किया हुआ खाता जो अब भी खुला दिख रहा है, चुकाया हुआ ऋण जो बकाया दिख रहा है, और व्यक्तिगत विवरण की गड़बड़ी जिससे किसी और का रिकॉर्ड आपके साथ जुड़ गया हो।',
          'सुधार की प्रक्रिया सीधी है। संबंधित क्रेडिट सूचना कंपनी के विवाद-पोर्टल पर आपत्ति दर्ज कीजिए, प्रमाण संलग्न कीजिए, और शिकायत संख्या सुरक्षित रखिए। कंपनी को एक निर्धारित अवधि में जाँच करनी होती है।',
          'यदि ग़लती किसी बैंक की रिपोर्टिंग से आई है, तो समानांतर रूप से उस बैंक को भी लिखिए। दोनों ओर से एक साथ चलने पर सुधार तेज़ होता है, और लिखित रिकॉर्ड बाद में काम आता है।',
        ],
        pEn: [
          'Errors in a credit report are not unusual, and they are usually discovered only when a loan is refused. Checking your own report once a year is therefore the cheapest preventive step available.',
          'The four most common errors are: an account that is not yours at all, a closed account still showing as open, a repaid loan still showing as outstanding, and a mix-up in personal details that has attached somebody else\'s record to yours.',
          'The correction process is straightforward. Raise a dispute on the credit information company\'s portal, attach evidence, and keep the complaint number. The company has to investigate within a set period.',
          'If the error originates in a bank\'s reporting, write to that bank in parallel. Working both sides at once speeds things up, and the written record is useful afterwards.',
        ],
      },
      {
        hHi: 'पुराने कार्ड बंद करने का उल्टा असर',
        hEn: 'Why closing old cards can work against you',
        pHi: [
          'सफ़ाई की भावना से लोग पुराने और अनुपयोगी क्रेडिट कार्ड बंद कर देते हैं, और यह सहज लगने वाला क़दम अक्सर स्कोर को नुक़सान पहुँचाता है।',
          'दो कारण हैं। पहला, आपके खातों की औसत आयु स्कोर का एक घटक है, और सबसे पुराना कार्ड बंद करने से वह औसत गिर जाती है। दूसरा, उस कार्ड की सीमा भी हट जाती है, जिससे कुल उपलब्ध सीमा घटती है और उपयोग अनुपात बढ़ जाता है।',
          'यानी एक ही क़दम से दो घटक बिगड़ते हैं। यदि कार्ड पर कोई वार्षिक शुल्क नहीं है, तो उसे खुला रखना और साल में एक-दो छोटे लेन-देन कर देना बेहतर है, ताकि वह निष्क्रिय होकर बंद न हो जाए।',
          'यदि वार्षिक शुल्क लग रहा है और कार्ड बेकार है, तो बंद करना ठीक है — पर उसे तब कीजिए जब आप निकट भविष्य में कोई ऋण नहीं लेने वाले हों, ताकि अस्थायी गिरावट किसी आवेदन के समय न पड़े।',
        ],
        pEn: [
          'Out of a sense of tidiness people close old and unused credit cards, and that intuitive step frequently damages the score instead of helping it.',
          'There are two reasons. First, the average age of your accounts is a component of the score, and closing the oldest card pulls that average down. Second, the limit on that card disappears too, which reduces the total available limit and pushes utilisation up.',
          'So a single action worsens two components at once. If the card carries no annual fee, keeping it open and putting one or two small transactions through it a year is better, so that it is not closed for inactivity.',
          'If an annual fee is being charged and the card is genuinely useless, closing it is fine — but do it when you are not about to apply for a loan, so the temporary dip does not land on an application.',
        ],
      },
      {
        hHi: 'सह-आवेदक और गारंटर बनने का जोखिम',
        hEn: 'The risk of being a co-applicant or guarantor',
        pHi: [
          'यह वह श्रेणी है जिसमें लोग अपने स्कोर को बिना कुछ उधार लिए नुक़सान पहुँचा बैठते हैं, और प्रायः किसी की मदद करने की नीयत से।',
          'जब आप किसी ऋण में सह-आवेदक या गारंटर बनते हैं, तो वह ऋण आपकी क्रेडिट रिपोर्ट में भी दर्ज होता है। यदि मुख्य उधारकर्ता चूकता है, तो वह चूक आपके रिकॉर्ड में भी जाती है।',
          'इसका एक दूसरा असर भी है जो कम दिखता है: वह ऋण आपके कुल दायित्व में गिना जाता है, इसलिए आपकी अपनी ऋण-क्षमता उतनी ही घट जाती है। कई लोगों को यह तब पता चलता है जब उनका अपना गृह ऋण अस्वीकार हो जाता है।',
          'इसका अर्थ यह नहीं कि किसी की मदद न कीजिए। इसका अर्थ केवल यह है कि यह निर्णय उतनी ही गंभीरता से लीजिए जितनी गंभीरता से आप अपने लिए वही ऋण लेते — क्योंकि रिकॉर्ड की दृष्टि से वह लगभग वही है।',
        ],
        pEn: [
          'This is the category in which people damage their own score without borrowing anything themselves, and usually while intending to help somebody.',
          'When you become a co-applicant or guarantor on a loan, that loan appears in your credit report as well. If the main borrower defaults, the default is recorded against you too.',
          'There is a second, less visible effect: the loan counts towards your total obligations, so your own borrowing capacity falls by that much. Many people discover this only when their own home loan application is refused.',
          'None of this means you should not help somebody. It only means treating the decision as seriously as you would treat taking that same loan for yourself — because in record terms it is very nearly the same thing.',
        ],
      },
      {
        hHi: 'शून्य इतिहास वाले व्यक्ति के लिए पहला क़दम',
        hEn: 'The first step for somebody with no history at all',
        pHi: [
          'एक स्थिति ऐसी है जिसकी चर्चा कम होती है: वह व्यक्ति जिसका स्कोर ख़राब नहीं बल्कि है ही नहीं, क्योंकि उसने कभी कुछ उधार नहीं लिया।',
          'यह विरोधाभासी लगता है पर ऋणदाता की दृष्टि से समझ में आता है — उसके पास आपके व्यवहार का कोई प्रमाण नहीं है, और प्रमाण न होना अच्छे प्रमाण जैसा नहीं होता।',
          'शुरुआत के दो सामान्य रास्ते हैं। पहला, किसी सावधि जमा के विरुद्ध जारी किया गया सुरक्षित क्रेडिट कार्ड, जिसमें ऋणदाता का जोखिम कम होता है और इसलिए स्वीकृति आसान होती है। दूसरा, कोई छोटा उपभोक्ता ऋण जिसे नियमित रूप से चुकाया जाए।',
          'दोनों में असली काम वही है जो हमेशा होता है: हर किश्त समय पर, हर बार। छह से बारह महीने का साफ़ रिकॉर्ड बन जाने पर स्कोर अपने आप अस्तित्व में आ जाता है और आगे के दरवाज़े खुलने लगते हैं।',
        ],
        pEn: [
          'One situation gets less discussion than it deserves: the person whose score is not bad but simply absent, because they have never borrowed anything.',
          'It sounds contradictory and it makes sense from a lender\'s side — they have no evidence of your behaviour, and no evidence is not the same as good evidence.',
          'There are two usual ways in. The first is a secured credit card issued against a fixed deposit, where the lender\'s risk is low and approval is therefore easy. The second is a small consumer loan repaid regularly.',
          'In both cases the real work is the same as always: every instalment on time, every time. Once six to twelve months of clean record exists, the score comes into being on its own and the further doors begin to open.',
        ],
      },
    ],
  },

  {
    slug: 'gullak-se-demat-tak-teen-padav',
    sections: [
      {
        hHi: 'किशोर की पहली कमाई',
        hEn: 'A teenager\'s first earnings',
        pHi: [
          'डिजिटल पैसे की समझ में सबसे बड़ा मोड़ तब आता है जब पैसा माता-पिता से नहीं, बाहर से आने लगता है। ट्यूशन, किसी छोटे काम, या किसी ऑनलाइन काम से मिली पहली कमाई एक अलग तरह का पाठ होती है।',
          'यहाँ पहली बात यह समझाने की है कि कमाई और ख़र्च के बीच का अंतर ही असली आय है। जो किशोर पाँच हज़ार कमाकर पाँच हज़ार ख़र्च कर देता है, उसने कमाया कुछ नहीं — यह वाक्य इस उम्र में सुनना बाद के बीस साल बचा सकता है।',
          'दूसरी बात कर की है, भले ही अभी लागू न हो। यह बता देना कि कमाई पर कर लगता है और उसका हिसाब रखना पड़ता है, बाद में उस झटके को कम कर देता है जो पहली नौकरी की पहली सैलरी स्लिप पर लगता है।',
          'तीसरी और सबसे उपयोगी बात यह है कि पहली कमाई का कुछ हिस्सा तुरंत अलग रख दिया जाए — किसी भी अनुपात में, पर हर बार। यह आदत राशि से कहीं अधिक महत्वपूर्ण है।',
        ],
        pEn: [
          'The largest turn in understanding digital money comes when money starts arriving from outside rather than from parents. First earnings from tutoring, a small job, or some online work are a different kind of lesson entirely.',
          'The first thing to explain here is that real income is the gap between earning and spending. A teenager who earns five thousand and spends five thousand has earned nothing — hearing that sentence at this age can save the next twenty years.',
          'The second is tax, even if none is payable yet. Explaining that earnings are taxed and have to be accounted for softens the shock that otherwise lands on the first salary slip of a first job.',
          'The third and most useful is setting aside part of the first earnings immediately — in any proportion at all, but every time. That habit matters far more than the amount involved.',
        ],
      },
      {
        hHi: 'किशोरों को निशाना बनाने वाली ठगी',
        hEn: 'The scams that target teenagers specifically',
        pHi: [
          'किशोर UPI और ऑनलाइन भुगतान में तकनीकी रूप से माता-पिता से अधिक सहज होते हैं, और यही सहजता एक अलग तरह का जोखिम बनाती है — वे झिझकते नहीं, इसलिए रुकते भी नहीं।',
          'उन तक पहुँचने वाली ठगी अलग शक्ल की होती है। गेम में मुफ़्त सामग्री का लालच, किसी प्रसिद्ध व्यक्ति के नाम पर बना नक़ली खाता, "आपका अकाउंट जीत गया है" जैसे संदेश, और सस्ते दाम पर कोई डिजिटल वस्तु बेचने का प्रस्ताव।',
          'इनमें सबसे आम वह है जिसमें भुगतान पहले माँगा जाता है और सामान बाद में देने का वादा होता है। किशोर अक्सर छोटी राशि होने के कारण जोखिम उठा लेते हैं, और ठग की पूरी अर्थव्यवस्था इसी छोटी राशि पर चलती है।',
          'बचाव का सबसे अच्छा तरीक़ा नियम नहीं बल्कि बातचीत है। यदि घर में यह तय हो कि किसी भी ऑनलाइन भुगतान से पहले एक बार किसी को बता देना है, तो अधिकांश ठगी उसी क्षण रुक जाती है — क्योंकि उन्हें बोलकर बताने से ही वे बेतुकी लगने लगती हैं।',
        ],
        pEn: [
          'Teenagers are technically more comfortable with UPI and online payments than their parents, and that ease creates a different kind of risk — they do not hesitate, so they do not pause either.',
          'The scams that reach them have a different shape. Free items inside a game, a fake account in a well-known person\'s name, messages saying "your account has won", and offers of some digital item at an unusually cheap price.',
          'The most common of these asks for payment first and promises delivery afterwards. Teenagers often take the risk because the amount is small, and the entire economics of that scam runs on exactly those small amounts.',
          'The best protection is a conversation rather than a rule. If the household agrees that any online payment gets mentioned to somebody first, most scams stop right there — because saying them out loud is usually enough to make them sound absurd.',
        ],
      },
      {
        hHi: 'किशोर का अपना बजट',
        hEn: 'A teenager\'s own budget',
        pHi: [
          'इस उम्र में बजट सिखाने का सबसे अच्छा तरीक़ा एक जटिल व्यवस्था नहीं बल्कि एक सरल, वास्तविक ज़िम्मेदारी है।',
          'एक कारगर तरीक़ा यह है कि महीने भर की किसी एक श्रेणी का पूरा ज़िम्मा किशोर को दे दिया जाए — मान लीजिए आने-जाने का ख़र्च, या फ़ोन का रिचार्ज, या कपड़ों का बजट। पूरी राशि एक बार दी जाए और महीने भर उसी से चलाना हो।',
          'इससे दो चीज़ें एक साथ होती हैं। पहली, योजना बनानी पड़ती है, क्योंकि पैसा महीने के बीच में ख़त्म हो सकता है। दूसरी, बचाने का लाभ सीधे उसी को मिलता है, जो प्रेरणा का सबसे मज़बूत रूप है।',
          'यहाँ माता-पिता के लिए सबसे कठिन हिस्सा हस्तक्षेप न करना है। यदि पैसा बीस तारीख़ को ख़त्म हो जाए, तो बाक़ी दस दिन कठिन होने चाहिए — यही पूरे अभ्यास का मूल्य है, और यह जितनी जल्दी सीखा जाए उतना सस्ता पड़ता है।',
        ],
        pEn: [
          'The best way to teach budgeting at this age is not an elaborate system but one simple, real responsibility.',
          'An effective method is to hand a teenager complete charge of a single category for the month — travel costs, say, or the phone recharge, or a clothing budget. The full amount is given once and has to last the month.',
          'Two things happen together. Planning becomes necessary, because the money can run out mid-month. And the benefit of saving goes directly to them, which is the strongest form of motivation there is.',
          'The hardest part here is for the parent: not intervening. If the money runs out on the twentieth, the remaining ten days should be difficult — that is the entire value of the exercise, and the earlier it is learned the cheaper it is.',
        ],
      },
      {
        hHi: 'साथियों का दबाव और दिखावे का ख़र्च',
        hEn: 'Peer pressure and spending to be seen',
        pHi: [
          'यह वह ख़र्च है जो किसी बजट में नहीं दिखता और फिर भी सबसे अधिक असर डालता है, और यह किशोरावस्था में सबसे तीव्र होता है।',
          'इस पर उपदेश काम नहीं करता, क्योंकि दबाव वास्तविक है और उसे नकारने से बातचीत ही बंद हो जाती है। जो काम करता है वह है इसे नाम देना — यह बता देना कि हाँ, यह दबाव मौजूद है, और सबको महसूस होता है।',
          'उसके बाद एक व्यावहारिक सवाल दिया जा सकता है जो जीवन भर काम आता है: "यदि कोई देखने वाला न होता, तो क्या मैं फिर भी यह ख़रीदता?" यह सवाल मना नहीं करता, केवल एक क्षण रुकवाता है — और अक्सर एक क्षण पर्याप्त होता है।',
          'तीसरी बात यह बताने की है कि दिखावे का ख़र्च वयस्कों में ख़त्म नहीं होता, केवल महँगा हो जाता है — गाड़ी, शादी, घर। इस उम्र में यह समझ लेना बाद के बड़े निर्णयों को कहीं शांत बना देता है।',
        ],
        pEn: [
          'This is the spending that appears in no budget and still does the most damage, and it is at its most intense during the teenage years.',
          'Lecturing does not work here, because the pressure is real and denying it simply ends the conversation. What works is naming it — saying plainly that yes, this pressure exists and everybody feels it.',
          'After that a practical question can be offered which keeps working for a lifetime: "if nobody were going to see it, would I still buy this?" The question forbids nothing; it only creates a pause — and a pause is very often enough.',
          'The third thing worth saying is that spending to be seen does not end in adulthood, it only gets more expensive — cars, weddings, houses. Understanding that at this age makes the larger decisions later considerably calmer.',
        ],
      },
      {
        hHi: 'कब पूरा नियंत्रण सौंप देना चाहिए',
        hEn: 'When to hand over full control',
        pHi: [
          'तीनों पड़ावों का अंतिम लक्ष्य निगरानी नहीं, स्वतंत्रता है — और यह तय करना ज़रूरी है कि वह स्वतंत्रता कब पूरी तरह सौंपी जाए।',
          'एक व्यावहारिक संकेत यह है: जब किशोर लगातार तीन-चार महीने तक अपने बजट में रह जाए, बिना याद दिलाए, तो अगला स्तर देने का समय आ गया है। नियंत्रण उम्र से नहीं, प्रदर्शन से सौंपा जाना चाहिए।',
          'सौंपने का अर्थ यह नहीं कि दरवाज़ा बंद हो जाए। एक अच्छा ढाँचा यह है: निर्णय पूरी तरह उनके, पर हर कुछ महीनों में एक बार साथ बैठकर देखना — सलाहकार की तरह, निरीक्षक की तरह नहीं।',
          'और सबसे महत्वपूर्ण: जब वे कोई ग़लत निर्णय लें, तो "मैंने कहा था" मत कहिए। जो बच्चा जानता है कि ग़लती के बाद भी बात की जा सकती है, वह अगली बार पहले पूछेगा — और यही इस पूरी प्रक्रिया का असली उद्देश्य है।',
        ],
        pEn: [
          'The goal of all three stages is not supervision but independence — and it is worth deciding when that independence is handed over completely.',
          'A practical signal: when a teenager stays inside their budget for three or four consecutive months without being reminded, it is time for the next level. Control should be handed over on the basis of performance rather than age.',
          'Handing over does not mean closing the door. A good arrangement is that the decisions are entirely theirs, with a joint review every few months — as an adviser rather than an inspector.',
          'And most importantly: when they make a bad decision, do not say "I told you so". A child who knows that a mistake can still be talked about will ask first next time — and that is the real purpose of the whole process.',
        ],
      },
      {
        hHi: 'तीनों पड़ावों में जो एक जैसा रहता है',
        hEn: 'What stays constant across all three stages',
        pHi: [
          'गुल्लक से UPI तक जो कुछ बदलता है वह माध्यम है; जो नहीं बदलता वह तीन बुनियादी विचार हैं, और उन्हें हर पड़ाव पर दोहराया जाना चाहिए।',
          'पहला: पैसा सीमित है, और हर हाँ किसी और चीज़ के लिए ना है। यह विचार पाँच साल की उम्र में गुल्लक से समझ आता है और पैंतीस की उम्र में गृह ऋण के समय भी वही रहता है।',
          'दूसरा: इंतज़ार का मूल्य होता है। जो आज छोड़ा गया वह कल बड़ा होकर लौटता है — पहले टॉफ़ी के रूप में, बाद में चक्रवृद्धि के रूप में, पर तर्क वही है।',
          'तीसरा: पैसा एक साधन है, माप नहीं। जो बच्चा यह मानकर बड़ा होता है कि उसकी क़ीमत उसके पास मौजूद चीज़ों से तय होती है, वह जीवन भर एक ऐसी दौड़ में रहता है जिसका कोई अंत नहीं।',
          'यदि ये तीन विचार पहुँच जाएँ, तो बाक़ी सब — खाता, ऐप, निवेश — केवल विवरण हैं, और वे अपने समय पर अपने आप सीख लिए जाते हैं।',
        ],
        pEn: [
          'What changes from piggy bank to UPI is the medium; what does not change are three basic ideas, and they deserve repeating at every stage.',
          'First: money is finite, and every yes is a no to something else. That idea is learned at five from a piggy bank and it is the same idea at thirty-five in front of a home loan.',
          'Second: waiting has value. What is given up today comes back larger tomorrow — first as a sweet, later as compounding, but the logic is identical.',
          'Third: money is a tool, not a measure. A child who grows up believing their worth is set by what they own spends a lifetime in a race that has no finishing line.',
          'If those three ideas land, everything else — the account, the app, the investing — is only detail, and detail gets picked up on its own at the right time.',
        ],
      },
    ],
  },

  // ---------------------------------------------------------------------
  // Batch three.
  // ---------------------------------------------------------------------

  {
    slug: 'emergency-fund-kaise-banaye',
    sections: [
      {
        hHi: 'जब बचाने को कुछ बचता ही नहीं',
        hEn: 'When there is nothing left over to save',
        pHi: [
          'सबसे ईमानदार आपत्ति यही है, और अधिकांश लेख इसे टाल जाते हैं। यदि महीने के अंत में कुछ बचता ही नहीं, तो कोष कैसे बने?',
          'पहला उत्तर क्रम बदलने का है। जो लोग "बचने के बाद बचाते हैं" वे कभी नहीं बचा पाते, क्योंकि ख़र्च हमेशा उपलब्ध राशि तक फैल जाता है। जो लोग वेतन आते ही एक छोटी राशि हटा देते हैं, वे बाक़ी में गुज़ारा कर लेते हैं — यही एकमात्र अंतर है।',
          'दूसरा उत्तर राशि छोटी करने का है। पाँच सौ रुपये महीने भी काम करते हैं। यह सुनने में बेकार लगता है, पर उद्देश्य पहले साल में कोष पूरा करना नहीं, यह साबित करना है कि यह संभव है।',
          'तीसरा उत्तर सबसे कठिन है: एक श्रेणी चुनकर उसे तीन महीने के लिए घटाना। कोई एक — बाहर का खाना, कोई सदस्यता, कोई आदत। तीन महीने बाद वह राशि अपने आप उपलब्ध लगने लगती है, और तब उसे स्थायी कर दीजिए।',
        ],
        pEn: [
          'This is the most honest objection to the whole idea, and most articles skirt around it. If nothing is left at the end of the month, how is a fund supposed to appear?',
          'The first answer is to reverse the order. People who save what is left never manage to save, because spending expands to fill whatever is available. People who move a small amount out the moment salary arrives manage on the rest — that is the entire difference.',
          'The second answer is to make the amount small. Five hundred rupees a month works. It sounds pointless, but the purpose in the first year is not to complete the fund; it is to prove to yourself that this is possible at all.',
          'The third answer is the hardest: pick one category and cut it for three months. Just one — eating out, a subscription, a habit. After three months that amount starts to feel available on its own, and at that point make it permanent.',
        ],
      },
      {
        hHi: 'अनपेक्षित पैसे का सही उपयोग',
        hEn: 'What to do with money you did not expect',
        pHi: [
          'कोष बनाने का सबसे तेज़ रास्ता मासिक बचत नहीं, बल्कि वह पैसा है जो अनपेक्षित रूप से आता है — बोनस, कर वापसी, कोई पुराना बकाया, कोई उपहार।',
          'ऐसा पैसा एक ख़ास मनोवैज्ञानिक श्रेणी में आता है। चूँकि वह बजट में था ही नहीं, इसलिए उसे ख़र्च करने पर कोई कमी महसूस नहीं होती — और इसी कारण वह लगभग हमेशा ख़र्च हो जाता है।',
          'एक सरल नियम इसे बदल देता है: कोई भी अनपेक्षित राशि आते ही उसका आधा सीधे कोष में। आधा इसलिए, क्योंकि पूरा रखने का नियम कोई निभा नहीं पाता, और आधा निभाया जा सकता है।',
          'इस एक नियम से कोष का वह हिस्सा बनता है जो मासिक बचत से बनने में वर्षों लेता। और इसका सबसे अच्छा पक्ष यह है कि इसमें जीवन-स्तर में कोई कटौती नहीं करनी पड़ती — बचा हुआ आधा अब भी आपका है।',
        ],
        pEn: [
          'The fastest route to building the fund is not monthly saving but money that arrives unexpectedly — a bonus, a tax refund, an old due finally paid, a gift.',
          'Money like this falls into a particular psychological category. Because it was never in the budget, spending it produces no feeling of loss — and that is precisely why it almost always gets spent.',
          'One simple rule changes this: half of any unexpected amount goes straight into the fund the day it arrives. Half rather than all, because nobody keeps a rule that demands all of it, and a rule about half can actually be kept.',
          'That single rule builds the portion of the fund that monthly saving would take years to reach. And the best part is that it requires no cut in your standard of living at all — the other half is still yours.',
        ],
      },
      {
        hHi: 'कोष को दो हिस्सों में बाँटना',
        hEn: 'Splitting the fund into two parts',
        pHi: [
          'एक व्यावहारिक ढाँचा जो कम चर्चा में आता है वह यह है कि पूरा कोष एक ही जगह न रखा जाए, बल्कि दो परतों में बाँटा जाए।',
          'पहली परत तत्काल पहुँच की है — लगभग एक महीने का ख़र्च, बचत खाते में, जिसे किसी भी क्षण निकाला जा सके। यह वह हिस्सा है जो आधी रात को काम आता है।',
          'दूसरी परत बाक़ी राशि है, जो ऐसी जगह रखी जा सकती है जहाँ से एक-दो दिन में पैसा आ जाए और जो थोड़ा बेहतर ब्याज दे। यह हिस्सा तत्काल नहीं चाहिए, क्योंकि पहली परत उतना समय ख़रीद देती है।',
          'इस बँटवारे के दो लाभ हैं। पहला, पूरे कोष पर बचत खाते जैसा कम ब्याज नहीं मिलता। दूसरा, और शायद अधिक महत्वपूर्ण, दूसरी परत तक पहुँचने में जो एक-दो दिन लगते हैं वे एक स्वाभाविक बाधा बन जाते हैं — और वह बाधा उसे अनावश्यक ख़र्च से बचा लेती है।',
        ],
        pEn: [
          'A practical structure that gets little discussion is not keeping the whole fund in one place but splitting it into two layers.',
          'The first layer is immediate access — roughly one month of expenses, in a savings account, reachable at any moment. This is the part that matters at midnight.',
          'The second layer is the remainder, which can sit somewhere that takes a day or two to reach and pays slightly better interest. This part does not need to be instant, because the first layer has already bought that much time.',
          'The split has two benefits. The first is that the whole fund is not earning savings-account interest. The second, and perhaps more important, is that the day or two needed to reach the second layer becomes a natural friction — and that friction protects it from unnecessary spending.',
        ],
      },
      {
        hHi: 'यदि क़र्ज़ भी है तो पहले क्या',
        hEn: 'If there is debt as well, what comes first',
        pHi: [
          'यह सबसे आम असली दुविधा है: कोष बनाना है या पहले क़र्ज़ चुकाना है? दोनों तर्क सही लगते हैं और दोनों को एक साथ करना संभव नहीं लगता।',
          'व्यावहारिक उत्तर बीच का है। पहले एक छोटा कोष बनाइए — एक महीने के ख़र्च जितना, पूरा छह महीने नहीं। फिर पूरी शक्ति ऊँचे ब्याज वाले क़र्ज़ पर लगाइए। क़र्ज़ ख़त्म होने के बाद कोष को उसके पूरे आकार तक ले जाइए।',
          'इस क्रम का कारण गणित नहीं, व्यवहार है। यदि कोष बिल्कुल न हो और कोई छोटी आपात स्थिति आए, तो आप फिर से क़र्ज़ लेंगे — और तब चुकाने की सारी मेहनत बेकार हो जाएगी। एक महीने का कोष उसी चक्र को तोड़ता है।',
          'यहाँ "ऊँचे ब्याज" का अर्थ स्पष्ट होना चाहिए: क्रेडिट कार्ड का बकाया और व्यक्तिगत ऋण इस श्रेणी में आते हैं। गृह ऋण जैसे कम ब्याज वाले दीर्घकालिक ऋण के लिए यह तर्क उतना मज़बूत नहीं है, और वहाँ कोष बनाना साथ-साथ चल सकता है।',
        ],
        pEn: [
          'This is the most common real dilemma: build the fund or clear the debt first? Both arguments sound correct and doing both at once feels impossible.',
          'The practical answer sits in the middle. Build a small fund first — about one month of expenses, not the full six. Then put everything into the high-interest debt. Once that is cleared, take the fund up to its full size.',
          'The reason for this order is behavioural rather than mathematical. With no fund at all, a small emergency means borrowing again — and then all the repayment effort is undone. A one-month fund is what breaks that cycle.',
          'What "high interest" means should be stated plainly here: credit card balances and personal loans belong in that category. For low-rate long-term borrowing such as a home loan the argument is much weaker, and there the fund can be built alongside.',
        ],
      },
      {
        hHi: 'क्रेडिट कार्ड आपातकालीन कोष नहीं है',
        hEn: 'A credit card is not an emergency fund',
        pHi: [
          'यह तर्क बार-बार सुनाई देता है: "मेरे पास कार्ड की सीमा है, वही मेरा कोष है।" यह तर्क तीन जगह टूटता है, और तीनों ठीक उसी समय टूटते हैं जब ज़रूरत होती है।',
          'पहला, कार्ड पैसा नहीं देता, उधार देता है। आपात के बाद आपके सामने वही समस्या रहती है और उसके ऊपर ब्याज भी जुड़ जाता है — और वह ब्याज दर सबसे ऊँची होती है जो आमतौर पर मिलती है।',
          'दूसरा, कई आपात स्थितियों में कार्ड चलता ही नहीं। नक़द की ज़रूरत, किसी छोटे शहर का अस्पताल, कोई ऐसा भुगतान जो कार्ड स्वीकार न करे — ये सब सामान्य स्थितियाँ हैं।',
          'तीसरा और सबसे गंभीर: जिस आपात में आय रुकती है, उसी में कार्ड चुकाने की क्षमता भी रुकती है। नौकरी जाने पर कार्ड का बकाया समस्या का समाधान नहीं, समस्या का दूसरा हिस्सा बन जाता है।',
          'कार्ड की सीमा को कोष का पूरक मानिए — पहले दिन का पुल, जब तक कोष तक पहुँचा न जाए। उसे कोष का विकल्प मानना ही वह ग़लती है जो सबसे महँगी पड़ती है।',
        ],
        pEn: [
          'This argument comes up constantly: "I have a card limit, that is my emergency fund." It breaks in three places, and all three break at exactly the moment the money is needed.',
          'First, a card does not give you money; it lends it. After the emergency the same problem remains with interest added on top — and that interest rate is the highest ordinarily available anywhere.',
          'Second, in many emergencies a card simply does not work. A need for cash, a hospital in a smaller town, a payment that does not accept cards — all of these are ordinary situations rather than unusual ones.',
          'Third and most serious: the emergency in which income stops is the same one in which the ability to repay the card stops. When a job goes, a card balance is not a solution to the problem but a second part of it.',
          'Treat a card limit as a supplement to the fund — a bridge for the first day, until the fund can be reached. Treating it as a replacement is the mistake that costs the most.',
        ],
      },
      {
        hHi: 'रास्ते के पड़ाव और यथार्थवादी समय',
        hEn: 'Milestones along the way, and a realistic timeline',
        pHi: [
          'छह महीने का लक्ष्य दूर से देखने पर हतोत्साहित करने वाला लगता है, और यही कारण है कि बहुत से लोग शुरू ही नहीं करते। इसे छोटे पड़ावों में बाँट देने से यह पूरी तरह अलग काम लगने लगता है।',
          'पहला पड़ाव: दस हज़ार रुपये। यह किसी भी बड़े ख़र्च के लिए पर्याप्त नहीं है, पर यह अधिकांश छोटी परेशानियों को सँभाल लेता है और सबसे महत्वपूर्ण बात — यह साबित कर देता है कि व्यवस्था चल रही है।',
          'दूसरा पड़ाव: एक महीने का ख़र्च। यहाँ पहुँचने पर मन का बोझ स्पष्ट रूप से हल्का होता है, और अधिकांश लोग यही बताते हैं कि असली बदलाव इसी बिंदु पर महसूस हुआ।',
          'तीसरा पड़ाव: तीन महीने। चौथा: आपकी अपनी गणना से निकली पूरी संख्या। हर पड़ाव पर रुककर उसे दर्ज कीजिए — प्रगति दिखना ही वह चीज़ है जो अगले पड़ाव तक ले जाती है।',
          'और समय के बारे में ईमानदारी: सामान्य आय पर पूरा कोष बनने में दो से तीन साल लग सकते हैं। यह लंबा लगता है, और फिर भी वह अवधि किसी भी हाल में बीतेगी — सवाल केवल यह है कि उसके अंत में कोष होगा या नहीं।',
        ],
        pEn: [
          'A six-month target looks discouraging from a distance, and that is exactly why many people never start. Broken into smaller milestones it becomes an entirely different task.',
          'First milestone: ten thousand rupees. It is not enough for any large expense, but it handles most small trouble, and more importantly it proves that the system is running.',
          'Second milestone: one month of expenses. Reaching this point lightens the mental load noticeably, and most people report that the real change in how they felt happened here.',
          'Third milestone: three months. Fourth: the full number your own calculation produced. Stop and record each one — seeing progress is the thing that carries you to the next stage.',
          'And an honest word about time: on an ordinary income the full fund can take two to three years. That sounds long, and those years will pass regardless — the only question is whether there is a fund at the end of them.',
        ],
      },
    ],
  },

  {
    slug: 'gold-investment-options-india',
    sections: [
      {
        hHi: 'गहनों की छिपी हुई लागत',
        hEn: 'The hidden costs inside jewellery',
        pHi: [
          'भारत में सोना ख़रीदने का सबसे आम रूप गहने हैं, और यही वह रूप है जिसमें निवेश के लिहाज़ से सबसे अधिक रिसाव होता है। यह बात गहनों के विरुद्ध नहीं है — यह केवल यह स्पष्ट करने के लिए है कि उन्हें निवेश मानने पर हिसाब कहाँ बिगड़ता है।',
          'पहली लागत मज़दूरी है। डिज़ाइन के अनुसार यह सोने की क़ीमत का एक ठीक-ठाक हिस्सा हो सकती है, और बेचते समय यह पूरी तरह चली जाती है — ख़रीदार केवल धातु का दाम देता है।',
          'दूसरी लागत शुद्धता की है। गहनों में शुद्ध सोना नहीं होता, क्योंकि उसे मज़बूत बनाने के लिए मिश्रण ज़रूरी है। बाईस कैरेट का अर्थ है कि वज़न का एक हिस्सा सोना नहीं है, और बेचते समय यही हिस्सा घटाया जाता है।',
          'तीसरी लागत बेचते समय की कटौती है, जो दुकान से दुकान अलग होती है। इन तीनों को जोड़ने पर जो अंतर बनता है, वह वर्षों की क़ीमत-वृद्धि खा सकता है।',
          'इसलिए एक साफ़ रेखा खींच लेना उपयोगी है: गहने पहनने और परंपरा के लिए ख़रीदिए, और निवेश के लिए अलग रूप चुनिए। दोनों उद्देश्य एक ही ख़रीद से पूरे करने की कोशिश ही वह जगह है जहाँ नुक़सान होता है।',
        ],
        pEn: [
          'The most common form in which gold is bought in India is jewellery, and it is also the form that leaks the most when treated as an investment. This is not an argument against jewellery — it is only about where the arithmetic breaks if it is counted as investing.',
          'The first cost is making charges. Depending on the design these can be a substantial share of the gold price, and they disappear completely at the time of sale — a buyer pays for the metal only.',
          'The second cost is purity. Jewellery is not pure gold, because an alloy is needed for strength. Twenty-two carat means part of the weight is not gold, and that part is deducted when selling.',
          'The third is the deduction applied at the point of sale, which varies from shop to shop. Added together, the resulting gap can consume years of price appreciation.',
          'So drawing a clear line helps: buy jewellery for wearing and for tradition, and choose a different form for investing. Trying to serve both purposes with one purchase is exactly where the loss occurs.',
        ],
      },
      {
        hHi: 'डिजिटल सोना, ETF और फंड — अंतर क्या है',
        hEn: 'Digital gold, ETFs and funds — what the differences are',
        pHi: [
          'सोने में निवेश के काग़ज़ी और डिजिटल रूपों को अक्सर एक साथ रख दिया जाता है, जबकि उनकी बनावट अलग है और यह अंतर मायने रखता है।',
          'गोल्ड ETF शेयर बाज़ार में सूचीबद्ध होता है और उसे ख़रीदने के लिए डीमैट खाता चाहिए। उसकी क़ीमत बाज़ार में तय होती है और उसमें एक प्रबंधन ख़र्च लगता है, जो आमतौर पर छोटा होता है।',
          'गोल्ड म्यूचुअल फंड इन्हीं ETF में निवेश करता है, इसलिए इसमें डीमैट खाते की ज़रूरत नहीं और SIP भी संभव है। बदले में इसमें एक अतिरिक्त परत का ख़र्च जुड़ जाता है।',
          '"डिजिटल गोल्ड" नाम से बिकने वाले उत्पाद इन दोनों से अलग हैं। वे किसी विनिमय पर सूचीबद्ध नहीं होते और उनका नियमन ढाँचा भी वही नहीं है, इसलिए ख़रीदने से पहले यह देखना ज़रूरी है कि पीछे कौन है और शर्तें क्या हैं।',
          'यह लेख इनमें से किसी की सिफ़ारिश नहीं करता। यहाँ केवल यह कहा जा रहा है कि "सोना ख़रीदना" एक काम नहीं बल्कि कई अलग-अलग काम हैं, और उनकी लागत, तरलता और नियमन अलग-अलग हैं।',
        ],
        pEn: [
          'The paper and digital forms of gold investment often get lumped together, even though their structures differ and the differences matter.',
          'A gold ETF is listed on the stock exchange and needs a demat account to buy. Its price is set in the market and it carries a management cost, which is usually small.',
          'A gold mutual fund invests in those same ETFs, so no demat account is needed and a SIP is possible. In exchange it carries the cost of an additional layer.',
          'Products sold under the name "digital gold" are different from both. They are not listed on an exchange and the regulatory framework around them is not the same, so it is worth checking who stands behind the product and what the terms are before buying.',
          'This article recommends none of them. The point being made is only that "buying gold" is not one activity but several different ones, with different costs, different liquidity and different regulation.',
        ],
      },
      {
        hHi: 'पोर्टफ़ोलियो में सोने की भूमिका',
        hEn: 'The role gold plays in a portfolio',
        pHi: [
          'सोने के बारे में सबसे उपयोगी बात यह समझना है कि वह किस काम के लिए रखा जाता है, क्योंकि उससे यह अपने आप तय हो जाता है कि कितना रखा जाए।',
          'सोना आय नहीं देता। न ब्याज, न लाभांश, न किराया। इसका पूरा प्रतिफल क़ीमत बदलने से आता है, और इसी कारण उसे किसी पोर्टफ़ोलियो का मुख्य इंजन नहीं बनाया जाता।',
          'जो काम वह करता है वह अलग है: वह प्रायः तब मज़बूत होता है जब बाक़ी चीज़ें कमज़ोर होती हैं — मुद्रा पर दबाव, अनिश्चितता, या भरोसे का संकट। इसी गुण के कारण उसे संतुलन के लिए रखा जाता है, वृद्धि के लिए नहीं।',
          'इसका व्यावहारिक अर्थ यह है कि सोने का हिस्सा सीमित रहना चाहिए। कितना — यह उम्र, लक्ष्य और बाक़ी पोर्टफ़ोलियो पर निर्भर करता है, और इस लेख में कोई प्रतिशत सुझाया नहीं जा रहा।',
          'एक बात ज़रूर कही जा सकती है: सोना इसलिए ख़रीदना कि उसकी क़ीमत हाल में तेज़ी से बढ़ी है, वही ग़लती है जो किसी भी परिसंपत्ति के साथ की जाती है, और वह किसी एक धातु से बँधी हुई नहीं है।',
        ],
        pEn: [
          'The most useful thing to understand about gold is what it is held for, because that settles by itself the question of how much to hold.',
          'Gold produces no income. No interest, no dividend, no rent. Its entire return comes from price movement, and that is why it is not made the main engine of a portfolio.',
          'What it does do is different: it tends to be strong when other things are weak — pressure on a currency, uncertainty, a crisis of confidence. That property is why it is held for balance rather than for growth.',
          'The practical consequence is that the gold share should stay limited. How much depends on age, goals and the rest of the portfolio, and no percentage is being suggested here.',
          'One thing can be said with confidence: buying gold because its price has risen sharply recently is the same mistake made with every asset, and it is not particular to this one metal.',
        ],
      },
      {
        hHi: 'भंडारण और शुद्धता की व्यावहारिक बातें',
        hEn: 'Practical matters of storage and purity',
        pHi: [
          'यदि आप भौतिक सोना रखते हैं, तो दो व्यावहारिक प्रश्न बचते हैं जिनका उत्तर ख़रीद के दिन ही तय हो जाना चाहिए।',
          'पहला शुद्धता का है। हॉलमार्क का उद्देश्य ठीक यही है — यह स्वतंत्र प्रमाण है कि धातु वह है जो बताई गई है। बिना हॉलमार्क के ख़रीदा गया सोना बेचते समय अतिरिक्त जाँच और अतिरिक्त कटौती दोनों माँगता है।',
          'दूसरा भंडारण का है। घर में रखे सोने का जोखिम स्पष्ट है, और बैंक लॉकर की अपनी लागत और अपनी सीमाएँ हैं। यह भी ध्यान रखिए कि लॉकर में रखी वस्तुओं पर बैंक की ज़िम्मेदारी सीमित होती है, और यह विवरण अनुबंध में लिखा होता है।',
          'तीसरा, और सबसे अधिक भुला दिया जाने वाला: रसीदें। बेचते समय और कर के समय ख़रीद का प्रमाण माँगा जाता है, और वर्षों बाद वह रसीद खोजना लगभग असंभव होता है यदि उसे शुरू में ही सँभालकर न रखा गया हो।',
        ],
        pEn: [
          'If you hold physical gold, two practical questions remain and both should be settled on the day of purchase rather than later.',
          'The first is purity. That is precisely what hallmarking exists for — independent evidence that the metal is what it is said to be. Gold bought without it demands both extra testing and an extra deduction at the time of sale.',
          'The second is storage. The risk of keeping gold at home is obvious, and a bank locker carries its own cost and its own limits. It is also worth knowing that a bank\'s liability for the contents of a locker is limited, and that detail is written into the agreement.',
          'The third, and the most consistently forgotten: receipts. Proof of purchase is asked for when selling and at tax time, and finding a receipt years later is very nearly impossible if it was not filed carefully at the start.',
        ],
      },
      {
        hHi: 'सोना और परिवार: एक संवेदनशील पहलू',
        hEn: 'Gold and the family: a sensitive dimension',
        pHi: [
          'भारत में सोने की चर्चा पूरी तरह वित्तीय नहीं हो सकती, क्योंकि उसका एक बड़ा हिस्सा परिवार, परंपरा और सुरक्षा की भावना से जुड़ा है।',
          'यह जुड़ाव अवैज्ञानिक नहीं है। पीढ़ियों तक सोना ही वह संपत्ति थी जो किसी महिला के अपने नाम रहती थी, जिसे कहीं भी बेचा जा सकता था, और जिसके लिए किसी दस्तावेज़ की ज़रूरत नहीं थी। यह इतिहास आज भी व्यवहार को आकार देता है।',
          'इसलिए किसी परिवार से यह कहना कि "गहनों में निवेश ग़लत है" न तो उपयोगी है और न ही पूरी तरह सही। अधिक उपयोगी बात यह है कि दोनों उद्देश्य अलग-अलग गिने जाएँ।',
          'व्यावहारिक सुझाव यह है: घर में जो सोना परंपरा और उपयोग के लिए है, उसे संपत्ति की सूची में उसकी वर्तमान बिक्री-योग्य क़ीमत पर गिनिए, ख़रीद क़ीमत पर नहीं। और भविष्य के लिए जो सोना निवेश के रूप में रखना हो, उसके लिए कोई और रूप चुनिए।',
        ],
        pEn: [
          'A discussion of gold in India cannot be purely financial, because a large part of it is tied to family, tradition and a sense of security.',
          'That attachment is not irrational. For generations gold was the one asset that stayed in a woman\'s own name, could be sold anywhere, and required no documentation. That history still shapes behaviour today.',
          'So telling a family that "investing in jewellery is wrong" is neither useful nor entirely correct. What is more useful is counting the two purposes separately.',
          'The practical suggestion: count the gold at home that exists for tradition and use at its current realisable value in your asset list, not at what was paid for it. And for gold held as an investment going forward, choose a different form.',
        ],
      },
      {
        hHi: 'ख़रीदने का समय: वह प्रश्न जिसका उत्तर नहीं है',
        hEn: 'Timing the purchase: the question with no answer',
        pHi: [
          'सबसे अधिक पूछा जाने वाला प्रश्न यही है — अभी ख़रीदें या रुकें? और सबसे ईमानदार उत्तर यह है कि यह किसी को नहीं पता।',
          'सोने की क़ीमत कई चीज़ों से चलती है जिनका पूर्वानुमान लगाना कठिन है: अंतरराष्ट्रीय दरें, रुपये की चाल, आयात शुल्क, और वैश्विक अनिश्चितता। इनमें से कोई भी भरोसेमंद रूप से पहले से नहीं जाना जा सकता।',
          'इसलिए जो तरीक़ा बचता है वह वही है जो हर अनिश्चित परिसंपत्ति के लिए काम करता है: एक बार में बड़ी राशि लगाने के बजाय समय पर फैलाकर ख़रीदना। इससे किसी एक दिन की क़ीमत का महत्व अपने आप घट जाता है।',
          'और एक चेतावनी जो हर साल दोहराने लायक़ है: त्योहारों के आसपास मिलने वाली "योजनाएँ" ध्यान से पढ़िए। कुछ में मासिक जमा के बाद एक अतिरिक्त किश्त मिलती है, पर शर्तें, निकासी के नियम और लागू होने वाली धातु-दर हर योजना में अलग होती है।',
        ],
        pEn: [
          'The most frequently asked question is exactly this — buy now or wait? And the most honest answer is that nobody knows.',
          'The gold price moves on several things that are hard to forecast: international rates, the rupee, import duties, and global uncertainty. None of these can be known reliably in advance by anybody.',
          'So the approach that remains is the one that works for every uncertain asset: spreading purchases over time rather than committing a large amount at once. That reduces the importance of any single day\'s price on its own.',
          'And one warning worth repeating every year: read the "schemes" offered around festivals carefully. Some add an extra instalment after a run of monthly deposits, but the conditions, the withdrawal rules and the metal rate applied differ from scheme to scheme.',
        ],
      },
    ],
  },

  {
    slug: 'five-modern-upi-scams-three-questions',
    sections: [
      {
        hHi: 'पहला प्रश्न: संपर्क किसने शुरू किया',
        hEn: 'The first question: who started the contact',
        pHi: [
          'तीन प्रश्नों में से यह सबसे शक्तिशाली है और सबसे कम इस्तेमाल किया जाता है, क्योंकि उत्तर इतना सरल होता है कि लोग उसे महत्वपूर्ण नहीं मानते।',
          'यदि कॉल, संदेश या लिंक उनकी ओर से आया है, तो पूरी बातचीत संदेह से शुरू होनी चाहिए — चाहे वे कुछ भी कहें। और यदि आपने ख़ुद किसी आधिकारिक नंबर पर कॉल किया है, तो जोखिम बहुत कम है।',
          'यह भेद इतना उपयोगी है कि उसे एक नियम बनाया जा सकता है: कभी भी उस नंबर पर वापस बात मत कीजिए जो उन्होंने दिया। यदि कोई बैंक होने का दावा करता है, तो कॉल काटिए और कार्ड के पीछे लिखे नंबर पर ख़ुद कॉल कीजिए।',
          'यही एक आदत उन ठगी को रोक देती है जो सबसे अधिक विश्वसनीय लगती हैं, क्योंकि उन सबकी एक ही ज़रूरत है — कि आप उनके चुने हुए रास्ते पर बने रहें।',
        ],
        pEn: [
          'Of the three questions this is the most powerful and the least used, because the answer is so simple that people do not treat it as significant.',
          'If the call, message or link came from their side, the entire conversation should begin in suspicion — whatever they say. And if you dialled an official number yourself, the risk is very much lower.',
          'The distinction is useful enough to turn into a rule: never call back on the number they gave you. If somebody claims to be from a bank, end the call and dial the number printed on the back of your card instead.',
          'That single habit stops the frauds that feel most convincing, because all of them need the same thing — for you to stay on the channel they chose.',
        ],
      },
      {
        hHi: 'दूसरा प्रश्न: वास्तव में माँगा क्या जा रहा है',
        hEn: 'The second question: what is actually being asked for',
        pHi: [
          'ठगी की भाषा हमेशा घुमावदार होती है, इसलिए यह प्रश्न पूछना कि "इस बातचीत के अंत में मुझसे क्या करवाया जा रहा है" चीज़ों को तुरंत साफ़ कर देता है।',
          'उत्तर लगभग हमेशा चार में से एक होता है: कोई ऐप इंस्टॉल करवाना, कोई कोड बताना, कोई लिंक खुलवाना, या कोई भुगतान करवाना। यदि बातचीत इन चारों में से किसी की ओर जा रही है, तो वह ठगी हो सकती है — चाहे कहानी कितनी भी विश्वसनीय हो।',
          'एक ख़ास बात ध्यान देने लायक़ है: असली संस्थाएँ जानकारी देती हैं, कार्रवाई नहीं करवातीं। बैंक आपको बताएगा कि आपके खाते में क्या हुआ; वह आपसे फ़ोन पर कुछ करवाने की कोशिश नहीं करेगा।',
          'इसलिए जब कोई कहे कि "बस यह एक काम कर दीजिए और मामला हल हो जाएगा", तो वही वाक्य सबसे बड़ा संकेत है। असली समस्याएँ फ़ोन पर तीन मिनट में हल नहीं होतीं।',
        ],
        pEn: [
          'The language of a scam is always indirect, so asking "what am I being made to do by the end of this conversation" clears things up immediately.',
          'The answer is almost always one of four: install an app, read out a code, open a link, or make a payment. If the conversation is heading towards any of those, it may well be fraud — however convincing the story around it.',
          'One thing is worth noting particularly: genuine institutions give information, they do not make you perform actions. A bank will tell you what happened in your account; it will not try to get you to do something on the phone.',
          'So when somebody says "just do this one thing and the matter will be resolved", that sentence is itself the largest signal. Real problems are not solved on a phone call in three minutes.',
        ],
      },
      {
        hHi: 'तीसरा प्रश्न: यदि मैं कल करूँ तो क्या होगा',
        hEn: 'The third question: what happens if I do this tomorrow',
        pHi: [
          'यह प्रश्न लगभग हर ठगी को अकेले तोड़ देता है, क्योंकि हर ठगी की एकमात्र वास्तविक ज़रूरत समय है — और यह प्रश्न उसे छीन लेता है।',
          'असली स्थितियों में देरी का परिणाम मामूली होता है। यदि सचमुच कोई भुगतान लंबित है, तो वह कल भी लंबित रहेगा। यदि सचमुच कोई KYC अधूरा है, तो उसके लिए शाखा में जाया जा सकता है।',
          'ठगी में उत्तर हमेशा उलटा होता है: "कल तक खाता बंद हो जाएगा", "यह ऑफ़र दस मिनट में ख़त्म है", "अभी नहीं किया तो मामला दर्ज हो जाएगा"। यह तात्कालिकता कहानी का हिस्सा नहीं, कहानी का पूरा उद्देश्य है।',
          'इसलिए इस प्रश्न का उत्तर सुनना ही पर्याप्त है। जो व्यक्ति आपको कल तक रुकने नहीं दे रहा, उसके पास कल तक टिकने वाली कोई बात नहीं है।',
        ],
        pEn: [
          'This question breaks nearly every scam on its own, because the one thing every scam truly needs is time — and this question takes it away.',
          'In genuine situations the consequence of a delay is minor. If a payment really is pending, it will still be pending tomorrow. If a KYC really is incomplete, it can be done at a branch.',
          'In a scam the answer always runs the other way: "the account will be frozen by tomorrow", "this offer ends in ten minutes", "a case will be filed if you do not act now". That urgency is not part of the story; it is the entire purpose of the story.',
          'So simply hearing the answer to this question is enough. Somebody who will not let you wait until tomorrow has nothing that would survive until tomorrow.',
        ],
      },
      {
        hHi: 'नौकरी और टास्क वाली ठगी',
        hEn: 'The job-offer and task scam',
        pHi: [
          'यह हाल के वर्षों में सबसे तेज़ी से बढ़ने वाली श्रेणी है और यह UPI ठगी की पुरानी सूची में नहीं आती, इसलिए इसे अलग से पहचानना ज़रूरी है।',
          'शुरुआत हानिरहित लगती है: किसी संदेश या समूह के माध्यम से घर बैठे काम का प्रस्ताव, जिसमें छोटे-छोटे "टास्क" पूरे करने पर भुगतान मिलता है। पहले कुछ भुगतान सचमुच आते हैं, और यही इस ठगी की असली चाल है।',
          'भरोसा बन जाने के बाद ढाँचा बदल जाता है। अब बड़े टास्क के लिए पहले "जमा" करनी पड़ती है, इस वादे के साथ कि वह भुगतान के साथ वापस मिलेगी। यह जमा बढ़ती जाती है, और किसी बिंदु पर खाता बंद हो जाता है।',
          'पहचान का नियम सरल है: किसी भी असली काम में आपसे पैसा नहीं लिया जाता। नियोक्ता भुगतान करता है, प्राप्त नहीं करता — और इस नियम का कोई अपवाद नहीं है।',
          'यह श्रेणी विशेष रूप से युवाओं और नई नौकरी खोज रहे लोगों तक पहुँचती है, इसलिए घर में इसकी चर्चा करना उतना ही उपयोगी है जितना UPI के PIN की।',
        ],
        pEn: [
          'This is the fastest-growing category of recent years and it does not appear on older lists of UPI frauds, so it is worth recognising separately.',
          'It begins harmlessly: an offer of work from home through a message or a group, with payment for completing small "tasks". The first few payments genuinely arrive, and that is the real mechanism of the scam.',
          'Once trust exists the structure changes. Larger tasks now require a "deposit" first, with a promise that it comes back along with the payment. The deposit grows, and at some point the account goes silent.',
          'The rule for recognising it is simple: no genuine work takes money from you. An employer pays, they do not receive — and there is no exception to that rule.',
          'This category reaches young people and those looking for a first job particularly, so discussing it at home is as useful as discussing the UPI PIN.',
        ],
      },
      {
        hHi: 'निवेश समूह वाली ठगी',
        hEn: 'The investment-group scam',
        pHi: [
          'यह दूसरी नई श्रेणी है और यह विशेष रूप से ख़तरनाक है क्योंकि इसमें बड़ी राशियाँ जाती हैं और शिकार अक्सर पढ़े-लिखे लोग होते हैं।',
          'ढाँचा यह होता है: किसी संदेश समूह में जुड़ाव, जहाँ "विशेषज्ञ" रोज़ सलाह देते हैं और अन्य सदस्य अपने मुनाफ़े की तस्वीरें साझा करते हैं। वे अन्य सदस्य प्रायः असली नहीं होते।',
          'कुछ दिनों बाद एक ऐप या मंच पर पैसा लगाने को कहा जाता है, जहाँ आपका "पोर्टफ़ोलियो" बढ़ता हुआ दिखता है। छोटी निकासी शुरू में सफल हो जाती है, जिससे भरोसा और बढ़ता है। बड़ी निकासी के समय कोई शुल्क, कोई कर, या कोई सत्यापन बीच में आ जाता है।',
          'पहचान के तीन संकेत हैं: कोई ऐसा मंच जो मान्यता-प्राप्त नहीं है, ऐसा प्रतिफल जो नियमित और असामान्य रूप से ऊँचा हो, और निकासी में कोई अतिरिक्त भुगतान की शर्त। इनमें से कोई एक भी पर्याप्त कारण है रुक जाने का।',
          'सामान्य नियम यह है कि निवेश केवल मान्यता-प्राप्त, नियमित संस्थाओं के माध्यम से किया जाए, और किसी भी मंच के बारे में उसकी अपनी वेबसाइट के बाहर से पुष्टि की जाए।',
        ],
        pEn: [
          'This is the second new category and it is particularly dangerous, because the amounts involved are large and the victims are frequently well-educated people.',
          'The structure runs like this: joining a messaging group where "experts" give daily advice and other members post screenshots of their profits. Those other members are usually not real people.',
          'After a few days you are asked to put money into an app or platform where your "portfolio" appears to grow. Small withdrawals succeed at first, which builds confidence further. At the point of a large withdrawal, a fee, a tax or a verification step suddenly appears.',
          'There are three signs: a platform that is not registered, returns that are regular and unusually high, and a condition requiring a further payment before withdrawal. Any one of them is sufficient reason to stop.',
          'The general rule is to invest only through registered, regulated entities, and to verify any platform from somewhere other than its own website.',
        ],
      },
      {
        hHi: 'शिकायत कैसे और कहाँ दर्ज करें',
        hEn: 'How and where to file a complaint',
        pHi: [
          'ठगी के बाद क्या करना है, यह जानना उतना ही महत्वपूर्ण है जितना ठगी पहचानना, क्योंकि पहले कुछ घंटों में की गई कार्रवाई ही सबसे अधिक असर रखती है।',
          'पहला चरण बैंक है — आधिकारिक नंबर पर कॉल करके लेन-देन की सूचना और खाते पर रोक की माँग। यह सूचना जितनी जल्दी दर्ज होती है, उत्तरदायित्व के नियम उतने ही अनुकूल रहते हैं।',
          'दूसरा चरण राष्ट्रीय साइबर अपराध रिपोर्टिंग व्यवस्था है, जिसमें ऑनलाइन शिकायत दर्ज की जा सकती है और एक समर्पित हेल्पलाइन भी है। शिकायत संख्या सुरक्षित रखिए, वह हर आगे के क़दम में माँगी जाती है।',
          'तीसरा चरण, यदि बैंक से संतोषजनक उत्तर न मिले, तो बैंकिंग लोकपाल के पास शिकायत है। यह रास्ता निशुल्क है और उसकी प्रक्रिया लिखित होती है।',
          'और एक अंतिम बात: तुरंत बताइए, चाहे शर्म आए। ठगी का शिकार होना बुद्धि की कमी नहीं है — ये तरकीबें पेशेवर रूप से बनाई जाती हैं। देरी ही एकमात्र चीज़ है जो नुक़सान को स्थायी बनाती है।',
        ],
        pEn: [
          'Knowing what to do after a fraud matters as much as recognising one, because action taken in the first few hours has by far the largest effect.',
          'The first step is the bank — call the official number, report the transaction and ask for the account to be blocked. The sooner that report is on record, the more favourable the liability rules remain.',
          'The second step is the national cybercrime reporting system, where a complaint can be filed online and where a dedicated helpline also exists. Keep the complaint number; it is asked for at every subsequent stage.',
          'The third step, if the bank\'s response is unsatisfactory, is a complaint to the banking ombudsman. That route costs nothing and its process is conducted in writing.',
          'And one last thing: report it immediately, however embarrassing it feels. Being defrauded is not a failure of intelligence — these tricks are built professionally. Delay is the only thing that makes the loss permanent.',
        ],
      },
    ],
  },

  {
    slug: 'credit-score-samjhiye',
    sections: [
      {
        hHi: 'चार ब्यूरो, चार अलग संख्याएँ',
        hEn: 'Four bureaus, four different numbers',
        pHi: [
          'बहुत से लोग यह मानते हैं कि क्रेडिट स्कोर एक ही संख्या है, और फिर दो अलग जगहों पर दो अलग आँकड़े देखकर उलझ जाते हैं।',
          'वास्तव में भारत में एक से अधिक क्रेडिट सूचना कंपनियाँ काम करती हैं, और हर एक अपना स्कोर निकालती है। उनके पास आने वाला डेटा भी पूरी तरह एक जैसा नहीं होता, क्योंकि हर ऋणदाता हर ब्यूरो को एक ही समय पर रिपोर्ट नहीं भेजता।',
          'इसका अर्थ यह नहीं कि कोई एक ग़लत है। इसका अर्थ केवल यह है कि संख्या को एक बिंदु के रूप में नहीं, एक दायरे के रूप में देखना चाहिए। तीस-चालीस अंकों का अंतर सामान्य है।',
          'व्यावहारिक सलाह यह है कि साल में एक बार एक से अधिक ब्यूरो की रिपोर्ट देख लीजिए — इसलिए नहीं कि स्कोर अलग है, बल्कि इसलिए कि किसी एक रिपोर्ट में मौजूद ग़लती दूसरी में न हो, और तुलना से वह पकड़ में आ जाती है।',
        ],
        pEn: [
          'Many people assume the credit score is a single number, and then get confused on finding two different figures in two different places.',
          'In fact more than one credit information company operates in India, and each produces its own score. The data reaching them is not identical either, because lenders do not report to every bureau at the same time.',
          'This does not mean one of them is wrong. It only means the number should be read as a range rather than as a point. A difference of thirty or forty points between bureaus is ordinary.',
          'The practical advice is to look at reports from more than one bureau once a year — not because the scores differ, but because an error present in one report may be absent from another, and comparing them is how it gets caught.',
        ],
      },
      {
        hHi: 'ऋणदाता संख्या से आगे क्या देखता है',
        hEn: 'What a lender looks at beyond the number',
        pHi: [
          'स्कोर एक सारांश है, और कोई भी गंभीर ऋणदाता केवल सारांश पर निर्णय नहीं लेता। रिपोर्ट में स्कोर के अलावा जो कुछ है, वह अक्सर उतना ही मायने रखता है।',
          'पहली चीज़ जो देखी जाती है वह हाल का व्यवहार है। दो साल पुरानी एक चूक और पिछले महीने की एक चूक स्कोर पर लगभग एक जैसा असर डाल सकती हैं, पर ऋणदाता की नज़र में वे बिल्कुल अलग हैं।',
          'दूसरी चीज़ खातों का मिश्रण है। केवल असुरक्षित ऋण — व्यक्तिगत ऋण और कार्ड — वाला रिकॉर्ड उसी स्कोर वाले उस रिकॉर्ड से कमज़ोर पढ़ा जाता है जिसमें कोई सुरक्षित ऋण भी नियमित रूप से चुकाया गया हो।',
          'तीसरी चीज़ हाल की पूछताछ है। यदि पिछले कुछ महीनों में कई ऋणदाताओं ने आपकी रिपोर्ट देखी है, तो यह संकेत जाता है कि आप कई जगह आवेदन कर रहे हैं — और यह अपने आप एक चेतावनी के रूप में पढ़ा जाता है।',
          'इसलिए स्कोर बढ़ाने का काम केवल संख्या का काम नहीं है। रिपोर्ट को एक कहानी की तरह पढ़ा जाता है, और वह कहानी सुसंगत होनी चाहिए।',
        ],
        pEn: [
          'The score is a summary, and no serious lender decides on a summary alone. What sits in the report besides the score often matters just as much.',
          'The first thing looked at is recent behaviour. A default two years old and one from last month may affect the score similarly, but in a lender\'s eyes they are entirely different things.',
          'The second is the mix of accounts. A record containing only unsecured borrowing — personal loans and cards — reads as weaker than a record with the same score that also shows a secured loan being repaid regularly.',
          'The third is recent enquiries. If several lenders have pulled your report over the past few months, that signals you are applying in many places — and it is read as a warning in itself.',
          'So improving a score is not purely a matter of the number. The report is read as a story, and the story needs to be coherent.',
        ],
      },
      {
        hHi: 'हार्ड और सॉफ़्ट पूछताछ का अंतर',
        hEn: 'The difference between hard and soft enquiries',
        pHi: [
          'यह अंतर छोटा लगता है और व्यवहार में महत्वपूर्ण है, ख़ासकर उन लोगों के लिए जो कोई बड़ा ऋण लेने की योजना बना रहे हैं।',
          'जब आप ख़ुद अपना स्कोर देखते हैं, तो वह सॉफ़्ट पूछताछ है और उससे स्कोर पर कोई असर नहीं पड़ता। यह डर कि "बार-बार देखने से स्कोर गिरता है" ग़लत है, और उसी डर के कारण बहुत से लोग अपनी रिपोर्ट कभी नहीं देखते।',
          'जब कोई ऋणदाता आपके आवेदन पर आपकी रिपोर्ट निकालता है, तो वह हार्ड पूछताछ है और वह रिपोर्ट में दर्ज होती है। एक-दो का असर मामूली है; कम समय में कई का असर जुड़ जाता है।',
          'इससे एक व्यावहारिक नियम निकलता है: कई जगह एक साथ आवेदन मत कीजिए। पहले पात्रता जाँचिए — कई ऋणदाता यह सुविधा बिना हार्ड पूछताछ के देते हैं — और फिर केवल वहीं औपचारिक आवेदन कीजिए जहाँ संभावना सबसे अच्छी हो।',
        ],
        pEn: [
          'This distinction sounds minor and matters in practice, particularly for anybody planning a large loan in the near future.',
          'When you check your own score it is a soft enquiry and has no effect at all. The fear that "checking repeatedly lowers the score" is simply wrong, and that fear is why many people never look at their own report.',
          'When a lender pulls your report against an application, that is a hard enquiry and it is recorded. One or two make little difference; several within a short period add up.',
          'A practical rule follows from this: do not apply in many places at once. Check eligibility first — many lenders offer this without a hard enquiry — and then submit a formal application only where the chance is best.',
        ],
      },
      {
        hHi: 'स्कोर का उपयोग ऋण के बाहर',
        hEn: 'Where the score is used outside lending',
        pHi: [
          'बहुत से लोग स्कोर की परवाह केवल तब करते हैं जब ऋण लेना हो, जबकि उसका उपयोग धीरे-धीरे अन्य जगहों तक फैला है।',
          'सबसे स्पष्ट उदाहरण क्रेडिट कार्ड की सीमा है। वही व्यक्ति, वही आय, पर बेहतर स्कोर पर अधिक सीमा और बेहतर शर्तें मिलती हैं — और सीमा अपने आप उपयोग अनुपात को सुधार देती है।',
          'दूसरा उदाहरण ऋण की दर है, जो शायद सबसे महँगा प्रभाव है। एक बड़े और लंबे ऋण पर ब्याज दर में आधे प्रतिशत का अंतर भी कुल भुगतान में बड़ी राशि बना देता है।',
          'तीसरा, कुछ मामलों में मकान किराए पर लेते समय या कुछ प्रकार की नौकरियों में — विशेष रूप से वित्तीय ज़िम्मेदारी वाले पदों पर — रिकॉर्ड देखा जा सकता है।',
          'इसका निष्कर्ष यह है कि स्कोर को केवल ऋण के समय याद करने वाली चीज़ मत मानिए। यह एक ऐसा रिकॉर्ड है जो चुपचाप बनता रहता है, और वह उसी दिन काम आता है जिस दिन उसे बनाना संभव नहीं होता।',
        ],
        pEn: [
          'Many people think about the score only when a loan is needed, whereas its use has gradually spread into other places.',
          'The clearest example is credit card limits. The same person on the same income gets a higher limit and better terms with a better score — and a higher limit improves the utilisation ratio on its own.',
          'The second example is the interest rate, which is probably the most expensive effect. On a large, long loan even half a percentage point of difference turns into a substantial sum across the total repayment.',
          'Third, in some cases the record may be looked at when renting a home, or for certain kinds of employment — particularly roles carrying financial responsibility.',
          'The conclusion is not to treat the score as something to remember at loan time. It is a record that builds quietly, and it becomes useful on precisely the day when building it is no longer possible.',
        ],
      },
      {
        hHi: 'सेटलमेंट और "क्लोज़्ड" के बीच का ख़तरनाक अंतर',
        hEn: 'The dangerous gap between "settled" and "closed"',
        pHi: [
          'यह रिपोर्ट का वह विवरण है जो सबसे अधिक नुक़सान करता है और जिसके बारे में सबसे कम बताया जाता है।',
          'जब कोई ऋण पूरी तरह चुका दिया जाता है, तो वह "क्लोज़्ड" के रूप में दर्ज होता है, और यह एक सकारात्मक प्रविष्टि है। जब ऋणदाता कम राशि लेकर मामला ख़त्म करने को राज़ी होता है, तो वह "सेटल्ड" दर्ज होता है — और यह एक नकारात्मक प्रविष्टि है जो वर्षों तक रहती है।',
          'बहुत से लोग सेटलमेंट को राहत समझकर स्वीकार कर लेते हैं, यह जाने बिना कि इसका दीर्घकालिक असर क्या है। भविष्य में कोई भी ऋणदाता इस प्रविष्टि को देखकर सावधान हो जाता है, क्योंकि उसका सीधा अर्थ है कि पिछला ऋणदाता पूरा पैसा वापस नहीं पा सका।',
          'यदि संभव हो, तो सेटलमेंट के बजाय पुनर्गठन या लंबी अवधि की बातचीत बेहतर है। और यदि सेटलमेंट पहले ही हो चुका है, तो शेष राशि चुकाकर "नो ड्यूज़" प्रमाणपत्र लेना और रिपोर्ट में स्थिति सुधरवाना संभव होता है — यह प्रक्रिया लंबी है, पर यह मौजूद है।',
        ],
        pEn: [
          'This is the detail in a report that does the most damage and about which the least is said.',
          'When a loan is repaid in full it is recorded as "closed", which is a positive entry. When a lender agrees to accept a lower amount and end the matter, it is recorded as "settled" — and that is a negative entry which stays for years.',
          'Many people accept a settlement as relief without knowing its longer-term effect. Any future lender seeing that entry becomes cautious, because it means directly that a previous lender did not get all its money back.',
          'Where possible, restructuring or negotiating a longer term is better than settling. And if a settlement has already happened, paying the remaining amount, obtaining a no-dues certificate and getting the report status corrected is possible — the process is slow, but it exists.',
        ],
      },
      {
        hHi: 'स्कोर की सीमाएँ',
        hEn: 'The limits of what a score can say',
        pHi: [
          'अंत में एक संतुलन ज़रूरी है, क्योंकि स्कोर को इतना महत्व दे दिया जाता है कि लोग उसे अपने वित्तीय स्वास्थ्य का माप समझने लगते हैं। वह यह नहीं है।',
          'स्कोर केवल एक बात मापता है: आपने उधार लिया हुआ पैसा कैसे चुकाया। वह यह नहीं जानता कि आपके पास कितनी बचत है, आपका आपातकालीन कोष है या नहीं, आपका बीमा है या नहीं, या आपकी आय स्थिर है या नहीं।',
          'इसका अर्थ यह है कि 800 का स्कोर रखने वाला व्यक्ति वित्तीय रूप से नाज़ुक हो सकता है, और बिना किसी स्कोर वाला व्यक्ति पूरी तरह सुरक्षित। स्कोर ऋणदाता के प्रश्न का उत्तर है, आपके प्रश्न का नहीं।',
          'इसलिए स्कोर को उतना ही महत्व दीजिए जितना उसका है — वह ऋण की क़ीमत तय करता है, और वही उसका पूरा काम है। बाक़ी वित्तीय स्वास्थ्य कहीं और मापा जाता है: बचत में, बीमा में, और उस अंतर में जो हर महीने आय और ख़र्च के बीच बचता है।',
        ],
        pEn: [
          'A balancing point belongs at the end, because the score is given so much weight that people begin to treat it as a measure of their financial health. It is not that.',
          'A score measures one thing only: how you repaid money you borrowed. It knows nothing about how much you have saved, whether you have an emergency fund, whether you are insured, or whether your income is stable.',
          'This means a person with a score of 800 can be financially fragile, and a person with no score at all can be entirely secure. The score answers a lender\'s question, not yours.',
          'So give it exactly the weight it deserves — it prices your borrowing, and that is the whole of its job. Financial health is measured elsewhere: in savings, in insurance, and in the gap that remains between income and spending each month.',
        ],
      },
    ],
  },

  {
    slug: 'tax-regime-kaise-chunein',
    sections: [
      {
        hHi: 'ब्रेक-ईवन कटौती: एक संख्या जो सब कुछ आसान कर देती है',
        hEn: 'The break-even deduction: one number that simplifies everything',
        pHi: [
          'पूरी तुलना को एक ही प्रश्न में बदला जा सकता है, और यह तरीक़ा हर साल की माथापच्ची बचा देता है।',
          'प्रश्न यह है: आपकी आय पर वह कुल कटौती कितनी है जिस पर दोनों व्यवस्थाओं में कर बराबर हो जाता है? यही ब्रेक-ईवन कटौती है। यदि आपकी वास्तविक कटौतियाँ इससे अधिक हैं, तो पुरानी व्यवस्था बेहतर है; यदि कम हैं, तो नई।',
          'इसका बड़ा लाभ यह है कि आपको हर बार पूरा कर-गणना नहीं करनी पड़ती। एक बार अपनी आय-श्रेणी के लिए यह संख्या निकाल लीजिए, फिर हर साल केवल यह देखिए कि आपकी कटौतियाँ उससे ऊपर हैं या नीचे।',
          'यह संख्या आय के साथ बदलती है, और दरों में बदलाव के साथ भी। इसलिए इसे हर वर्ष एक बार दोबारा निकालना ज़रूरी है — पर वह काम पूरी तुलना दोहराने से कहीं छोटा है।',
        ],
        pEn: [
          'The whole comparison can be reduced to a single question, and doing so saves the annual head-scratching entirely.',
          'The question is: at what level of total deductions does the tax under both regimes come out the same for your income? That is the break-even deduction. If your actual deductions exceed it, the old regime is better; if they fall short, the new one is.',
          'The advantage is that you no longer have to run a full tax calculation each time. Work the number out once for your income band, and thereafter simply check whether your deductions sit above or below it.',
          'This figure moves with income and also whenever rates change, so it needs recalculating once a year — but that is a far smaller task than repeating the entire comparison.',
        ],
      },
      {
        hHi: 'प्रोफ़ाइल के अनुसार एक सरल निर्णय-वृक्ष',
        hEn: 'A simple decision tree by profile',
        pHi: [
          'हर किसी की स्थिति अलग होती है, फिर भी कुछ सामान्य प्रोफ़ाइल हैं जिनके लिए दिशा प्रायः स्पष्ट होती है। यह अंतिम उत्तर नहीं, केवल शुरुआती बिंदु है।',
          'युवा वेतनभोगी, कोई ऋण नहीं, कोई बड़ा बीमा नहीं, किराए के मकान में नहीं: इनके पास गिनने को अधिक कटौतियाँ नहीं होतीं, इसलिए यह प्रोफ़ाइल प्रायः नई व्यवस्था की ओर झुकती है।',
          'परिवार, गृह ऋण, स्वास्थ्य बीमा, बच्चों की फ़ीस: यहाँ कटौतियाँ आसानी से बड़ी हो जाती हैं, और यह प्रोफ़ाइल प्रायः पुरानी व्यवस्था की ओर झुकती है — विशेषकर ऋण के शुरुआती वर्षों में।',
          'किराए के मकान में रहने वाला वेतनभोगी जिसे किराए से जुड़ी छूट मिलती है: यह श्रेणी बीच में है और यहाँ वास्तविक गणना सबसे अधिक ज़रूरी है, क्योंकि किराए की राशि शहर के हिसाब से बहुत बदलती है।',
          'व्यवसाय या पेशे से आय: यहाँ नियम अलग हैं और चुनाव बदलने की स्वतंत्रता सीमित है, इसलिए यह वह प्रोफ़ाइल है जिसे सामान्य सलाह के बजाय व्यक्तिगत सलाह चाहिए।',
        ],
        pEn: [
          'Everybody\'s position differs, and some common profiles nevertheless point in a fairly clear direction. This is a starting point rather than a final answer.',
          'A young salaried person with no loan, no large insurance and not paying rent: there are not many deductions to count, so this profile usually leans towards the new regime.',
          'A family with a home loan, health insurance and school fees: deductions add up quickly here, and this profile usually leans towards the old regime, particularly in the early years of the loan.',
          'A salaried person living in rented accommodation and eligible for rent-related relief: this sits in the middle and is where an actual calculation matters most, because rent varies enormously between cities.',
          'Business or professional income: the rules differ here and the freedom to switch is limited, so this is the profile that needs individual advice rather than general guidance.',
        ],
      },
      {
        hHi: 'नियोक्ता को दी गई घोषणा और असली स्थिति',
        hEn: 'The declaration given to an employer versus the real position',
        pHi: [
          'वेतनभोगी लोगों के लिए एक व्यावहारिक उलझन यह है कि घोषणा वर्ष की शुरुआत में देनी होती है, जब बहुत कुछ अभी हुआ ही नहीं होता।',
          'इसका परिणाम यह होता है कि साल भर TDS एक व्यवस्था के अनुसार कटता है, और रिटर्न भरते समय पता चलता है कि दूसरी व्यवस्था बेहतर थी। यह कोई आपदा नहीं है — रिटर्न में सही व्यवस्था चुनी जा सकती है और अंतर वापसी के रूप में मिल जाता है।',
          'पर इसमें एक असुविधा है: वह पैसा साल भर आपके पास नहीं रहता। इसलिए शुरुआत में की गई घोषणा जितनी सटीक होगी, नक़दी का प्रवाह उतना ही बेहतर रहेगा।',
          'इसका व्यावहारिक हल यह है कि घोषणा भरते समय केवल वे कटौतियाँ लिखिए जिनके बारे में आप निश्चित हैं। बाद में यदि कोई और कटौती जुड़ जाए, तो नियोक्ता को बीच वर्ष में सूचित किया जा सकता है, और अधिकांश नियोक्ता यह अनुमति देते हैं।',
        ],
        pEn: [
          'A practical difficulty for salaried people is that the declaration has to be given at the start of the year, when much of what it covers has not happened yet.',
          'The result is that TDS is deducted all year under one regime, and at filing time it turns out the other one was better. This is not a disaster — the correct regime can be chosen in the return and the difference comes back as a refund.',
          'There is an inconvenience in it though: that money is not with you during the year. So the more accurate the declaration made at the start, the better the cash flow across the year.',
          'The practical fix is to declare only those deductions you are certain about. If something else is added later, the employer can be informed mid-year, and most employers allow this.',
        ],
      },
      {
        hHi: 'जब दोनों व्यवस्थाओं का अंतर बहुत छोटा हो',
        hEn: 'When the difference between the two is very small',
        pHi: [
          'कभी-कभी पूरी गणना के बाद अंतर कुछ हज़ार रुपये का ही निकलता है, और तब यह प्रश्न उठता है कि इतनी मेहनत का क्या अर्थ है।',
          'ऐसी स्थिति में एक अतिरिक्त कारक देखा जा सकता है जो पैसों में नहीं गिना जाता: सरलता। नई व्यवस्था में प्रमाण जुटाने, रसीदें सँभालने और घोषणाएँ करने का काम कम है।',
          'दूसरा कारक लचीलापन है। यदि पुरानी व्यवस्था चुनने का अर्थ यह है कि आपको कर बचाने के लिए हर साल एक निश्चित राशि किसी लंबी अवधि के उत्पाद में डालनी पड़ेगी, तो वह बाध्यता अपने आप में एक लागत है।',
          'तीसरा कारक स्थिरता है। यदि अगले कुछ वर्षों में आपकी स्थिति बदलने वाली है — ऋण समाप्त होने वाला है, या कोई बड़ा ख़र्च शुरू होने वाला है — तो आज का छोटा अंतर वैसे भी टिकने वाला नहीं है।',
          'इन तीनों को जोड़कर देखने पर छोटे अंतर वाली स्थिति में निर्णय प्रायः आसान हो जाता है, और वह निर्णय अक्सर सरलता के पक्ष में जाता है।',
        ],
        pEn: [
          'Sometimes the whole calculation produces a difference of only a few thousand rupees, and then the question arises of what all the effort was for.',
          'In that situation an additional factor can be weighed which does not appear in rupees: simplicity. The new regime involves less work in gathering proofs, keeping receipts and making declarations.',
          'The second factor is flexibility. If choosing the old regime means committing a fixed amount to a long-term product every year purely to save tax, that obligation is itself a cost.',
          'The third factor is stability. If your position is going to change over the next few years — a loan ending, or a large expense beginning — then today\'s small difference will not hold anyway.',
          'Taking those three together usually makes the decision easy when the gap is small, and the decision quite often goes in favour of simplicity.',
        ],
      },
      {
        hHi: 'सामान्य ग़लतफ़हमियाँ जो हर साल दोहराई जाती हैं',
        hEn: 'The misconceptions repeated every year',
        pHi: [
          'इस विषय पर कुछ धारणाएँ इतनी बार दोहराई जाती हैं कि वे तथ्य लगने लगती हैं। उन्हें अलग से स्पष्ट कर देना उपयोगी है।',
          'पहली: "नई व्यवस्था सबके लिए बेहतर है क्योंकि दरें कम हैं।" दरें कम हैं और कटौतियाँ भी कम हैं, इसलिए यह तुलना अधूरी है। कुल कर मायने रखता है, दर नहीं।',
          'दूसरी: "पुरानी व्यवस्था हमेशा बेहतर है अगर गृह ऋण हो।" यह प्रायः सच होता है पर हमेशा नहीं, क्योंकि यह ऋण की उम्र, ब्याज की राशि और आय के स्तर पर निर्भर करता है।',
          'तीसरी: "एक बार चुन लिया तो हमेशा के लिए।" वेतनभोगी व्यक्ति के लिए यह ग़लत है, और यह ग़लतफ़हमी लोगों को अनावश्यक रूप से चिंतित करती है।',
          'चौथी: "नई व्यवस्था में कोई राहत नहीं मिलती।" कुछ राहतें दोनों में लागू रहती हैं, और उन्हें छोड़ देना गणना को ग़लत कर देता है। इसीलिए अपनी पूरी सूची बनाकर देखना आवश्यक है।',
        ],
        pEn: [
          'A few beliefs on this subject are repeated so often that they start to sound like facts. They are worth clearing up separately.',
          'First: "the new regime is better for everybody because the rates are lower." The rates are lower and so are the deductions, which makes that comparison incomplete. Total tax is what matters, not the rate.',
          'Second: "the old regime is always better if there is a home loan." That is often true but not always, because it depends on the age of the loan, the amount of interest and the level of income.',
          'Third: "once chosen, it is fixed forever." For a salaried person that is wrong, and the misconception causes unnecessary anxiety.',
          'Fourth: "nothing at all is allowed under the new regime." Some reliefs apply under both, and leaving them out makes the calculation wrong. This is why building your own complete list is necessary.',
        ],
      },
      {
        hHi: 'जब निश्चित न हो पाएँ',
        hEn: 'When you cannot be certain',
        pHi: [
          'कुछ स्थितियाँ ऐसी होती हैं जिनमें ईमानदार उत्तर यही है कि अभी तय नहीं किया जा सकता, और यह मान लेना कमज़ोरी नहीं है।',
          'सबसे आम ऐसी स्थिति वह है जब वर्ष के बीच में कोई बड़ा बदलाव आने वाला हो — नई नौकरी, घर की ख़रीद, विवाह, या आय में बड़ा परिवर्तन। ऐसे वर्ष में शुरुआत में की गई कोई भी गणना अनुमान ही रहेगी।',
          'ऐसी स्थिति में व्यावहारिक रास्ता यह है: नियोक्ता को वह विकल्प बताइए जो आज की स्थिति में ठीक बैठता है, पूरे वर्ष के दस्तावेज़ सँभालिए, और अंतिम निर्णय रिटर्न भरते समय लीजिए, जब सारे आँकड़े सामने हों।',
          'और यदि राशि बड़ी है या स्थिति उलझी हुई है, तो एक बार किसी योग्य कर सलाहकार से पूछ लेना सबसे सस्ता निर्णय होता है। एक परामर्श का शुल्क प्रायः उस अंतर से बहुत कम होता है जो एक ग़लत चुनाव पैदा कर देता है।',
        ],
        pEn: [
          'Some situations genuinely cannot be settled in advance, and accepting that is not a weakness.',
          'The most common of these is a year in which something large is going to change — a new job, buying a house, a marriage, or a significant change in income. In such a year any calculation made at the start remains an estimate.',
          'The practical route then is: tell your employer the option that fits today\'s position, keep documentation across the whole year, and take the final decision at filing time when all the figures are actually available.',
          'And if the amounts are large or the situation is tangled, asking a qualified tax adviser once is the cheapest decision available. The fee for a single consultation is usually far smaller than the difference a wrong choice produces.',
        ],
      },
    ],
  },

  // ---------------------------------------------------------------------
  // Batch four.
  // ---------------------------------------------------------------------

  {
    slug: 'term-insurance-kyon-kaafi-hai',
    sections: [
      {
        hHi: 'कवर की राशि कैसे निकालें',
        hEn: 'How to arrive at the cover amount',
        pHi: [
          'टर्म बीमा का सबसे महत्वपूर्ण निर्णय कंपनी का चुनाव नहीं, राशि का चुनाव है — और यही वह जगह है जहाँ सबसे अधिक लोग अंदाज़े से काम चला लेते हैं।',
          'सबसे सरल तरीक़ा आय-प्रतिस्थापन का है: आपकी वार्षिक आय को उन वर्षों से गुणा कीजिए जितने वर्षों तक परिवार को आपकी आय की ज़रूरत रहेगी। यदि बच्चे छोटे हैं, तो यह अवधि लंबी होगी।',
          'इसमें दो जोड़ लगते हैं। पहला, सभी बकाया ऋण — गृह ऋण, वाहन ऋण, कोई व्यक्तिगत ऋण। दूसरा, कोई बड़ा भविष्य का ख़र्च जो निश्चित है, जैसे बच्चों की उच्च शिक्षा।',
          'और एक घटाव: वह संपत्ति जो पहले से मौजूद है और तुरंत काम आ सकती है। बाक़ी बची संख्या ही आपका कवर है।',
          'यह गणना दस मिनट की है और अधिकांश लोगों को चौंका देती है, क्योंकि वास्तविक ज़रूरत उस राशि से कहीं बड़ी निकलती है जो आमतौर पर ख़रीदी जाती है।',
        ],
        pEn: [
          'The most important decision in term insurance is not which company but how much — and this is exactly where most people settle for a guess.',
          'The simplest method is income replacement: multiply your annual income by the number of years your family would need that income for. If the children are young, that period is long.',
          'Two additions follow. First, all outstanding debt — home loan, vehicle loan, any personal borrowing. Second, any large future expense that is effectively certain, such as higher education for children.',
          'And one subtraction: assets that already exist and could be used immediately. What remains is your cover.',
          'This calculation takes ten minutes and surprises most people, because the real requirement comes out considerably larger than the amount usually bought.',
        ],
      },
      {
        hHi: 'जो राइडर लेने लायक़ हैं और जो नहीं',
        hEn: 'Which riders are worth taking and which are not',
        pHi: [
          'टर्म पॉलिसी के साथ कई अतिरिक्त सुविधाएँ बेची जाती हैं, और वे सब एक जैसी उपयोगी नहीं हैं। कुछ वास्तविक अंतराल भरती हैं और कुछ केवल प्रीमियम बढ़ाती हैं।',
          'सबसे उपयोगी आमतौर पर वह है जो स्थायी विकलांगता की स्थिति में भविष्य के प्रीमियम माफ़ कर देती है। इसका कारण यह है कि विकलांगता में आय रुक जाती है पर पॉलिसी की ज़रूरत बनी रहती है — यानी ठीक उसी समय प्रीमियम भरना कठिन हो जाता है।',
          'दूसरा जो कई लोगों के लिए उपयोगी है वह दुर्घटना से जुड़ा अतिरिक्त कवर है, विशेषकर उन लोगों के लिए जिनका काम यात्रा या क्षेत्र में अधिक है।',
          'गंभीर बीमारी वाला राइडर अलग विचार माँगता है। वह उपयोगी हो सकता है, पर उसकी शर्तें विस्तृत होती हैं और वह प्रायः एक अलग स्वास्थ्य पॉलिसी से बेहतर ढंग से हल किया जा सकता है। दोनों की तुलना किए बिना उसे जोड़ लेना ठीक नहीं।',
          'सामान्य नियम यह है: हर राइडर के लिए यह पूछिए कि वह किस विशिष्ट स्थिति को कवर करता है और क्या वह स्थिति पहले से किसी और पॉलिसी में शामिल है। दोहराव के लिए प्रीमियम देना सबसे आम अनावश्यक ख़र्च है।',
        ],
        pEn: [
          'Several add-ons are sold alongside a term policy, and they are not equally useful. Some fill a real gap and some only add to the premium.',
          'The most useful is generally the one that waives future premiums in the event of permanent disability. The logic is that disability stops income while the need for the policy continues — so paying the premium becomes hard at precisely the wrong moment.',
          'The second that suits many people is additional accident cover, particularly for those whose work involves travel or fieldwork.',
          'A critical illness rider needs separate thought. It can be useful, but its conditions run into detail and the same need is often better addressed by a separate health policy. Adding it without comparing the two is not sensible.',
          'The general rule: for every rider, ask which specific situation it covers and whether that situation is already covered elsewhere. Paying premium for duplication is the most common unnecessary cost in insurance.',
        ],
      },
      {
        hHi: 'क्लेम सेटलमेंट अनुपात को कैसे पढ़ें',
        hEn: 'How to read the claim settlement ratio',
        pHi: [
          'यह वह आँकड़ा है जिसे विज्ञापनों में सबसे अधिक दिखाया जाता है और जिसे सबसे कम समझा जाता है।',
          'यह अनुपात बताता है कि किसी वर्ष में कंपनी ने कितने प्रतिशत दावों का भुगतान किया। यह उपयोगी है, पर यह अकेला पर्याप्त नहीं है, क्योंकि इसमें छोटी और बड़ी राशि के दावे एक साथ गिने जाते हैं।',
          'एक अधिक उपयोगी दृष्टि यह देखना है कि अनुपात कई वर्षों तक कैसा रहा। किसी एक वर्ष का ऊँचा आँकड़ा उतना नहीं बताता जितना पाँच वर्षों की स्थिरता बताती है।',
          'और सबसे महत्वपूर्ण बात यह है कि दावा अस्वीकार होने का सबसे आम कारण कंपनी नहीं बल्कि प्रस्ताव-पत्र में दी गई अधूरी जानकारी होती है। यानी इस आँकड़े को सुधारने में आपकी अपनी भूमिका सबसे बड़ी है।',
          'इसलिए कंपनी चुनते समय अनुपात देखिए, पर उसे निर्णायक मत बनाइए। पूरी और सच्ची घोषणा किसी भी अनुपात से अधिक सुरक्षा देती है।',
        ],
        pEn: [
          'This is the figure shown most often in advertising and understood least often by the people reading it.',
          'The ratio states what percentage of claims a company paid in a given year. It is useful, and it is not sufficient on its own, because small and large claims are counted together within it.',
          'A more useful view is how the ratio has held across several years. A high number in one year says considerably less than steadiness across five.',
          'And the most important point is that the commonest reason for a rejected claim is not the company but incomplete information given on the proposal form. Your own role in that figure is therefore the largest one.',
          'So look at the ratio when choosing an insurer, but do not make it decisive. Full and truthful disclosure gives more protection than any ratio.',
        ],
      },
      {
        hHi: 'यदि आपके पास पहले से एंडोमेंट पॉलिसी है',
        hEn: 'If you already hold an endowment policy',
        pHi: [
          'यह सबसे व्यावहारिक प्रश्न है, क्योंकि बहुत से लोग यह लेख पढ़ते समय पहले से ऐसी पॉलिसी रखे हुए होते हैं और सोचते हैं कि अब क्या किया जाए।',
          'पहला क़दम यह देखना है कि पॉलिसी को कितने वर्ष हो चुके हैं। शुरुआती वर्षों में समर्पण मूल्य बहुत कम होता है, इसलिए वहाँ बाहर निकलने की लागत सबसे अधिक है।',
          'दूसरा विकल्प "पेड-अप" है, जिसमें आप आगे प्रीमियम देना बंद कर देते हैं पर पॉलिसी घटे हुए कवर के साथ चलती रहती है। यह प्रायः समर्पण से बेहतर होता है, क्योंकि इसमें तुरंत बड़ा नुक़सान नहीं होता।',
          'तीसरा और सबसे महत्वपूर्ण नियम: पुरानी पॉलिसी बंद करने से पहले नई टर्म पॉलिसी जारी हो जानी चाहिए। बीच में कोई अंतराल नहीं रहना चाहिए, क्योंकि उस अंतराल में परिवार असुरक्षित रहता है।',
          'और यह निर्णय गणना पर आधारित होना चाहिए, भावना पर नहीं। अब तक दिया गया प्रीमियम डूबी हुई लागत है; प्रश्न यह नहीं है कि पहले क्या दिया, बल्कि यह है कि आगे के वर्षों के लिए सबसे अच्छा क्या है।',
        ],
        pEn: [
          'This is the most practical question of all, because many people reading an article like this already hold such a policy and are wondering what to do now.',
          'The first step is to see how many years the policy has run. Surrender value is very low in the early years, so that is where the cost of exiting is highest.',
          'The second option is making it "paid-up", where you stop paying further premiums and the policy continues at a reduced cover. This is often better than surrendering, because it avoids an immediate large loss.',
          'The third and most important rule: the new term policy must be issued before the old one is stopped. There should be no gap in between, because during that gap the family is unprotected.',
          'And the decision should rest on arithmetic rather than feeling. Premiums already paid are a sunk cost; the question is not what was paid before but what is best for the years ahead.',
        ],
      },
      {
        hHi: 'नामांकन और परिवार तक पैसा पहुँचना',
        hEn: 'Nomination, and the money actually reaching the family',
        pHi: [
          'बीमा का पूरा उद्देश्य यह है कि ज़रूरत के समय पैसा परिवार तक पहुँचे, और इस अंतिम चरण पर सबसे कम ध्यान दिया जाता है।',
          'सबसे बुनियादी बात नामांकन है। यह भरा हुआ, सही और अद्यतन होना चाहिए। विवाह, तलाक़, या किसी नामित व्यक्ति की मृत्यु के बाद इसे बदलना ज़रूरी है, और यह काम प्रायः वर्षों तक टलता रहता है।',
          'दूसरी बात परिवार को बताने की है। एक ऐसी पॉलिसी जिसके बारे में किसी को पता ही नहीं, उसका मूल्य शून्य है। पॉलिसी नंबर, कंपनी का नाम और दस्तावेज़ कहाँ रखे हैं — यह जानकारी कम से कम दो लोगों के पास होनी चाहिए।',
          'तीसरी, कुछ स्थितियों में विवाहित महिला संपत्ति अधिनियम के अंतर्गत पॉलिसी लेना विचारणीय होता है, क्योंकि इससे राशि पत्नी और बच्चों के लिए सुरक्षित हो जाती है और अन्य दावेदारों की पहुँच से बाहर रहती है। यह हर किसी के लिए ज़रूरी नहीं, पर जिन पर लागू है उनके लिए महत्वपूर्ण है।',
          'ये तीनों काम मिलाकर एक घंटे के हैं और पॉलिसी के पूरे मूल्य को सुरक्षित करते हैं। इनके बिना सबसे अच्छी पॉलिसी भी अधूरी है।',
        ],
        pEn: [
          'The entire purpose of insurance is that money reaches the family when it is needed, and this final stage receives the least attention of all.',
          'The most basic point is nomination. It should be filled in, correct and current. Updating it after a marriage, a divorce, or the death of a nominee is necessary, and it is a task that gets postponed for years.',
          'The second is telling the family. A policy nobody knows about has a value of zero. The policy number, the insurer\'s name and where the documents are kept should be known to at least two people.',
          'Third, in some situations taking the policy under the Married Women\'s Property Act is worth considering, because it secures the amount for a wife and children and keeps it beyond the reach of other claimants. It is not necessary for everybody, but it matters greatly for those it applies to.',
          'Those three tasks take about an hour between them and they protect the entire value of the policy. Without them even the best policy is incomplete.',
        ],
      },
      {
        hHi: 'प्रीमियम सस्ता रखने के वैध तरीक़े',
        hEn: 'Legitimate ways to keep the premium low',
        pHi: [
          'टर्म बीमा पहले से ही सस्ता है, पर कुछ वैध तरीक़े हैं जिनसे वह और सस्ता रहता है — और इनमें से कोई भी जानकारी छिपाने से जुड़ा नहीं है।',
          'सबसे बड़ा कारक उम्र है। प्रीमियम पूरी अवधि के लिए ख़रीद के समय की उम्र पर तय होता है, इसलिए एक वर्ष की देरी भी पूरे तीस वर्षों के प्रीमियम को ऊपर कर देती है। जल्दी ख़रीदना अकेला सबसे बड़ा बचत का तरीक़ा है।',
          'दूसरा कारक धूम्रपान है, और इसका असर बहुत बड़ा होता है। यहाँ यह स्पष्ट कर देना ज़रूरी है कि इसे छिपाना नहीं चाहिए — जाँच में यह पकड़ा जाता है और दावा अस्वीकार हो सकता है।',
          'तीसरा, वार्षिक भुगतान चुनना प्रायः मासिक से सस्ता पड़ता है, क्योंकि किश्तों में भुगतान पर एक छोटा अतिरिक्त शुल्क लगता है।',
          'और चौथा: सीधे ऑनलाइन ख़रीदी गई पॉलिसी प्रायः सस्ती होती है, क्योंकि उसमें वितरण की लागत कम होती है। यह सुविधा हर किसी के लिए उपयुक्त नहीं है, पर जो लोग स्वयं तुलना कर सकते हैं उनके लिए यह एक वास्तविक बचत है।',
        ],
        pEn: [
          'Term insurance is already inexpensive, and a few legitimate approaches keep it cheaper still — none of which involve withholding information.',
          'The largest factor is age. The premium is fixed for the whole term at the age of purchase, so even a year\'s delay raises the cost across all thirty years. Buying early is the single biggest saving available.',
          'The second factor is smoking, and its effect is large. It has to be said plainly that this must not be concealed — testing catches it and a claim can be rejected on that basis.',
          'Third, paying annually is usually cheaper than paying monthly, because instalment payment carries a small additional charge.',
          'And fourth: a policy bought directly online is often cheaper, because the distribution cost is lower. That route does not suit everybody, but for people able to compare on their own it is a genuine saving.',
        ],
      },
    ],
  },

  {
    slug: 'fd-vs-debt-funds',
    sections: [
      {
        hHi: 'समय से पहले पैसा निकालने पर क्या होता है',
        hEn: 'What happens when money is taken out early',
        pHi: [
          'दोनों विकल्पों की तुलना में यह पहलू सबसे कम देखा जाता है और व्यवहार में सबसे अधिक काम आता है, क्योंकि योजना चाहे जो हो, पैसे की ज़रूरत कभी भी आ सकती है।',
          'सावधि जमा में समय से पहले निकासी संभव है, पर उस पर एक दंड लगता है — प्रायः ब्याज दर घटा दी जाती है और कुछ मामलों में एक अतिरिक्त कटौती भी। यानी आपको वह दर नहीं मिलती जिसका वादा था, बल्कि उस अवधि के लिए लागू कम दर मिलती है।',
          'डेब्ट फंड में कोई दंड नहीं होता, पर एक अलग बात है: आपको उस दिन का मूल्य मिलता है। यदि बाज़ार में ब्याज दरें बढ़ी हुई हैं, तो वह मूल्य आपकी अपेक्षा से कम हो सकता है।',
          'कुछ फंडों में बहुत जल्दी निकासी पर एक छोटा निकास भार भी लगता है, इसलिए योजना बनाते समय यह देख लेना चाहिए कि वह अवधि कितनी है।',
          'निष्कर्ष यह है कि दोनों में तरलता है पर उसकी क़ीमत अलग रूप में चुकानी पड़ती है — एक में निश्चित दंड, दूसरे में अनिश्चित मूल्य।',
        ],
        pEn: [
          'This is the least examined aspect when the two are compared and the most useful in practice, because whatever the plan, money can be needed at any time.',
          'A fixed deposit can be broken early, and a penalty applies — usually the interest rate is reduced and in some cases a further deduction is made. You do not receive the promised rate but the lower rate applicable to the period actually completed.',
          'A debt fund carries no penalty, but something different applies: you receive that day\'s value. If interest rates in the market have risen, that value can be lower than you expected.',
          'Some funds also apply a small exit load on very early redemption, so it is worth checking how long that period runs before planning around it.',
          'The conclusion is that both are liquid, but the cost of that liquidity is paid in different currencies — a fixed penalty in one, an uncertain value in the other.',
        ],
      },
      {
        hHi: 'ब्याज दर जोखिम को सरल भाषा में',
        hEn: 'Interest rate risk in plain language',
        pHi: [
          'यह वह अवधारणा है जो डेब्ट फंड को समझने के लिए सबसे ज़रूरी है और जिसे सबसे कम समझाया जाता है।',
          'सरल तरीक़े से: जब बाज़ार में नई ब्याज दरें बढ़ती हैं, तो पहले से जारी कम ब्याज वाले बॉन्ड कम आकर्षक हो जाते हैं और उनका बाज़ार मूल्य गिर जाता है। चूँकि डेब्ट फंड ऐसे ही बॉन्ड रखता है, इसलिए दरें बढ़ने पर उसका मूल्य गिर सकता है।',
          'यही बात उल्टी दिशा में भी काम करती है: दरें गिरने पर पुराने ऊँची दर वाले बॉन्ड अधिक मूल्यवान हो जाते हैं और फंड का मूल्य बढ़ता है।',
          'इस उतार-चढ़ाव की तीव्रता इस बात पर निर्भर करती है कि फंड कितनी लंबी अवधि के बॉन्ड रखता है। बहुत कम अवधि वाले फंडों में यह असर छोटा होता है; लंबी अवधि वाले फंडों में यह उल्लेखनीय हो सकता है।',
          'इसलिए "डेब्ट फंड सुरक्षित है" कहना अधूरा है। प्रश्न यह है कि कौन-सा डेब्ट फंड, कितनी अवधि का, और आपको पैसा कब चाहिए।',
        ],
        pEn: [
          'This is the concept most necessary for understanding debt funds and the one explained least often.',
          'Put simply: when new interest rates in the market rise, existing bonds paying lower rates become less attractive and their market value falls. Because a debt fund holds such bonds, its value can fall when rates rise.',
          'The same works in the other direction: when rates fall, older bonds paying higher rates become more valuable and the fund\'s value rises.',
          'How strongly this moves depends on the maturity of the bonds the fund holds. In very short-duration funds the effect is small; in long-duration funds it can be substantial.',
          'So saying "debt funds are safe" is incomplete. The question is which debt fund, of what duration, and when you need the money.',
        ],
      },
      {
        hHi: 'क्रेडिट जोखिम: दूसरा और कम दिखने वाला जोखिम',
        hEn: 'Credit risk: the second and less visible risk',
        pHi: [
          'ब्याज दर जोखिम के अलावा एक दूसरा जोखिम है जो कम बार होता है पर जब होता है तो अधिक गंभीर होता है: वह उधार लेने वाला जिसने बॉन्ड जारी किया, समय पर भुगतान न कर पाए।',
          'सरकारी प्रतिभूतियों में यह जोखिम व्यावहारिक रूप से नहीं होता। कंपनियों के बॉन्ड में यह होता है, और जितनी कम रेटिंग वाली कंपनी होगी, उतना अधिक ब्याज वह देगी — क्योंकि ऊँचा ब्याज ही उस जोखिम का मुआवज़ा है।',
          'यही कारण है कि दो डेब्ट फंड, जो देखने में एक जैसे लगते हैं, बहुत अलग प्रतिफल दिखा सकते हैं। ऊँचा प्रतिफल प्रायः बेहतर प्रबंधन का नहीं, अधिक जोखिम का संकेत होता है।',
          'व्यावहारिक रूप से इसका अर्थ यह है कि फंड चुनते समय केवल पिछला प्रतिफल मत देखिए। यह देखिए कि फंड किस प्रकार की प्रतिभूतियाँ रखता है, और यह जानकारी हर फंड अपने दस्तावेज़ों में प्रकाशित करता है।',
          'सुरक्षित पैसे के लिए, जहाँ पूँजी की रक्षा मुख्य उद्देश्य है, वहाँ अधिक प्रतिफल का पीछा करना उद्देश्य के ही विरुद्ध है।',
        ],
        pEn: [
          'Besides interest rate risk there is a second risk that occurs less often and is more serious when it does: the borrower who issued the bond failing to pay on time.',
          'In government securities this risk is practically absent. In corporate bonds it exists, and the lower the rating of the company the higher the interest it pays — because that higher interest is the compensation for the risk.',
          'This is why two debt funds that look similar can show very different returns. A higher return is usually a signal of more risk rather than of better management.',
          'Practically, this means not judging a fund by its past return alone. Look at what kind of securities it holds, and every fund publishes that information in its documents.',
          'For safe money, where protecting the capital is the main objective, chasing a higher return works against the objective itself.',
        ],
      },
      {
        hHi: 'सावधि जमा की सीढ़ी बनाना',
        hEn: 'Building a ladder of fixed deposits',
        pHi: [
          'सावधि जमा की सबसे बड़ी शिकायत यह है कि पैसा एक अवधि के लिए बँध जाता है। एक साधारण तकनीक इस समस्या का बड़ा हिस्सा हल कर देती है, और उसे सीढ़ी कहते हैं।',
          'पूरी राशि एक ही जमा में डालने के बजाय उसे कई हिस्सों में बाँटिए और अलग-अलग अवधि की जमाओं में रखिए — मान लीजिए एक, दो, तीन, चार और पाँच साल।',
          'इसका परिणाम यह होता है कि हर साल एक जमा परिपक्व होती है। यदि पैसे की ज़रूरत है तो वह उपलब्ध है, और यदि नहीं है तो उसे फिर से सबसे लंबी अवधि के लिए लगा दीजिए।',
          'इससे दो लाभ एक साथ मिलते हैं: हर साल तरलता, और औसतन लंबी अवधि की बेहतर दर। और कोई जमा तोड़नी नहीं पड़ती, इसलिए दंड का प्रश्न ही नहीं उठता।',
          'यह तकनीक विशेष रूप से उन लोगों के लिए उपयोगी है जो सेवानिवृत्त हैं और जिन्हें नियमित अंतराल पर पैसे की ज़रूरत होती है।',
        ],
        pEn: [
          'The main complaint about fixed deposits is that money gets locked in for a term. A simple technique solves a large part of that problem, and it is called laddering.',
          'Instead of putting the whole amount into one deposit, split it into several parts and place them in deposits of different terms — say one, two, three, four and five years.',
          'The result is that one deposit matures every year. If the money is needed it is available, and if it is not, it goes back in for the longest term again.',
          'That gives two benefits at once: liquidity every year, and on average the better rate that longer terms carry. And no deposit ever has to be broken, so the question of a penalty does not arise.',
          'The technique is particularly useful for retired people who need money at regular intervals rather than all at once.',
        ],
      },
      {
        hHi: '"सुरक्षित" शब्द का सही अर्थ',
        hEn: 'What the word "safe" actually means here',
        pHi: [
          'दोनों विकल्पों की चर्चा में "सुरक्षित" शब्द बार-बार आता है, और उसका अर्थ हर बार एक जैसा नहीं होता। इस अस्पष्टता से बहुत सी ग़लतफ़हमियाँ पैदा होती हैं।',
          'सावधि जमा में सुरक्षा का अर्थ है कि राशि निश्चित है — आपको ठीक पता है कि परिपक्वता पर कितना मिलेगा। यह निश्चितता वास्तविक है और इसका मूल्य है।',
          'पर एक दूसरी दृष्टि से यही सुरक्षा अधूरी है: यदि मुद्रास्फीति उस दर से अधिक है जो आपको मिल रही है, तो आपकी राशि बढ़ रही है पर उसकी क्रय-शक्ति घट रही है। यह भी एक प्रकार का नुक़सान है, केवल कम दिखने वाला।',
          'डेब्ट फंड में राशि निश्चित नहीं है, पर वह मुद्रास्फीति के साथ थोड़ा बेहतर चल सकती है। यहाँ अनिश्चितता दिखती है, इसलिए वह अधिक डरावनी लगती है, भले ही दीर्घकाल में परिणाम बेहतर हो।',
          'इसलिए चुनाव "सुरक्षित बनाम जोखिम भरा" नहीं है। यह चुनाव है कि आप कौन-सी अनिश्चितता स्वीकार करना चाहते हैं — दिखने वाली, या न दिखने वाली।',
        ],
        pEn: [
          'The word "safe" appears repeatedly in this comparison, and it does not mean the same thing each time. A great deal of confusion comes from that ambiguity.',
          'In a fixed deposit, safety means the amount is certain — you know exactly what will arrive at maturity. That certainty is real and it has value.',
          'But from another angle that safety is incomplete: if inflation runs above the rate you are receiving, your amount is growing while its purchasing power shrinks. That is also a loss, merely a less visible one.',
          'In a debt fund the amount is not certain, but it may keep up with inflation somewhat better. The uncertainty here is visible, which makes it feel more frightening even where the long-run result is better.',
          'So the choice is not "safe versus risky". It is a choice about which uncertainty you would rather accept — the visible one, or the invisible one.',
        ],
      },
      {
        hHi: 'एक व्यावहारिक बँटवारा',
        hEn: 'A practical way to split between them',
        pHi: [
          'चूँकि दोनों की ताक़त अलग है, इसलिए अधिकांश लोगों के लिए सही उत्तर "कौन-सा" नहीं बल्कि "कितना-कितना" है।',
          'एक सरल ढाँचा उद्देश्य के अनुसार बाँटने का है। जो पैसा एक वर्ष के भीतर चाहिए — आपातकालीन कोष, कोई तय ख़र्च — उसके लिए निश्चितता अधिक मायने रखती है, और वहाँ बचत खाता या छोटी अवधि की जमा उपयुक्त है।',
          'जो पैसा एक से तीन वर्ष में चाहिए, वहाँ दोनों काम कर सकते हैं, और चुनाव कर की स्थिति तथा आपकी सुविधा पर निर्भर करता है।',
          'जो पैसा तीन वर्ष से अधिक के लिए है और जिसका उद्देश्य वृद्धि है, वह वास्तव में इस पूरी तुलना के बाहर है — वहाँ प्रश्न सुरक्षित साधनों के बीच का नहीं रह जाता।',
          'यह ढाँचा किसी विशेष उत्पाद की सिफ़ारिश नहीं करता। यह केवल यह कहता है कि पैसे को उसके उद्देश्य और समय-सीमा से मिलाइए — और यही एकमात्र नियम है जो हर स्थिति में काम करता है।',
        ],
        pEn: [
          'Because the two have different strengths, the right answer for most people is not "which one" but "how much of each".',
          'A simple framework divides by purpose. Money needed within a year — an emergency fund, a committed expense — is money where certainty matters more, and a savings account or short deposit fits there.',
          'Money needed in one to three years can sit in either, and the choice depends on the tax position and on what you find convenient to manage.',
          'Money for more than three years whose purpose is growth is really outside this comparison altogether — there the question is no longer one between safe instruments.',
          'This framework recommends no particular product. It only says to match money to its purpose and its horizon — which is the one rule that works in every situation.',
        ],
      },
    ],
  },

  {
    slug: 'sip-band-karne-ki-galti',
    sections: [
      {
        hHi: 'वे स्थितियाँ जिनमें रोकना सही है',
        hEn: 'The situations in which stopping is the right call',
        pHi: [
          'यह लेख गिरावट में SIP रोकने के विरुद्ध है, पर ईमानदारी यह माँगती है कि उन स्थितियों को भी बताया जाए जिनमें रोकना सही निर्णय है — क्योंकि वे मौजूद हैं।',
          'पहली स्थिति: आय रुक गई हो। नौकरी जाने या व्यवसाय बंद होने पर SIP रोकना समझदारी है, क्योंकि तब प्राथमिकता निवेश नहीं, गुज़ारा है। यहाँ रोकना घबराहट नहीं, प्रबंधन है।',
          'दूसरी: कोई ऊँचे ब्याज वाला क़र्ज़ चुकाना हो। यदि क्रेडिट कार्ड पर बकाया है, तो उस पर लगने वाला ब्याज किसी भी अपेक्षित निवेश-प्रतिफल से अधिक है। ऐसे में SIP रोककर क़र्ज़ चुकाना गणितीय रूप से बेहतर है।',
          'तीसरी: लक्ष्य पास आ गया हो। यदि जिस काम के लिए निवेश किया गया था वह अगले एक-दो साल में आने वाला है, तो योजना के अनुसार जोखिम कम करना और अंततः रोकना सही है।',
          'इन तीनों में एक बात समान है: निर्णय का कारण बाज़ार नहीं, आपकी अपनी परिस्थिति है। यही वह रेखा है जो सही रोकने और ग़लत रोकने को अलग करती है।',
        ],
        pEn: [
          'This article argues against stopping a SIP during a fall, and honesty requires naming the situations in which stopping is the correct decision — because they exist.',
          'The first: income has stopped. Halting a SIP after losing a job or closing a business is sensible, because the priority then is getting by rather than investing. Stopping here is management, not panic.',
          'The second: there is high-interest debt to clear. If a credit card balance is outstanding, the interest on it exceeds any expected investment return. Stopping the SIP to clear it is mathematically better.',
          'The third: the goal is close. If whatever the money was for arrives in the next year or two, reducing risk on schedule and eventually stopping is exactly right.',
          'All three share one feature: the reason for the decision is your own circumstance rather than the market. That line is what separates stopping correctly from stopping wrongly.',
        ],
      },
      {
        hHi: 'रोकना और रुकवाना — दो अलग चीज़ें',
        hEn: 'Pausing and cancelling are two different things',
        pHi: [
          'बहुत से लोगों को यह पता नहीं होता कि SIP को स्थायी रूप से रद्द करने के अलावा कुछ समय के लिए रोकने की सुविधा भी होती है, और यह अनजानापन अनावश्यक नुक़सान करता है।',
          'अधिकांश फंड हाउस कुछ महीनों के लिए किश्तें रोकने का विकल्प देते हैं। इस अवधि में कोई किश्त नहीं कटती, और अवधि समाप्त होने पर वही SIP अपने आप फिर शुरू हो जाती है।',
          'यह अंतर व्यवहार में बहुत बड़ा है। रद्द की गई SIP को दोबारा शुरू करने के लिए फिर से पूरी प्रक्रिया करनी पड़ती है, और अनुभव यह बताता है कि जो लोग रद्द करते हैं उनमें से बहुत से कभी दोबारा शुरू नहीं करते।',
          'इसलिए यदि किसी कारण से कुछ महीने कठिन हैं, तो पहला विकल्प रोकना होना चाहिए, रद्द करना नहीं। और दूसरा विकल्प — जो और भी बेहतर है — राशि घटा देना, ताकि आदत बनी रहे।',
        ],
        pEn: [
          'Many people do not know that besides cancelling a SIP permanently there is an option to pause it for a while, and that ignorance causes unnecessary loss.',
          'Most fund houses allow instalments to be paused for some months. No instalment is debited during that period, and at the end of it the same SIP restarts on its own.',
          'The difference matters enormously in practice. Restarting a cancelled SIP means going through the whole process again, and experience shows that many people who cancel never restart at all.',
          'So if some months are going to be difficult, the first option should be pausing rather than cancelling. And the second option — better still — is reducing the amount, so that the habit stays intact.',
        ],
      },
      {
        hHi: 'स्टेप-अप: वह सुविधा जिसका उपयोग कम होता है',
        hEn: 'Step-up: the facility that goes unused',
        pHi: [
          'रोकने के विपरीत छोर पर एक ऐसी सुविधा है जिसका उपयोग बहुत कम लोग करते हैं, और जो लंबी अवधि में सबसे बड़ा अंतर पैदा करती है।',
          'स्टेप-अप SIP में आप शुरुआत में ही यह तय कर देते हैं कि हर वर्ष किश्त एक निश्चित प्रतिशत या राशि से बढ़ जाएगी। इसके बाद यह अपने आप होता रहता है और आपको हर साल निर्णय नहीं लेना पड़ता।',
          'इसका लाभ यह है कि आपकी बचत आपकी आय के साथ बढ़ती है, न कि पीछे रह जाती है। अधिकांश लोगों की समस्या यह नहीं है कि उन्होंने शुरू नहीं किया; समस्या यह है कि दस साल बाद भी वे उतना ही डाल रहे हैं जितना पहले साल डालते थे।',
          'व्यावहारिक सुझाव यह है कि वृद्धि का प्रतिशत अपनी अपेक्षित वेतन-वृद्धि से थोड़ा कम रखिए। इससे बढ़ोतरी कभी बोझ नहीं लगती, और जीवन-स्तर भी सुधरता रहता है।',
        ],
        pEn: [
          'At the opposite end from stopping sits a facility very few people use, and it makes the largest difference of all over a long period.',
          'In a step-up SIP you decide at the outset that the instalment will rise by a fixed percentage or amount every year. After that it happens automatically and no annual decision is required.',
          'The benefit is that your saving grows with your income rather than falling behind it. Most people\'s problem is not that they never started; it is that ten years later they are still putting in what they put in during the first year.',
          'A practical suggestion is to set the increase slightly below your expected salary growth. Then the rise never feels like a burden, and the standard of living improves alongside it.',
        ],
      },
      {
        hHi: 'फंड ख़राब प्रदर्शन कर रहा हो तो',
        hEn: 'When the fund itself is doing badly',
        pHi: [
          'यह एक अलग प्रश्न है और इसे बाज़ार की गिरावट से गड्ड-मड्ड नहीं करना चाहिए। बाज़ार गिरने पर सब गिरते हैं; यहाँ प्रश्न यह है कि आपका फंड अपने जैसे फंडों से लगातार पीछे रह रहा हो।',
          'पहला क़दम सही तुलना है। किसी फंड को उसकी अपनी श्रेणी और उसके सूचकांक से तुलना कीजिए, किसी और श्रेणी के फंड से नहीं। एक डेब्ट फंड की तुलना इक्विटी फंड से करना कोई जानकारी नहीं देता।',
          'दूसरा क़दम धैर्य है। एक या दो तिमाही का पिछड़ना सामान्य है। जिस चीज़ पर ध्यान देना चाहिए वह है तीन से पाँच वर्ष का लगातार पिछड़ना, विशेषकर जब कारण स्पष्ट न हो।',
          'तीसरा, यदि बदलना ही हो तो पुरानी SIP रोककर नई शुरू कीजिए — पुराने निवेश को तुरंत निकालना ज़रूरी नहीं है। इससे कर और निकास भार दोनों का प्रभाव कम रहता है, और निर्णय क्रमशः लागू होता है।',
          'और एक सावधानी: फंड बदलना अपने आप में एक लागत है। जो व्यक्ति हर साल फंड बदलता रहता है, वह प्रायः उससे कम पाता है जो एक ठीक-ठाक फंड में टिका रहता है।',
        ],
        pEn: [
          'This is a separate question and should not be confused with a market fall. When the market falls everything falls; the question here is a fund consistently lagging others of its own kind.',
          'The first step is comparing correctly. Compare a fund with its own category and its own benchmark, not with a fund of a different type. Comparing a debt fund with an equity fund conveys no information at all.',
          'The second step is patience. Lagging for a quarter or two is ordinary. What deserves attention is consistent underperformance across three to five years, particularly when the reason is not clear.',
          'Third, if a change is warranted, stop the old SIP and start a new one — the existing investment does not have to be redeemed immediately. That limits the impact of both tax and exit load, and applies the decision gradually.',
          'And a caution: changing funds is itself a cost. Somebody who switches every year usually ends up with less than somebody who stayed put in a reasonable fund.',
        ],
      },
      {
        hHi: 'डरावने महीनों के लिए एक लिखित नियम',
        hEn: 'A written rule for the frightening months',
        pHi: [
          'सबसे प्रभावी बचाव यह है कि निर्णय उस समय लिया जाए जब आप डरे हुए न हों — यानी अभी, न कि तब जब बाज़ार गिर रहा हो।',
          'व्यावहारिक तरीक़ा यह है कि एक पन्ने पर तीन वाक्य लिख लिए जाएँ और उसे वहीं रखा जाए जहाँ ज़रूरत के समय दिख जाए। पहला वाक्य: मैंने यह पैसा किस लक्ष्य के लिए लगाया है और वह लक्ष्य कब है।',
          'दूसरा वाक्य: मैं किन तीन परिस्थितियों में यह SIP रोकूँगा — और उनमें "बाज़ार गिर रहा है" शामिल नहीं है।',
          'तीसरा वाक्य: यदि मैं फिर भी रोकना चाहूँ, तो मैं पहले सात दिन प्रतीक्षा करूँगा। यह सात दिन का नियम अकेला अधिकांश घबराहट भरे निर्णयों को रोक देता है, क्योंकि डर सात दिन नहीं टिकता।',
          'यह अभ्यास मूर्खतापूर्ण लग सकता है, और यह काम करता है — क्योंकि अपने ही हाथ का लिखा हुआ वाक्य किसी भी सलाह से अधिक भारी पड़ता है।',
        ],
        pEn: [
          'The most effective protection is to take the decision at a time when you are not frightened — that is, now, rather than while the market is falling.',
          'The practical method is to write three sentences on one page and keep it where it will be seen when needed. The first: what goal I invested this money for, and when that goal is.',
          'The second: the three circumstances in which I will stop this SIP — and "the market is falling" is not among them.',
          'The third: if I still want to stop, I will wait seven days first. That seven-day rule alone prevents most panicked decisions, because fear does not last seven days.',
          'The exercise can feel silly, and it works — because a sentence in your own handwriting carries more weight than any amount of advice from elsewhere.',
        ],
      },
      {
        hHi: 'बाद में देखने पर क्या दिखता है',
        hEn: 'What it looks like in hindsight',
        pHi: [
          'जिन लोगों ने किसी बड़ी गिरावट के दौरान अपनी SIP जारी रखी, वे लगभग हमेशा एक ही बात कहते हैं: उस समय यह सबसे कठिन लगा और बाद में यह सबसे अच्छा निर्णय निकला।',
          'इसका कारण यह है कि गिरावट के महीनों में ख़रीदी गई यूनिट सबसे सस्ती होती हैं, और जब बाज़ार लौटता है तो उन्हीं यूनिटों से सबसे बड़ा अंतर आता है। यह हिसाब बाद में साफ़ दिखता है और उस समय बिल्कुल नहीं दिखता।',
          'दूसरी ओर, जिन्होंने रोक दिया, उनके साथ प्रायः दो चीज़ें होती हैं। पहली, वे सस्ती यूनिट चूक जाते हैं। दूसरी, और अधिक महँगी, वे तब वापस लौटते हैं जब बाज़ार ऊपर जा चुका होता है — यानी महँगे में।',
          'यही वह पैटर्न है जो व्यवहार-वित्त में बार-बार दर्ज किया गया है, और यह किसी की बुद्धि की कमी नहीं है। यह केवल यह दिखाता है कि डर किसी भी गणना से तेज़ काम करता है।',
          'इसलिए इस पूरे विषय का सार यह है कि व्यवस्था को अपने भरोसे मत छोड़िए। स्वचालित किश्त, लिखा हुआ नियम, और खाता न देखने की आदत — ये तीनों उस क्षण के लिए बनाए जाते हैं जब तर्क काम करना बंद कर देता है।',
        ],
        pEn: [
          'People who kept their SIPs running through a major fall almost all say the same thing: it felt hardest at the time and turned out to be the best decision afterwards.',
          'The reason is that units bought during the falling months are the cheapest, and when the market recovers it is those units that make the largest difference. That arithmetic is obvious in hindsight and invisible at the time.',
          'On the other side, two things usually happen to those who stopped. First, they miss the cheap units. Second, and more expensively, they return once the market has already risen — that is, they buy dear.',
          'This is the pattern recorded over and over in behavioural finance, and it is not a failure of intelligence in anybody. It only shows that fear operates faster than arithmetic does.',
          'So the whole point of this subject is not to rely on yourself in the moment. An automatic instalment, a written rule, and the habit of not looking at the account — all three exist for the moment when reasoning stops working.',
        ],
      },
    ],
  },

  {
    slug: 'lifestyle-inflation-hindi',
    sections: [
      {
        hHi: 'इंक्रीमेंट के पहले तीस दिन',
        hEn: 'The first thirty days after an increment',
        pHi: [
          'जीवन-स्तर की मुद्रास्फीति किसी एक बड़े निर्णय से नहीं आती। वह वेतन बढ़ने के बाद के पहले महीने में, कई छोटे निर्णयों से आती है — और उसी महीने में वह लगभग स्थायी हो जाती है।',
          'इसका कारण यह है कि बढ़ी हुई राशि खाते में आते ही "उपलब्ध" महसूस होने लगती है, और जो उपलब्ध महसूस होता है वह कुछ हफ़्तों में सामान्य बन जाता है। एक बार सामान्य बन जाने के बाद उसे कम करना कहीं अधिक कठिन होता है।',
          'सबसे प्रभावी हस्तक्षेप इसी खिड़की में होता है। वेतन बढ़ने की सूचना मिलते ही, और उसके खाते में आने से पहले, बढ़ी हुई राशि का एक हिस्सा स्वचालित रूप से किसी और जगह भेजने का निर्देश दे दीजिए।',
          'यह क्रम महत्वपूर्ण है। यदि आप पहले पैसा देख लेंगे और फिर बचाने की कोशिश करेंगे, तो आप उस राशि से लड़ रहे होंगे जिसकी आदत बन चुकी है। यदि वह कभी दिखे ही नहीं, तो कोई लड़ाई ही नहीं होती।',
        ],
        pEn: [
          'Lifestyle inflation does not arrive through one large decision. It arrives in the first month after a raise, through many small ones — and within that same month it becomes very nearly permanent.',
          'The reason is that the increased amount starts to feel "available" the moment it lands, and what feels available becomes normal within a few weeks. Once it has become normal, reducing it is far harder.',
          'The most effective intervention sits inside that window. As soon as a raise is confirmed and before it reaches the account, set up an automatic instruction sending part of the increase somewhere else.',
          'The order matters. If you see the money first and then try to save it, you are fighting an amount you have already grown used to. If it never appears, there is no fight at all.',
        ],
      },
      {
        hHi: 'वे ख़र्च जो एक बार बढ़ने के बाद घटते नहीं',
        hEn: 'The costs that do not come back down',
        pHi: [
          'सभी ख़र्च बराबर नहीं हैं। कुछ ऐसे हैं जिन्हें ज़रूरत पड़ने पर घटाया जा सकता है, और कुछ ऐसे हैं जो एक बार बढ़ जाने के बाद वर्षों तक बँधे रहते हैं। यह भेद सबसे महत्वपूर्ण है।',
          'पहली श्रेणी में हैं: बाहर खाना, मनोरंजन, कपड़े, यात्रा। ये बढ़ते हैं और घटाए भी जा सकते हैं। ये जीवन-स्तर की मुद्रास्फीति का दिखने वाला हिस्सा हैं, और वास्तव में कम ख़तरनाक हिस्सा हैं।',
          'दूसरी श्रेणी में हैं: बड़े मकान का किराया, गाड़ी की EMI, बच्चों का महँगा स्कूल, और कोई भी दीर्घकालिक अनुबंध। ये एक बार शुरू होने के बाद अगले कई वर्षों के लिए तय हो जाते हैं।',
          'यही दूसरी श्रेणी असली जोखिम है, क्योंकि यदि आय गिर जाए तो इन्हें तुरंत घटाया नहीं जा सकता। मकान बदलना, स्कूल बदलना या गाड़ी बेचना महीनों का काम है और भावनात्मक रूप से कठिन भी।',
          'इसलिए व्यावहारिक नियम यह है: वेतन बढ़ने पर पहली श्रेणी में ख़र्च बढ़ाइए, दूसरी में नहीं। पहली श्रेणी सुख देती है और लचीली रहती है; दूसरी सुख देती है और आपको बाँध देती है।',
        ],
        pEn: [
          'Not all spending is equal. Some can be cut when necessary, and some locks you in for years once it rises. This distinction is the most important one in the subject.',
          'The first category holds eating out, entertainment, clothes and travel. These rise and they can also be reduced. They are the visible part of lifestyle inflation, and in truth the less dangerous part.',
          'The second holds rent on a larger home, a car EMI, an expensive school, and any long-term contract. Once started, these are fixed for several years ahead.',
          'That second category is the real risk, because if income falls they cannot be reduced quickly. Changing a home, changing a school or selling a car takes months and is emotionally difficult besides.',
          'So the practical rule is: when the salary rises, raise spending in the first category and not the second. The first gives pleasure and stays flexible; the second gives pleasure and ties you down.',
        ],
      },
      {
        hHi: 'बचत दर बनाम बचत की राशि',
        hEn: 'Savings rate against the amount saved',
        pHi: [
          'यह माप का प्रश्न है और यह पूरे विषय को स्पष्ट कर देता है, क्योंकि ग़लत माप ग़लत संतोष पैदा करता है।',
          'बहुत से लोग यह देखते हैं कि वे पहले से अधिक बचा रहे हैं और मान लेते हैं कि स्थिति सुधर रही है। यह भ्रामक हो सकता है: यदि आय दोगुनी हो गई और बचत केवल बीस प्रतिशत बढ़ी, तो वास्तव में स्थिति ख़राब हुई है।',
          'सही माप बचत दर है — आय का कितना प्रतिशत बच रहा है। यह संख्या आय बढ़ने के साथ ऊपर जानी चाहिए या कम से कम स्थिर रहनी चाहिए। यदि वह गिर रही है, तो जीवन-स्तर की मुद्रास्फीति काम कर रही है, चाहे राशि बढ़ती दिखे।',
          'इस माप का एक और लाभ है: यह अलग-अलग आय-स्तरों पर तुलना योग्य है। एक व्यक्ति जो कम कमाकर तीस प्रतिशत बचाता है, उस व्यक्ति से बेहतर स्थिति में है जो अधिक कमाकर दस प्रतिशत बचाता है।',
          'इसलिए हर वर्ष एक ही संख्या दर्ज कीजिए: इस वर्ष मैंने अपनी आय का कितना प्रतिशत बचाया। यह अकेली संख्या पूरे वित्तीय वर्ष का सबसे ईमानदार सारांश है।',
        ],
        pEn: [
          'This is a question of measurement, and it clarifies the whole subject, because the wrong measure produces the wrong satisfaction.',
          'Many people see that they are saving more than before and conclude that things are improving. That can mislead: if income doubled and saving rose by only twenty percent, the position has actually worsened.',
          'The correct measure is the savings rate — what percentage of income is being saved. That number should rise as income rises, or at minimum stay flat. If it is falling, lifestyle inflation is at work whatever the amount appears to say.',
          'The measure has another advantage: it is comparable across different income levels. A person earning less and saving thirty percent is in a better position than one earning more and saving ten.',
          'So record one number every year: what percentage of my income did I save this year. That single figure is the most honest summary of an entire financial year.',
        ],
      },
      {
        hHi: 'सामाजिक तुलना की भूमिका',
        hEn: 'The part played by social comparison',
        pHi: [
          'जीवन-स्तर की मुद्रास्फीति का एक बड़ा हिस्सा भीतर से नहीं, बाहर से आता है — और इसे स्वीकार कर लेना उससे लड़ने का पहला क़दम है।',
          'हम अपनी स्थिति को निरपेक्ष रूप से नहीं मापते, बल्कि अपने आसपास के लोगों की तुलना में मापते हैं। और आसपास के लोग बदलते रहते हैं: नई नौकरी, नया मोहल्ला, नया समूह — हर बदलाव तुलना का नया पैमाना ले आता है।',
          'सबसे कठिन हिस्सा यह है कि तुलना दिखने वाली चीज़ों से होती है। आप किसी की गाड़ी देखते हैं, उसका क़र्ज़ नहीं; उसकी छुट्टी की तस्वीर देखते हैं, उसका बैंक विवरण नहीं।',
          'इसका कोई सरल समाधान नहीं है, पर एक उपयोगी अभ्यास है: किसी भी बड़ी ख़रीद से पहले यह लिख लीजिए कि आप इसे क्यों चाहते हैं। यदि कारण में कहीं किसी और का ज़िक्र आ जाए, तो एक सप्ताह और सोचिए।',
          'और दूसरा अभ्यास और भी सरल है: अपनी बचत दर की तुलना केवल अपने पिछले वर्ष से कीजिए, किसी और से नहीं। यही एकमात्र तुलना है जो सटीक भी है और उपयोगी भी।',
        ],
        pEn: [
          'A large part of lifestyle inflation comes from outside rather than from within — and accepting that is the first step in dealing with it.',
          'We do not measure our position in absolute terms but against the people around us. And those people keep changing: a new job, a new neighbourhood, a new circle — each change brings a new yardstick with it.',
          'The hardest part is that comparison runs on what is visible. You see somebody\'s car, not their loan; their holiday photograph, not their bank statement.',
          'There is no simple solution, but there is a useful exercise: before any large purchase, write down why you want it. If the reason mentions somebody else anywhere in it, think for another week.',
          'And a second exercise, simpler still: compare your savings rate only with your own previous year, never with anybody else\'s. That is the one comparison which is both accurate and useful.',
        ],
      },
      {
        hHi: 'बिना अपराध-बोध के ख़र्च करना',
        hEn: 'Spending without guilt',
        pHi: [
          'इस विषय की सबसे बड़ी ग़लतफ़हमी यह है कि इसका उद्देश्य कम ख़र्च करना है। उद्देश्य यह नहीं है। उद्देश्य यह है कि ख़र्च चुना हुआ हो, बहा हुआ नहीं।',
          'जो व्यक्ति हर ख़रीद पर अपराध-बोध महसूस करता है, वह उतनी ही ख़राब स्थिति में है जितना वह जो कुछ नहीं सोचता — क्योंकि दोनों ही स्थितियों में पैसा तनाव पैदा कर रहा है, आराम नहीं।',
          'व्यावहारिक हल वही तीन-डिब्बों वाला ढाँचा है। जब बचत का हिस्सा वेतन के दिन ही अलग हो जाता है और ज़रूरतें पूरी हो जाती हैं, तो जो बचता है वह पूरी तरह ख़र्च करने के लिए है — और उसे ख़र्च करते समय हिसाब लगाने की कोई ज़रूरत नहीं।',
          'यही इस पूरे अनुशासन का असली प्रतिफल है: यह कम ख़र्च नहीं करवाता, यह ख़र्च से चिंता निकाल देता है।',
          'और इसी कारण वेतन बढ़ने पर ख़र्च बढ़ाना ग़लत नहीं है। ग़लत केवल यह है कि पूरी बढ़ोतरी ख़र्च में चली जाए और बचत दर वहीं रह जाए जहाँ पाँच साल पहले थी।',
        ],
        pEn: [
          'The biggest misunderstanding in this subject is that its purpose is to spend less. That is not the purpose. The purpose is that spending should be chosen rather than drifted into.',
          'Somebody who feels guilty about every purchase is in as poor a position as somebody who never thinks at all — because in both cases money is producing anxiety rather than ease.',
          'The practical answer is the three-bucket structure. When the savings share leaves on payday and the needs are met, whatever remains exists entirely to be spent — and spending it requires no calculation whatsoever.',
          'That is the real return on this whole discipline: it does not make you spend less, it takes the worry out of spending.',
          'And for the same reason, spending more after a raise is not wrong. What is wrong is the entire increase going into spending while the savings rate stays exactly where it was five years ago.',
        ],
      },
      {
        hHi: 'साल में एक बार की जाँच',
        hEn: 'The once-a-year check',
        pHi: [
          'यह पूरा विषय एक वार्षिक अभ्यास में समेटा जा सकता है, और वह अभ्यास बीस मिनट का है।',
          'पहला क़दम: पिछले बारह महीनों की कुल आय और कुल बचत निकालिए, और बचत दर की गणना कीजिए। उसे कहीं लिख लीजिए, ताकि अगले वर्ष तुलना हो सके।',
          'दूसरा क़दम: अपने स्थायी मासिक दायित्वों की सूची बनाइए — किराया या EMI, फ़ीस, प्रीमियम, सदस्यताएँ। देखिए कि यह सूची पिछले वर्ष से कितनी बढ़ी। यही वह संख्या है जो आपकी लचीलापन बताती है।',
          'तीसरा क़दम: सदस्यताओं की सूची में से हर उस चीज़ को रद्द कीजिए जिसका उपयोग पिछले तीन महीनों में नहीं हुआ। यह छोटा काम प्रायः आश्चर्यजनक रूप से बड़ी राशि निकाल देता है।',
          'और चौथा: यदि इस वर्ष कोई वेतन-वृद्धि हुई है, तो जाँचिए कि उसका कितना हिस्सा बचत में गया। यदि उत्तर शून्य है, तो अगले वर्ष के लिए स्वचालित निर्देश अभी बदल दीजिए — क्योंकि अगली बढ़ोतरी तक याद नहीं रहेगा।',
        ],
        pEn: [
          'The whole subject can be reduced to one annual exercise, and that exercise takes twenty minutes.',
          'First: work out total income and total savings across the past twelve months and calculate the savings rate. Write it down somewhere so next year can be compared against it.',
          'Second: list your fixed monthly obligations — rent or EMI, fees, premiums, subscriptions. See how much that list has grown since last year. That figure is what tells you how flexible your position is.',
          'Third: from the list of subscriptions, cancel everything not used in the past three months. This small task usually frees up a surprisingly large amount.',
          'And fourth: if there was a raise this year, check how much of it went into savings. If the answer is nothing, change the automatic instruction for next year today — because by the time the next raise arrives it will have been forgotten.',
        ],
      },
    ],
  },

  {
    slug: 'home-loan-prepay-or-invest',
    sections: [
      {
        hHi: 'तुलना का सही आधार: दोनों तरफ़ कर के बाद',
        hEn: 'Comparing correctly: after tax on both sides',
        pHi: [
          'इस प्रश्न का उत्तर एक तुलना पर टिका है, और वह तुलना तभी सही होती है जब दोनों पक्षों को एक ही पैमाने पर रखा जाए — यानी कर के बाद।',
          'एक ओर आपके ऋण की प्रभावी लागत है। यदि आपको ब्याज पर कोई कर राहत मिलती है, तो आपकी वास्तविक लागत घोषित दर से कम है। यदि नहीं मिलती — जैसे किसी व्यवस्था में नहीं मिलती — तो लागत पूरी दर के बराबर है।',
          'दूसरी ओर निवेश से अपेक्षित प्रतिफल है, और उसे भी कर के बाद देखना चाहिए। सकल प्रतिफल की तुलना शुद्ध लागत से करना ही वह ग़लती है जो अधिकांश गणनाओं को बेकार कर देती है।',
          'जब दोनों संख्याएँ एक ही आधार पर आ जाएँ, तो तुलना सीधी है। और यहाँ एक और अंतर याद रखना ज़रूरी है: ऋण की लागत निश्चित है, जबकि निवेश का प्रतिफल अपेक्षित है। बराबर संख्याओं पर भी ये दोनों बराबर नहीं हैं।',
        ],
        pEn: [
          'The answer to this question rests on a comparison, and that comparison is only valid when both sides are placed on the same footing — that is, after tax.',
          'On one side is the effective cost of your loan. If you receive any tax relief on the interest, your real cost is below the stated rate. If you receive none — as under some arrangements — the cost equals the full rate.',
          'On the other side is the expected return from investing, and that also has to be looked at after tax. Comparing a gross return against a net cost is the error that renders most of these calculations useless.',
          'Once both numbers sit on the same basis the comparison is straightforward. And one further difference is worth remembering: the loan cost is certain while the investment return is expected. Even at equal numbers, the two are not equal.',
        ],
      },
      {
        hHi: 'समय का असर: शुरुआती वर्ष क्यों निर्णायक हैं',
        hEn: 'The effect of timing: why the early years decide everything',
        pHi: [
          'यदि आप पूर्व-भुगतान करने का निर्णय लेते हैं, तो उसका समय उसकी राशि से अधिक मायने रखता है, और यह बात अधिकांश लोगों को देर से पता चलती है।',
          'गृह ऋण की किश्त शुरुआती वर्षों में लगभग पूरी ब्याज होती है और मूल का हिस्सा बहुत छोटा। जैसे-जैसे वर्ष बीतते हैं, यह अनुपात उलटता जाता है।',
          'इसका अर्थ यह है कि पाँचवें वर्ष में किया गया एक लाख का पूर्व-भुगतान पंद्रहवें वर्ष में किए गए उतने ही भुगतान से कहीं अधिक ब्याज बचाता है। एक ही राशि, बहुत अलग असर।',
          'व्यावहारिक निष्कर्ष यह है कि यदि पूर्व-भुगतान करना है, तो जितनी जल्दी हो उतना अच्छा। और उसका उल्टा भी सही है: ऋण के अंतिम वर्षों में पूर्व-भुगतान का लाभ बहुत कम रह जाता है, इसलिए तब वही पैसा कहीं और लगाना अक्सर बेहतर होता है।',
          'इसे देखने का सबसे अच्छा तरीक़ा अपने ऋण की परिशोधन तालिका है, जो हर बैंक देता है। उसमें साफ़ दिख जाता है कि आपकी हर किश्त का कितना हिस्सा कहाँ जा रहा है।',
        ],
        pEn: [
          'If you decide to prepay, the timing of it matters more than the amount, and most people learn this later than they would have liked.',
          'In the early years a home loan instalment is very nearly all interest, with only a small part going to principal. As the years pass that ratio reverses.',
          'This means a prepayment of one lakh in year five saves considerably more interest than the same amount paid in year fifteen. Identical sums, very different effects.',
          'The practical conclusion is that if prepayment is going to happen, sooner is better. And the reverse holds too: in the final years of a loan the benefit of prepaying shrinks a great deal, so putting that money elsewhere is often better then.',
          'The best way to see this is your own amortisation schedule, which every bank provides. It shows exactly how much of each instalment is going where.',
        ],
      },
      {
        hHi: 'अवधि घटाएँ या किश्त — दो अलग नतीजे',
        hEn: 'Reduce the tenure or the instalment — two different outcomes',
        pHi: [
          'पूर्व-भुगतान करते समय बैंक प्रायः एक विकल्प देता है, और यह विकल्प इतना महत्वपूर्ण है कि उसे बिना सोचे नहीं छोड़ना चाहिए।',
          'पहला विकल्प यह है कि किश्त वही रहे और ऋण की अवधि घट जाए। दूसरा यह कि अवधि वही रहे और किश्त घट जाए।',
          'ब्याज बचाने की दृष्टि से पहला विकल्प स्पष्ट रूप से बेहतर है, क्योंकि ऋण जल्दी समाप्त होता है और कुल ब्याज कम लगता है। यदि आपकी किश्त देने की क्षमता में कोई बदलाव नहीं आया है, तो यही चुनना चाहिए।',
          'दूसरा विकल्प तब समझ में आता है जब मासिक नक़दी का दबाव हो — मान लीजिए आय घटी हो या कोई नया बड़ा ख़र्च शुरू हुआ हो। यहाँ आप ब्याज की बचत का कुछ हिस्सा छोड़कर मासिक राहत ख़रीद रहे हैं, और कभी-कभी यही सही निर्णय होता है।',
          'ध्यान देने वाली बात यह है कि यदि आप कुछ न कहें, तो बैंक प्रायः दूसरा विकल्प अपने आप लागू कर देता है। इसलिए पूर्व-भुगतान करते समय यह स्पष्ट रूप से लिखकर देना चाहिए कि आप क्या चाहते हैं।',
        ],
        pEn: [
          'When prepaying, a bank usually offers a choice, and that choice is important enough that it should not be left to default.',
          'The first option is that the instalment stays the same and the tenure shortens. The second is that the tenure stays the same and the instalment falls.',
          'For saving interest the first is clearly better, because the loan ends sooner and less total interest is charged. If your ability to pay the instalment has not changed, this is the one to choose.',
          'The second makes sense when monthly cash flow is under pressure — income has fallen, say, or a large new expense has begun. Here you are giving up part of the interest saving to buy monthly relief, and sometimes that is exactly right.',
          'What is worth knowing is that if you say nothing, the bank often applies the second option automatically. So state in writing what you want at the time of prepaying.',
        ],
      },
      {
        hHi: 'पूर्व-भुगतान की प्रक्रिया और शुल्क',
        hEn: 'The process and charges around prepayment',
        pHi: [
          'व्यावहारिक विवरण अक्सर छूट जाते हैं, और वही देरी और झुँझलाहट पैदा करते हैं।',
          'सबसे पहले शुल्क का प्रश्न। भारत में परिवर्तनीय दर वाले व्यक्तिगत गृह ऋणों पर पूर्व-भुगतान शुल्क आमतौर पर नहीं लगता। स्थिर दर वाले ऋणों और कुछ अन्य श्रेणियों पर स्थिति अलग हो सकती है, इसलिए अपने ऋण अनुबंध की शर्त देख लेना ज़रूरी है।',
          'दूसरा, भुगतान के बाद बैंक से संशोधित परिशोधन तालिका माँगिए। यह पुष्टि करती है कि राशि मूल में समायोजित हुई है और नया ढाँचा वही है जो आपने चुना था।',
          'तीसरा, हर पूर्व-भुगतान की रसीद सँभालिए। ऋण समाप्त होने पर आपको सभी मूल दस्तावेज़ और एक "नो ड्यूज़" प्रमाणपत्र मिलना चाहिए, और उस समय यह रिकॉर्ड काम आता है।',
          'चौथा और अंतिम: ऋण पूरा होने के बाद यह भी सुनिश्चित कीजिए कि संपत्ति पर से बैंक का भार हटा दिया गया है और यह सरकारी अभिलेख में दर्ज हो गया है। यह क़दम बहुत लोग छोड़ देते हैं और वर्षों बाद संपत्ति बेचते समय उसमें उलझते हैं।',
        ],
        pEn: [
          'Practical details often get skipped, and those are exactly what cause delay and irritation afterwards.',
          'First, charges. In India prepayment charges generally do not apply to floating-rate individual home loans. The position can differ for fixed-rate loans and some other categories, so check the terms of your own agreement.',
          'Second, ask the bank for a revised amortisation schedule after the payment. It confirms that the amount was applied to principal and that the new structure is the one you chose.',
          'Third, keep the receipt for every prepayment. When the loan closes you should receive all original documents and a no-dues certificate, and this record is useful at that point.',
          'Fourth and last: after the loan is closed, make sure the bank\'s charge on the property has been removed and recorded in the official register. A great many people skip this step and get tangled in it years later when selling.',
        ],
      },
      {
        hHi: 'दोनों में से किसी से पहले जो ज़रूरी है',
        hEn: 'What comes before either of them',
        pHi: [
          'यह पूरा प्रश्न — पूर्व-भुगतान या निवेश — तभी सार्थक है जब दो चीज़ें पहले से मौजूद हों। यदि वे नहीं हैं, तो अतिरिक्त पैसा उन्हीं में जाना चाहिए।',
          'पहली है आपातकालीन कोष। ऋण चुकाने में लगाया गया पैसा वापस नहीं आता; यदि उसके बाद कोई आपात स्थिति आए, तो आपको ऊँचे ब्याज पर उधार लेना पड़ेगा — और तब आपने सस्ते ऋण को महँगे ऋण से बदल दिया।',
          'दूसरी है पर्याप्त बीमा — जीवन और स्वास्थ्य दोनों। गृह ऋण एक लंबा दायित्व है, और यदि कमाने वाले के साथ कुछ हो जाए तो परिवार पर वह दायित्व बना रहता है। टर्म बीमा की राशि में बकाया ऋण जुड़ा होना चाहिए, यह पहले ही कहा जा चुका है।',
          'जब ये दोनों मौजूद हों, तभी अतिरिक्त पैसे पर यह प्रश्न लागू होता है। इससे पहले उत्तर सरल है और दोनों विकल्पों से बाहर है।',
        ],
        pEn: [
          'This whole question — prepay or invest — only becomes meaningful once two things are already in place. If they are not, the surplus money should go into them instead.',
          'The first is an emergency fund. Money put into repaying a loan does not come back; if an emergency follows, you will borrow at a higher rate — and at that point you have swapped cheap debt for expensive debt.',
          'The second is adequate insurance, both life and health. A home loan is a long obligation, and if something happens to the earner the obligation remains with the family. The outstanding loan should be included in the term cover, as noted earlier.',
          'Only when both of those exist does this question apply to surplus money. Before that, the answer is simple and lies outside both options.',
        ],
      },
      {
        hHi: 'गणित के बाद जो बचता है',
        hEn: 'What remains after the arithmetic',
        pHi: [
          'सारी गणना कर लेने के बाद भी एक हिस्सा बचता है जिसे संख्याओं में नहीं रखा जा सकता, और उसे नज़रअंदाज़ करना ईमानदार नहीं होगा।',
          'बहुत से लोगों के लिए क़र्ज़-मुक्त होना केवल आर्थिक स्थिति नहीं बल्कि एक मानसिक स्थिति है। यदि ऋण की वजह से नींद ख़राब होती है, तो थोड़ा कम प्रतिफल स्वीकार करके उसे चुका देना एक वैध निर्णय है।',
          'इसका उल्टा भी उतना ही वैध है। जो व्यक्ति ऋण को केवल एक अनुबंध की तरह देखता है और उससे कोई तनाव महसूस नहीं करता, उसके लिए गणित का पालन करना ठीक है।',
          'सबसे व्यावहारिक रास्ता प्रायः बीच का होता है: अतिरिक्त राशि का एक हिस्सा पूर्व-भुगतान में और एक हिस्सा निवेश में। इससे ब्याज भी घटता है, संपत्ति भी बनती है, और मन भी हल्का रहता है।',
          'और यही शायद सबसे महत्वपूर्ण बात है — वह योजना जिसे आप वर्षों तक निभा सकें, उस योजना से बेहतर है जो काग़ज़ पर थोड़ी अधिक कुशल हो पर जिसे आप छह महीने में छोड़ दें।',
        ],
        pEn: [
          'Even after all the arithmetic, a part remains that cannot be put into numbers, and ignoring it would not be honest.',
          'For many people being debt-free is not only a financial position but a mental one. If a loan is costing you sleep, accepting a slightly lower return in order to clear it is a legitimate decision.',
          'The reverse is equally legitimate. Somebody who treats a loan as simply a contract and feels no stress about it is right to follow the arithmetic instead.',
          'The most practical route is often in the middle: part of the surplus towards prepayment and part into investing. Interest falls, assets build, and the mind stays lighter than under either extreme.',
          'And that may be the most important point of all — a plan you can keep to for years beats a plan that is marginally more efficient on paper but which you abandon after six months.',
        ],
      },
    ],
  },

  // ---------------------------------------------------------------------
  // Batch five.
  // ---------------------------------------------------------------------

  {
    slug: 'term-insurance-guide',
    sections: [
      {
        hHi: 'चिकित्सा जाँच से घबराने की ज़रूरत नहीं',
        hEn: 'The medical test is not something to fear',
        pHi: [
          'बहुत से लोग केवल इसलिए टर्म बीमा टालते रहते हैं कि उसमें चिकित्सा जाँच करानी पड़ती है, और यह हिचक अक्सर ग़लतफ़हमी पर टिकी होती है।',
          'जाँच आमतौर पर बुनियादी होती है: रक्तचाप, कुछ रक्त परीक्षण, मूत्र परीक्षण, और उम्र या राशि के अनुसार कभी-कभी हृदय की जाँच। यह अधिकांश लोगों के लिए एक-दो घंटे का काम है और कई कंपनियाँ इसे घर पर करा देती हैं।',
          'और यह जाँच वास्तव में आपके पक्ष में है। जिस पॉलिसी में पूरी जाँच हुई हो, उसमें दावे के समय स्वास्थ्य-संबंधी विवाद की गुंजाइश बहुत कम रह जाती है, क्योंकि कंपनी आपकी स्थिति पहले से देख चुकी है।',
          'इसके उलट, "बिना जाँच के तुरंत पॉलिसी" वाले प्रस्ताव सुविधाजनक लगते हैं पर वहाँ जोखिम बाद के लिए टाल दिया जाता है — और वह बाद प्रायः दावे का दिन होता है।',
          'यदि जाँच में कुछ निकल आए, तो वह भी बुरी ख़बर नहीं है। पॉलिसी थोड़े अधिक प्रीमियम पर मिल जाती है, और उससे भी महत्वपूर्ण यह कि आपको अपनी स्थिति का पता समय पर चल जाता है।',
        ],
        pEn: [
          'Many people postpone term insurance purely because a medical test is involved, and that hesitation usually rests on a misunderstanding of what the test is.',
          'The examination is generally basic: blood pressure, some blood work, a urine test, and depending on age or sum insured sometimes a cardiac check. For most people it is an hour or two, and many insurers arrange it at home.',
          'And the test genuinely works in your favour. A policy issued after full medical examination leaves very little room for health-related dispute at claim time, because the insurer has already seen your condition.',
          'By contrast, offers of "instant policy without tests" look convenient but simply postpone the assessment — and that later assessment usually happens on the day of the claim.',
          'If something does show up in the test, that is not bad news either. The policy is issued at a somewhat higher premium, and more importantly you have found out about your own condition in time to act on it.',
        ],
      },
      {
        hHi: 'पॉलिसी की अवधि कब तक रखनी चाहिए',
        hEn: 'How long the policy term should run',
        pHi: [
          'यह प्रश्न कवर की राशि जितना ही महत्वपूर्ण है और उससे कहीं कम पूछा जाता है। अवधि बहुत छोटी हो तो सुरक्षा उसी समय समाप्त हो जाती है जब ज़रूरत बची होती है; बहुत लंबी हो तो अनावश्यक प्रीमियम जाता रहता है।',
          'सोचने का सही तरीक़ा यह है: पॉलिसी तब तक चलनी चाहिए जब तक कोई आपकी आय पर निर्भर है। इसलिए उत्तर आपकी उम्र से नहीं, आपके परिवार की स्थिति से निकलता है।',
          'व्यावहारिक रूप से इसका अर्थ प्रायः यह होता है कि अवधि आपकी अपेक्षित सेवानिवृत्ति तक जाए, या सबसे छोटे बच्चे के आर्थिक रूप से स्वतंत्र होने तक — इनमें से जो बाद में आए।',
          'यदि गृह ऋण है, तो एक तीसरी शर्त जुड़ जाती है: पॉलिसी कम से कम ऋण की शेष अवधि तक चलनी चाहिए, क्योंकि वह दायित्व आपके बाद भी बना रहता है।',
          'और एक बात जो अक्सर नहीं बताई जाती: बहुत लंबी अवधि — जैसे निन्यानबे वर्ष तक — का प्रीमियम काफ़ी अधिक होता है, और उसकी उपयोगिता तभी है जब आप संपत्ति हस्तांतरण की कोई विशेष योजना बना रहे हों। अधिकांश परिवारों के लिए यह आवश्यक नहीं।',
        ],
        pEn: [
          'This question matters as much as the sum insured and is asked far less often. Too short a term and the protection ends while the need continues; too long and premium is spent unnecessarily.',
          'The right way to think about it: the policy should run for as long as somebody depends on your income. So the answer comes from your family\'s position rather than from your age.',
          'In practice that usually means running the term until your expected retirement, or until the youngest child becomes financially independent — whichever comes later.',
          'If there is a home loan, a third condition attaches: the policy should run at least as long as the remaining loan, because that obligation survives you.',
          'And one point that is rarely made: very long terms — to ninety-nine, say — carry substantially higher premiums, and are useful only if you are planning something particular around transferring wealth. For most families they are not necessary.',
        ],
      },
      {
        hHi: 'फ़्री-लुक अवधि: पंद्रह दिन जो लोग गँवा देते हैं',
        hEn: 'The free-look period: fifteen days people waste',
        pHi: [
          'पॉलिसी दस्तावेज़ मिलने के बाद एक निश्चित अवधि होती है जिसमें आप उसे पढ़कर वापस कर सकते हैं और प्रीमियम लौटा दिया जाता है। यह अधिकार मौजूद है और लगभग कोई इसका उपयोग नहीं करता।',
          'इसका कारण सरल है: लोग पॉलिसी दस्तावेज़ पढ़ते ही नहीं। वह एक मोटा लिफ़ाफ़ा है जो अलमारी में चला जाता है, और उसे पहली बार तब खोला जाता है जब दावा करना होता है।',
          'इस अवधि में जो पढ़ना चाहिए वह चार चीज़ें हैं: कवर की राशि सही लिखी है या नहीं, अवधि सही है या नहीं, नामांकित व्यक्ति का नाम और विवरण ठीक है या नहीं, और वे सभी घोषणाएँ जो आपने दी थीं वैसी ही दर्ज हुई हैं या नहीं।',
          'चौथी बात सबसे महत्वपूर्ण है। कई बार एजेंट प्रस्ताव-पत्र भरते समय कोई जानकारी छोड़ देता है या ग़लत लिख देता है, और वह ग़लती दस्तावेज़ में चली आती है। दावे के समय वही ग़लती आपके ख़िलाफ़ पढ़ी जाती है, भले वह आपने न की हो।',
          'इसलिए दस्तावेज़ आते ही एक घंटा निकालकर उसे पढ़िए। यदि कुछ ग़लत हो, तो उसी अवधि में लिखित रूप से सुधार माँगिए — बाद में यह प्रक्रिया कहीं अधिक कठिन हो जाती है।',
        ],
        pEn: [
          'After the policy document arrives there is a defined period in which you can read it and return it, with the premium refunded. This right exists and almost nobody uses it.',
          'The reason is simple: people do not read the policy document. It is a thick envelope that goes into a cupboard, and it is opened for the first time when a claim has to be made.',
          'Four things should be read during this window: whether the sum insured is stated correctly, whether the term is right, whether the nominee\'s name and details are accurate, and whether every declaration you made has been recorded as you made it.',
          'The fourth matters most. Agents sometimes leave something out of the proposal form or record it wrongly, and that error carries into the document. At claim time it is read against you, even though you did not make it.',
          'So take an hour when the document arrives and read it. If anything is wrong, ask for a correction in writing within that period — the process becomes considerably harder later.',
        ],
      },
      {
        hHi: 'दो पॉलिसियों की तुलना कैसे करें',
        hEn: 'How to compare two policies properly',
        pHi: [
          'तुलना करते समय अधिकांश लोग केवल प्रीमियम देखते हैं, और यही सबसे सीमित तुलना है। कुछ और बिंदु हैं जिन्हें साथ रखना चाहिए।',
          'पहला, समान शर्तों पर तुलना कीजिए। वही कवर राशि, वही अवधि, वही राइडर। अलग-अलग शर्तों पर दो प्रीमियम की तुलना कोई जानकारी नहीं देती।',
          'दूसरा, यह देखिए कि भुगतान किस रूप में होगा — एकमुश्त, मासिक आय के रूप में, या दोनों का मिश्रण। एक परिवार के लिए जो पैसे के प्रबंधन में सहज नहीं है, मासिक आय का विकल्प अधिक उपयोगी हो सकता है।',
          'तीसरा, यह जाँचिए कि क्या भविष्य में कवर बढ़ाने की सुविधा है — विवाह या बच्चे के जन्म जैसी घटनाओं पर, बिना नई चिकित्सा जाँच के। यह सुविधा कुछ पॉलिसियों में होती है और लंबे समय में मूल्यवान है।',
          'चौथा, कंपनी की दावा-प्रक्रिया देखिए: दस्तावेज़ कहाँ जमा होते हैं, कितना समय लगता है, और क्या कोई समर्पित सहायता है। यह विवरण तब मायने रखता है जब परिवार पहले ही तनाव में हो।',
        ],
        pEn: [
          'When comparing, most people look only at the premium, and that is the narrowest possible comparison. Several other points belong alongside it.',
          'First, compare on identical terms. Same sum insured, same term, same riders. Comparing two premiums quoted on different terms conveys nothing at all.',
          'Second, look at how the payout is made — as a lump sum, as a monthly income, or a mixture of the two. For a family not comfortable managing a large sum, the monthly income option can be considerably more useful.',
          'Third, check whether cover can be increased later — at events such as marriage or the birth of a child, without a fresh medical test. Some policies offer this and it is valuable over a long term.',
          'Fourth, look at the insurer\'s claim process: where documents are submitted, how long it takes, and whether dedicated assistance exists. These details matter when the family is already under strain.',
        ],
      },
      {
        hHi: 'प्रीमियम चूक जाने पर क्या होता है',
        hEn: 'What happens if a premium is missed',
        pHi: [
          'यह वह स्थिति है जिसके बारे में ख़रीदते समय कोई नहीं सोचता और जो वर्षों में किसी न किसी को पेश आती ही है।',
          'हर पॉलिसी में एक छूट अवधि होती है — प्रीमियम की तारीख़ के बाद कुछ दिनों तक पॉलिसी चालू रहती है और भुगतान स्वीकार किया जाता है। इस अवधि में कोई नुक़सान नहीं होता।',
          'छूट अवधि बीत जाने पर पॉलिसी लैप्स हो जाती है, और तब कवर समाप्त हो जाता है। यह सबसे ख़तरनाक स्थिति है, क्योंकि परिवार को यह पता ही नहीं होता कि सुरक्षा अब मौजूद नहीं है।',
          'लैप्स हुई पॉलिसी को एक निश्चित अवधि के भीतर पुनर्जीवित कराया जा सकता है — छूटा हुआ प्रीमियम, कुछ ब्याज, और कभी-कभी नई घोषणा या जाँच के साथ। यह संभव है, पर यह देरी करने का कारण नहीं होना चाहिए।',
          'सबसे अच्छा बचाव स्वचालित भुगतान है, और उसके साथ एक कैलेंडर अनुस्मारक। और यदि किसी वर्ष भुगतान कठिन हो, तो कंपनी से पहले बात कीजिए — विकल्प मौजूद होते हैं, पर वे तभी काम आते हैं जब पॉलिसी अभी चालू हो।',
        ],
        pEn: [
          'This is the situation nobody thinks about while buying and which happens to somebody or other over a long enough period.',
          'Every policy has a grace period — the cover continues for some days past the due date and payment is still accepted. Nothing is lost during that window.',
          'Once the grace period passes the policy lapses, and the cover ends. This is the most dangerous state, because the family does not know that the protection is no longer there.',
          'A lapsed policy can be revived within a defined period — the missed premium, some interest, and sometimes a fresh declaration or examination. It is possible, and it should not be a reason to delay.',
          'The best protection is automatic payment plus a calendar reminder. And if a payment is going to be difficult in some year, speak to the insurer first — options exist, but they only help while the policy is still in force.',
        ],
      },
      {
        hHi: 'दंपती के लिए: अलग-अलग या संयुक्त',
        hEn: 'For couples: separate policies or a joint one',
        pHi: [
          'जहाँ दोनों साथी कमाते हैं, वहाँ एक व्यावहारिक प्रश्न उठता है: दो अलग पॉलिसियाँ लें या एक संयुक्त पॉलिसी।',
          'सामान्य तौर पर दो अलग पॉलिसियाँ अधिक लचीली होती हैं। हर एक की अवधि, राशि और राइडर उसकी अपनी स्थिति के अनुसार चुने जा सकते हैं, और भविष्य में एक को बदलने के लिए दूसरे को छूने की ज़रूरत नहीं होती।',
          'यह भी ध्यान देने योग्य है कि केवल कमाने वाले साथी के लिए कवर पर्याप्त नहीं है। जो साथी सीधे आय नहीं कमाता पर घर और बच्चों की देखभाल करता है, उसके न रहने पर भी वास्तविक आर्थिक ख़र्च बढ़ता है — और यह पहलू प्रायः पूरी तरह छूट जाता है।',
          'दोनों की राशि तय करते समय हर एक की आय, हर एक के हिस्से का ऋण, और घर की ज़िम्मेदारियाँ अलग-अलग गिनी जानी चाहिए, न कि केवल जोड़कर आधा-आधा कर दिया जाए।',
          'और अंत में वही बात जो हर पॉलिसी पर लागू है: दोनों को एक-दूसरे की पॉलिसी के बारे में पूरी जानकारी होनी चाहिए — कंपनी, संख्या, राशि और दस्तावेज़ कहाँ रखे हैं।',
        ],
        pEn: [
          'Where both partners earn, a practical question arises: two separate policies or one joint policy.',
          'In general two separate policies are more flexible. The term, the sum and the riders on each can be chosen for that person\'s own position, and changing one later does not require touching the other.',
          'It is also worth noting that covering only the earning partner is not sufficient. A partner who does not earn directly but runs the home and looks after children leaves real financial costs behind as well — and this dimension is very often missed entirely.',
          'When deciding the amounts, each person\'s income, each person\'s share of any loan, and the responsibilities at home should be counted separately rather than added together and halved.',
          'And finally the same point that applies to every policy: each partner should know the full details of the other\'s — insurer, number, sum, and where the documents are kept.',
        ],
      },
    ],
  },

  {
    slug: 'nps-retirement-guide',
    sections: [
      {
        hHi: 'टियर I और टियर II का व्यावहारिक अंतर',
        hEn: 'The practical difference between Tier I and Tier II',
        pHi: [
          'NPS में दो खाते होते हैं और उनका उद्देश्य पूरी तरह अलग है, इसलिए उन्हें एक ही चीज़ के दो हिस्से समझना ग़लत है।',
          'टियर I वह मुख्य सेवानिवृत्ति खाता है। उसमें सख़्त निकासी नियम हैं और उसी पर कर लाभ लागू होते हैं। यही वह खाता है जिसकी वजह से NPS "सेवानिवृत्ति की व्यवस्था" कहलाता है।',
          'टियर II एक वैकल्पिक खाता है जो तभी खोला जा सकता है जब टियर I मौजूद हो। इसमें से पैसा कभी भी निकाला जा सकता है, पर सामान्य ग्राहकों के लिए उस पर वे कर लाभ नहीं होते जो टियर I पर हैं।',
          'इसका व्यावहारिक अर्थ यह है कि टियर II को एक बचत या निवेश खाते की तरह देखा जाना चाहिए, न कि सेवानिवृत्ति की व्यवस्था की तरह। उसकी तुलना किसी म्यूचुअल फंड से करना अधिक उचित है।',
          'बहुत से लोग यह मानकर टियर II खोल लेते हैं कि यह अतिरिक्त कर लाभ देगा, और बाद में निराश होते हैं। इसलिए खोलने से पहले यह स्पष्ट कर लेना ज़रूरी है कि आप उससे क्या चाहते हैं।',
        ],
        pEn: [
          'NPS has two accounts and their purposes are entirely different, so treating them as two halves of the same thing is a mistake.',
          'Tier I is the main retirement account. It carries strict withdrawal rules and it is the one to which the tax benefits attach. This is the account that makes NPS a retirement arrangement at all.',
          'Tier II is an optional account which can only be opened once Tier I exists. Money can be withdrawn from it at any time, but for ordinary subscribers it does not carry the tax benefits that Tier I does.',
          'The practical consequence is that Tier II should be viewed as a savings or investment account rather than as retirement provision. Comparing it with a mutual fund is more appropriate than comparing it with Tier I.',
          'Many people open Tier II expecting an additional tax benefit and are disappointed later. So it is worth being clear about what you want from it before opening one.',
        ],
      },
      {
        hHi: 'ऑटो और एक्टिव विकल्प में क्या चुनें',
        hEn: 'Choosing between the auto and active options',
        pHi: [
          'NPS में यह तय करना होता है कि आपका पैसा इक्विटी, कॉर्पोरेट बॉन्ड और सरकारी प्रतिभूतियों में किस अनुपात में जाएगा, और इसके दो रास्ते हैं।',
          'ऑटो विकल्प में यह अनुपात आपकी उम्र के अनुसार अपने आप बदलता रहता है — युवावस्था में इक्विटी अधिक, और उम्र बढ़ने के साथ धीरे-धीरे कम। इसमें आपको कुछ करना नहीं पड़ता।',
          'एक्टिव विकल्प में आप ख़ुद अनुपात तय करते हैं, एक निर्धारित ऊपरी सीमा के भीतर। यह उन लोगों के लिए है जो अपनी पूरी संपत्ति की तस्वीर देखकर निर्णय लेना चाहते हैं।',
          'व्यावहारिक सलाह यह है: यदि NPS आपकी एकमात्र दीर्घकालिक व्यवस्था है और आप हर साल उसकी समीक्षा नहीं करने वाले, तो ऑटो बेहतर है, क्योंकि वह जोखिम को अपने आप घटाता रहता है।',
          'और यदि आपके पास अन्य निवेश भी हैं, तो एक्टिव विकल्प इसलिए उपयोगी है कि आप NPS के हिस्से को अपने बाक़ी पोर्टफ़ोलियो के साथ संतुलित कर सकें। दोनों में से कोई भी अपने आप बेहतर नहीं है।',
        ],
        pEn: [
          'NPS requires deciding in what proportion your money goes into equity, corporate bonds and government securities, and there are two routes to that decision.',
          'Under the auto option the proportion changes on its own with your age — more equity when young, tapering gradually as you get older. Nothing is required from you.',
          'Under the active option you set the proportions yourself, within defined upper limits. This suits people who want to decide while looking at their whole asset picture.',
          'The practical advice: if NPS is your only long-term arrangement and you are not going to review it each year, auto is better, because it reduces risk on its own.',
          'And if you hold other investments too, the active option is useful because it lets you balance the NPS portion against the rest of the portfolio. Neither is better in itself.',
        ],
      },
      {
        hHi: 'फंड प्रबंधक बदलने की सुविधा',
        hEn: 'The facility to change fund managers',
        pHi: [
          'यह NPS की एक ऐसी विशेषता है जो बहुत कम लोगों को पता है और जो लंबी अवधि में उपयोगी हो सकती है।',
          'आपका पैसा किसी एक पेंशन फंड प्रबंधक द्वारा प्रबंधित होता है, और यदि आप चाहें तो एक निर्धारित प्रक्रिया के तहत प्रबंधक बदला जा सकता है। यह बदलाव पूरे संचित कोष पर लागू होता है, और इसके लिए कोई कर-घटना नहीं बनती।',
          'यह अंतिम बात महत्वपूर्ण है। सामान्य निवेश में एक विकल्प से दूसरे में जाना प्रायः एक बिक्री मानी जाती है और उस पर कर का प्रश्न उठता है। NPS के भीतर यह बदलाव उस समस्या के बिना होता है।',
          'इसका उपयोग बार-बार नहीं करना चाहिए। लगातार प्रबंधक बदलते रहना उसी ग़लती का दूसरा रूप है जो लोग फंड बदलते रहने में करते हैं।',
          'उपयोगी अभ्यास यह है कि कुछ वर्षों में एक बार अपने प्रबंधक के प्रदर्शन को उसी श्रेणी के अन्य प्रबंधकों से तुलना कर लीजिए, और केवल लगातार और स्पष्ट पिछड़ने की स्थिति में बदलाव पर विचार कीजिए।',
        ],
        pEn: [
          'This is a feature of NPS that very few people know about and which can be useful over a long horizon.',
          'Your money is managed by one pension fund manager, and if you wish, the manager can be changed through a defined process. The change applies to the whole accumulated corpus, and it does not create a tax event.',
          'That last point matters. In ordinary investing, moving from one option to another is usually treated as a sale and raises a tax question. Inside NPS the switch happens without that problem.',
          'The facility should not be used often. Changing managers repeatedly is another version of the same mistake people make by changing funds repeatedly.',
          'A useful habit is to compare your manager\'s performance against others in the same category once every few years, and to consider a change only in the case of consistent and clear underperformance.',
        ],
      },
      {
        hHi: 'निकासी के नियम और एन्युइटी का सच',
        hEn: 'Exit rules and the truth about the annuity',
        pHi: [
          'यह NPS का सबसे अधिक बहस वाला हिस्सा है, और उसे स्पष्ट रूप से समझना ज़रूरी है, क्योंकि यही तय करता है कि यह व्यवस्था आपके लिए उपयुक्त है या नहीं।',
          'निर्धारित आयु पर बाहर निकलते समय संचित राशि का एक हिस्सा एकमुश्त लिया जा सकता है, और शेष हिस्से से एक एन्युइटी ख़रीदनी होती है, जो जीवन भर एक नियमित आय देती है।',
          'यही बाध्यता कुछ लोगों को असुविधाजनक लगती है, और यह असुविधा वास्तविक है — वह पैसा फिर आपकी मर्ज़ी से इस्तेमाल नहीं किया जा सकता, और एन्युइटी से मिलने वाली आय आमतौर पर कर-योग्य होती है।',
          'दूसरी ओर, इसी बाध्यता का एक उद्देश्य है। यह उस जोखिम से बचाती है जिसे लोग कम आँकते हैं: सेवानिवृत्ति की राशि का अपेक्षा से जल्दी समाप्त हो जाना, विशेषकर तब जब आयु लंबी हो।',
          'इसलिए यह प्रश्न "अच्छा या बुरा" नहीं है। यह प्रश्न है कि आप अपने लिए एक अनिवार्य आजीवन आय चाहते हैं या अपनी राशि पर पूरा नियंत्रण। दोनों उत्तर वैध हैं, और नियम समय-समय पर संशोधित भी होते रहते हैं, इसलिए निर्णय से पहले वर्तमान प्रावधान देख लेना ज़रूरी है।',
        ],
        pEn: [
          'This is the most debated part of NPS, and it deserves clarity, because it is what decides whether the arrangement suits you at all.',
          'On exit at the prescribed age, part of the accumulated amount can be taken as a lump sum and the remainder must be used to buy an annuity, which pays a regular income for life.',
          'That requirement feels restrictive to some people, and the restriction is real — the money can no longer be used as you choose, and annuity income is generally taxable.',
          'On the other side, the requirement exists for a reason. It protects against a risk people consistently underestimate: retirement money running out sooner than expected, particularly where life is long.',
          'So the question is not "good or bad". It is whether you want a compulsory lifetime income for yourself or full control over the corpus. Both answers are legitimate, and the rules are revised from time to time, so the current provisions should be checked before deciding.',
        ],
      },
      {
        hHi: 'नियोक्ता के माध्यम से योगदान',
        hEn: 'Contributing through an employer',
        pHi: [
          'NPS में योगदान के दो रास्ते हैं — अपने आप, और नियोक्ता के माध्यम से — और इन दोनों का कर-व्यवहार एक जैसा नहीं है।',
          'नियोक्ता द्वारा किया गया योगदान एक अलग प्रावधान के अंतर्गत आता है और उसकी सीमा भी अलग होती है। यही कारण है कि कुछ वेतन-ढाँचों में यह विकल्प उपलब्ध कराया जाता है।',
          'व्यावहारिक बात यह है कि यदि आपका नियोक्ता यह सुविधा देता है, तो उसे समझना उपयोगी है, क्योंकि यह वेतन के उसी हिस्से को अलग ढंग से संरचित करने का तरीक़ा हो सकता है।',
          'साथ ही यह याद रखना ज़रूरी है कि यह पैसा टियर I में जाता है, यानी वही निकासी के नियम उस पर भी लागू होंगे। यह कोई अतिरिक्त बचत खाता नहीं है।',
          'यहाँ सटीक सीमाएँ और धाराएँ जानबूझकर नहीं लिखी जा रही हैं, क्योंकि वे बजट के साथ बदलती हैं। अपने वेतन-ढाँचे में कोई बदलाव करने से पहले वर्तमान नियम अपने नियोक्ता या किसी कर सलाहकार से पुष्ट कर लीजिए।',
        ],
        pEn: [
          'There are two routes for contributing to NPS — on your own and through an employer — and the tax treatment of the two is not the same.',
          'An employer contribution falls under a separate provision with its own limit. That is why some salary structures make this option available.',
          'The practical point is that if your employer offers it, understanding it is worthwhile, because it may be a way of structuring the same part of your salary differently.',
          'It is also worth remembering that this money goes into Tier I, so the same withdrawal rules apply to it. It is not an additional savings account.',
          'Exact limits and sections are deliberately not quoted here because they change with each budget. Confirm the current position with your employer or a tax adviser before changing anything in your salary structure.',
        ],
      },
      {
        hHi: 'NPS, EPF और PPF को साथ कैसे रखें',
        hEn: 'Holding NPS, EPF and PPF together',
        pHi: [
          'भारत में सेवानिवृत्ति की तीन प्रमुख व्यवस्थाएँ हैं और उन्हें प्रतिस्पर्धी मानने के बजाय पूरक मानना अधिक उपयोगी है।',
          'EPF वेतनभोगी लोगों के लिए अपने आप चलता है और उसमें नियोक्ता का योगदान भी शामिल होता है। इसका चरित्र निश्चित प्रतिफल वाला है और उस पर आपका कोई निवेश-निर्णय नहीं होता।',
          'PPF स्वैच्छिक है, सबके लिए उपलब्ध है, और उसकी अवधि तथा सीमाएँ निश्चित हैं। यह पोर्टफ़ोलियो का स्थिर, कर-अनुकूल सिरा बनाता है।',
          'NPS इन दोनों से अलग है क्योंकि इसमें इक्विटी का हिस्सा होता है और लागत बहुत कम होती है। यही वह घटक है जो लंबी अवधि में मुद्रास्फीति से आगे रहने की संभावना देता है — और यही वह घटक है जिसमें उतार-चढ़ाव भी है।',
          'तीनों को साथ रखने पर एक संतुलित ढाँचा बनता है: EPF और PPF निश्चितता देते हैं, NPS वृद्धि की संभावना। अनुपात हर व्यक्ति की उम्र और स्थिति से तय होगा, और इस लेख में कोई अनुपात सुझाया नहीं जा रहा।',
        ],
        pEn: [
          'India has three main retirement arrangements, and it is more useful to treat them as complementary than as competing with one another.',
          'EPF runs automatically for salaried people and includes an employer contribution. Its character is fixed-return and no investment decision rests with you.',
          'PPF is voluntary, available to everybody, and its term and limits are fixed. It forms the stable, tax-efficient end of a portfolio.',
          'NPS differs from both because it contains an equity component and its costs are very low. That is the component offering the possibility of staying ahead of inflation over a long horizon — and it is also the component that moves.',
          'Held together the three make a balanced structure: EPF and PPF supply certainty, NPS supplies the possibility of growth. The proportion depends on each person\'s age and position, and no proportion is suggested here.',
        ],
      },
    ],
  },

  {
    slug: 'elss-tax-saving-funds',
    sections: [
      {
        hHi: 'लॉक-इन हर किश्त पर अलग चलता है',
        hEn: 'The lock-in runs separately on every instalment',
        pHi: [
          'यह ELSS का वह विवरण है जो सबसे अधिक ग़लतफ़हमी पैदा करता है, और यह तब सामने आता है जब लोग पहली बार पैसा निकालने जाते हैं।',
          'तीन वर्ष की अवधि पूरे निवेश पर एक साथ नहीं, बल्कि हर किश्त पर अलग-अलग गिनी जाती है। यदि आप SIP के माध्यम से हर महीने निवेश कर रहे हैं, तो हर महीने की किश्त की अपनी तीन-वर्षीय घड़ी है।',
          'इसका व्यावहारिक अर्थ यह है कि तीन साल पूरे होने पर आपका पूरा पैसा मुक्त नहीं होता — केवल पहली किश्त मुक्त होती है। बारह किश्तों वाले एक वर्ष का पूरा निवेश मुक्त होने में चार वर्ष लगते हैं।',
          'यह कोई समस्या नहीं है यदि आप इसे पहले से जानते हों। समस्या तब बनती है जब कोई तीन साल बाद पूरी राशि की अपेक्षा करके योजना बना लेता है।',
          'इसलिए यदि किसी निश्चित लक्ष्य के लिए ELSS का उपयोग कर रहे हैं, तो अपनी गणना अंतिम किश्त की तारीख़ से कीजिए, पहली से नहीं।',
        ],
        pEn: [
          'This is the detail of ELSS that causes the most confusion, and it surfaces when people go to withdraw money for the first time.',
          'The three-year period is not counted once across the whole investment but separately on each instalment. If you invest monthly through a SIP, every month\'s instalment carries its own three-year clock.',
          'The practical meaning is that your money is not all free after three years — only the first instalment is. A full year of twelve instalments takes four years before all of it is free.',
          'None of this is a problem if you know it in advance. It becomes a problem when somebody plans around having the whole amount available after three years.',
          'So if ELSS is being used for a defined goal, calculate from the date of the last instalment rather than the first.',
        ],
      },
      {
        hHi: 'मार्च में एकमुश्त डालने की समस्या',
        hEn: 'The problem with a lump sum in March',
        pHi: [
          'यह भारत में सबसे आम निवेश-आदत है और सबसे कम उपयोगी: कर बचाने की समय-सीमा से कुछ दिन पहले पूरी राशि एक साथ डाल देना।',
          'इसमें दो अलग नुक़सान हैं। पहला यह कि आप बाज़ार में एक ही दिन प्रवेश कर रहे हैं, और वह दिन ऊँचा या नीचा — कुछ भी हो सकता है। पूरे वर्ष फैलाकर निवेश करने से यह जोखिम अपने आप बँट जाता है।',
          'दूसरा नुक़सान यह है कि जल्दबाज़ी में चुनाव अच्छा नहीं होता। मार्च के अंतिम दिनों में लोग वही चुन लेते हैं जो सबसे पहले सामने आ जाए, और वह चुनाव अगले तीन वर्षों तक साथ रहता है।',
          'सरल समाधान यह है कि अप्रैल में ही तय कर लीजिए कि इस वर्ष कितना जाना है, और उसे बारह हिस्सों में बाँटकर SIP चालू कर दीजिए।',
          'इससे तीन चीज़ें एक साथ मिलती हैं: बेहतर औसत क़ीमत, बिना दबाव के लिया गया निर्णय, और मार्च का वह तनाव जो हर साल दोहराया जाता है, पूरी तरह समाप्त।',
        ],
        pEn: [
          'This is the most common investing habit in India and the least useful one: putting the whole amount in a few days before the tax deadline.',
          'There are two separate costs in it. The first is that you enter the market on a single day, and that day could be high or low — spreading across the year distributes that risk automatically.',
          'The second is that choices made in haste are not good ones. In the last days of March people pick whatever appears first, and that choice then stays with them for three years.',
          'The simple fix is to decide in April how much is going in this year and split it into twelve, running as a SIP.',
          'That produces three things at once: a better average price, a decision made without pressure, and the complete end of the March scramble that otherwise repeats every year.',
        ],
      },
      {
        hHi: '80C के अन्य विकल्पों से तुलना',
        hEn: 'Comparing with the other options in that basket',
        pHi: [
          'कर बचाने वाले विकल्पों की सूची लंबी है, और ELSS को उसमें रखकर देखने से उसकी जगह स्पष्ट हो जाती है।',
          'लॉक-इन की दृष्टि से ELSS इस सूची में सबसे छोटी अवधि वाला है। अन्य कई विकल्पों में अवधि पाँच वर्ष या उससे बहुत अधिक होती है, और कुछ में तो पंद्रह।',
          'प्रतिफल की दृष्टि से ELSS इक्विटी से जुड़ा है, इसलिए उसका प्रतिफल निश्चित नहीं है — न ऊपर की सीमा है, न नीचे की गारंटी। अधिकांश अन्य विकल्प निश्चित या अपेक्षाकृत स्थिर प्रतिफल देते हैं।',
          'इसका अर्थ यह नहीं कि ELSS बेहतर है। इसका अर्थ यह है कि वह अलग है, और उसका चुनाव इस पर निर्भर करता है कि आपके पोर्टफ़ोलियो में इक्विटी की जगह पहले से कितनी भरी हुई है।',
          'एक व्यावहारिक ढाँचा यह है: पहले देखिए कि आपकी सूची में कौन-सी कटौतियाँ अपने आप हो रही हैं — भविष्य निधि, बीमा प्रीमियम, गृह ऋण की मूल राशि। जो जगह बचती है, उसी के लिए यह प्रश्न लागू होता है।',
        ],
        pEn: [
          'The list of tax-saving options is long, and placing ELSS inside it makes its position clear.',
          'On lock-in, ELSS carries the shortest period in that list. Several other options run for five years or considerably longer, and some for fifteen.',
          'On return, ELSS is linked to equity, so its return is not fixed — there is no ceiling and no floor. Most of the other options offer fixed or relatively stable returns.',
          'That does not make ELSS better. It makes it different, and choosing it depends on how much of the equity space in your portfolio is already occupied.',
          'A practical framework: first see which deductions in your list are happening automatically — provident fund, insurance premiums, home loan principal. The question applies only to whatever space is left after those.',
        ],
      },
      {
        hHi: 'लॉक-इन ख़त्म होने के बाद क्या करें',
        hEn: 'What to do once the lock-in ends',
        pHi: [
          'तीन वर्ष पूरे होने पर एक निर्णय सामने आता है, और अधिकांश लोग उसे बिना सोचे ले लेते हैं — या तो सब निकाल लेते हैं, या भूल जाते हैं।',
          'पहली बात यह समझने की है कि लॉक-इन समाप्त होने का अर्थ यह नहीं है कि निकालना ज़रूरी है। यदि फंड ठीक चल रहा है और आपको पैसे की ज़रूरत नहीं है, तो उसे बनाए रखना पूरी तरह उचित है — अब वह एक सामान्य इक्विटी फंड की तरह काम करता है।',
          'दूसरी बात यह है कि निकालकर उसी योजना में दोबारा डालने का कोई अतिरिक्त कर लाभ नहीं होता, यदि आप उसी वर्ष की सीमा पहले ही पूरी कर चुके हैं। यह "चक्रीकरण" कई लोग बिना कारण करते हैं।',
          'तीसरी, यदि निकालना है तो यह देख लीजिए कि किस किश्त की अवधि पूरी हुई है और कर की स्थिति क्या बनती है। यहाँ भी दरें बदलती रहती हैं, इसलिए उस वर्ष के नियम देखना ज़रूरी है।',
          'सबसे उपयोगी आदत यह है कि हर वर्ष एक बार यह देख लिया जाए कि कौन-सी किश्तें मुक्त हो चुकी हैं। इससे यह निर्णय आपात के समय जल्दबाज़ी में नहीं लेना पड़ता।',
        ],
        pEn: [
          'When three years complete, a decision arrives, and most people take it without thinking — either withdrawing everything or forgetting about it entirely.',
          'The first thing to understand is that the lock-in ending does not mean withdrawal is required. If the fund is doing reasonably and you do not need the money, staying invested is entirely sensible — it now behaves like an ordinary equity fund.',
          'The second is that withdrawing and reinvesting in the same scheme gives no additional tax benefit if you have already used that year\'s limit. A number of people carry out this cycling for no reason.',
          'Third, if you are withdrawing, check which instalments have completed their period and what the tax position is. Rates change here too, so the rules for that year need looking up.',
          'The most useful habit is to check once a year which instalments have become free. Then the decision does not have to be taken in a hurry during an emergency.',
        ],
      },
      {
        hHi: 'ग्रोथ और IDCW विकल्प की उलझन',
        hEn: 'The confusion between growth and payout options',
        pHi: [
          'हर फंड में दो विकल्प होते हैं और उनका चुनाव नतीजे को बदल देता है, फिर भी बहुत से लोग यह चुनाव बिना समझे कर देते हैं।',
          'ग्रोथ विकल्प में सारा प्रतिफल फंड के भीतर ही रहता है और यूनिट का मूल्य बढ़ता जाता है। आपको बीच में कुछ नहीं मिलता, और यही उसका उद्देश्य है।',
          'दूसरे विकल्प में समय-समय पर कुछ राशि वितरित की जाती है। यहाँ एक महत्वपूर्ण बात समझने की है: यह वितरण कोई अतिरिक्त कमाई नहीं है, वह आपके ही निवेश में से निकाला जाता है और उतना ही यूनिट का मूल्य घट जाता है।',
          'बहुत से लोग दूसरे विकल्प को "नियमित आय" समझकर चुन लेते हैं, यह जाने बिना कि वे अपनी ही पूँजी निकाल रहे हैं — और लंबी अवधि में चक्रवृद्धि का लाभ खो रहे हैं।',
          'सामान्य नियम यह है: यदि उद्देश्य लंबी अवधि में संपत्ति बनाना है, तो ग्रोथ विकल्प ही उपयुक्त है। नियमित आय की ज़रूरत को हल करने के बेहतर और अधिक पारदर्शी तरीक़े मौजूद हैं।',
        ],
        pEn: [
          'Every fund carries two options and the choice changes the outcome, yet many people make it without understanding what they are choosing.',
          'Under the growth option all returns stay inside the fund and the unit value rises. Nothing comes to you along the way, and that is precisely its purpose.',
          'Under the other option amounts are distributed from time to time. One thing needs understanding here: that distribution is not additional earnings, it comes out of your own investment and the unit value falls by the same amount.',
          'Many people choose the second thinking of it as "regular income", without realising they are withdrawing their own capital — and losing the benefit of compounding over a long period.',
          'The general rule: if the purpose is building wealth over a long horizon, the growth option is the appropriate one. There are better and more transparent ways of meeting a need for regular income.',
        ],
      },
      {
        hHi: 'नई कर व्यवस्था चुनने पर ELSS का क्या',
        hEn: 'What happens to ELSS under the new tax regime',
        pHi: [
          'यह प्रश्न हाल के वर्षों में सबसे अधिक पूछा गया है, और इसका उत्तर दो हिस्सों में है।',
          'पहला हिस्सा: यदि आप ऐसी व्यवस्था चुनते हैं जिसमें यह कटौती लागू नहीं होती, तो ELSS का कर लाभ आपके लिए समाप्त हो जाता है। इसमें कोई अस्पष्टता नहीं है।',
          'दूसरा हिस्सा अधिक दिलचस्प है: इसका यह अर्थ नहीं कि ELSS बेकार हो गया। वह अब भी एक इक्विटी फंड है, और यदि वह आपकी योजना में उपयुक्त बैठता है तो उसे रखा जा सकता है।',
          'पर एक अंतर ज़रूर बनता है। कर लाभ के बिना, तीन वर्ष का लॉक-इन एक शुद्ध बाधा है — और उसी श्रेणी के बिना लॉक-इन वाले फंड उपलब्ध हैं। इसलिए कर लाभ के बिना ELSS चुनने का कारण कमज़ोर हो जाता है।',
          'और यदि आपके पास पहले से ELSS है और आप व्यवस्था बदल रहे हैं, तो पुराने निवेश पर कुछ नहीं होता — लॉक-इन अपने समय पर पूरा होगा और उसके बाद वह सामान्य फंड की तरह चलेगा। केवल नई किश्तों पर यह प्रश्न लागू होता है।',
        ],
        pEn: [
          'This has been the most frequently asked question in recent years, and the answer comes in two parts.',
          'The first part: if you choose a regime under which this deduction does not apply, the tax benefit of ELSS ends for you. There is no ambiguity about that.',
          'The second part is more interesting: it does not follow that ELSS has become useless. It is still an equity fund, and if it fits your plan it can be held.',
          'One difference does arise though. Without the tax benefit, the three-year lock-in is a pure constraint — and funds in the same category exist without one. So the case for choosing ELSS specifically becomes weaker.',
          'And if you already hold ELSS and are changing regime, nothing happens to the existing investment — the lock-in completes on schedule and the holding then behaves like an ordinary fund. The question applies only to new instalments.',
        ],
      },
    ],
  },

  {
    slug: 'budget-50-30-20-rule',
    sections: [
      {
        hHi: 'जब अनुपात आपकी स्थिति में बैठता ही नहीं',
        hEn: 'When the ratios simply do not fit your situation',
        pHi: [
          'यह नियम एक औसत के लिए बना है, और भारत में बहुत सी स्थितियाँ उस औसत से दूर हैं। जब संख्याएँ न बैठें, तो नियम छोड़ देना ग़लत निष्कर्ष है — उसे समायोजित करना सही है।',
          'सबसे आम स्थिति महँगे शहरों की है, जहाँ अकेले किराया ही आय का चालीस प्रतिशत ले लेता है। यहाँ पचास प्रतिशत में सभी ज़रूरतें रखना असंभव है।',
          'ऐसी स्थिति में सही तरीक़ा यह है कि बचत का हिस्सा पहले तय कीजिए — भले वह बीस प्रतिशत न होकर दस हो — और शेष को ज़रूरतों तथा इच्छाओं में बाँटिए। बचत का प्रतिशत घट सकता है; बचत का शून्य होना नहीं चाहिए।',
          'दूसरी आम स्थिति कम आय की है, जहाँ ज़रूरतें अपने आप सत्तर या अस्सी प्रतिशत ले लेती हैं। यहाँ नियम का उद्देश्य अनुपात नहीं बल्कि दिशा है: कोई भी राशि, नियमित रूप से, ज़रूरतों के हिस्से से बाहर।',
          'और तीसरी स्थिति उल्टी है — ऊँची आय, जहाँ ज़रूरतें तीस प्रतिशत में पूरी हो जाती हैं। यहाँ बीस प्रतिशत बचाना बहुत कम है, और अनुपात को ऊपर की ओर समायोजित करना चाहिए।',
        ],
        pEn: [
          'This rule was built for an average, and many situations in India sit far from that average. When the numbers do not fit, abandoning the rule is the wrong conclusion — adjusting it is the right one.',
          'The most common case is expensive cities, where rent alone takes forty percent of income. Fitting all needs inside fifty percent is simply impossible there.',
          'In that situation the correct approach is to fix the savings share first — even if it is ten percent rather than twenty — and divide what remains between needs and wants. The savings percentage can fall; it should not become zero.',
          'The second common case is low income, where needs take seventy or eighty percent on their own. Here the purpose of the rule is not the ratio but the direction: some amount, regularly, kept outside the needs share.',
          'And the third case is the reverse — high income, where needs are met within thirty percent. Saving twenty percent is far too little there, and the ratio should be adjusted upward.',
        ],
      },
      {
        hHi: 'जो ख़र्च बीच में आते हैं उन्हें कैसे बाँटें',
        hEn: 'How to classify the expenses that sit in between',
        pHi: [
          'नियम लागू करते समय सबसे अधिक समय इसी पर जाता है: यह ख़र्च ज़रूरत है या इच्छा? और यही वह जगह है जहाँ लोग सबसे अधिक बहस करते हैं — अपने आप से।',
          'एक उपयोगी परीक्षण यह है: यदि यह ख़र्च रुक जाए तो क्या आपकी आय, स्वास्थ्य या सुरक्षा पर सीधा असर पड़ेगा? यदि हाँ, तो यह ज़रूरत है। यदि केवल असुविधा होगी, तो यह इच्छा है।',
          'इस परीक्षण से कुछ स्पष्ट उत्तर मिलते हैं। काम पर जाने का ख़र्च ज़रूरत है; उसी रास्ते पर अधिक आरामदायक विकल्प चुनना इच्छा है। फ़ोन का बुनियादी रिचार्ज ज़रूरत है; उससे ऊपर का पैकेज इच्छा है।',
          'एक और उपयोगी तरीक़ा यह है कि एक ही श्रेणी को दो हिस्सों में बाँट दिया जाए। भोजन का राशन ज़रूरत के डिब्बे में जाता है और बाहर खाना इच्छा के डिब्बे में — दोनों "खाना" हैं पर वे एक चीज़ नहीं हैं।',
          'और सबसे महत्वपूर्ण: इस वर्गीकरण को ईमानदार रखिए, पर उस पर बहुत समय मत लगाइए। एक मोटा और ईमानदार बँटवारा उस सटीक बँटवारे से कहीं अधिक उपयोगी है जो कभी पूरा नहीं होता।',
        ],
        pEn: [
          'Most of the time spent applying the rule goes into this: is this expense a need or a want? And this is where people argue the most — with themselves.',
          'A useful test: if this expense stopped, would your income, health or safety be directly affected? If yes, it is a need. If it would only be inconvenient, it is a want.',
          'The test produces some clear answers. Getting to work is a need; choosing a more comfortable way along the same route is a want. A basic phone recharge is a need; the package above it is a want.',
          'Another useful approach is splitting a single category in two. Groceries go in the needs bucket and eating out in the wants bucket — both are "food" and they are not the same thing.',
          'And most importantly: keep the classification honest but do not spend long on it. A rough, honest split is far more useful than a precise one that never gets finished.',
        ],
      },
      {
        hHi: 'साल में एक-दो बार आने वाले ख़र्च',
        hEn: 'The expenses that arrive once or twice a year',
        pHi: [
          'यह वह जगह है जहाँ अधिकांश मासिक बजट टूटते हैं, और वे इसलिए टूटते हैं क्योंकि बजट मासिक है जबकि ये ख़र्च नहीं हैं।',
          'बीमा का प्रीमियम, स्कूल की फ़ीस, त्योहार, कोई वार्षिक सदस्यता, वाहन का बीमा और सर्विस, कोई शादी या यात्रा — ये सब निश्चित हैं और फिर भी मासिक बजट में कहीं नहीं दिखते।',
          'इसका समाधान सरल है और उसे सिंकिंग फंड कहते हैं। इन सभी वार्षिक ख़र्चों को जोड़िए, बारह से भाग दीजिए, और उतनी राशि हर महीने अलग रख दीजिए — ठीक वैसे ही जैसे कोई और मासिक बिल।',
          'इससे दो चीज़ें होती हैं। पहली, जिस महीने वह ख़र्च आता है, वह महीना संकट नहीं बनता। दूसरी, और अधिक महत्वपूर्ण, आपकी बचत उन महीनों में नहीं टूटती, क्योंकि पैसा पहले से अलग रखा हुआ है।',
          'यह एक छोटा-सा जोड़ है जो पूरे बजट को स्थिर कर देता है, और यही वह हिस्सा है जो अधिकांश सरल नियमों में नहीं बताया जाता।',
        ],
        pEn: [
          'This is where most monthly budgets break, and they break because the budget is monthly while these expenses are not.',
          'Insurance premiums, school fees, festivals, an annual subscription, vehicle insurance and servicing, a wedding or a trip — all of them are predictable and none of them appears anywhere in a monthly budget.',
          'The solution is simple and it is called a sinking fund. Add up all these annual expenses, divide by twelve, and set that amount aside every month — exactly like any other monthly bill.',
          'Two things follow. First, the month in which the expense arrives is no longer a crisis. Second and more importantly, your saving does not break in those months, because the money was set aside in advance.',
          'It is a small addition that stabilises the entire budget, and it is the part most simple rules leave out.',
        ],
      },
      {
        hHi: 'अनियमित आय वाले लोगों के लिए',
        hEn: 'For people whose income is irregular',
        pHi: [
          'यह नियम मासिक वेतन को मानकर बना है, और भारत में बहुत बड़ी संख्या में लोगों की आय ऐसी नहीं है — दुकानदार, किसान, फ्रीलांसर, कमीशन पर काम करने वाले।',
          'ऐसी स्थिति में पहला क़दम प्रतिशत नहीं, एक आधार-रेखा तय करना है। पिछले बारह महीनों में जो सबसे कम आय वाला महीना रहा, उसे अपना "वेतन" मानिए और उसी पर बजट बनाइए।',
          'जिन महीनों में आय उससे अधिक हो, वह अतिरिक्त राशि सीधे दो जगह जाए: पहले आपातकालीन कोष तक, और उसके बाद निवेश तक। उसे मासिक ख़र्च में मत मिलाइए, क्योंकि तब वह आधार-रेखा को ऊपर खींच देगी।',
          'यह तरीक़ा शुरू में कठिन लगता है क्योंकि अच्छे महीनों में भी संयम रखना पड़ता है। पर यही वह अनुशासन है जो बुरे महीनों को सामान्य बना देता है, और अनियमित आय में यही सबसे बड़ी राहत है।',
          'और एक अतिरिक्त सुझाव: अनियमित आय वाले लोगों के लिए आपातकालीन कोष का आकार बड़ा होना चाहिए — नौ से बारह महीने — क्योंकि यहाँ आय का रुकना असामान्य घटना नहीं, व्यवसाय का सामान्य हिस्सा है।',
        ],
        pEn: [
          'This rule assumes a monthly salary, and a very large number of people in India do not have one — shopkeepers, farmers, freelancers, anybody working on commission.',
          'The first step in that situation is not a percentage but a baseline. Take the lowest-income month of the past year and treat that as your "salary", budgeting against it.',
          'In months where income exceeds it, the surplus goes to two places directly: first towards the emergency fund, then towards investing. Do not mix it into monthly spending, because that pulls the baseline upward.',
          'This feels hard at first because restraint is required even in good months. But that discipline is what makes the bad months ordinary, and in irregular income that is the largest relief available.',
          'One additional suggestion: for irregular income the emergency fund should be larger — nine to twelve months — because here a pause in income is not an unusual event but a normal part of the business.',
        ],
      },
      {
        hHi: 'बिना ऐप के हिसाब रखना',
        hEn: 'Tracking without an app',
        pHi: [
          'बहुत से लोग बजट इसलिए छोड़ देते हैं कि हर ख़र्च दर्ज करना थका देता है, और यह आपत्ति उचित है। पर हिसाब रखने का यह एकमात्र तरीक़ा नहीं है।',
          'सबसे सरल विकल्प खातों को अलग करना है। एक खाता जिससे बिल और ज़रूरतें जाती हैं, एक जिसमें इच्छाओं का हिस्सा रहता है, और एक जहाँ बचत जाती है। फिर हर ख़र्च दर्ज करने की ज़रूरत नहीं — शेष राशि अपने आप बता देती है।',
          'दूसरा विकल्प महीने में एक बार का पुनरावलोकन है। हर लेन-देन के समय कुछ मत कीजिए; महीने के अंत में एक बार विवरण देखिए और तीन डिब्बों में जोड़ लीजिए। इसमें बीस मिनट लगते हैं।',
          'तीसरा विकल्प और भी सरल है और उन लोगों के लिए अच्छा है जो नक़द अधिक इस्तेमाल करते हैं: इच्छाओं का पूरा हिस्सा महीने की शुरुआत में नक़द निकाल लीजिए। जब लिफ़ाफ़ा ख़ाली हो जाए, महीना समाप्त।',
          'इनमें से कोई भी तरीक़ा "सही" नहीं है। सही वही है जिसे आप छह महीने तक निभा सकें, और यह जान लेना कि ऐप ही एकमात्र रास्ता नहीं है, बहुत लोगों को दोबारा शुरू करने का मौक़ा दे देता है।',
        ],
        pEn: [
          'Many people abandon budgeting because recording every expense is exhausting, and that objection is fair. But recording is not the only way to keep track.',
          'The simplest alternative is separating accounts. One account from which bills and needs are paid, one holding the wants share, and one where savings go. Then nothing needs recording — the balance tells you by itself.',
          'The second alternative is a monthly review. Do nothing at the time of each transaction; look at the statement once at month-end and total it into three buckets. That takes twenty minutes.',
          'The third is simpler still and suits people who use more cash: withdraw the entire wants share in cash at the start of the month. When the envelope is empty, the month is over.',
          'None of these is the "correct" method. The correct one is whichever you can keep up for six months, and knowing that an app is not the only route gives a lot of people the chance to start again.',
        ],
      },
      {
        hHi: 'बजट कब बदलना चाहिए',
        hEn: 'When the budget itself should change',
        pHi: [
          'एक बार बना बजट हमेशा के लिए नहीं होता, और उसे न बदलना उतनी ही आम ग़लती है जितना उसे बहुत बार बदलना।',
          'चार घटनाएँ ऐसी हैं जिनके बाद बजट दोबारा बनाना चाहिए: आय में बड़ा बदलाव, घर या शहर बदलना, परिवार में कोई नया सदस्य या नया आश्रित, और कोई बड़ा दायित्व शुरू या समाप्त होना।',
          'इनके अलावा वर्ष में एक बार सामान्य समीक्षा पर्याप्त है। हर महीने अनुपात बदलते रहना बजट को व्यवस्था के बजाय एक और चिंता बना देता है।',
          'समीक्षा में देखने लायक़ एक ही मुख्य संख्या है: बचत का प्रतिशत पिछले वर्ष से ऊपर है या नीचे। यदि ऊपर है, तो बाक़ी विवरण पर बहस करने की ज़रूरत नहीं।',
          'और यदि नीचे है, तो कारण खोजने के लिए स्थायी दायित्वों की सूची से शुरू कीजिए, दैनिक ख़र्चों से नहीं। लगभग हर बार असली कारण वहीं मिलता है।',
        ],
        pEn: [
          'A budget made once is not permanent, and never changing it is as common a mistake as changing it constantly.',
          'Four events should trigger rebuilding it: a large change in income, moving home or city, a new family member or dependant, and a large obligation beginning or ending.',
          'Beyond those, an ordinary review once a year is enough. Changing the ratios every month turns the budget from a system into one more thing to worry about.',
          'There is one main number to look at in that review: is the savings percentage above or below last year\'s. If it is above, the remaining detail does not need arguing over.',
          'And if it is below, start looking for the reason in the list of fixed obligations rather than in daily spending. Almost every time, that is where it turns out to be.',
        ],
      },
    ],
  },

  {
    slug: 'index-fund-vs-active-fund',
    sections: [
      {
        hHi: 'सूचकांक असल में है क्या',
        hEn: 'What an index actually is',
        pHi: [
          'तुलना समझने से पहले यह जान लेना ज़रूरी है कि सूचकांक कोई निवेश-रणनीति नहीं बल्कि एक माप है — बाज़ार के किसी हिस्से का प्रतिनिधित्व करने वाली कंपनियों की एक सूची, जिसे तय नियमों से बनाया जाता है।',
          'उस सूची में कौन आएगा और किस अनुपात में, यह किसी की राय से नहीं बल्कि पहले से घोषित नियमों से तय होता है — आमतौर पर कंपनी के आकार और उसके व्यापार योग्य हिस्से के आधार पर।',
          'सूचकांक फंड का काम केवल उसी सूची की नक़ल करना है, उसी अनुपात में। उसका उद्देश्य बाज़ार से बेहतर करना नहीं है; उसका उद्देश्य बाज़ार जितना करना है, कम से कम ख़र्च में।',
          'यह अंतर छोटा लगता है और वास्तव में पूरा दर्शन बदल देता है। सक्रिय फंड में एक व्यक्ति चुनाव करता है और उसके चुनाव पर आपका प्रतिफल निर्भर करता है। सूचकांक फंड में कोई चुनाव नहीं होता, इसलिए किसी की ग़लती का जोखिम भी नहीं।',
          'बदले में यह भी सच है कि सूचकांक फंड कभी बाज़ार से आगे नहीं जाएगा। वह वही देगा जो बाज़ार देगा, ख़र्च घटाकर — न उससे अधिक, न बहुत कम।',
        ],
        pEn: [
          'Before the comparison makes sense it helps to know that an index is not an investment strategy but a measurement — a list of companies representing some part of the market, built according to fixed rules.',
          'Who enters that list and in what proportion is decided not by anybody\'s opinion but by rules announced in advance, usually based on the size of a company and the tradable portion of it.',
          'An index fund\'s job is only to copy that list in the same proportions. Its purpose is not to beat the market; its purpose is to match the market at the lowest possible cost.',
          'The distinction sounds small and it changes the entire philosophy. In an active fund a person makes choices and your return depends on those choices. In an index fund no choices are made, so there is no risk of somebody choosing badly either.',
          'In exchange it is equally true that an index fund will never go ahead of the market. It gives whatever the market gives, minus costs — no more, and not much less.',
        ],
      },
      {
        hHi: 'ख़र्च अनुपात का चक्रवृद्धि प्रभाव',
        hEn: 'How the expense ratio compounds',
        pHi: [
          'ख़र्च अनुपात एक ऐसी संख्या है जो देखने में छोटी है और लंबी अवधि में असामान्य रूप से बड़ी हो जाती है, और यही इस पूरी बहस का केंद्र है।',
          'यह अंतर हर वर्ष लगता है, चाहे फंड ने अच्छा किया हो या बुरा। और चूँकि वह उस राशि पर लगता है जो अपने आप बढ़ रही है, इसलिए उसका असर भी चक्रवृद्धि की तरह बढ़ता है।',
          'इसे समझने का सबसे सरल तरीक़ा यह है कि इसे प्रतिफल से घटाकर देखिए। यदि दो फंड एक जैसा प्रदर्शन करें, तो जिसका ख़र्च कम है वह हर वर्ष उतना ही आगे रहेगा — और बीस वर्षों में यह छोटा अंतर एक बड़ी राशि बन जाता है।',
          'यहाँ यह भी ध्यान देने योग्य है कि ख़र्च निश्चित है और प्रतिफल अनिश्चित। आप यह नहीं जानते कि कोई फंड आगे कैसा करेगा, पर आप यह ठीक-ठीक जानते हैं कि वह कितना शुल्क लेगा।',
          'इसीलिए अनुभवी निवेशक कहते हैं कि ख़र्च उन कुछ चीज़ों में है जिन्हें आप नियंत्रित कर सकते हैं। प्रतिफल का पीछा करना अनिश्चित है; ख़र्च घटाना निश्चित है।',
        ],
        pEn: [
          'The expense ratio is a number that looks small and becomes unusually large over long periods, and it sits at the centre of this entire debate.',
          'The charge applies every year, whether the fund did well or badly. And because it applies to an amount that is itself growing, its effect compounds in the same way returns do.',
          'The simplest way to see it is to subtract it from the return. If two funds perform identically, the one charging less stays ahead by that much every year — and across twenty years the small difference becomes a large sum.',
          'It is also worth noticing that the cost is certain while the return is not. You do not know how a fund will perform in future, but you know exactly what it will charge.',
          'This is why experienced investors say that cost is one of the few things you can actually control. Chasing returns is uncertain; reducing costs is certain.',
        ],
      },
      {
        hHi: 'ट्रैकिंग एरर: वह विवरण जिसे लोग नहीं देखते',
        hEn: 'Tracking error: the detail nobody checks',
        pHi: [
          'सूचकांक फंड चुनते समय अधिकांश लोग केवल ख़र्च अनुपात देखते हैं, और एक दूसरा माप छोड़ देते हैं जो उतना ही महत्वपूर्ण है।',
          'ट्रैकिंग एरर यह बताता है कि फंड अपने सूचकांक से कितना भटका। सिद्धांत में यह शून्य होना चाहिए, पर व्यवहार में कभी नहीं होता — नक़दी का प्रबंधन, लेन-देन की लागत और सूचकांक में होने वाले बदलाव सब मिलकर थोड़ा अंतर पैदा करते हैं।',
          'दो सूचकांक फंड जो एक ही सूचकांक की नक़ल करते हैं और जिनका ख़र्च लगभग बराबर है, उनका ट्रैकिंग एरर अलग हो सकता है। और वही अंतर बताता है कि किसका क्रियान्वयन बेहतर है।',
          'व्यावहारिक सलाह यह है कि दोनों संख्याएँ साथ देखिए: कम ख़र्च और कम ट्रैकिंग एरर। इनमें से केवल एक देखना अधूरी तस्वीर देता है।',
          'और यह जानकारी छिपी हुई नहीं है — फंड अपने नियमित प्रकटीकरण में इसे प्रकाशित करते हैं, और उसे देखने में कुछ मिनट लगते हैं।',
        ],
        pEn: [
          'When choosing an index fund most people look only at the expense ratio and skip a second measure that matters just as much.',
          'Tracking error states how far the fund drifted from its index. In theory it should be zero, and in practice it never is — cash management, transaction costs and changes to the index itself all produce a small gap.',
          'Two index funds tracking the same index with almost identical costs can have different tracking errors. That difference is what tells you whose execution is better.',
          'The practical advice is to look at both numbers together: low cost and low tracking error. Looking at only one gives an incomplete picture.',
          'And the information is not hidden — funds publish it in their regular disclosures, and finding it takes a few minutes.',
        ],
      },
      {
        hHi: 'सक्रिय फंड के लिए मुश्किल क्यों बढ़ती जाती है',
        hEn: 'Why it keeps getting harder for active funds',
        pHi: [
          'यह पूछना उचित है कि यदि सक्रिय प्रबंधन में विशेषज्ञता है, तो उसे लगातार आगे क्यों नहीं रहना चाहिए। उत्तर संरचनात्मक है, व्यक्तिगत नहीं।',
          'पहला कारण गणितीय है। सभी निवेशक मिलकर ही बाज़ार हैं, इसलिए ख़र्च से पहले औसत निवेशक को औसत प्रतिफल ही मिलेगा। ख़र्च घटाने के बाद औसत निवेशक बाज़ार से पीछे रहेगा — यह किसी की कुशलता का प्रश्न नहीं है।',
          'दूसरा कारण प्रतिस्पर्धा है। जैसे-जैसे बाज़ार में जानकारी तेज़ी से फैलती है और पेशेवर प्रतिभागी बढ़ते हैं, वैसे-वैसे किसी एक को बाक़ी सबसे बेहतर जानकारी मिलने की संभावना घटती है।',
          'तीसरा कारण आकार है। जो फंड सफल होता है उसमें अधिक पैसा आता है, और अधिक पैसे को उसी कुशलता से लगाना कठिन हो जाता है। सफलता अपने आप अगली सफलता को कठिन बना देती है।',
          'इसका अर्थ यह नहीं कि कोई सक्रिय फंड आगे नहीं रह सकता। इसका अर्थ यह है कि पहले से यह पहचानना कि कौन-सा रहेगा, बहुत कठिन है — और पिछला प्रदर्शन इस पहचान के लिए भरोसेमंद आधार नहीं है।',
        ],
        pEn: [
          'It is fair to ask why expertise in active management should not keep it consistently ahead. The answer is structural rather than personal.',
          'The first reason is arithmetic. All investors together are the market, so before costs the average investor gets the average return. After costs, the average investor sits behind the market — this is not a question of anybody\'s skill.',
          'The second is competition. As information spreads faster and the number of professional participants grows, the chance of any one of them knowing more than the rest declines.',
          'The third is size. A fund that succeeds attracts more money, and deploying more money with the same skill becomes harder. Success makes the next success more difficult on its own.',
          'None of this means no active fund can stay ahead. It means identifying in advance which one will is very difficult — and past performance is not a reliable basis for that identification.',
        ],
      },
      {
        hHi: 'भारत में यह बहस थोड़ी अलग क्यों है',
        hEn: 'Why the debate is somewhat different in India',
        pHi: [
          'यह चर्चा पश्चिमी बाज़ारों से आयातित है, और भारत में उसे लागू करते समय कुछ स्थानीय बातें ध्यान में रखनी चाहिए।',
          'पहली, भारतीय बाज़ार अपेक्षाकृत नया और कम गहरा है, विशेषकर छोटी कंपनियों के हिस्से में। इसका अर्थ यह है कि वहाँ जानकारी का अंतर अब भी मौजूद हो सकता है, और सक्रिय प्रबंधन के लिए गुंजाइश बड़ी कंपनियों की तुलना में अधिक हो सकती है।',
          'दूसरी, यही कारण है कि बड़ी कंपनियों के हिस्से में सूचकांक फंड की स्थिति मज़बूत मानी जाती है, जबकि छोटी और मध्यम कंपनियों के हिस्से में बहस अब भी खुली है।',
          'तीसरी, भारत में सूचकांक फंडों का इतिहास छोटा है, इसलिए बहुत लंबी अवधि के आँकड़े सीमित हैं। यह किसी निष्कर्ष को ग़लत नहीं करता, पर उसे कम निश्चित बनाता है।',
          'इसलिए यहाँ सबसे संतुलित निष्कर्ष यह है कि यह "या तो यह या वह" का प्रश्न नहीं है। बहुत से निवेशक अपने पोर्टफ़ोलियो के मुख्य हिस्से के लिए सूचकांक और किसी विशिष्ट हिस्से के लिए सक्रिय चुनते हैं, और यह एक उचित रुख़ है।',
        ],
        pEn: [
          'This discussion is imported from Western markets, and applying it in India calls for a few local considerations.',
          'First, the Indian market is relatively young and less deep, particularly in the smaller-company segment. That means information gaps may still exist there, and the room for active management may be larger than among large companies.',
          'Second, this is why the case for index funds is considered strong in the large-company segment while the debate remains genuinely open in the small and mid segments.',
          'Third, index funds have a short history in India, so very long-period data is limited. That does not make any conclusion wrong, but it does make it less certain.',
          'The most balanced conclusion here is therefore that this is not an either-or question. Many investors use index funds for the core of a portfolio and active funds for a specific portion, and that is a reasonable position.',
        ],
      },
      {
        hHi: 'सूचकांक फंड चुनते समय क्या देखें',
        hEn: 'What to look at when choosing an index fund',
        pHi: [
          'यदि आप सूचकांक की ओर जा रहे हैं, तो एक बात याद रखने लायक़ है: सारे सूचकांक फंड एक जैसे नहीं हैं, और चुनाव अब भी करना पड़ता है।',
          'पहला प्रश्न यह है कि फंड किस सूचकांक की नक़ल कर रहा है। बड़ी कंपनियों का व्यापक सूचकांक, कोई क्षेत्रीय सूचकांक, या कोई विशेष रणनीति वाला सूचकांक — ये सब बहुत अलग चीज़ें हैं, भले सब "सूचकांक फंड" कहलाएँ।',
          'दूसरा, ख़र्च अनुपात और ट्रैकिंग एरर, जिनकी चर्चा ऊपर हुई। इन दोनों को साथ देखिए।',
          'तीसरा, फंड का आकार और उसकी तरलता। बहुत छोटे फंड में लेन-देन की लागत अधिक पड़ सकती है, जो ट्रैकिंग एरर में दिखती है।',
          'और चौथा, अपनी ओर से एक चीज़: बार-बार मत बदलिए। सूचकांक निवेश का पूरा लाभ ही धैर्य और कम लागत से आता है, और हर साल फंड बदलना उन दोनों को नष्ट कर देता है। यह लेख किसी विशेष फंड की सिफ़ारिश नहीं करता; यह केवल यह बताता है कि किन बिंदुओं पर देखना चाहिए।',
        ],
        pEn: [
          'If you are moving towards index funds, one thing is worth remembering: they are not all the same, and a choice still has to be made.',
          'The first question is which index the fund is tracking. A broad large-company index, a sector index, or an index built around a particular strategy — these are very different things even though all of them are called index funds.',
          'Second, the expense ratio and tracking error discussed above. Look at both together rather than either alone.',
          'Third, the size and liquidity of the fund. A very small fund can incur higher transaction costs, and that shows up in the tracking error.',
          'And fourth, something from your own side: do not keep switching. The whole benefit of index investing comes from patience and low cost, and changing funds every year destroys both. This article recommends no particular fund; it only sets out what to look at.',
        ],
      },
    ],
  },

  // ---------------------------------------------------------------------
  // Batch six — the investing articles from the shorter batches.
  // ---------------------------------------------------------------------

  {
    slug: 'asset-allocation-basics-india',
    sections: [
      {
        hHi: 'आवंटन तय करने से पहले तीन प्रश्न',
        hEn: 'Three questions before deciding an allocation',
        pHi: [
          'आवंटन का कोई सार्वभौमिक सही उत्तर नहीं है, इसलिए किसी सूत्र से शुरू करने के बजाय तीन प्रश्नों से शुरू करना अधिक उपयोगी है।',
          'पहला प्रश्न समय का है: यह पैसा कब चाहिए? एक वर्ष में चाहिए तो जोखिम की कोई गुंजाइश नहीं; पंद्रह वर्ष में चाहिए तो उतार-चढ़ाव सहन किया जा सकता है, क्योंकि उसे सुधरने का समय मिलेगा।',
          'दूसरा प्रश्न क्षमता का है: यदि यह राशि एक वर्ष में बीस प्रतिशत घट जाए, तो क्या आपकी योजना टूट जाएगी? यह भावना का नहीं, तथ्य का प्रश्न है, और इसका उत्तर आपकी आय, कोष और दायित्वों से निकलता है।',
          'तीसरा प्रश्न स्वभाव का है: यदि वही गिरावट हो जाए, तो क्या आप बेच देंगे? यहाँ ईमानदारी सबसे ज़रूरी है, क्योंकि वह आवंटन बेकार है जिसे आप बुरे समय में निभा न सकें।',
          'तीनों उत्तर मिलकर एक दायरा बना देते हैं, और उसी दायरे के भीतर कोई भी अनुपात काम कर जाता है। जो काम नहीं करता वह है किसी और के अनुपात की नक़ल करना।',
        ],
        pEn: [
          'There is no universally correct allocation, so starting with three questions is more useful than starting with a formula.',
          'The first is about time: when is this money needed? Needed in a year, there is no room for risk; needed in fifteen, movement can be tolerated because there is time for it to recover.',
          'The second is about capacity: if this amount fell twenty percent in a year, would your plan break? That is a question of fact rather than feeling, and the answer comes from your income, your fund and your obligations.',
          'The third is about temperament: if that same fall happened, would you sell? Honesty matters most here, because an allocation you cannot keep to in a bad period is useless.',
          'The three answers together produce a range, and any proportion inside that range works. What does not work is copying somebody else\'s proportion.',
        ],
      },
      {
        hHi: 'पुनर्संतुलन: नियम और उसकी मुश्किल',
        hEn: 'Rebalancing: the rule and its difficulty',
        pHi: [
          'आवंटन तय करना आधा काम है; उसे बनाए रखना दूसरा आधा है, और यही हिस्सा प्रायः छूट जाता है।',
          'समय के साथ जो श्रेणी तेज़ी से बढ़ती है, उसका हिस्सा अपने आप बड़ा हो जाता है। यदि आपने साठ-चालीस तय किया था और इक्विटी तेज़ी से बढ़ी, तो कुछ वर्षों बाद वह पचहत्तर-पच्चीस हो चुका होगा — और आपका जोखिम आपकी जानकारी के बिना बढ़ गया।',
          'पुनर्संतुलन का अर्थ है उसे वापस मूल अनुपात पर लाना — यानी जो बढ़ा है उसका कुछ हिस्सा बेचकर जो पिछड़ा है उसमें डालना।',
          'यह करना मनोवैज्ञानिक रूप से कठिन है, क्योंकि इसका अर्थ है अच्छा प्रदर्शन करने वाली चीज़ बेचना और ख़राब प्रदर्शन करने वाली में डालना। यह हर सहज प्रवृत्ति के विरुद्ध लगता है।',
          'इसीलिए इसे एक यांत्रिक नियम बना देना चाहिए, निर्णय नहीं — साल में एक तय तारीख़ पर, या जब कोई श्रेणी अपने लक्ष्य से एक निश्चित सीमा से अधिक भटक जाए। नियम बना लेने पर उस दिन सोचना नहीं पड़ता।',
        ],
        pEn: [
          'Deciding the allocation is half the work; maintaining it is the other half, and that half is usually skipped.',
          'Over time whichever category grows fastest takes a larger share on its own. If you set sixty-forty and equity ran up, a few years later it may be seventy-five to twenty-five — and your risk has risen without your knowing it.',
          'Rebalancing means bringing it back to the original proportion — selling part of what has grown and putting it into what has lagged.',
          'Doing this is psychologically hard, because it means selling what is performing well and buying what is performing badly. It runs against every instinct.',
          'Which is why it should be a mechanical rule rather than a decision — on a fixed date once a year, or whenever a category drifts beyond a set band from its target. With a rule in place, nothing has to be decided on the day.',
        ],
      },
      {
        hHi: 'उम्र आधारित सूत्रों की सीमाएँ',
        hEn: 'The limits of age-based formulas',
        pHi: [
          'आवंटन पर सबसे प्रसिद्ध सलाह उम्र से जुड़ी है — किसी संख्या में से अपनी उम्र घटाइए और उतना प्रतिशत इक्विटी में रखिए। यह सूत्र सरल है और इसीलिए लोकप्रिय है।',
          'इसकी उपयोगिता एक ही बात में है: यह याद दिलाता है कि उम्र बढ़ने के साथ जोखिम घटना चाहिए। यह सिद्धांत सही है।',
          'पर सूत्र इसे बहुत मोटे तरीक़े से लागू करता है। दो व्यक्ति जिनकी उम्र एक है, उनमें से एक की पेंशन तय हो सकती है और दूसरे की कोई आय-सुरक्षा नहीं — इन दोनों का आवंटन एक कैसे हो सकता है?',
          'दूसरी सीमा यह है कि सूत्र केवल उम्र देखता है, लक्ष्य नहीं। साठ वर्ष का व्यक्ति जो अपने पोते की शिक्षा के लिए पैसा रख रहा है, उसका समय-क्षितिज पंद्रह वर्ष का है, उसकी उम्र चाहे जो हो।',
          'इसलिए सूत्र को शुरुआती बिंदु मानिए और उसे अपनी तीन बातों से समायोजित कीजिए: आय की स्थिरता, अन्य सुरक्षा, और पैसे का वास्तविक उद्देश्य।',
        ],
        pEn: [
          'The most famous advice on allocation is tied to age — subtract your age from some number and hold that percentage in equity. The formula is simple, which is why it is popular.',
          'Its usefulness lies in one thing: it reminds you that risk should decline as age rises. That principle is correct.',
          'But the formula applies it very crudely. Of two people the same age, one may have a guaranteed pension and the other no income security at all — how can their allocations be identical?',
          'The second limit is that the formula looks only at age and not at goals. A sixty-year-old setting money aside for a grandchild\'s education has a fifteen-year horizon, whatever their own age happens to be.',
          'So treat the formula as a starting point and adjust it against three things: the stability of your income, whatever other security exists, and the actual purpose of the money.',
        ],
      },
      {
        hHi: 'सोने और अचल संपत्ति को कैसे गिनें',
        hEn: 'How to count gold and property',
        pHi: [
          'भारतीय परिवारों में आवंटन की गणना प्रायः इसलिए ग़लत निकलती है क्योंकि दो बड़ी श्रेणियाँ उसमें गिनी ही नहीं जातीं।',
          'पहली सोना है। घर में मौजूद गहनों को लोग "संपत्ति" नहीं मानते और इसलिए आवंटन में नहीं गिनते। पर वह वास्तविक मूल्य रखता है, और यदि उसे गिना जाए तो कई परिवारों का सोने का हिस्सा अपेक्षा से कहीं बड़ा निकलता है।',
          'दूसरी अचल संपत्ति है, और यहाँ एक भेद ज़रूरी है। जिस घर में आप रहते हैं वह निवेश नहीं है — वह उपभोग है, क्योंकि उसे बेचकर आप उसका उपयोग नहीं कर सकते। जो संपत्ति किराए पर दी है या निवेश के लिए रखी है, वही आवंटन में गिनी जानी चाहिए।',
          'इस भेद को न करने से एक आम स्थिति बनती है: परिवार यह मानता है कि वह संतुलित है, जबकि उसकी लगभग पूरी संपत्ति एक ही श्रेणी में है — अचल संपत्ति में — और वह श्रेणी सबसे कम तरल है।',
          'इसलिए आवंटन की गणना करते समय एक बार पूरी सूची बनाइए: बैंक, निवेश, भविष्य निधि, सोना, और निवेश वाली संपत्ति। इस पूरी सूची पर देखने से तस्वीर प्रायः बहुत अलग दिखती है।',
        ],
        pEn: [
          'Allocation calculations in Indian households often come out wrong because two large categories are simply not counted.',
          'The first is gold. Jewellery in the house is not thought of as an asset and so does not enter the allocation. But it holds real value, and once counted, many families find their gold share is considerably larger than they assumed.',
          'The second is property, and a distinction matters here. The home you live in is not an investment — it is consumption, because you cannot use it after selling it. Property let out or held for investment is what belongs in the allocation.',
          'Failing to make that distinction produces a common situation: a family believes it is balanced while very nearly all of its wealth sits in one category — property — and that category is the least liquid of them all.',
          'So when calculating allocation, build the complete list once: bank, investments, provident fund, gold, and investment property. Seen against that full list, the picture usually looks very different.',
        ],
      },
      {
        hHi: 'लक्ष्य-आधारित आवंटन: एक बेहतर ढाँचा',
        hEn: 'Goal-based allocation: a better framework',
        pHi: [
          'एक ही आवंटन पूरी संपत्ति पर लागू करने के बजाय, एक अधिक व्यावहारिक तरीक़ा यह है कि हर लक्ष्य का अपना आवंटन हो।',
          'इसका कारण सीधा है: अलग-अलग लक्ष्यों की समय-सीमा अलग है, इसलिए उनका जोखिम भी अलग होना चाहिए। बच्चे की फ़ीस जो अगले साल देनी है और सेवानिवृत्ति जो बीस साल दूर है — इन दोनों का पैसा एक जैसा नहीं रखा जा सकता।',
          'व्यावहारिक तरीक़ा यह है कि हर लक्ष्य को एक पंक्ति में लिखिए, उसके सामने उसकी तारीख़ लिखिए, और उसी तारीख़ से उसका आवंटन तय कीजिए। पास के लक्ष्य सुरक्षित साधनों में, दूर के लक्ष्य वृद्धि वाले साधनों में।',
          'इसका एक बड़ा मनोवैज्ञानिक लाभ भी है। जब बाज़ार गिरता है, तो आप यह देख पाते हैं कि गिरने वाला पैसा वह है जिसकी ज़रूरत पंद्रह साल बाद है — और यह जानकारी घबराहट को बहुत कम कर देती है।',
          'और जैसे-जैसे कोई लक्ष्य पास आता है, उसका आवंटन अपने आप बदलना चाहिए। यही वह क्रमिक कमी है जो लक्ष्य की तारीख़ पर बाज़ार की स्थिति को अप्रासंगिक बना देती है।',
        ],
        pEn: [
          'Rather than applying one allocation across all your wealth, a more practical approach gives each goal its own.',
          'The reason is straightforward: different goals have different horizons, so they should carry different risk. A school fee due next year and a retirement twenty years away cannot hold their money the same way.',
          'The practical method is to write each goal on a line, put its date beside it, and set its allocation from that date. Near goals into safe instruments, distant goals into growth instruments.',
          'There is a large psychological benefit as well. When the market falls, you can see that the money falling is the money needed in fifteen years — and knowing that reduces panic enormously.',
          'And as a goal approaches, its allocation should shift on its own. That gradual reduction is what makes the state of the market on the goal date irrelevant.',
        ],
      },
      {
        hHi: 'सबसे आम आवंटन-ग़लतियाँ',
        hEn: 'The most common allocation mistakes',
        pHi: [
          'कुछ ग़लतियाँ इतनी बार दोहराई जाती हैं कि उन्हें अलग से सूचीबद्ध करना उपयोगी है।',
          'पहली: बहुत सारे फंड रखना और यह मान लेना कि यह विविधीकरण है। यदि आठ फंड एक ही श्रेणी के हैं, तो वे लगभग एक ही चीज़ हैं — विविधीकरण श्रेणियों के बीच होता है, नामों के बीच नहीं।',
          'दूसरी: सबसे हाल में अच्छा प्रदर्शन करने वाली श्रेणी में हिस्सा बढ़ा देना। यह आवंटन का उल्टा है — आप महँगे में ख़रीद रहे हैं और आगे के लिए जोखिम बढ़ा रहे हैं।',
          'तीसरी: आपातकालीन कोष को आवंटन में गिन लेना। वह कोष निवेश नहीं है और उसे इस गणना से बाहर रखना चाहिए, वरना आपका सुरक्षित हिस्सा असल से बड़ा दिखता है।',
          'चौथी: भविष्य निधि को भूल जाना। वेतनभोगी लोगों के लिए यह प्रायः सबसे बड़ी एकल राशि होती है और वह पूरी तरह ऋण श्रेणी में है। उसे न गिनने से लोग अपने आवंटन को वास्तव में जितना है उससे कहीं अधिक जोखिम भरा समझ लेते हैं।',
        ],
        pEn: [
          'A few mistakes are repeated often enough to be worth listing separately.',
          'First: holding many funds and treating that as diversification. If eight funds sit in the same category they are very nearly the same thing — diversification happens across categories, not across names.',
          'Second: increasing the share of whichever category performed best most recently. That is allocation in reverse — buying dear and raising the risk carried forward.',
          'Third: counting the emergency fund inside the allocation. That fund is not an investment and belongs outside the calculation, otherwise the safe portion looks larger than it really is.',
          'Fourth: forgetting the provident fund. For salaried people it is often the single largest amount held and it sits entirely in the debt category. Leaving it out makes people believe their allocation is far riskier than it actually is.',
        ],
      },
      {
        hHi: 'सरल रखने का तर्क',
        hEn: 'The case for keeping it simple',
        pHi: [
          'आवंटन के बारे में पढ़ते-पढ़ते एक ख़तरा यह है कि व्यवस्था इतनी जटिल बना ली जाए कि उसे निभाना असंभव हो जाए।',
          'व्यवहार में तीन या चार श्रेणियाँ अधिकांश लोगों के लिए पर्याप्त हैं, और उनमें से हर एक में एक या दो साधन। इससे अधिक जोड़ने पर परिणाम में सुधार बहुत कम होता है और प्रबंधन की कठिनाई बहुत बढ़ जाती है।',
          'इसका एक और कारण है जो कम कहा जाता है: एक जटिल पोर्टफ़ोलियो केवल आपके लिए ही कठिन नहीं है। यदि कभी उसे किसी और को सँभालना पड़े — परिवार में किसी को — तो सरलता एक वास्तविक सुरक्षा बन जाती है।',
          'सबसे अच्छी परीक्षा यह है: क्या आप अपना पूरा आवंटन एक काग़ज़ पर पाँच पंक्तियों में लिख सकते हैं? यदि नहीं, तो वह शायद उससे अधिक जटिल है जितनी ज़रूरत है।',
          'और अंत में वही बात जो हर वित्तीय विषय पर लागू होती है: वह योजना जो निभाई जा सके, उस योजना से बेहतर है जो काग़ज़ पर अधिक कुशल हो।',
        ],
        pEn: [
          'One risk in reading about allocation is building a system so complex that keeping to it becomes impossible.',
          'In practice three or four categories are enough for most people, with one or two instruments inside each. Adding beyond that improves the outcome very little and increases the difficulty of managing it a great deal.',
          'There is another reason that is said less often: a complex portfolio is not only hard for you. If somebody else ever has to handle it — somebody in the family — simplicity becomes a real protection.',
          'The best test is this: can you write your entire allocation on one sheet of paper in five lines? If not, it is probably more complicated than it needs to be.',
          'And finally the point that applies to every financial subject: a plan you can keep to beats a plan that is more efficient on paper.',
        ],
      },
    ],
  },

  {
    slug: 'sovereign-gold-bonds-guide',
    sections: [
      {
        hHi: 'ब्याज वाला हिस्सा — और उस पर कर',
        hEn: 'The interest component — and the tax on it',
        pHi: [
          'यह वह विशेषता है जो इन बॉन्डों को सोने के अन्य रूपों से अलग करती है: सोने की क़ीमत के अलावा इन पर एक निश्चित ब्याज भी मिलता है, जो आमतौर पर वर्ष में दो बार जमा होता है।',
          'यह छोटा-सा जोड़ इसलिए महत्वपूर्ण है कि सोने का कोई भी अन्य रूप कुछ नहीं देता। भौतिक सोना, ETF, डिजिटल सोना — इन सबका पूरा प्रतिफल केवल क़ीमत बदलने से आता है।',
          'पर एक बात स्पष्ट होनी चाहिए: यह ब्याज आमतौर पर कर-योग्य होता है और आपकी आय में जुड़ता है। इसलिए इसकी तुलना करते समय कर के बाद की राशि देखनी चाहिए, घोषित दर नहीं।',
          'यह ब्याज उस मूल राशि पर मिलता है जो आपने निवेश के समय दी थी, न कि सोने के वर्तमान मूल्य पर। यह विवरण छोटा है और इसे न जानने से बाद में अपेक्षा ग़लत बन जाती है।',
          'दरें और कर के प्रावधान समय के साथ बदलते हैं, इसलिए किसी भी नई किश्त में निवेश से पहले उस समय की शर्तें पढ़ लेना ज़रूरी है।',
        ],
        pEn: [
          'This is the feature that separates these bonds from every other form of gold: besides the gold price they pay a fixed rate of interest, usually credited twice a year.',
          'That small addition matters because no other form of gold pays anything at all. Physical gold, ETFs, digital gold — their entire return comes from price movement alone.',
          'One point should be clear though: this interest is generally taxable and adds to your income. So when comparing, look at the post-tax amount rather than the stated rate.',
          'The interest is paid on the original amount invested rather than on the current value of the gold. That detail is small and not knowing it produces the wrong expectation later.',
          'Rates and tax provisions change over time, so the terms applicable at the time should be read before investing in any new tranche.',
        ],
      },
      {
        hHi: 'आठ वर्ष की अवधि और बीच में निकलने के रास्ते',
        hEn: 'The eight-year term and the ways out of it',
        pHi: [
          'इन बॉन्डों की पूरी अवधि आठ वर्ष है, और यही वह बात है जो कई लोगों को हिचकिचाहट में डालती है। पर बीच में निकलने के रास्ते मौजूद हैं और उन्हें जान लेना ज़रूरी है।',
          'पहला रास्ता निर्धारित निकासी खिड़कियाँ हैं, जो एक तय अवधि के बाद ब्याज भुगतान की तारीख़ों पर उपलब्ध होती हैं। यह रास्ता सीधा है पर वह हर समय खुला नहीं रहता।',
          'दूसरा रास्ता द्वितीयक बाज़ार है, क्योंकि ये बॉन्ड शेयर बाज़ार में सूचीबद्ध होते हैं। यहाँ कभी भी बेचा जा सकता है, पर दो बातें ध्यान रखने की हैं: तरलता हर किश्त में एक जैसी नहीं होती, और बाज़ार मूल्य सोने के मूल्य से थोड़ा अलग हो सकता है।',
          'इसका व्यावहारिक अर्थ यह है कि यदि आपको निश्चित रूप से तीन-चार वर्षों में पैसा चाहिए, तो यह सबसे उपयुक्त साधन नहीं है — या कम से कम, उसकी पूरी राशि इसमें नहीं होनी चाहिए।',
          'और यदि आप पूरी अवधि तक रखने की योजना बना सकते हैं, तो यही वह स्थिति है जिसमें इस साधन की सारी विशेषताएँ आपके पक्ष में काम करती हैं।',
        ],
        pEn: [
          'The full term of these bonds is eight years, and that is what makes many people hesitate. But routes out exist partway and they are worth knowing.',
          'The first is the defined redemption windows, available after a set period on interest payment dates. That route is straightforward but it is not open at all times.',
          'The second is the secondary market, since these bonds are listed on the exchange. They can be sold at any time there, with two caveats: liquidity is not the same across every tranche, and the market price can differ somewhat from the gold price.',
          'The practical consequence is that if you definitely need the money in three or four years, this is not the most suitable instrument — or at least, not the whole of that money.',
          'And if you can plan to hold for the full term, that is exactly the situation in which all of this instrument\'s features work in your favour.',
        ],
      },
      {
        hHi: 'प्राथमिक निर्गम बनाम बाज़ार से ख़रीद',
        hEn: 'Buying at issue against buying in the market',
        pHi: [
          'इन बॉन्डों को ख़रीदने के दो रास्ते हैं और उनके बीच का अंतर व्यावहारिक रूप से मायने रखता है।',
          'पहला रास्ता प्राथमिक निर्गम है — जब कोई नई किश्त खुलती है और एक निर्धारित अवधि में आवेदन किया जा सकता है। इसका लाभ यह है कि क़ीमत एक घोषित सूत्र से तय होती है और प्रायः ऑनलाइन आवेदन पर एक छोटी छूट भी मिलती है।',
          'दूसरा रास्ता द्वितीयक बाज़ार है, जहाँ पहले से जारी बॉन्ड ख़रीदे जा सकते हैं। यहाँ का लाभ यह है कि आपको नई किश्त का इंतज़ार नहीं करना पड़ता, और कभी-कभी बॉन्ड सोने के मूल्य से थोड़ा नीचे भी मिल सकता है।',
          'द्वितीयक बाज़ार का एक और कोण यह है कि वहाँ आप कम शेष अवधि वाले बॉन्ड चुन सकते हैं — यदि आपको आठ वर्ष नहीं बल्कि तीन वर्ष के लिए चाहिए, तो कोई पुरानी किश्त उपयुक्त हो सकती है।',
          'इसकी सीमा तरलता है। कुछ किश्तों में सौदे कम होते हैं, इसलिए ख़रीदते और बेचते समय क़ीमत में अंतर बड़ा हो सकता है। छोटी राशि के लिए यह प्रायः समस्या नहीं है; बड़ी राशि के लिए इसे देख लेना चाहिए।',
        ],
        pEn: [
          'There are two routes to buying these bonds and the difference between them matters in practice.',
          'The first is the primary issue — when a new tranche opens and applications are accepted for a defined period. Its advantage is that the price is set by a published formula and a small discount is usually available on applying online.',
          'The second is the secondary market, where previously issued bonds can be bought. The advantage there is not having to wait for a new tranche, and bonds sometimes trade a little below the gold price.',
          'Another angle on the secondary market is that you can pick bonds with less remaining term — if you want three years rather than eight, an older tranche may suit.',
          'Its limitation is liquidity. Some tranches trade thinly, so the gap between buying and selling prices can be wide. For small amounts this is usually not a problem; for large ones it should be checked.',
        ],
      },
      {
        hHi: 'भंडारण और शुद्धता की चिंता से मुक्ति',
        hEn: 'Freedom from storage and purity worries',
        pHi: [
          'इन बॉन्डों का सबसे कम गिना जाने वाला लाभ वह है जो दिखाई नहीं देता: वे चीज़ें जो आपको करनी नहीं पड़तीं।',
          'भौतिक सोने के साथ तीन स्थायी चिंताएँ जुड़ी हैं — शुद्धता की जाँच, सुरक्षित भंडारण, और बेचते समय कटौती। इनमें से कोई भी यहाँ लागू नहीं होती।',
          'लॉकर का वार्षिक शुल्क अपने आप में एक लागत है जिसे लोग सोने के प्रतिफल से घटाकर नहीं देखते। यदि उसे घटाया जाए, तो भौतिक सोने और इन बॉन्डों का अंतर और बढ़ जाता है।',
          'और बेचते समय कोई मज़दूरी नहीं कटती, कोई शुद्धता की बहस नहीं होती, और कोई ख़रीदार खोजने की ज़रूरत नहीं होती। परिपक्वता पर राशि सीधे खाते में आती है।',
          'यह सुविधा किसी संख्या में नहीं दिखती, और फिर भी यही वह अंतर है जो अधिकांश लोगों के लिए व्यावहारिक रूप से सबसे बड़ा है।',
        ],
        pEn: [
          'The least counted benefit of these bonds is the invisible one: the things you no longer have to do.',
          'Physical gold carries three permanent worries — verifying purity, storing it safely, and the deduction taken when selling. None of them applies here.',
          'The annual locker fee is itself a cost that people do not subtract from the return on their gold. Once subtracted, the gap between physical gold and these bonds widens further.',
          'And on selling there are no making charges lost, no argument about purity, and no need to find a buyer. At maturity the amount arrives in the account directly.',
          'None of this appears as a number, and it is nevertheless the difference that matters most in practice for most people.',
        ],
      },
      {
        hHi: 'नामांकन और उत्तराधिकार',
        hEn: 'Nomination and inheritance',
        pHi: [
          'चूँकि यह आठ वर्षों तक चलने वाला साधन है, इसलिए उत्तराधिकार का पक्ष यहाँ विशेष रूप से महत्वपूर्ण है — और यह प्रायः पूरी तरह छूट जाता है।',
          'नामांकन आवेदन के समय ही किया जा सकता है और बाद में बदला भी जा सकता है। यह सबसे बुनियादी क़दम है और उसमें कुछ मिनट लगते हैं।',
          'दूसरा क़दम रिकॉर्ड का है। यदि बॉन्ड डीमैट रूप में हैं तो वे उसी खाते में दिखेंगे; यदि प्रमाणपत्र के रूप में हैं तो वह काग़ज़ सुरक्षित और खोजने योग्य जगह पर होना चाहिए।',
          'तीसरा और सबसे महत्वपूर्ण: परिवार में किसी को इसके बारे में पता होना चाहिए। यह भौतिक सोने से अलग है — घर में रखा सोना अपने आप मिल जाता है, जबकि एक बॉन्ड जिसके बारे में किसी को पता नहीं, वह वर्षों तक अनदेखा रह सकता है।',
          'यह बात हर वित्तीय साधन पर लागू होती है, पर लंबी अवधि वाले साधनों पर विशेष रूप से — क्योंकि वहीं भूल जाने की संभावना सबसे अधिक है।',
        ],
        pEn: [
          'Because this instrument runs for eight years, the inheritance side matters particularly here — and it is very often skipped entirely.',
          'Nomination can be done at the time of application and changed later. It is the most basic step and takes a few minutes.',
          'The second step is the record. If the bonds are held in demat form they appear in that account; if they are held as certificates, the paper needs to be somewhere safe and findable.',
          'Third and most important: somebody in the family should know about them. This differs from physical gold — gold kept at home is found on its own, whereas a bond nobody knows about can go unnoticed for years.',
          'The point applies to every financial instrument, and to long-term ones especially — because that is where being forgotten is most likely.',
        ],
      },
      {
        hHi: 'यह किसके लिए उपयुक्त है और किसके लिए नहीं',
        hEn: 'Who this suits and who it does not',
        pHi: [
          'हर साधन की तरह इसकी भी एक जगह है, और उसे स्पष्ट कर देना चुनाव को आसान बना देता है।',
          'यह उन लोगों के लिए उपयुक्त है जो सोने को अपने पोर्टफ़ोलियो में एक दीर्घकालिक, संतुलन देने वाले हिस्से के रूप में रखना चाहते हैं और जिन्हें अगले कुछ वर्षों में वह पैसा नहीं चाहिए।',
          'यह उन लोगों के लिए उपयुक्त नहीं है जिन्हें सोना पहनने या किसी अवसर पर देने के लिए चाहिए। यह एक वित्तीय साधन है, कोई वस्तु नहीं, और वह ज़रूरत इससे पूरी नहीं होती।',
          'यह उन लोगों के लिए भी उपयुक्त नहीं है जिन्हें बार-बार ख़रीदने और बेचने की सुविधा चाहिए, क्योंकि द्वितीयक बाज़ार में तरलता सीमित हो सकती है।',
          'और एक अंतिम बात जो हर सोने पर लागू है: यह पोर्टफ़ोलियो का एक हिस्सा है, पूरा पोर्टफ़ोलियो नहीं। यह लेख कोई अनुपात नहीं सुझाता, पर यह ज़रूर कहता है कि किसी एक श्रेणी में बहुत बड़ा हिस्सा रखना — चाहे वह कितनी भी परिचित हो — अपने आप एक जोखिम है।',
        ],
        pEn: [
          'Like every instrument this one has a place, and stating it plainly makes the choice easier.',
          'It suits people who want gold in their portfolio as a long-term balancing component and who will not need that money over the next several years.',
          'It does not suit people who want gold to wear or to give on an occasion. This is a financial instrument rather than an object, and that need is not met by it.',
          'It also does not suit people who want the ability to buy and sell frequently, because liquidity in the secondary market can be limited.',
          'And a final point that applies to all gold: this is one part of a portfolio, not the portfolio. This article suggests no proportion, but it does say that holding a very large share in any single category — however familiar it feels — is itself a risk.',
        ],
      },
      {
        hHi: 'निर्गम के समय ध्यान देने की बातें',
        hEn: 'Things to watch at the time of issue',
        pHi: [
          'यदि आप प्राथमिक निर्गम में आवेदन करने जा रहे हैं, तो कुछ व्यावहारिक विवरण उपयोगी हैं।',
          'पहला, आवेदन की खिड़की छोटी होती है — आमतौर पर कुछ दिन। इसलिए यदि आप इसका उपयोग करने वाले हैं, तो घोषणाओं पर ध्यान रखना पड़ता है, अन्यथा किश्त निकल जाती है।',
          'दूसरा, न्यूनतम और अधिकतम मात्रा निर्धारित होती है, और अधिकतम सीमा व्यक्ति और वित्तीय वर्ष के हिसाब से लागू होती है। परिवार के लिए योजना बनाते समय यह देख लेना चाहिए।',
          'तीसरा, आवेदन बैंक, डाकघर, मान्यता-प्राप्त स्टॉक एक्सचेंज और कुछ अन्य निर्धारित माध्यमों से किया जा सकता है। ऑनलाइन आवेदन और डिजिटल भुगतान पर प्रायः एक छोटी छूट मिलती है।',
          'चौथा, आवंटन के बाद प्रमाणपत्र या डीमैट प्रविष्टि की पुष्टि कर लीजिए। यह छोटा-सा काम बाद के हर चरण को सरल बना देता है — ब्याज की प्राप्ति से लेकर परिपक्वता तक।',
        ],
        pEn: [
          'If you are going to apply in a primary issue, a few practical details help.',
          'First, the application window is short — usually a few days. So if you intend to use it, the announcements need watching, or the tranche passes.',
          'Second, minimum and maximum quantities are prescribed, and the maximum applies per person per financial year. This is worth checking when planning for a family.',
          'Third, applications can be made through banks, post offices, recognised stock exchanges and certain other designated channels. A small discount is usually available for applying online with digital payment.',
          'Fourth, confirm the certificate or the demat entry after allotment. That small task simplifies every later stage — from receiving interest through to maturity.',
        ],
      },
    ],
  },

  {
    slug: 'reit-investment-india',
    sections: [
      {
        hHi: 'आय कहाँ से आती है',
        hEn: 'Where the income actually comes from',
        pHi: [
          'REIT को समझने का सबसे साफ़ रास्ता यह देखना है कि पैसा किस रास्ते से आपके पास पहुँचता है, क्योंकि यही उसे शेयर और संपत्ति दोनों से अलग करता है।',
          'REIT कई व्यावसायिक इमारतें रखता है और उन्हें कंपनियों को किराए पर देता है। जो किराया आता है, उसमें से ख़र्च और ऋण की अदायगी घटाकर शेष का एक बड़ा हिस्सा नियमित रूप से यूनिटधारकों को वितरित करना होता है।',
          'यह अनिवार्य वितरण ही इस साधन की सबसे विशिष्ट बात है। सामान्य कंपनी यह तय कर सकती है कि लाभ बाँटे या न बाँटे; REIT के लिए यह नियम से बँधा है।',
          'इसका अर्थ यह है कि REIT की मुख्य पहचान नियमित नक़दी है, न कि तेज़ पूँजी-वृद्धि। जो व्यक्ति इसे शेयर की तरह देखकर तेज़ वृद्धि की अपेक्षा करता है, वह प्रायः निराश होता है।',
          'यहाँ यह भी जानना ज़रूरी है कि वितरण के अलग-अलग हिस्सों — किराया, ब्याज, पूँजी की वापसी — का कर-व्यवहार अलग हो सकता है। यह विवरण जटिल है और नियम बदलते रहते हैं, इसलिए निवेश से पहले उस वर्ष की स्थिति देखना ज़रूरी है।',
        ],
        pEn: [
          'The clearest way into REITs is to look at how money reaches you, because that is what separates them from both shares and property.',
          'A REIT owns several commercial buildings and lets them to companies. From the rent received, after costs and debt servicing, a large part of what remains has to be distributed to unit holders at regular intervals.',
          'That mandatory distribution is the most distinctive feature of the instrument. An ordinary company may decide whether to pay out profits; for a REIT it is bound by rule.',
          'This means the defining characteristic of a REIT is regular cash rather than rapid capital growth. Somebody treating it like a share and expecting fast appreciation is usually disappointed.',
          'It is also worth knowing that different components of a distribution — rent, interest, return of capital — can be taxed differently. The detail is intricate and the rules change, so the position for the relevant year should be checked before investing.',
        ],
      },
      {
        hHi: 'REIT और अपना फ़्लैट ख़रीदना — दोनों एक नहीं हैं',
        hEn: 'A REIT and buying your own flat are not the same thing',
        pHi: [
          'दोनों को "अचल संपत्ति में निवेश" कहा जाता है, और इसी वजह से लोग उनकी तुलना सीधे कर बैठते हैं। व्यवहार में वे बहुत अलग अनुभव हैं।',
          'सबसे बड़ा अंतर तरलता का है। REIT की यूनिट किसी भी कारोबारी दिन बेची जा सकती है; एक फ़्लैट बेचने में महीने लगते हैं और उसमें दलाली, दस्तावेज़ और बातचीत सब शामिल है।',
          'दूसरा अंतर राशि का है। REIT में छोटी राशि से शुरू किया जा सकता है, जबकि संपत्ति के लिए एक बड़ी राशि और प्रायः एक ऋण चाहिए।',
          'तीसरा अंतर विविधीकरण का है। एक फ़्लैट का पूरा भाग्य एक इमारत, एक इलाक़े और एक किरायेदार पर टिका है। REIT में कई इमारतें, कई शहर और कई किरायेदार होते हैं।',
          'और एक अंतर जो REIT के विरुद्ध जाता है: उसमें कोई नियंत्रण नहीं है। आप न किरायेदार चुनते हैं, न किराया तय करते हैं, न यह तय करते हैं कि कब बेचा जाए। जो व्यक्ति नियंत्रण चाहता है, उसके लिए यह साधन असंतोषजनक लगेगा।',
        ],
        pEn: [
          'Both are called "investing in real estate", and because of that people compare them directly. In practice they are very different experiences.',
          'The largest difference is liquidity. REIT units can be sold on any trading day; selling a flat takes months and involves brokerage, documentation and negotiation.',
          'The second is the amount. A REIT can be started with a small sum, while property requires a large one and usually a loan alongside it.',
          'The third is diversification. A flat\'s entire fortune rests on one building, one locality and one tenant. A REIT holds several buildings across several cities with many tenants.',
          'And one difference that runs against REITs: there is no control. You do not choose the tenant, do not set the rent, and do not decide when to sell. Somebody who wants control will find the instrument unsatisfying.',
        ],
      },
      {
        hHi: 'जो जोखिम सबसे कम दिखते हैं',
        hEn: 'The risks that are least visible',
        pHi: [
          'REIT के जोखिमों की सूची में सबसे स्पष्ट है किराए का कम हो जाना, पर कुछ और हैं जिन पर ध्यान कम जाता है।',
          'पहला ब्याज दरों का है। जब दरें बढ़ती हैं तो दो चीज़ें एक साथ होती हैं: REIT का अपना ऋण महँगा हो जाता है, और उसका वितरण अन्य सुरक्षित विकल्पों की तुलना में कम आकर्षक लगने लगता है। दोनों कारण मिलकर यूनिट की क़ीमत पर दबाव डालते हैं।',
          'दूसरा किरायेदारों की संरचना है। यदि किराए का बड़ा हिस्सा कुछ ही किरायेदारों या किसी एक उद्योग से आता है, तो उस उद्योग की कमज़ोरी सीधे असर करती है।',
          'तीसरा पट्टों की समाप्ति का है। सभी पट्टे किसी न किसी दिन नवीनीकरण के लिए खुलते हैं, और यदि उस समय बाज़ार कमज़ोर हो तो नया किराया पुराने से कम हो सकता है।',
          'चौथा, और यह सबसे कम गिना जाता है: काम करने के तरीक़ों में बदलाव। व्यावसायिक कार्यालयों की माँग इस बात पर निर्भर करती है कि कंपनियाँ कितनी जगह चाहती हैं, और यह अपेक्षा समय के साथ बदल सकती है।',
        ],
        pEn: [
          'The most obvious risk in a REIT is rents falling, but several others receive far less attention.',
          'The first is interest rates. When rates rise two things happen together: the REIT\'s own borrowing becomes costlier, and its distribution looks less attractive against other safe options. Both press on the unit price.',
          'The second is the tenant mix. If a large share of the rent comes from a few tenants or from one industry, weakness in that industry transmits directly.',
          'The third is lease expiry. Every lease comes up for renewal at some point, and if the market is weak then, the new rent can be lower than the old one.',
          'The fourth, and the least counted: changes in how work is done. Demand for commercial office space depends on how much space companies want, and that expectation can shift over time.',
        ],
      },
      {
        hHi: 'यूनिट की क़ीमत और अंतर्निहित मूल्य',
        hEn: 'Unit price against underlying value',
        pHi: [
          'यह एक ऐसा बिंदु है जो REIT को समझने में बहुत मदद करता है और जिसे प्रायः नहीं बताया जाता।',
          'REIT की संपत्तियों का एक मूल्यांकन किया जाता है और वह नियमित रूप से प्रकाशित होता है। साथ ही उसकी यूनिट बाज़ार में एक क़ीमत पर कारोबार करती है। ये दोनों संख्याएँ हमेशा बराबर नहीं होतीं।',
          'कभी यूनिट अपने अंतर्निहित मूल्य से नीचे मिलती है और कभी ऊपर। इसका कारण वही है जो किसी भी सूचीबद्ध साधन के साथ होता है — बाज़ार की धारणा, ब्याज दरों की अपेक्षा, और तात्कालिक माँग-आपूर्ति।',
          'इसका व्यावहारिक उपयोग यह है कि निवेश से पहले दोनों संख्याएँ देखी जा सकती हैं। यदि यूनिट लगातार अपने मूल्यांकन से काफ़ी ऊपर कारोबार कर रही है, तो यह जानना उपयोगी है, भले ही वह अकेला कारण न बने।',
          'यह जानकारी REIT के नियमित प्रकटीकरण में उपलब्ध होती है। यह लेख किसी REIT की सिफ़ारिश नहीं करता; यह केवल यह बता रहा है कि देखने लायक़ कौन-सी संख्या है।',
        ],
        pEn: [
          'This is a point that helps a great deal in understanding REITs and is rarely explained.',
          'A REIT\'s properties are valued and that valuation is published regularly. Separately, its units trade at a price in the market. Those two numbers are not always equal.',
          'Sometimes units trade below the underlying value and sometimes above. The reason is the same as for any listed instrument — market sentiment, expectations about interest rates, and immediate supply and demand.',
          'The practical use of this is that both numbers can be looked at before investing. If units are consistently trading well above the valuation, that is worth knowing, even if it is not a reason on its own.',
          'The information appears in the REIT\'s regular disclosures. This article recommends no particular REIT; it only points out which number is worth looking at.',
        ],
      },
      {
        hHi: 'पोर्टफ़ोलियो में इसे कहाँ रखें',
        hEn: 'Where this fits in a portfolio',
        pHi: [
          'REIT को किस श्रेणी में गिनें, यह प्रश्न लगता है तकनीकी पर उसका असर व्यावहारिक है।',
          'यह न पूरी तरह इक्विटी है और न पूरी तरह ऋण। इसकी क़ीमत बाज़ार में चलती है, इसलिए उसमें उतार-चढ़ाव है; पर उसकी आय किराए से आती है, इसलिए वह अपेक्षाकृत स्थिर होती है।',
          'इसीलिए अधिकांश लोग इसे एक अलग, छोटी श्रेणी के रूप में रखते हैं — पोर्टफ़ोलियो का वह हिस्सा जो नियमित नक़दी देता है और जिसकी चाल इक्विटी से थोड़ी अलग रहती है।',
          'हिस्सा छोटा रखने का एक कारण यह भी है कि भारत में यह बाज़ार अभी नया है और सूचीबद्ध विकल्पों की संख्या सीमित है। जहाँ विकल्प कम हों, वहाँ बड़ा हिस्सा रखना अपने आप एक संकेंद्रण है।',
          'और यदि आपके पास पहले से कोई निवेश-संपत्ति है, तो यह ध्यान रखना ज़रूरी है कि आप उसी श्रेणी में और जोड़ रहे हैं। आवंटन की पूरी सूची देखे बिना यह अंतर दिखता नहीं।',
        ],
        pEn: [
          'Which category to count a REIT in sounds like a technical question and its effect is practical.',
          'It is neither fully equity nor fully debt. Its price moves in the market, so it carries volatility; but its income comes from rent, so that part is relatively steady.',
          'This is why most people hold it as a separate, small category — the part of a portfolio producing regular cash and moving somewhat differently from equity.',
          'One reason for keeping the share small is that this market is still young in India and the number of listed options is limited. Where options are few, a large holding is itself a concentration.',
          'And if you already own an investment property, it is worth noticing that you are adding to the same category. Without looking at the full allocation list, that overlap is invisible.',
        ],
      },
      {
        hHi: 'ख़रीदने से पहले क्या पढ़ें',
        hEn: 'What to read before buying',
        pHi: [
          'REIT के बारे में जानकारी उपलब्ध है और वह पढ़ने योग्य है, इसलिए यहाँ अंदाज़े से काम चलाने की ज़रूरत नहीं।',
          'पहला, अधिभोग दर देखिए — कुल जगह का कितना हिस्सा किराए पर चढ़ा हुआ है। यह एक ही संख्या स्वास्थ्य के बारे में बहुत कुछ बता देती है।',
          'दूसरा, पट्टों की औसत शेष अवधि देखिए। लंबी शेष अवधि का अर्थ है कि आय की दृश्यता अधिक है, और छोटी अवधि का अर्थ है कि निकट भविष्य में नवीनीकरण का जोखिम है।',
          'तीसरा, ऋण का स्तर देखिए। अधिक ऋण वाला REIT ब्याज दरों के प्रति अधिक संवेदनशील होता है, और उसकी वितरण-क्षमता पर भी दबाव अधिक रहता है।',
          'चौथा, किरायेदारों की संरचना देखिए — कितने किरायेदार, किन उद्योगों से, और सबसे बड़े कुछ किरायेदारों का हिस्सा कितना है।',
          'ये चारों जानकारियाँ नियमित प्रस्तुतियों में उपलब्ध होती हैं और इन्हें देखने में आधा घंटा लगता है। किसी भी लंबी अवधि के निवेश के लिए यह बहुत कम समय है।',
        ],
        pEn: [
          'Information about REITs is available and readable, so there is no need to work from guesswork here.',
          'First, look at the occupancy rate — what share of the total space is actually let. That single number says a great deal about health.',
          'Second, look at the weighted average remaining lease term. A longer term means better visibility of income; a shorter one means renewal risk in the near future.',
          'Third, look at the level of debt. A REIT carrying more debt is more sensitive to interest rates, and the pressure on its ability to distribute is correspondingly higher.',
          'Fourth, look at the tenant mix — how many tenants, from which industries, and what share the largest few account for.',
          'All four are available in the regular presentations and take about half an hour to review. For a long-term investment that is very little time.',
        ],
      },
      {
        hHi: 'यथार्थवादी अपेक्षा',
        hEn: 'A realistic expectation',
        pHi: [
          'अंत में सबसे उपयोगी बात अपेक्षा को सही जगह रखना है, क्योंकि इसी से अधिकांश निराशा या संतोष तय होता है।',
          'REIT से नियमित, अपेक्षाकृत अनुमान योग्य नक़दी की अपेक्षा करना उचित है। तेज़ पूँजी-वृद्धि की अपेक्षा करना उचित नहीं — वह कभी-कभी हो सकती है पर वह इस साधन का उद्देश्य नहीं है।',
          'दूसरी उचित अपेक्षा यह है कि उसकी क़ीमत चलेगी। सूचीबद्ध साधन होने के कारण वह बाज़ार के साथ ऊपर-नीचे होगा, भले ही उसकी अंतर्निहित इमारतें वही रहें।',
          'तीसरी, और यह सबसे महत्वपूर्ण: वितरण की राशि निश्चित नहीं है। वह किराए, अधिभोग और ख़र्चों पर निर्भर करती है, और घट भी सकती है। पिछले वर्ष की दर अगले वर्ष की गारंटी नहीं है।',
          'इन तीनों अपेक्षाओं के साथ यह साधन एक उपयोगी भूमिका निभा सकता है। इनके बिना, वही साधन एक निराशा बन जाता है — और अंतर साधन में नहीं, अपेक्षा में होता है।',
        ],
        pEn: [
          'The most useful thing at the end is placing expectations correctly, because most disappointment or satisfaction is decided there.',
          'Expecting regular, relatively predictable cash from a REIT is reasonable. Expecting rapid capital growth is not — it can happen occasionally, but it is not what the instrument is for.',
          'The second reasonable expectation is that the price will move. Being listed, it will rise and fall with the market even while the underlying buildings stay exactly the same.',
          'The third, and most important: the distribution is not guaranteed. It depends on rent, occupancy and costs, and it can fall. Last year\'s rate is not a promise about next year.',
          'Held with those three expectations, the instrument can play a useful role. Without them, the same instrument becomes a disappointment — and the difference lies in the expectation rather than in the instrument.',
        ],
      },
    ],
  },

  {
    slug: 'rupee-cost-averaging-explained',
    sections: [
      {
        hHi: 'औसत निकलता कैसे है — एक सरल उदाहरण',
        hEn: 'How the average actually forms — a simple illustration',
        pHi: [
          'यह अवधारणा तब सबसे साफ़ होती है जब उसे संख्याओं में देखा जाए, इसलिए एक सरल उदाहरण उपयोगी है।',
          'मान लीजिए आप हर महीने एक हज़ार रुपये लगाते हैं। पहले महीने यूनिट का दाम सौ है, तो आपको दस यूनिट मिलीं। दूसरे महीने दाम गिरकर पचास हो गया, तो उसी हज़ार रुपये से बीस यूनिट मिलीं।',
          'अब आपने कुल दो हज़ार रुपये लगाकर तीस यूनिट ख़रीदीं। आपकी औसत लागत हुई लगभग सड़सठ रुपये प्रति यूनिट — जबकि दोनों दामों का साधारण औसत पचहत्तर होता।',
          'यह अंतर संयोग नहीं है। जब दाम कम होता है तो वही राशि अधिक यूनिट ख़रीदती है, इसलिए सस्ती यूनिटों का भार अपने आप अधिक हो जाता है — और औसत नीचे खिंच जाता है।',
          'यही पूरा तंत्र है। इसमें कोई भविष्यवाणी नहीं है, कोई चतुराई नहीं — केवल यह तथ्य कि एक तय राशि सस्ते में अधिक ख़रीदती है।',
        ],
        pEn: [
          'The concept is clearest when seen in numbers, so a simple illustration helps.',
          'Suppose you invest a thousand rupees each month. In the first month the unit price is a hundred, so you receive ten units. In the second the price falls to fifty, so the same thousand buys twenty units.',
          'You have now invested two thousand rupees and hold thirty units. Your average cost is about sixty-seven rupees a unit — while the simple average of the two prices would have been seventy-five.',
          'The difference is not a coincidence. When the price is low the same amount buys more units, so the cheaper units carry more weight automatically — and the average is pulled downward.',
          'That is the whole mechanism. There is no forecasting in it and no cleverness — only the fact that a fixed sum buys more when things are cheap.',
        ],
      },
      {
        hHi: 'यह कब मदद नहीं करता',
        hEn: 'When it does not help',
        pHi: [
          'ईमानदार तस्वीर के लिए यह बताना ज़रूरी है कि यह तरीक़ा हर स्थिति में बेहतर नहीं होता, और यह बात प्रायः छोड़ दी जाती है।',
          'सबसे स्पष्ट स्थिति वह है जब आपके पास पहले से एकमुश्त राशि मौजूद हो और बाज़ार लगातार ऊपर जाता रहे। ऐसी स्थिति में धीरे-धीरे लगाने से आप बढ़ते दामों पर ख़रीदते रहते हैं, और एक बार में लगा देना बेहतर रहता।',
          'पर यहाँ एक ज़रूरी बात है: यह केवल बाद में पता चलता है। पहले से यह जानना कि बाज़ार ऊपर जाएगा या नीचे, संभव नहीं — और इसीलिए धीरे-धीरे लगाना एक जोखिम-प्रबंधन का निर्णय है, अधिकतम प्रतिफल का नहीं।',
          'दूसरी स्थिति यह है कि यदि आपकी नियमित आय है और आप वैसे भी हर महीने ही निवेश कर सकते हैं, तो यह प्रश्न ही नहीं उठता। वहाँ यह तरीक़ा चुनाव नहीं, परिस्थिति है।',
          'इसलिए सही निष्कर्ष यह है: यह तरीक़ा प्रतिफल बढ़ाने की गारंटी नहीं देता। यह उस सबसे बुरी स्थिति की संभावना घटाता है जिसमें आपने पूरी राशि एक ही ऊँचे दिन लगा दी हो।',
        ],
        pEn: [
          'For an honest picture it has to be said that this method is not better in every situation, and that part is usually left out.',
          'The clearest case is having a lump sum already available while the market rises steadily. Investing gradually then means buying at rising prices, and putting it in at once would have been better.',
          'There is an important qualification though: that is only known afterwards. Knowing in advance whether the market will rise or fall is not possible — which is why investing gradually is a risk-management decision rather than a maximum-return one.',
          'The second case is that if you have regular income and can only invest monthly anyway, the question does not arise. There the method is a circumstance rather than a choice.',
          'So the correct conclusion is: this method does not guarantee a higher return. It reduces the chance of the worst case, in which the whole amount went in on a single expensive day.',
        ],
      },
      {
        hHi: 'एकमुश्त राशि हो तो STP का रास्ता',
        hEn: 'The STP route when you have a lump sum',
        pHi: [
          'यदि आपके पास एक बड़ी राशि आ गई है — बोनस, कोई बिक्री, कोई विरासत — तो एक व्यावहारिक बीच का रास्ता मौजूद है जिसके बारे में बहुत कम बात होती है।',
          'यह रास्ता व्यवस्थित हस्तांतरण योजना है। इसमें पूरी राशि पहले किसी कम जोखिम वाले फंड में रखी जाती है, और वहाँ से हर महीने एक तय राशि लक्ष्य फंड में स्वतः चली जाती है।',
          'इसका लाभ यह है कि पैसा बेकार पड़ा नहीं रहता — प्रतीक्षा की अवधि में भी वह कुछ कमाता है — और साथ ही प्रवेश समय पर फैल जाता है।',
          'अवधि कितनी रखनी चाहिए, इसका कोई नियम नहीं है। छह महीने से दो वर्ष तक का दायरा आमतौर पर उपयोग होता है, और वह इस पर निर्भर करता है कि राशि कितनी बड़ी है और आपको उतार-चढ़ाव कितना असहज करता है।',
          'यहाँ भी एक बात ध्यान रखने की है: हर हस्तांतरण को कर की दृष्टि से एक भुनाई माना जा सकता है। इसलिए बड़ी राशि पर यह रास्ता चुनने से पहले उस वर्ष के नियम देख लेना या सलाह ले लेना ज़रूरी है।',
        ],
        pEn: [
          'If a large amount has arrived — a bonus, a sale, an inheritance — there is a practical middle route that gets very little discussion.',
          'That route is a systematic transfer plan. The whole amount is first placed in a lower-risk fund, and from there a fixed sum moves automatically into the target fund each month.',
          'The benefit is that the money is not sitting idle — it earns something during the waiting period — while entry is still spread across time.',
          'There is no rule about how long the period should be. Six months to two years is the range commonly used, depending on how large the amount is and how uncomfortable movement makes you.',
          'One thing needs noting here too: each transfer may be treated as a redemption for tax purposes. So on a large amount, check the rules for that year or take advice before choosing this route.',
        ],
      },
      {
        hHi: 'अलग-अलग तरह के बाज़ारों में क्या होता है',
        hEn: 'What happens in different kinds of markets',
        pHi: [
          'यह समझना उपयोगी है कि यह तरीक़ा तीन अलग स्थितियों में कैसे व्यवहार करता है, क्योंकि इससे अपेक्षा यथार्थवादी बनती है।',
          'लगातार गिरते बाज़ार में यह सबसे अच्छा काम करता है — हर किश्त पिछली से सस्ती होती है, और जब सुधार आता है तो पूरी राशि पर लाभ मिलता है। यही वह स्थिति है जिसमें लोग सबसे अधिक घबराते हैं और सबसे अधिक चूकते हैं।',
          'उतार-चढ़ाव वाले, दिशाहीन बाज़ार में भी यह अच्छा काम करता है, क्योंकि औसत लागत बीच में बैठ जाती है और ऊँचे दिनों की ख़रीद का भार अपने आप कम रहता है।',
          'लगातार चढ़ते बाज़ार में यह सबसे कम प्रभावी है, जैसा पहले कहा गया। पर यहाँ भी यह नुक़सान नहीं करता — यह केवल उतना अच्छा नहीं करता जितना एकमुश्त निवेश करता।',
          'तीनों स्थितियों को साथ देखने पर एक बात साफ़ होती है: यह तरीक़ा किसी एक स्थिति के लिए अनुकूलित नहीं है। यह उस अनिश्चितता के लिए बनाया गया है कि आप नहीं जानते कि कौन-सी स्थिति आने वाली है।',
        ],
        pEn: [
          'It helps to understand how this method behaves in three different conditions, because that keeps expectations realistic.',
          'In a steadily falling market it works best — each instalment is cheaper than the last, and when recovery comes the gain applies to the whole amount. This is precisely the situation in which people panic most and miss out most.',
          'In a volatile, directionless market it also works well, because the average cost settles in the middle and purchases on expensive days carry less weight automatically.',
          'In a steadily rising market it is least effective, as noted earlier. Even there it does no harm — it simply does not do as well as a lump sum would have.',
          'Looking at all three together makes one thing clear: this method is not optimised for any single condition. It is built for the uncertainty of not knowing which condition is coming.',
        ],
      },
      {
        hHi: 'एक ग़लत उपयोग जो अक्सर देखा जाता है',
        hEn: 'One misuse that is seen often',
        pHi: [
          'इस अवधारणा का एक ऐसा प्रयोग है जो सुनने में इसी जैसा लगता है पर वास्तव में उलटा है, और उसे अलग करना ज़रूरी है।',
          'वह प्रयोग है किसी एक गिरते हुए शेयर में बार-बार पैसा डालते जाना, यह सोचकर कि "औसत नीचे आ रहा है"। यह रुपया-लागत औसत नहीं है; यह एक अलग चीज़ है।',
          'अंतर यह है कि विविध पोर्टफ़ोलियो पूरे बाज़ार का प्रतिनिधित्व करता है और उसका दीर्घकालिक सुधार अर्थव्यवस्था की वृद्धि पर टिका है। एक अकेली कंपनी का गिरना किसी वास्तविक और स्थायी समस्या के कारण हो सकता है, जिसका सुधार कभी न आए।',
          'औसत नीचे लाने से केवल यह होता है कि आपने उसी समस्या में और पैसा लगा दिया। यहाँ जो चीज़ मदद करती दिखती है, वह वास्तव में जोखिम बढ़ा रही है।',
          'इसलिए यह याद रखना ज़रूरी है: यह तरीक़ा एक विविध, नियम-आधारित निवेश पर लागू होता है, किसी एक चीज़ पर दृढ़ता दिखाने के लिए नहीं।',
        ],
        pEn: [
          'There is one use of this idea that sounds like the same thing and is actually its opposite, and it deserves separating out.',
          'That use is putting money repeatedly into a single falling stock on the reasoning that "the average is coming down". This is not rupee-cost averaging; it is something else.',
          'The difference is that a diversified portfolio represents the whole market and its long-term recovery rests on economic growth. A single company can be falling because of a real and permanent problem, from which no recovery arrives.',
          'Bringing the average down in that case only means putting more money into the same problem. What appears to be helping is in fact increasing the risk.',
          'So it is worth remembering: this method applies to a diversified, rule-based investment, not to persisting with one particular holding.',
        ],
      },
      {
        hHi: 'असली शक्ति गणित में नहीं, आदत में है',
        hEn: 'The real power is in the habit rather than the arithmetic',
        pHi: [
          'इस पूरे विषय पर बहुत गणित लिखा जाता है, और वह उपयोगी है — पर व्यवहार में जो चीज़ सबसे अधिक अंतर बनाती है वह गणितीय नहीं है।',
          'सबसे बड़ा लाभ यह है कि निर्णय हट जाता है। जिस व्यक्ति की किश्त स्वचालित है, उसे हर महीने यह नहीं सोचना पड़ता कि आज लगाऊँ या रुकूँ — और यही सोचना वह जगह है जहाँ अधिकांश नुक़सान होता है।',
          'दूसरा लाभ यह है कि निवेश एक घटना के बजाय एक आदत बन जाता है। घटनाएँ भुला दी जाती हैं; आदतें चलती रहती हैं, और बीस वर्षों में यही अंतर सबसे बड़ा निकलता है।',
          'तीसरा लाभ भावनात्मक है। जब बाज़ार गिरता है, तो जिस व्यक्ति की किश्त चल रही है वह उसे "और ख़रीदने का मौक़ा" के रूप में देख पाता है — और यह दृष्टिकोण अपने आप घबराहट कम कर देता है।',
          'इसलिए यदि इस विषय से केवल एक बात लेनी हो, तो वह गणित नहीं बल्कि यह है: व्यवस्था बना दीजिए और उसे चलने दीजिए। बाक़ी सब उसी से निकल आता है।',
        ],
        pEn: [
          'A great deal of arithmetic is written about this subject, and it is useful — but what makes the largest practical difference is not arithmetic at all.',
          'The biggest benefit is that the decision disappears. Somebody whose instalment is automatic does not have to think each month about whether to invest today or wait — and that thinking is where most of the damage happens.',
          'The second benefit is that investing becomes a habit rather than an event. Events get forgotten; habits keep running, and across twenty years that difference turns out to be the largest one.',
          'The third is emotional. When the market falls, somebody with an instalment running can see it as "a chance to buy more" — and that framing reduces panic on its own.',
          'So if only one thing is to be taken from this subject, it is not the arithmetic but this: set the system up and let it run. Everything else follows from that.',
        ],
      },
      {
        hHi: 'शुरू करने से पहले तीन जाँचें',
        hEn: 'Three checks before starting',
        pHi: [
          'यह तरीक़ा तभी काम करता है जब कुछ बुनियादी चीज़ें पहले से मौजूद हों, इसलिए शुरू करने से पहले तीन जाँचें उपयोगी हैं।',
          'पहली: क्या आपका आपातकालीन कोष बना हुआ है? यदि नहीं, तो पहली आपात स्थिति में किश्त रुक जाएगी, और रुकी हुई किश्त इस पूरे तंत्र को तोड़ देती है।',
          'दूसरी: क्या राशि ऐसी है जिसे आप बुरे महीनों में भी चला सकें? यह प्रश्न पहले भी आया है और यहाँ दोहराने लायक़ है, क्योंकि यही सबसे आम कारण है जिससे लोग रुकते हैं।',
          'तीसरी: क्या आपका समय-क्षितिज पर्याप्त लंबा है? यदि पैसा दो साल में चाहिए, तो यह तरीक़ा उपयुक्त साधन में लागू नहीं हो रहा — समस्या तरीक़े की नहीं, चुने हुए साधन की है।',
          'तीनों उत्तर हाँ हों, तो शुरू करने में देर करने का कोई कारण नहीं। और यदि कोई उत्तर नहीं है, तो पहले उसी को ठीक कीजिए — वह भी इसी योजना का हिस्सा है।',
        ],
        pEn: [
          'This method only works when a few basics are already in place, so three checks before starting are useful.',
          'First: is your emergency fund built? If not, the first emergency will stop the instalment, and a stopped instalment breaks the whole mechanism.',
          'Second: is the amount one you can sustain through bad months? The question has come up before and is worth repeating, because it is the most common reason people stop.',
          'Third: is your horizon long enough? If the money is needed in two years, the method is not being applied to a suitable instrument — the problem is the instrument chosen rather than the method.',
          'If all three answers are yes, there is no reason to delay starting. And if any answer is no, fix that first — that too is part of the same plan.',
        ],
      },
    ],
  },

  {
    slug: 'compounding-start-early-hindi',
    sections: [
      {
        hHi: 'दोगुना होने की अवधि: एक सरल गणना',
        hEn: 'Doubling time: one simple calculation',
        pHi: [
          'चक्रवृद्धि को महसूस करने का सबसे तेज़ तरीक़ा दर के बजाय दोगुना होने की अवधि में सोचना है, और उसके लिए एक सरल अनुमान मौजूद है।',
          'बहत्तर को अपनी वार्षिक दर से भाग दीजिए, और जो संख्या आए वह लगभग उतने वर्ष है जिनमें राशि दोगुनी हो जाएगी। छह प्रतिशत पर लगभग बारह वर्ष, नौ प्रतिशत पर लगभग आठ, बारह प्रतिशत पर लगभग छह।',
          'यह अनुमान सटीक नहीं है पर इतना क़रीब है कि मन में हिसाब लगाया जा सके, और यही उसकी असली उपयोगिता है।',
          'इसका सबसे उपयोगी प्रयोग यह देखना है कि आपके पास कितने "दोगुने" बचे हैं। तीस साल और आठ वर्ष की दोगुनी अवधि का अर्थ है लगभग पौने चार दोगुने — यानी राशि लगभग तेरह गुना।',
          'और यहीं यह भी दिखता है कि आख़िरी दोगुना सबसे बड़ा होता है। पहले दोगुने में एक इकाई जुड़ती है; चौथे में आठ जुड़ती हैं। यही कारण है कि अंतिम वर्ष सबसे अधिक देते हैं — और यही कारण है कि लोग बीच में हार मान जाते हैं।',
        ],
        pEn: [
          'The fastest way to feel compounding is to think in doubling periods rather than in rates, and a simple approximation exists for that.',
          'Divide seventy-two by your annual rate, and the answer is roughly the number of years in which the amount doubles. At six percent about twelve years, at nine about eight, at twelve about six.',
          'The approximation is not exact but it is close enough to work out in your head, and that is its real usefulness.',
          'Its most useful application is seeing how many doublings you have left. Thirty years with an eight-year doubling period means about three and three-quarter doublings — roughly thirteen times the amount.',
          'And this is where it also becomes visible that the last doubling is the largest. The first adds one unit; the fourth adds eight. That is why the final years give the most — and why people give up in the middle.',
        ],
      },
      {
        hHi: 'पाँच साल की देरी की क़ीमत',
        hEn: 'The price of a five-year delay',
        pHi: [
          'देरी की लागत को समझने का सबसे स्पष्ट तरीक़ा यह देखना है कि वह किस सिरे से कटती है।',
          'लोग यह मान लेते हैं कि पाँच साल देर से शुरू करने का अर्थ है पाँच साल की किश्तें छूट जाना। यह ग़लत है। असल में जो छूटता है वह अंत के पाँच वर्ष हैं — और वही सबसे मूल्यवान हैं।',
          'इसका कारण वही दोगुना होने वाला तर्क है। शुरुआत के वर्षों में राशि छोटी है, इसलिए उन वर्षों में जुड़ने वाली वृद्धि भी छोटी है। अंत के वर्षों में राशि बड़ी है, इसलिए उन्हीं वर्षों में सबसे बड़ी वृद्धि होती है।',
          'देर से शुरू करने पर आप शुरुआत नहीं खोते — आप अंत खोते हैं। यह वाक्य समझ में आ जाए तो यह पूरा विषय समझ में आ जाता है।',
          'और इसी से यह भी निकलता है कि छोटी राशि से आज शुरू करना बड़ी राशि से पाँच साल बाद शुरू करने से प्रायः बेहतर है। समय वह चीज़ है जिसे बाद में पैसे से नहीं ख़रीदा जा सकता।',
        ],
        pEn: [
          'The clearest way to understand the cost of delay is to see which end it is taken from.',
          'People assume that starting five years late means missing five years of instalments. That is wrong. What is actually lost is the last five years — and those are the most valuable ones.',
          'The reason is the same doubling logic. In the early years the amount is small, so the growth added in those years is small too. In the final years the amount is large, so the largest growth happens exactly there.',
          'Starting late does not cost you the beginning — it costs you the end. Once that sentence lands, the whole subject becomes clear.',
          'And it follows from this that starting today with a small amount is usually better than starting in five years with a large one. Time is the thing that cannot be bought back with money later.',
        ],
      },
      {
        hHi: 'चक्रवृद्धि उल्टी दिशा में: क़र्ज़',
        hEn: 'Compounding in reverse: debt',
        pHi: [
          'वही गणित जो आपके पक्ष में काम करता है, उधार में आपके विरुद्ध काम करता है — और उस दिशा में वह प्रायः तेज़ होता है।',
          'क्रेडिट कार्ड के बकाया पर लगने वाली दर आमतौर पर किसी भी अपेक्षित निवेश-प्रतिफल से कहीं ऊपर होती है, और वह मासिक रूप से जुड़ती है। इसका अर्थ है कि उसका दोगुना होने का समय बहुत छोटा है।',
          'यही कारण है कि किसी भी वित्तीय योजना में ऊँचे ब्याज वाला क़र्ज़ पहले आता है। उस क़र्ज़ को चुकाना एक निश्चित प्रतिफल है — उतना ही जितनी उसकी ब्याज दर है — और वह किसी भी अनिश्चित निवेश-प्रतिफल से अधिक भरोसेमंद है।',
          'इसका उल्टा भी सच है और उसे संतुलन के लिए कहना ज़रूरी है: कम ब्याज वाला दीर्घकालिक क़र्ज़, जैसे गृह ऋण, इस श्रेणी में नहीं आता। वहाँ गणना अलग है और उसे अलग से करना चाहिए।',
          'सार यह है कि चक्रवृद्धि एक शक्ति है, दिशा नहीं। वह जिस ओर लगी हो उसी ओर काम करती है — और यह तय करना कि वह किस ओर लगे, पूरी तरह आपके हाथ में है।',
        ],
        pEn: [
          'The same arithmetic that works for you works against you in borrowing — and in that direction it usually works faster.',
          'The rate charged on a credit card balance is generally well above any expected investment return, and it is applied monthly. That means its doubling time is very short.',
          'This is why high-interest debt comes first in any financial plan. Repaying it is a certain return — exactly equal to its interest rate — and that is more reliable than any uncertain investment return.',
          'The reverse is also true and deserves saying for balance: low-rate long-term borrowing such as a home loan does not belong in this category. The calculation there is different and should be done separately.',
          'The point is that compounding is a force rather than a direction. It works whichever way it is pointed — and deciding which way it points is entirely in your hands.',
        ],
      },
      {
        hHi: 'मुद्रास्फीति: चुपचाप चलने वाली उल्टी चक्रवृद्धि',
        hEn: 'Inflation: the quiet compounding that runs the other way',
        pHi: [
          'एक तीसरी दिशा भी है जिसकी चर्चा कम होती है, और वह हर किसी पर लागू होती है — चाहे वह निवेश करे या न करे।',
          'मुद्रास्फीति भी चक्रवृद्धि है, बस उल्टी ओर। यदि क़ीमतें हर वर्ष एक निश्चित दर से बढ़ती हैं, तो आपके पैसे की क्रय-शक्ति उसी दर से हर वर्ष घटती है — और वह घटाव भी जुड़ता जाता है।',
          'इसी वही "बहत्तर वाला" नियम यहाँ भी लागू होता है। छह प्रतिशत की मुद्रास्फीति पर आपके पैसे की क्रय-शक्ति लगभग बारह वर्षों में आधी रह जाती है, भले ही खाते में संख्या वही दिख रही हो।',
          'यही कारण है कि "पैसा बैंक में सुरक्षित रखा है" वाला संतोष अधूरा है। वह राशि सुरक्षित है, पर वह जो ख़रीद सकती है वह लगातार घट रही है।',
          'इसलिए निवेश का पहला उद्देश्य अमीर बनना नहीं है। पहला उद्देश्य केवल इस उल्टी चक्रवृद्धि से आगे रहना है — और उसके बाद जो कुछ मिलता है, वह असली वृद्धि है।',
        ],
        pEn: [
          'There is a third direction that gets less discussion, and it applies to everybody — whether they invest or not.',
          'Inflation is also compounding, simply pointed the other way. If prices rise at a certain rate each year, the purchasing power of your money falls at that rate each year — and that reduction accumulates too.',
          'The same "seventy-two" rule applies here. At six percent inflation the purchasing power of your money halves in roughly twelve years, even though the number in the account has not changed.',
          'This is why the comfort of "the money is safe in the bank" is incomplete. The amount is safe, and what it can buy is steadily shrinking.',
          'So the first purpose of investing is not becoming wealthy. The first purpose is merely staying ahead of this reverse compounding — and whatever comes after that is real growth.',
        ],
      },
      {
        hHi: 'निकासी: वह जगह जहाँ लोग चक्र तोड़ देते हैं',
        hEn: 'Withdrawal: where people break the chain',
        pHi: [
          'चक्रवृद्धि का सबसे नाज़ुक हिस्सा उसकी निरंतरता है, और वह निरंतरता प्रायः किसी बड़ी गिरावट से नहीं बल्कि एक साधारण निकासी से टूटती है।',
          'हर बार जब आप बीच में से पैसा निकालते हैं, तो आप केवल वह राशि नहीं निकालते — आप उस राशि की आगे की सारी वृद्धि भी निकाल देते हैं। और चूँकि वृद्धि अंत में सबसे बड़ी होती है, इसलिए यह लागत बाद में दिखती है।',
          'यही कारण है कि आपातकालीन कोष और दीर्घकालिक निवेश को अलग रखना इतना महत्वपूर्ण है। कोष का पूरा काम ही यह है कि वह निवेश को छूने से बचाए।',
          'एक दूसरी और अधिक चुपचाप होने वाली टूट है: बार-बार बदलना। हर बार जब आप एक साधन से दूसरे में जाते हैं, तो लागत, कर और समय — तीनों लगते हैं, और शृंखला फिर से शुरू होती है।',
          'इसलिए चक्रवृद्धि का असली रहस्य कोई विशेष साधन नहीं है। रहस्य यह है कि उसे बिना रुकावट के बहुत लंबे समय तक चलने दिया जाए, और यही सबसे कठिन हिस्सा है।',
        ],
        pEn: [
          'The most fragile part of compounding is its continuity, and that continuity usually breaks not through a large fall but through an ordinary withdrawal.',
          'Every time you take money out partway, you remove more than the amount — you remove all the growth that amount would have produced afterwards. And because growth is largest at the end, the cost shows up later.',
          'This is why keeping the emergency fund and long-term investments separate matters so much. The entire job of the fund is to keep the investments from being touched.',
          'There is a second, quieter break: switching repeatedly. Each move from one instrument to another costs money, tax and time — and the chain starts again from the beginning.',
          'So the real secret of compounding is not any particular instrument. The secret is letting it run uninterrupted for a very long time, and that is the hardest part.',
        ],
      },
      {
        hHi: 'यदि आपकी उम्र निकल चुकी है',
        hEn: 'If your own early years have already passed',
        pHi: [
          'यह लेख जल्दी शुरू करने के बारे में है, और उसका एक अनचाहा असर यह हो सकता है कि जिनकी उम्र निकल चुकी है वे हतोत्साहित हो जाएँ। यह निष्कर्ष ग़लत है।',
          'पहली बात: आज सबसे जल्दी है। कल की तुलना में आज ही शुरुआत का सबसे अच्छा दिन है, और यह बात हर उम्र पर समान रूप से लागू होती है।',
          'दूसरी बात: देर से शुरू करने वालों के पास प्रायः वह चीज़ अधिक होती है जो युवा लोगों के पास कम होती है — आय। तीस की उम्र में हज़ार रुपये और पैंतालीस की उम्र में दस हज़ार, दोनों अलग रास्ते हैं जो एक ही दिशा में जाते हैं।',
          'तीसरी बात: यदि आपके बच्चे हैं, तो आप उनके लिए वह कर सकते हैं जो आपके लिए नहीं हुआ। उनके नाम पर बहुत छोटी राशि से शुरू किया गया निवेश उन्हें वह दशक दे देता है जो सबसे मूल्यवान है।',
          'और चौथी, जो सबसे व्यावहारिक है: देर से शुरू करने पर लक्ष्य अधिक स्पष्ट और अवधि अधिक निश्चित होती है, इसलिए योजना प्रायः बेहतर बनती है। समय कम है, पर स्पष्टता अधिक — और यह कोई छोटा लाभ नहीं है।',
        ],
        pEn: [
          'This article is about starting early, and an unintended effect of that can be discouraging people whose early years have gone. That conclusion is wrong.',
          'First: today is the earliest available. Compared with tomorrow, today is the best day to start, and this holds equally at every age.',
          'Second: people starting late usually have more of the thing young people have less of — income. A thousand rupees at thirty and ten thousand at forty-five are two different routes travelling in the same direction.',
          'Third: if you have children, you can do for them what was not done for you. A very small amount started in their name gives them the decade that matters most.',
          'And fourth, the most practical: starting late usually comes with clearer goals and a more definite horizon, so the plan tends to be better made. Less time, more clarity — and that is not a small advantage.',
        ],
      },
      {
        hHi: 'इसे बच्चों को कैसे दिखाएँ',
        hEn: 'How to show this to children',
        pHi: [
          'चक्रवृद्धि को शब्दों में समझाना कठिन है और दिखाकर समझाना आसान — और बचपन में देखी हुई यह एक चीज़ पूरे जीवन काम आती है।',
          'सबसे अच्छा तरीक़ा एक काग़ज़ है। एक कॉलम में हर महीने जमा की गई राशि, दूसरे में कुल राशि। कुछ महीनों बाद दूसरा कॉलम पहले से तेज़ बढ़ने लगता है, और वह अंतर आँखों से दिखता है।',
          'दूसरा तरीक़ा और भी सरल है, और छोटे बच्चों के लिए बेहतर: हर हफ़्ते बचाई गई राशि पर आप ख़ुद एक छोटा "ब्याज" जोड़ दीजिए। जब बच्चा देखता है कि बिना कुछ किए राशि बढ़ रही है, तो अवधारणा बिना किसी व्याख्या के पहुँच जाती है।',
          'तीसरा, किशोरों के लिए: उन्हें ख़ुद बहत्तर वाला नियम बताइए और अपनी संख्या डालकर गणना करने दीजिए। ख़ुद निकाला गया उत्तर बताए गए उत्तर से कहीं अधिक टिकता है।',
          'और चौथा, जो सबसे महत्वपूर्ण है: उन्हें यह भी दिखाइए कि यही गणित क़र्ज़ पर उल्टा चलता है। जो बच्चा दोनों दिशाएँ देख चुका है, वह अपने पहले क्रेडिट कार्ड के सामने बहुत बेहतर तैयार खड़ा होगा।',
        ],
        pEn: [
          'Compounding is hard to explain in words and easy to show — and this one thing seen in childhood keeps working for a lifetime.',
          'The best method is a sheet of paper. One column for the amount saved each month, another for the total. After a few months the second column starts growing faster than the first, and the gap is visible to the eye.',
          'The second method is simpler still and better for younger children: add a small "interest" of your own to whatever they saved each week. When a child sees the amount growing without doing anything, the concept arrives without explanation.',
          'Third, for teenagers: give them the seventy-two rule and let them work out the calculation with their own numbers. An answer worked out oneself lasts far longer than one that was told.',
          'And fourth, the most important: show them that the same arithmetic runs in reverse on debt. A child who has seen both directions stands much better prepared in front of their first credit card.',
        ],
      },
    ],
  },
  {
    slug: 'recurring-deposit-guide',
    sections: [
      {
        hHi: 'RD और FD में असली फ़र्क़',
        hEn: 'The real difference between an RD and an FD',
        pHi: [
          'दोनों बैंक में जमा हैं, दोनों की ब्याज दर पहले से तय होती है, और दोनों में बाज़ार का उतार-चढ़ाव नहीं आता। इसलिए बहुत से लोग मान लेते हैं कि दोनों एक ही चीज़ हैं। फ़र्क़ पैसे में नहीं, समय में है।',
          'FD में पूरी राशि पहले दिन जमा होती है और पूरी अवधि तक ब्याज कमाती है। RD में राशि हर महीने थोड़ी-थोड़ी जाती है, इसलिए पहली किश्त पूरी अवधि तक ब्याज कमाती है, दूसरी उससे एक महीना कम, और आख़िरी किश्त सिर्फ़ एक महीना।',
          'इसका सीधा नतीजा यह है कि एक ही दर पर, एक ही कुल राशि पर, RD का ब्याज FD के ब्याज से कम बैठता है। यह बैंक की चालाकी नहीं है, यह गणित है। जिसने यह समझ लिया, वह RD के नतीजे देखकर निराश नहीं होता।',
          'तो फिर RD क्यों? क्योंकि FD के लिए आपके पास पहले दिन पूरी राशि होनी चाहिए। अगर वह होती, तो सवाल ही नहीं उठता। RD उनके लिए है जिनके पास एकमुश्त राशि नहीं है लेकिन हर महीने एक निश्चित रक़म बचाने की क्षमता है।',
          'व्यावहारिक तरीक़ा यह है कि दोनों को साथ चलाया जाए। RD से राशि इकट्ठी कीजिए, और परिपक्व होने पर उसे FD में डाल दीजिए। RD आदत बनाता है, FD उस आदत के नतीजे को सँभालता है।',
        ],
        pEn: [
          'Both are bank deposits, both carry a rate fixed in advance, and neither moves with the market. So many people assume they are the same thing. The difference is not in the money — it is in the timing.',
          'In an FD the whole amount goes in on day one and earns interest for the full term. In an RD the money goes in a little each month, so the first instalment earns for the full term, the second earns one month less, and the last instalment earns for a single month.',
          'The direct consequence is that at the same rate, on the same total amount, an RD produces less interest than an FD. This is not a trick by the bank; it is arithmetic. Anyone who understands it is not disappointed when the maturity figure arrives.',
          'So why an RD at all? Because an FD needs the whole amount on day one. If you had that, the question would not arise. An RD is for people who do not have a lump sum but do have the capacity to set aside a fixed amount every month.',
          'The practical approach is to run both. Accumulate through an RD, and on maturity move the proceeds into an FD. The RD builds the habit; the FD looks after what the habit produced.',
        ],
      },
      {
        hHi: 'समय से पहले तोड़ने पर क्या होता है',
        hEn: 'What happens if you break it early',
        pHi: [
          'RD खोलते समय यह सवाल कोई नहीं पूछता, और बाद में यही सबसे ज़्यादा पूछा जाता है। ज़्यादातर बैंकों में RD समय से पहले बंद की जा सकती है, लेकिन उस पर एक जुर्माना लगता है।',
          'जुर्माना आमतौर पर ब्याज दर में कटौती के रूप में आता है। यानी आपको वह दर नहीं मिलती जो खाता खोलते समय तय हुई थी, बल्कि उस अवधि के लिए लागू दर में से आधा या एक प्रतिशत घटाकर दिया जाता है। मूलधन आमतौर पर सुरक्षित रहता है।',
          'दूसरी बात जो कम लोग जानते हैं: किश्त चूकने पर भी छोटा शुल्क लग सकता है। लगातार कई किश्तें चूक जाएँ तो कुछ बैंक खाता ही बंद कर देते हैं और जमा राशि बचत खाते में लौटा देते हैं।',
          'इसलिए किश्त की राशि तय करते समय उदार होने के बजाय सावधान रहना बेहतर है। जो व्यक्ति पाँच हज़ार की RD चलाकर बीच में तोड़ देता है, उससे बेहतर वह है जो तीन हज़ार की RD पूरी अवधि चलाता है।',
          'और अगर बीच में तोड़ना ही पड़े, तो यह विफलता नहीं है। RD का काम पैसा जमा करना था, और वह जमा हुआ। जुर्माना उस सुविधा की क़ीमत है, और वह क़ीमत क़र्ज़ लेने से लगभग हमेशा कम पड़ती है।',
        ],
        pEn: [
          'Nobody asks this question while opening an RD, and afterwards it is the most asked question of all. At most banks an RD can be closed early, but there is a penalty for it.',
          'The penalty usually arrives as a cut in the interest rate. You do not get the rate agreed when the account opened; instead you get the rate applicable to the period actually completed, minus something like half or one per cent. The principal is normally safe.',
          'A second point fewer people know: missing an instalment can attract a small charge of its own. Miss several in a row and some banks will close the account and return the balance to your savings account.',
          'So when fixing the instalment amount, caution beats ambition. Somebody who runs a five-thousand RD and breaks it midway is worse off than somebody who runs a three-thousand RD to full term.',
          'And if you do have to break it, that is not a failure. The RD’s job was to gather money, and the money was gathered. The penalty is the price of that flexibility, and that price is almost always lower than the cost of borrowing instead.',
        ],
      },
      {
        hHi: 'टैक्स का हिस्सा, जिसे लोग भूल जाते हैं',
        hEn: 'The tax side, which people forget',
        pHi: [
          'RD पर मिलने वाला ब्याज कर-मुक्त नहीं है। वह आपकी कुल आय में जुड़ता है और आपके स्लैब के हिसाब से कर लगता है। यह बात बहुत से लोगों को परिपक्वता के बाद पता चलती है, जो सबसे ग़लत समय है।',
          'इसके अलावा, अगर एक वित्तीय वर्ष में एक ही बैंक से मिलने वाला ब्याज एक तय सीमा से ऊपर चला जाए तो बैंक स्रोत पर कर काट लेता है। कटा हुआ कर आपके फ़ॉर्म 26AS में दिखता है और रिटर्न भरते समय समायोजित हो जाता है।',
          'यहाँ एक आम ग़लतफ़हमी है: लोग सोचते हैं कि अगर बैंक ने कर काट लिया तो मामला ख़त्म। ऐसा नहीं है। बैंक एक निश्चित दर पर काटता है; अगर आपका स्लैब उससे ऊँचा है तो बाक़ी कर आपको ख़ुद भरना होगा।',
          'दूसरी तरफ़, अगर आपकी कुल आय कर योग्य सीमा से नीचे है, तो साल की शुरुआत में उपयुक्त घोषणा-पत्र जमा करके कटौती रुकवाई जा सकती है। यह काम अप्रैल में करना होता है, मार्च में नहीं।',
          'सबसे व्यावहारिक आदत यह है कि ब्याज को साल-दर-साल आय मानकर चलिए, चाहे वह हाथ में आया हो या नहीं। इससे परिपक्वता के साल में अचानक बड़ा कर-बोझ नहीं पड़ता।',
        ],
        pEn: [
          'Interest from an RD is not tax-free. It is added to your total income and taxed at your slab. Many people discover this only after maturity, which is the worst possible moment.',
          'On top of that, if the interest from a single bank crosses a set threshold in a financial year, the bank deducts tax at source. The deducted amount shows up in your Form 26AS and is adjusted when you file your return.',
          'There is a common misunderstanding here: people think that if the bank has deducted tax, the matter is closed. It is not. The bank deducts at a fixed rate; if your own slab is higher, the balance is still yours to pay.',
          'On the other side, if your total income is below the taxable limit, the deduction can be stopped by filing the appropriate declaration at the start of the year. That is an April job, not a March one.',
          'The most practical habit is to treat the interest as income year by year, whether or not it has reached your hands. That way the maturity year does not arrive with a sudden lump of tax attached.',
        ],
      },
      {
        hHi: 'RD किसके लिए सबसे उपयुक्त है',
        hEn: 'Who an RD suits best',
        pHi: [
          'RD का सबसे अच्छा उपयोग एक से तीन साल के भीतर आने वाले निश्चित ख़र्च के लिए है — फ़ीस, बीमा का सालाना प्रीमियम, कोई नियोजित यात्रा, या घर का कोई काम जिसकी तारीख़ पहले से पता है।',
          'यह उस व्यक्ति के लिए भी उपयुक्त है जो पहली बार बचत शुरू कर रहा है। बाज़ार से जुड़े विकल्प में शुरुआती गिरावट देखकर बहुत से नए बचतकर्ता हमेशा के लिए डर जाते हैं। RD में वह डर नहीं आता, और आदत बन जाती है।',
          'यह उनके लिए भी अच्छा है जिनकी आय निश्चित है लेकिन अनुशासन कमज़ोर है। किश्त खाते से अपने-आप कट जाती है, और चूकने पर शुल्क लगता है — यह हल्का दबाव कई लोगों के लिए ठीक उतना ही होता है जितना चाहिए।',
          'यह किसके लिए उपयुक्त नहीं है? दस-पंद्रह साल के लक्ष्य के लिए। इतनी लंबी अवधि में महँगाई ब्याज को खा जाती है, और उस काम के लिए बाज़ार से जुड़े विकल्प ऐतिहासिक रूप से बेहतर रहे हैं, हालाँकि उनमें उतार-चढ़ाव सहना पड़ता है।',
          'और यह आपातकालीन कोष का विकल्प भी नहीं है। आपात राशि ऐसी जगह होनी चाहिए जहाँ से वह उसी दिन निकल सके, बिना जुर्माने के।',
        ],
        pEn: [
          'An RD is at its best for a definite expense arriving within one to three years — school fees, an annual insurance premium, a planned journey, or a piece of household work whose date is already known.',
          'It also suits somebody saving for the first time. In a market-linked option, an early fall frightens many new savers away permanently. That fright does not arrive in an RD, and the habit gets a chance to form.',
          'It suits people with steady income and weak discipline too. The instalment leaves the account by itself, and missing it carries a charge — that gentle pressure is exactly the right amount of pressure for a lot of people.',
          'Who is it not for? Ten- and fifteen-year goals. Over that length of time inflation eats the interest, and for that job market-linked options have historically done better, though they demand that you sit through the swings.',
          'It is also not a substitute for an emergency fund. Emergency money has to sit somewhere it can leave the same day, without a penalty attached.',
        ],
      },
      {
        hHi: 'RD चलाते समय होने वाली आम ग़लतियाँ',
        hEn: 'Common mistakes while running an RD',
        pHi: [
          'पहली ग़लती: किश्त इतनी बड़ी रखना कि वह महीने के आख़िरी हफ़्ते में तकलीफ़ दे। बचत जो तकलीफ़ देती है, वह टिकती नहीं। जो राशि आराम से चल जाए, वही असली राशि है।',
          'दूसरी: किश्त की तारीख़ वेतन आने से बहुत बाद रखना। महीने की पच्चीस तारीख़ को कटने वाली किश्त हर महीने एक छोटी लड़ाई बन जाती है। वेतन आने के अगले दिन कटने वाली किश्त कभी लड़ाई नहीं बनती।',
          'तीसरी: परिपक्वता के बाद राशि को बचत खाते में छोड़ देना। यह सबसे महँगी ग़लती है, क्योंकि जिस अनुशासन से पैसा जमा हुआ, वह बिना योजना के कुछ ही हफ़्तों में ख़र्च हो जाता है।',
          'चौथी: एक ही समय पर तीन-चार RD चलाना। यह अनुशासित लगता है, पर असल में हर एक की किश्त छोटी होती है और मिलाकर वही बोझ बनता है, बस हिसाब रखना मुश्किल हो जाता है। एक RD, सही राशि पर, बेहतर है।',
          'पाँचवीं: दर की तुलना किए बिना खाता खोल लेना। अलग-अलग बैंकों और डाकघर की दरें अलग होती हैं, और छोटे अंतर भी लंबी अवधि में दिखते हैं। दस मिनट की तुलना पूरे साल काम आती है।',
        ],
        pEn: [
          'First mistake: setting an instalment so large that it hurts in the last week of the month. Saving that hurts does not survive. The amount that passes comfortably is the real amount.',
          'Second: setting the instalment date long after payday. A deduction on the twenty-fifth becomes a small monthly battle. A deduction the day after salary arrives never becomes a battle at all.',
          'Third: leaving the maturity amount sitting in the savings account. This is the most expensive mistake of all, because money gathered with discipline gets spent without a plan in a matter of weeks.',
          'Fourth: running three or four RDs at once. It feels disciplined, but each instalment is small, the combined burden is the same, and the bookkeeping gets harder. One RD at the right amount is better.',
          'Fifth: opening the account without comparing rates. Banks and the post office differ, and small gaps still show up over a term. Ten minutes of comparison pays for the whole year.',
        ],
      },
    ],
  },
  {
    slug: 'automate-your-savings-india',
    sections: [
      {
        hHi: 'ऑटोमेशन कहाँ टूटता है',
        hEn: 'Where automation breaks',
        pHi: [
          'स्वचालित बचत की सबसे बड़ी ताक़त यह है कि उसमें हर महीने फ़ैसला नहीं लेना पड़ता। और उसकी सबसे बड़ी कमज़ोरी भी यही है — क्योंकि जब कोई फ़ैसला नहीं लिया जाता, तो कोई जाँच भी नहीं होती।',
          'सबसे आम टूटन खाते में पर्याप्त शेष न होने से आती है। निर्देश असफल होता है, बैंक शुल्क लगाता है, और उस महीने की बचत नहीं होती। दो-तीन बार यह हो जाए तो लोग निर्देश ही बंद कर देते हैं।',
          'दूसरी टूटन कार्ड या खाता बदलने पर आती है। पुराना ऑटो-डेबिट चुपचाप विफल होने लगता है और कई महीनों तक किसी को पता नहीं चलता, क्योंकि विफलता की सूचना उस पुराने नंबर पर जाती है जो अब इस्तेमाल में नहीं है।',
          'तीसरी, और सबसे सूक्ष्म: वेतन बढ़ता रहता है और बचत की राशि वही रहती है। ऑटोमेशन ईमानदारी से वही करता रहता है जो पाँच साल पहले कहा गया था, और पाँच साल पहले की राशि आज छोटी पड़ चुकी होती है।',
          'इसलिए ऑटोमेशन "लगाकर भूल जाना" नहीं है। यह "लगाकर साल में दो बार देख लेना" है। जुलाई और जनवरी, दस मिनट — इतना काफ़ी है।',
        ],
        pEn: [
          'The great strength of automated saving is that no decision has to be taken each month. And that is also its great weakness — because where no decision is taken, no check is made either.',
          'The commonest break is an insufficient balance. The instruction fails, the bank levies a charge, and that month’s saving does not happen. After two or three of those, people cancel the instruction altogether.',
          'The second break comes when a card or an account changes. The old auto-debit quietly starts failing and nobody notices for months, because the failure notice goes to a number that is no longer in use.',
          'The third, and the subtlest: the salary keeps rising and the saving amount does not. Automation honestly keeps doing what it was told five years ago, and what was told five years ago has become small.',
          'So automation is not "set it and forget it". It is "set it and look at it twice a year". July and January, ten minutes each — that is enough.',
        ],
      },
      {
        hHi: 'तारीख़ का चुनाव, जो सब कुछ बदल देता है',
        hEn: 'Choosing the date, which changes everything',
        pHi: [
          'यह छोटा-सा विवरण लगता है और असल में सबसे बड़ा है। बचत का निर्देश वेतन आने के एक या दो दिन बाद होना चाहिए — तब जब खाते में सबसे ज़्यादा पैसा हो और सबसे कम ख़र्च हुआ हो।',
          'जो लोग बचत की तारीख़ महीने के मध्य या अंत में रखते हैं, वे असल में यह कह रहे होते हैं कि "जो बचेगा वह बचाऊँगा"। और जो बचेगा, वह लगभग हमेशा उम्मीद से कम होता है।',
          'अगर आय महीने में दो बार आती है, तो निर्देश भी दो हिस्सों में बाँटा जा सकता है। इससे एक बार में बड़ा झटका नहीं लगता और खाते में शेष की समस्या भी कम होती है।',
          'अगर वेतन की तारीख़ पक्की नहीं है — जैसा कई छोटे संस्थानों में होता है — तो निर्देश को औसत तारीख़ के तीन दिन बाद रखिए, और खाते में एक छोटा बफ़र हमेशा छोड़िए ताकि एक दिन की देरी निर्देश को विफल न कर दे।',
          'और एक व्यावहारिक बात: बिल की तारीख़ें और बचत की तारीख़ एक ही दिन न रखिए। सब कुछ एक साथ कटने पर खाता एक दिन के लिए ख़ाली दिखता है, और उसी दिन कोई अप्रत्याशित भुगतान आ जाए तो सब गड़बड़ा जाता है।',
        ],
        pEn: [
          'This looks like a small detail and is in fact the biggest one. The saving instruction should fall one or two days after payday — when the account holds the most and has spent the least.',
          'People who put the saving date in the middle or at the end of the month are really saying "I will save what is left". And what is left is almost always less than expected.',
          'If income arrives twice a month, the instruction can be split in two as well. That avoids one large jolt and reduces the balance problem at the same time.',
          'If the salary date is not firm — as happens at many smaller employers — set the instruction three days after the usual date and keep a small buffer in the account, so that a day’s delay does not fail the mandate.',
          'And one practical point: do not put the bill dates and the saving date on the same day. When everything leaves together the account looks empty for a day, and an unexpected payment on that day upsets the whole arrangement.',
        ],
      },
      {
        hHi: 'एक खाता या कई खाते',
        hEn: 'One account or several',
        pHi: [
          'एक लोकप्रिय सलाह यह है कि हर लक्ष्य के लिए अलग खाता खोला जाए। यह सुनने में साफ़-सुथरा लगता है और कुछ लोगों के लिए वाक़ई काम करता है, पर सबके लिए नहीं।',
          'अलग खातों का फ़ायदा दृश्यता है। जब यात्रा का खाता अलग है, तो उसमें से पैसा निकालते समय एक क्षण रुकना पड़ता है, और वह क्षण अक्सर ख़र्च रोक देता है।',
          'नुक़सान यह है कि हर खाते का न्यूनतम शेष, हर खाते की जाँच, और हर खाते का विवरण — यह सब मिलकर एक ऐसा बोझ बना देता है जिसे लोग कुछ महीनों बाद छोड़ देते हैं।',
          'बीच का रास्ता आमतौर पर बेहतर है: एक बचत खाता, और उसके भीतर अलग-अलग जमा या फ़ंड जिनके नाम लक्ष्य के हिसाब से हों। नाम अलग होने से वही मानसिक दीवार बन जाती है, बिना कई खातों के झंझट के।',
          'जो भी ढाँचा चुनें, एक नियम स्थिर रखिए: रोज़मर्रा का ख़र्च और बचत कभी एक ही खाते में न रहें। यह अकेला नियम बाक़ी सारी जटिलता से ज़्यादा काम करता है।',
        ],
        pEn: [
          'A popular piece of advice is to open a separate account for every goal. It sounds tidy, and for some people it genuinely works — but not for everyone.',
          'The benefit of separate accounts is visibility. When the travel account is its own account, taking money out of it requires a moment’s pause, and that pause often stops the spending.',
          'The cost is that every account carries a minimum balance, a statement to check and a login to remember, and together these become a burden people abandon after a few months.',
          'The middle path is usually better: one savings account, and within it separate deposits or funds named after the goals. The naming builds the same mental wall without the overhead of many accounts.',
          'Whichever structure you choose, hold one rule steady: everyday spending and savings should never live in the same account. That single rule does more work than all the rest of the complexity.',
        ],
      },
      {
        hHi: 'सालाना बढ़ोतरी, जो सबसे ज़्यादा फ़र्क़ लाती है',
        hEn: 'The annual step-up, which makes the biggest difference',
        pHi: [
          'अगर इस पूरे लेख से केवल एक चीज़ याद रखनी हो, तो यह हो: हर साल बचत की राशि बढ़ाइए, चाहे थोड़ी ही सही। वेतन बढ़ने पर बचत का न बढ़ना ही वह जगह है जहाँ अधिकांश लोगों की योजना चुपचाप रुक जाती है।',
          'सबसे आसान नियम यह है कि वेतन बढ़ने पर बढ़ी हुई राशि का आधा हिस्सा बचत में जोड़ दीजिए और आधा जीवनशैली में। इससे न तो जीवन कसा हुआ लगता है और न बचत पिछड़ती है।',
          'यह काम बढ़ोतरी मिलने के उसी हफ़्ते कर लेना चाहिए। जैसे ही बढ़ी हुई राशि कुछ महीने हाथ में आ जाती है, वह "सामान्य" बन जाती है और फिर उसे बचत में डालना कटौती जैसा महसूस होता है।',
          'कई जगहों पर यह सुविधा स्वचालित रूप से उपलब्ध होती है — निर्देश में ही हर साल एक तय प्रतिशत की बढ़ोतरी दर्ज की जा सकती है। जहाँ यह उपलब्ध हो, वहाँ इसका उपयोग कीजिए, क्योंकि तब यह याद रखने का काम नहीं रहता।',
          'और जिस साल बढ़ोतरी न मिले, उस साल राशि वही रहने दीजिए। इसे रोकना विफलता नहीं है; योजना का मक़सद हर हाल में बढ़ना नहीं, बल्कि कभी पीछे न जाना है।',
        ],
        pEn: [
          'If only one thing is to be carried away from this article, let it be this: raise the saving amount every year, even slightly. A salary that rises while the saving stays flat is exactly where most plans quietly stall.',
          'The easiest rule is to put half of any raise into saving and half into living. That way life does not feel squeezed and the saving does not fall behind.',
          'Do it in the same week the raise arrives. Once the extra money has been in hand for a few months it becomes "normal", and moving it into saving then feels like a cut.',
          'In many places this can be automated — the mandate itself can carry a fixed annual increase. Where that facility exists, use it, because then it stops being something you have to remember.',
          'And in a year with no raise, let the amount stand. Pausing is not failure; the point of the plan is never to go backwards, not to rise no matter what.',
        ],
      },
      {
        hHi: 'जब आय अनियमित हो',
        hEn: 'When income is irregular',
        pHi: [
          'स्वचालित बचत की सारी सलाह उस व्यक्ति को मानकर लिखी जाती है जिसे हर महीने एक ही तारीख़ को एक ही राशि मिलती है। दुकानदार, ठेके पर काम करने वाले, किसान और स्वतंत्र पेशेवर उस श्रेणी में नहीं आते।',
          'ऐसी स्थिति में पहला क़दम यह है कि पिछले बारह महीनों की आय देखकर सबसे कम वाला महीना निकालिए। स्वचालित बचत उसी सबसे कम महीने के हिसाब से तय कीजिए, न कि औसत के हिसाब से।',
          'यह राशि छोटी लगेगी, और लगनी भी चाहिए। इसका काम अच्छे महीनों में ज़्यादा बचाना नहीं है; इसका काम बुरे महीनों में भी बचत का सिलसिला न टूटने देना है।',
          'अच्छे महीनों के लिए एक अलग, हाथ से किया जाने वाला नियम रखिए: जो भी अतिरिक्त आया, उसका एक तय प्रतिशत उसी हफ़्ते बचत में डाल दीजिए। यह स्वचालित नहीं होगा, पर नियम स्पष्ट होने से फ़ैसला हर बार नया नहीं लेना पड़ता।',
          'और अनियमित आय वालों के लिए आपातकालीन कोष तीन महीने का नहीं, छह से नौ महीने का होना चाहिए। यह उनके लिए विलासिता नहीं, बुनियादी ढाँचा है।',
        ],
        pEn: [
          'All the advice about automated saving is written for somebody who receives the same amount on the same date every month. Shopkeepers, contract workers, farmers and freelancers are not in that category.',
          'For them the first step is to look back over twelve months of income and find the worst month. Set the automated saving against that worst month, not against the average.',
          'The amount will look small, and it should. Its job is not to save a lot in the good months; its job is to keep the sequence unbroken in the bad ones.',
          'For the good months keep a separate, manual rule: whatever came in above the usual, move a fixed percentage of it into savings the same week. It will not be automatic, but a clear rule means the decision is not made afresh each time.',
          'And for irregular earners the emergency fund should hold six to nine months, not three. For them that is not a luxury; it is basic infrastructure.',
        ],
      },
    ],
  },
  {
    slug: 'sinking-fund-for-big-expenses',
    sections: [
      {
        hHi: 'साल भर की सूची कैसे बनाएँ',
        hEn: 'Building the year’s list',
        pHi: [
          'सिंकिंग फ़ंड की सारी ताक़त सूची में है, गणित में नहीं। जिस दिन आप बैठकर अगले बारह महीनों के बड़े ख़र्च लिख लेते हैं, उसी दिन आधा काम हो जाता है।',
          'शुरुआत पिछले साल के बैंक विवरण से कीजिए, याददाश्त से नहीं। याददाश्त छोटे-छोटे बड़े ख़र्च भूल जाती है — गाड़ी का बीमा, स्कूल की किताबें, त्योहार, कोई शादी, दाँत का इलाज, फ़ोन का बदलना।',
          'हर मद के सामने तीन चीज़ें लिखिए: अनुमानित राशि, अनुमानित महीना, और यह कि राशि पक्की है या अंदाज़ा। पक्की और अंदाज़े वाली मदों को अलग रखना बाद में बहुत काम आता है।',
          'फिर कुल जोड़िए और बारह से भाग दीजिए। यही आपकी मासिक सिंकिंग राशि है। पहली बार यह संख्या देखकर अधिकांश लोग चौंकते हैं, और वह चौंकना ही इस अभ्यास का सबसे उपयोगी हिस्सा है।',
          'अगर संख्या असंभव लगे, तो सूची को दो हिस्सों में बाँटिए: जो टाला नहीं जा सकता और जो टाला जा सकता है। पहले हिस्से के लिए फ़ंड शुरू कीजिए, दूसरे के लिए अगले साल।',
        ],
        pEn: [
          'All the power of a sinking fund is in the list, not the arithmetic. The day you sit down and write out the big expenses of the next twelve months, half the work is done.',
          'Start from last year’s bank statement, not from memory. Memory forgets the small large expenses — vehicle insurance, school books, festivals, a wedding, dental work, replacing a phone.',
          'Against each item write three things: the estimated amount, the expected month, and whether the amount is fixed or a guess. Keeping the fixed items separate from the guesses pays off later.',
          'Then add it up and divide by twelve. That is your monthly sinking amount. Most people are startled the first time they see that number, and the startle is the most useful part of the exercise.',
          'If the number looks impossible, split the list in two: what cannot be postponed and what can. Start the fund for the first half, and leave the second half for next year.',
        ],
      },
      {
        hHi: 'एक जगह या अलग-अलग जगह',
        hEn: 'One pot or many',
        pHi: [
          'यहाँ दो स्कूल हैं। एक कहता है कि हर मद का अपना अलग खाता या जमा हो; दूसरा कहता है कि एक ही जगह रखिए और काग़ज़ पर हिसाब बाँटिए।',
          'व्यवहार में दूसरा तरीक़ा अधिकांश लोगों के लिए बेहतर चलता है, क्योंकि उसमें रखरखाव कम है। एक खाता, और एक साधारण सूची जिसमें हर मद के सामने अब तक जमा हुई राशि लिखी हो।',
          'लेकिन इस तरीक़े में एक ख़तरा है: चूँकि पैसा एक ही जगह है, इसलिए एक मद का पैसा दूसरी मद में चुपचाप चला जाता है, और साल के अंत में पता चलता है कि बीमा का पैसा त्योहार में ख़र्च हो गया।',
          'इसका इलाज यह है कि जब भी किसी मद से पैसा निकालें, सूची उसी दिन अपडेट करें और उधार को उधार लिखें। "बीमा से पाँच हज़ार लिए, तीन महीने में वापस" — यह एक पंक्ति पूरी व्यवस्था बचा लेती है।',
          'जिन मदों की राशि बड़ी है और तारीख़ दूर है — जैसे सालाना बीमा प्रीमियम — उनके लिए अलग जमा बनाना ठीक रहता है, क्योंकि वहाँ ग़लती की क़ीमत सबसे ज़्यादा होती है।',
        ],
        pEn: [
          'There are two schools here. One says every item should have its own account or deposit; the other says keep it in one place and divide it on paper.',
          'In practice the second works better for most people, because it needs less maintenance. One account, and a simple list showing how much has accumulated against each item.',
          'But this method carries a danger: because the money sits together, one item’s money quietly walks into another item, and at the end of the year you find the insurance money went on the festival.',
          'The cure is to update the list the same day you take money out, and to write a borrowing as a borrowing. "Took five thousand from insurance, back within three months" — that one line saves the whole arrangement.',
          'For items that are large and far away — an annual insurance premium, say — a separate deposit is worth it, because that is where a mistake costs the most.',
        ],
      },
      {
        hHi: 'जब अनुमान ग़लत निकले',
        hEn: 'When the estimate turns out wrong',
        pHi: [
          'अनुमान ग़लत निकलेंगे। यह व्यवस्था की विफलता नहीं है; यह पहले साल की सामान्य बात है। सवाल यह है कि ग़लती पकड़ में कब आती है — ख़र्च से तीन महीने पहले, या उसी दिन।',
          'सिंकिंग फ़ंड का असली फ़ायदा यही है कि वह ग़लती को जल्दी दिखा देता है। जब जून में दिखता है कि दिसंबर वाली मद के लिए जमा राशि आधी ही है, तो छह महीने का समय बचा होता है, और छह महीने में विकल्प होते हैं।',
          'अगर कमी छोटी है, तो बाक़ी महीनों की किश्त थोड़ी बढ़ा दीजिए। अगर कमी बड़ी है, तो ख़र्च का आकार घटाना ही ईमानदार रास्ता है — छोटा आयोजन, सस्ता मॉडल, या तारीख़ आगे बढ़ाना।',
          'दूसरी तरफ़, अगर ख़र्च अनुमान से कम निकले तो बची हुई राशि को तुरंत किसी और मद में डाल दीजिए, या आपातकालीन कोष में। उसे खाते में "अतिरिक्त" के रूप में छोड़ना उसे ख़र्च होने का न्योता देना है।',
          'और हर साल के अंत में पिछली सूची और असली ख़र्च को आमने-सामने रखिए। दूसरे साल के अनुमान पहले साल से काफ़ी बेहतर होते हैं, और तीसरे साल तक वे लगभग सटीक हो जाते हैं।',
        ],
        pEn: [
          'The estimates will be wrong. That is not a failure of the system; it is what a first year looks like. The question is when the error surfaces — three months before the expense, or on the day itself.',
          'This is the real benefit of a sinking fund: it surfaces the error early. When June shows that the December item is only half funded, six months remain, and six months contain options.',
          'If the shortfall is small, raise the instalment slightly for the remaining months. If it is large, shrinking the expense is the honest route — a smaller event, a cheaper model, or a later date.',
          'On the other side, if an expense comes in under the estimate, move the leftover into another item or into the emergency fund immediately. Leaving it in the account as "extra" is an invitation for it to be spent.',
          'And at the end of each year, put last year’s list next to what was actually spent. The second year’s estimates are much better than the first year’s, and by the third they are close to accurate.',
        ],
      },
      {
        hHi: 'सिंकिंग फ़ंड और आपातकालीन कोष अलग चीज़ें हैं',
        hEn: 'A sinking fund and an emergency fund are different things',
        pHi: [
          'दोनों को एक मान लेना सबसे आम ग़लती है, और सबसे महँगी। दोनों पैसे को अलग रखते हैं, पर वे बिल्कुल अलग सवालों के जवाब हैं।',
          'सिंकिंग फ़ंड ज्ञात ख़र्च के लिए है — आप जानते हैं कि क्या आएगा और लगभग कब आएगा। आपातकालीन कोष अज्ञात के लिए है — नौकरी जाना, अस्पताल, कोई दुर्घटना।',
          'अगर आपात राशि सिंकिंग फ़ंड में मिली हुई है, तो पहली सच्ची आपात स्थिति में आपकी पूरी साल भर की योजना ख़त्म हो जाती है, और उसे दोबारा खड़ा करने में महीनों लगते हैं।',
          'क्रम भी मायने रखता है। पहले एक छोटा आपातकालीन कोष खड़ा कीजिए — एक महीने का ख़र्च ही सही। उसके बाद सिंकिंग फ़ंड शुरू कीजिए। फिर आपातकालीन कोष को तीन से छह महीने तक बढ़ाइए।',
          'जगह भी अलग होनी चाहिए। आपात राशि वहाँ जहाँ से उसी दिन निकल सके; सिंकिंग फ़ंड वहाँ जहाँ से ख़र्च की तारीख़ पर निकल सके। दूसरे में थोड़ी कम तरलता चल जाती है।',
        ],
        pEn: [
          'Treating the two as one thing is the commonest mistake here, and the costliest. Both set money aside, but they answer entirely different questions.',
          'A sinking fund is for known expenses — you know what is coming and roughly when. An emergency fund is for the unknown — a job loss, a hospital, an accident.',
          'If the emergency money is mixed into the sinking fund, the first real emergency wipes out the whole year’s plan, and rebuilding it takes months.',
          'The order matters too. Build a small emergency fund first — even one month of expenses. Then start the sinking fund. Then grow the emergency fund towards three to six months.',
          'The location should differ as well. Emergency money belongs where it can leave the same day; sinking money belongs where it can leave on the expense date. The second can tolerate slightly less liquidity.',
        ],
      },
      {
        hHi: 'पहला साल सबसे कठिन होता है',
        hEn: 'The first year is the hardest',
        pHi: [
          'ईमानदारी से कहा जाए तो पहले साल में सिंकिंग फ़ंड दोहरा बोझ लगता है — इस साल के ख़र्च भी चल रहे हैं और अगले साल के लिए जमा भी हो रहा है। यह असहज है, और यह अस्थायी है।',
          'दूसरे साल से तस्वीर बदल जाती है। तब जो ख़र्च आता है, उसके लिए पैसा पहले से मौजूद होता है, और मासिक किश्त एक बोझ की जगह एक सामान्य लाइन बन जाती है।',
          'पहले साल को आसान बनाने का एक तरीक़ा यह है कि सारी मदों से शुरुआत न कीजिए। दो या तीन सबसे बड़ी और सबसे निश्चित मदें चुनिए — बीमा, फ़ीस, त्योहार — और बाक़ी अगले साल जोड़िए।',
          'दूसरा तरीक़ा यह है कि साल में मिलने वाली कोई एकमुश्त राशि, जैसे बोनस या कर वापसी, पूरी की पूरी सिंकिंग फ़ंड में डाल दीजिए। इससे पहला साल कई महीने आगे कूद जाता है।',
          'और एक बात याद रखिए: इस व्यवस्था का असली फल पैसा नहीं है, बल्कि यह है कि बड़े ख़र्च आने पर घबराहट नहीं होती। जिस घर में दिसंबर का ख़र्च अगस्त में तय हो चुका है, वहाँ दिसंबर एक झटका नहीं रहता।',
        ],
        pEn: [
          'Honestly stated, in the first year a sinking fund feels like a double burden — this year’s expenses are still running while next year’s money is being set aside. It is uncomfortable, and it is temporary.',
          'From the second year the picture changes. The expense arrives and the money for it is already there, and the monthly instalment stops being a burden and becomes an ordinary line.',
          'One way to make the first year easier is not to start with every item. Pick the two or three largest and most certain — insurance, fees, festivals — and add the rest next year.',
          'Another is to put any lump sum the year brings, a bonus or a tax refund, straight into the sinking fund. That jumps the first year forward by several months at once.',
          'And remember what the real fruit of this system is. It is not the money; it is that a large expense no longer causes panic. In a household where December was settled in August, December stops being a shock.',
        ],
      },
    ],
  },
  {
    slug: 'automate-your-savings-india',
    sections: [
      {
        hHi: 'साल में दो बार की जाँच-सूची',
        hEn: 'The twice-a-year checklist',
        pHi: [
          'हर छह महीने में दस मिनट निकालिए और पाँच चीज़ें देखिए। पहली: क्या पिछले छह महीनों की सारी किश्तें सचमुच कटीं, या कोई चुपचाप विफल हुई? यह बैंक विवरण में एक नज़र में दिख जाता है।',
          'दूसरी: क्या वेतन बढ़ा है, और क्या बचत की राशि उसके साथ बढ़ी? तीसरी: क्या कोई ऐसा स्वचालित भुगतान चल रहा है जिसकी अब ज़रूरत नहीं — कोई पुरानी सदस्यता, कोई बंद हो चुकी सेवा?',
          'चौथी: क्या बैंक में दर्ज मोबाइल नंबर और ईमेल अब भी वही हैं जो आप इस्तेमाल करते हैं? विफलता की सूचना वहीं जाती है, और ग़लत पते पर गई सूचना किसी काम की नहीं।',
          'और पाँचवीं: क्या नामांकन यानी नॉमिनी हर खाते और हर जमा में दर्ज है? यह बचत का हिस्सा नहीं लगता, पर परिवार के लिए यह उतना ही ज़रूरी है जितनी बचत ख़ुद।',
        ],
        pEn: [
          'Every six months, take ten minutes and look at five things. First: did every instalment of the last six months actually go through, or did one quietly fail? A single glance at the statement shows this.',
          'Second: has the salary risen, and did the saving amount rise with it? Third: is any automatic payment still running that is no longer needed — an old subscription, a service that has been discontinued?',
          'Fourth: are the mobile number and email registered with the bank still the ones you actually use? Failure notices go there, and a notice sent to the wrong address is worth nothing.',
          'And fifth: is a nominee recorded on every account and every deposit? It does not feel like part of saving, but for the family it matters as much as the saving itself.',
        ],
      },
    ],
  },
  {
    slug: 'sinking-fund-for-big-expenses',
    sections: [
      {
        hHi: 'एक पन्ने का ढाँचा जिससे शुरुआत हो सकती है',
        hEn: 'A one-page format you can start from',
        pHi: [
          'इस पूरे विचार को एक पन्ने पर उतारा जा सकता है, और उतारना ही चाहिए, क्योंकि जो चीज़ लिखी नहीं जाती वह चलती नहीं। ऊपर चार शीर्षक लिखिए: मद, अनुमानित राशि, महीना, मासिक हिस्सा।',
          'हर पंक्ति में एक ख़र्च। मासिक हिस्सा निकालना आसान है — अनुमानित राशि को उस ख़र्च तक बचे महीनों से भाग दे दीजिए। पहले साल में यह संख्या हर मद के लिए अलग होगी, और यह ठीक है।',
          'नीचे तीन पंक्तियाँ छोड़िए: कुल मासिक हिस्सा, अब तक जमा कुल राशि, और आख़िरी बार जाँच की तारीख़। बस इतना ही चाहिए। इससे ज़्यादा जटिल कोई भी ढाँचा तीन महीने में छूट जाता है।',
          'इस पन्ने को वहीं रखिए जहाँ महीने में एक बार आपकी नज़र पड़े — बैंक विवरण के साथ, या फ़ोन में उसी जगह जहाँ बिल की तारीख़ें हैं। जो सूची दिखती नहीं, वह चलती नहीं।',
        ],
        pEn: [
          'The whole idea fits on one page, and it should be put there, because what is not written down does not run. Write four headings across the top: item, estimated amount, month, monthly share.',
          'One expense per line. Working out the monthly share is easy — divide the estimated amount by the number of months left before it falls due. In the first year that figure will differ for every item, and that is fine.',
          'Leave three lines at the bottom: total monthly share, total accumulated so far, and the date of the last review. That is all that is needed. Any format more complicated than this gets abandoned within three months.',
          'Keep the page where your eye falls on it once a month — beside the bank statement, or in the same place on your phone as the bill dates. A list that is not seen is a list that does not run.',
        ],
      },
    ],
  },
  {
    slug: 'savings-account-smart-use',
    sections: [
      {
        hHi: 'ब्याज असल में गिना कैसे जाता है',
        hEn: 'How the interest is actually counted',
        pHi: [
          'बचत खाते का ब्याज अब दैनिक शेष पर गिना जाता है और आमतौर पर तिमाही में जमा होता है। यानी हर दिन का शेष मायने रखता है, महीने के आख़िरी दिन का नहीं।',
          'इसका एक व्यावहारिक नतीजा है जो बहुत कम लोग इस्तेमाल करते हैं: अगर कोई बड़ा भुगतान महीने के आख़िर में करना है, तो उसे पहले कर देने से कुछ नहीं बचता, पर उसे नियत तारीख़ तक टालने से कुछ दिन का ब्याज मिल जाता है।',
          'दूसरा नतीजा यह है कि खाते में औसत शेष बढ़ाने की कोशिश करना बेकार है, क्योंकि औसत का कोई महत्व नहीं है। हर दिन का शेष अपने-आप में गिना जाता है।',
          'यह भी याद रखिए कि बचत खाते का ब्याज एक तय सीमा तक कटौती के योग्य होता है, और उससे ऊपर कर योग्य। यह सीमा वरिष्ठ नागरिकों के लिए अलग होती है, और नियम समय-समय पर बदलते हैं।',
          'सबसे ज़रूरी बात: बचत खाते का ब्याज लगभग हमेशा महँगाई से कम रहता है। यानी वहाँ रखा पैसा हर साल थोड़ी क्रय-शक्ति खोता है। यह खाता पैसा बढ़ाने के लिए नहीं, पैसा पहुँच में रखने के लिए है।',
        ],
        pEn: [
          'Savings-account interest is now calculated on the daily balance and usually credited quarterly. That means every day’s balance counts, not the balance on the last day of the month.',
          'There is a practical consequence that very few people use: if a large payment is due at month end, paying it early gains nothing, while holding it to the due date earns a few days of interest.',
          'A second consequence is that trying to raise the "average balance" is pointless, because the average has no meaning here. Each day’s balance is counted in its own right.',
          'Remember too that savings interest is deductible up to a set limit and taxable above it. That limit differs for senior citizens, and the rules change from time to time.',
          'The most important point: savings-account interest is almost always below inflation. Money parked there loses a little purchasing power every year. This account exists to keep money reachable, not to grow it.',
        ],
      },
      {
        hHi: 'वे शुल्क जो छिपकर बैठे रहते हैं',
        hEn: 'The charges that sit hidden',
        pHi: [
          'बचत खाता मुफ़्त लगता है, और अक्सर होता नहीं। शुल्क छोटे होते हैं, इसलिए विवरण में दिखकर भी नज़र नहीं आते, और साल भर में मिलकर उस ब्याज से बड़े हो सकते हैं जो खाते ने कमाया।',
          'सबसे आम है न्यूनतम शेष न रखने पर लगने वाला शुल्क। यह हर महीने लगता है और कई बैंकों में कमी के अनुपात में बढ़ता है। जिस व्यक्ति का शेष अक्सर कम रहता है, उसके लिए शून्य-शेष खाता कहीं बेहतर विकल्प है।',
          'दूसरा है दूसरे बैंक के एटीएम से तय संख्या से ज़्यादा बार पैसा निकालने का शुल्क। तीसरा, डेबिट कार्ड का सालाना शुल्क, जो अक्सर उसी महीने कट जाता है जिस महीने कार्ड जारी हुआ था और किसी को याद नहीं रहता।',
          'चौथा, एसएमएस सूचना शुल्क — छोटी राशि, पर हर तिमाही। पाँचवाँ, चेक बुक, डिमांड ड्राफ्ट, और खाता बंद करने का शुल्क अगर खाता एक साल के भीतर बंद किया जाए।',
          'साल में एक बार विवरण में सिर्फ़ शुल्क वाली पंक्तियाँ छाँटकर देखिए। यह पंद्रह मिनट का काम है और अक्सर यह बता देता है कि खाता बदलने का समय आ गया है।',
        ],
        pEn: [
          'A savings account feels free, and often is not. The charges are small, so they appear on the statement without being noticed, and over a year they can add up to more than the interest the account earned.',
          'The commonest is the charge for not maintaining a minimum balance. It applies monthly, and at many banks it scales with the size of the shortfall. For anyone whose balance often runs low, a zero-balance account is a far better fit.',
          'Second is the fee for using another bank’s ATM beyond a set number of times. Third is the annual debit-card fee, which usually lands in the same month the card was issued and which nobody remembers.',
          'Fourth, the SMS alert charge — a small amount, but every quarter. Fifth, cheque books, demand drafts, and a closure charge if the account is shut within a year of opening.',
          'Once a year, filter the statement down to the charge lines alone. It is fifteen minutes of work and it often tells you the account needs changing.',
        ],
      },
      {
        hHi: 'स्वीप-इन सुविधा और उसकी शर्तें',
        hEn: 'The sweep-in facility and its conditions',
        pHi: [
          'कई बैंक एक सुविधा देते हैं जिसमें बचत खाते में तय सीमा से ऊपर की राशि अपने-आप एक सावधि जमा में चली जाती है, और ज़रूरत पड़ने पर वापस आ जाती है। इसे स्वीप-इन या ऑटो-स्वीप कहते हैं।',
          'फ़ायदा साफ़ है: निष्क्रिय पड़ा पैसा बचत खाते की दर के बजाय जमा की दर कमाता है, और तरलता लगभग वैसी ही बनी रहती है। जिनके खाते में हर महीने कुछ राशि यूँ ही पड़ी रहती है, उनके लिए यह उपयोगी है।',
          'पर शर्तें पढ़नी ज़रूरी हैं। पहली: वापसी आमतौर पर टुकड़ों में होती है, और जिस टुकड़े को तोड़ा जाता है उस पर समय-पूर्व निकासी वाला नियम लगता है, यानी उस हिस्से पर ब्याज कम मिलता है।',
          'दूसरी: कुछ बैंकों में वापसी "पहले जो बना, पहले वही टूटे" के क्रम में होती है और कुछ में उल्टे क्रम में। यह छोटा-सा अंतर बार-बार निकासी करने वालों के लिए मायने रखता है।',
          'तीसरी: इस जमा पर भी ब्याज कर योग्य है और स्रोत पर कटौती लागू हो सकती है। स्वीप-इन कोई कर-बचत योजना नहीं है; यह सिर्फ़ निष्क्रिय पैसे को थोड़ा बेहतर काम पर लगाने का तरीक़ा है।',
        ],
        pEn: [
          'Many banks offer a facility where any balance above a set threshold moves automatically into a fixed deposit and comes back when it is needed. It is called sweep-in or auto-sweep.',
          'The benefit is plain: idle money earns the deposit rate instead of the savings rate, and liquidity stays much the same. For anyone whose account carries a spare balance every month, it is worth having.',
          'But the conditions need reading. First: the money usually comes back in pieces, and the piece that is broken is treated as a premature withdrawal, so it earns a reduced rate.',
          'Second: some banks unwind on a first-in-first-broken basis and others in reverse order. That small difference matters to anyone who withdraws often.',
          'Third: the interest here is taxable too and tax may be deducted at source. Sweep-in is not a tax-saving scheme; it is simply a way of putting idle money to slightly better use.',
        ],
      },
      {
        hHi: 'एक से ज़्यादा बचत खाते रखने की क़ीमत',
        hEn: 'The cost of holding more than one savings account',
        pHi: [
          'ज़्यादातर लोगों के पास तीन-चार बचत खाते होते हैं और उनमें से दो का इस्तेमाल नहीं होता। पुराना नियोक्ता, पुराना शहर, कोई योजना जो कभी शुरू ही नहीं हुई।',
          'हर निष्क्रिय खाता एक छोटा जोखिम है। उसमें न्यूनतम शेष का शुल्क लगता रह सकता है, उसका डेबिट कार्ड कहीं पड़ा रहता है, और उसका पता या मोबाइल नंबर अक्सर पुराना होता है — यानी धोखाधड़ी की सूचना आप तक नहीं पहुँचेगी।',
          'बंद करना उतना कठिन नहीं जितना लगता है, पर उससे पहले तीन काम कीजिए: उस खाते से जुड़े सारे स्वचालित निर्देश हटाइए, आयकर विभाग और अन्य जगहों पर दर्ज खाता विवरण बदलिए, और अंतिम विवरण डाउनलोड करके रख लीजिए।',
          'कितने खाते ठीक हैं? अधिकांश परिवारों के लिए दो पर्याप्त हैं: एक जिसमें वेतन आता है और रोज़मर्रा का ख़र्च चलता है, दूसरा जिसमें बचत रहती है और जिसका कार्ड घर पर रहता है।',
          'दूसरे खाते का डेबिट कार्ड न बनवाना, या बनवाकर उसे रोज़ की जेब से बाहर रखना, एक छोटा-सा क़दम है जो ख़र्च पर सबसे ज़्यादा असर डालता है।',
        ],
        pEn: [
          'Most people hold three or four savings accounts and use two of them. An old employer, an old city, a scheme that never quite started.',
          'Every dormant account is a small risk. Minimum-balance charges may keep running, its debit card is lying somewhere, and its registered address or mobile number is usually out of date — meaning a fraud alert will not reach you.',
          'Closing one is less difficult than it seems, but do three things first: remove every standing instruction attached to it, update the account details recorded with the tax department and elsewhere, and download and keep the final statement.',
          'How many accounts are right? For most households two are enough: one that receives the salary and runs daily spending, and one that holds savings and whose card stays at home.',
          'Not taking a debit card for the second account — or taking one and keeping it out of the everyday wallet — is a small step with an outsized effect on spending.',
        ],
      },
      {
        hHi: 'निष्क्रिय खाता, केवाईसी और नामांकन',
        hEn: 'Dormant accounts, KYC and the nominee',
        pHi: [
          'अगर किसी बचत खाते में दो साल तक ग्राहक की ओर से कोई लेन-देन न हो, तो वह निष्क्रिय मान लिया जाता है। ब्याज तब भी जमा होता रहता है, पर खाते से पैसा निकालना बंद हो जाता है जब तक उसे दोबारा चालू न कराया जाए।',
          'दोबारा चालू कराना मुश्किल नहीं है — शाखा में पहचान और पते के दस्तावेज़ के साथ एक आवेदन — पर यह उसी समय याद आता है जब पैसे की सख़्त ज़रूरत हो। इससे बचने का तरीक़ा सरल है: साल में एक बार, किसी भी राशि का, एक लेन-देन कर दीजिए।',
          'केवाईसी को भी समय-समय पर अद्यतन करना पड़ता है। बैंक इसकी सूचना उसी नंबर पर भेजता है जो दर्ज है, इसलिए नंबर बदलने पर उसे हर खाते में बदलवाना ज़रूरी है, सिर्फ़ मुख्य खाते में नहीं।',
          'नामांकन सबसे ज़्यादा टाला जाने वाला काम है और परिवार के लिए सबसे ज़रूरी। नामांकन दर्ज न होने पर उत्तराधिकारियों को उत्तराधिकार प्रमाणपत्र की लंबी प्रक्रिया से गुज़रना पड़ता है, जो महीनों ले सकती है।',
          'नामांकन हर खाते, हर जमा, हर लॉकर और हर निवेश में अलग-अलग दर्ज होता है। एक जगह दर्ज करने से बाक़ी जगह अपने-आप नहीं होता। यह एक दोपहर का काम है और उसका मूल्य किसी भी ब्याज दर से बड़ा है।',
        ],
        pEn: [
          'If a savings account sees no customer-initiated transaction for two years, it is treated as dormant. Interest keeps accruing, but withdrawals stop until the account is reactivated.',
          'Reactivating is not hard — an application at the branch with identity and address documents — but it is remembered at exactly the moment the money is urgently needed. The way to avoid it is simple: make one transaction of any size once a year.',
          'KYC also has to be refreshed periodically. The bank sends that notice to the number on record, so when a number changes it has to be updated on every account, not only the main one.',
          'Naming a nominee is the most postponed task of all and the most important one for the family. Without a nominee, heirs must go through the long succession-certificate route, which can take months.',
          'A nomination is recorded separately for each account, each deposit, each locker and each investment. Recording it in one place does not carry it to the others. It is an afternoon’s work and it is worth more than any interest rate.',
        ],
      },
    ],
  },
  {
    slug: 'tax-saving-80c-options',
    sections: [
      {
        hHi: 'पहले देखिए कि सीमा पहले से कितनी भर चुकी है',
        hEn: 'First see how much of the limit is already filled',
        pHi: [
          'हर साल जनवरी से मार्च के बीच लाखों लोग कर बचाने के लिए कुछ नया ख़रीदते हैं, यह जाँचे बिना कि उनकी 80C की सीमा पहले से कितनी भर चुकी है। यह इस पूरे विषय की सबसे महँगी चूक है।',
          'कई चीज़ें अपने-आप इस सीमा में जुड़ती हैं और लोग उन्हें गिनते नहीं। भविष्य निधि में वेतन से हर महीने कटने वाला कर्मचारी अंशदान इसमें आता है, और अकेले वही कई लोगों की आधी सीमा भर देता है।',
          'इसी तरह जीवन बीमा का प्रीमियम, गृह ऋण की मूल राशि की वापसी, और बच्चों की ट्यूशन फ़ीस — ये सब पहले से चल रहे होते हैं और सीमा में गिने जाते हैं।',
          'इसलिए सही क्रम यह है: पहले काग़ज़ पर पहले से भर चुकी राशि जोड़िए, फिर बची हुई जगह निकालिए, और उसके बाद ही तय कीजिए कि नया कुछ चाहिए भी या नहीं। कई लोगों के लिए जवाब "नहीं" निकलता है।',
          'सीमा से ऊपर डाला गया पैसा कर नहीं बचाता। वह निवेश के रूप में अच्छा हो सकता है, पर तब उसे निवेश के तर्क पर परखा जाना चाहिए, कर-बचत के तर्क पर नहीं।',
        ],
        pEn: [
          'Every year between January and March, lakhs of people buy something new to save tax without first checking how much of their 80C limit is already used. This is the costliest slip in the whole subject.',
          'Several things fill the limit by themselves and go uncounted. The employee’s provident-fund contribution deducted from salary each month sits inside it, and for many people that alone fills half the limit.',
          'Life-insurance premiums, the principal portion of home-loan repayment, and children’s tuition fees do the same — all already running, all counted against the limit.',
          'So the right order is: add up what is already filled, on paper; work out the space that remains; and only then decide whether anything new is needed at all. For many people the answer turns out to be no.',
          'Money put in above the limit saves no tax. It may still be a good investment, but then it should be judged on investment logic, not on tax logic.',
        ],
      },
      {
        hHi: 'लॉक-इन की तुलना, जो असली फ़र्क़ है',
        hEn: 'Comparing the lock-ins, which is the real difference',
        pHi: [
          '80C के विकल्पों में सबसे बड़ा फ़र्क़ रिटर्न का नहीं, लॉक-इन का है — यानी कितने साल तक पैसा बाहर नहीं निकाला जा सकता। यही तय करता है कि कोई विकल्प आपके लिए उपयुक्त है या नहीं।',
          'कुछ विकल्पों में लॉक-इन तीन साल का है, कुछ में पाँच, और कुछ में पंद्रह साल तक। लंबा लॉक-इन अपने-आप बुरा नहीं है; अगर लक्ष्य भी उतना ही दूर है तो वह अनुशासन का काम करता है।',
          'समस्या तब आती है जब पंद्रह साल के लॉक-इन वाले विकल्प में वह पैसा डाल दिया जाए जिसकी ज़रूरत तीन साल में पड़नी है। तब कर बचता है और तरलता चली जाती है, और अक्सर उसकी भरपाई क़र्ज़ से करनी पड़ती है।',
          'दूसरी तुलना योग्य बात यह है कि परिपक्वता पर मिलने वाली राशि कर-मुक्त है या कर योग्य। दो विकल्प जो आज एक जैसी कटौती देते हैं, अंत में बहुत अलग हाथ में आ सकते हैं।',
          'इसलिए चुनाव का सही सवाल यह नहीं है कि "कौन सा सबसे ज़्यादा देता है", बल्कि यह है कि "मुझे यह पैसा कब चाहिए, और उस तारीख़ पर यह विकल्प क्या हाथ में देगा"।',
        ],
        pEn: [
          'The biggest difference between 80C options is not the return but the lock-in — how many years the money cannot be taken out. That is what decides whether an option suits you at all.',
          'Some options lock money for three years, some for five, some for as long as fifteen. A long lock-in is not bad in itself; when the goal is equally far away it does the work of discipline.',
          'The trouble starts when money needed in three years goes into a fifteen-year lock-in. Tax is saved and liquidity is lost, and the gap usually ends up being filled by borrowing.',
          'The second thing worth comparing is whether the maturity amount is tax-free or taxable. Two options that give the same deduction today can hand over very different amounts at the end.',
          'So the right question is not "which one gives the most" but "when do I need this money, and what will this option put in my hand on that date".',
        ],
      },
      {
        hHi: 'मार्च की जल्दबाज़ी वाली ख़रीद',
        hEn: 'The March-rush purchase',
        pHi: [
          'मार्च के आख़िरी दस दिन इस देश में सबसे ख़राब वित्तीय फ़ैसलों का मौसम हैं। समय कम होता है, दबाव ज़्यादा, और बेचने वाले को यह पता होता है।',
          'जल्दबाज़ी में जो सबसे ज़्यादा बिकता है वह अक्सर वही होता है जिस पर बेचने वाले को सबसे ज़्यादा कमीशन मिलता है, न कि वह जो ख़रीदने वाले के लिए सबसे उपयुक्त है। यह किसी की बदनीयती नहीं, यह प्रोत्साहन का सीधा असर है।',
          'सबसे आम नुक़सान लंबी अवधि की उन बीमा पॉलिसियों में होता है जो कर बचाने के नाम पर बेची जाती हैं। उनमें हर साल प्रीमियम देना पड़ता है, और दो-तीन साल बाद छोड़ देने पर जमा राशि का बड़ा हिस्सा चला जाता है।',
          'इसका इलाज कैलेंडर है, समझदारी नहीं। कर-बचत का फ़ैसला अप्रैल में लीजिए, मार्च में नहीं, और उसे बारह मासिक किश्तों में बाँट दीजिए। तब न जल्दबाज़ी रहती है और न एकमुश्त बोझ।',
          'अगर मार्च आ ही गया है और कुछ करना ज़रूरी है, तो सबसे सरल और सबसे कम प्रतिबद्धता वाला विकल्प चुनिए। एक साल की सादा चीज़ पंद्रह साल की जटिल चीज़ से बेहतर है जब सोचने का समय न हो।',
        ],
        pEn: [
          'The last ten days of March are this country’s worst season for financial decisions. There is little time, a lot of pressure, and the seller knows it.',
          'What sells most in a rush is often what pays the seller the most, not what fits the buyer best. This is not anybody’s bad faith; it is the plain effect of incentives.',
          'The commonest damage happens in long-term insurance policies sold in the name of tax saving. They demand a premium every year, and abandoning one after two or three years surrenders a large part of what was paid in.',
          'The cure is a calendar, not cleverness. Take the tax-saving decision in April, not March, and spread it over twelve monthly instalments. Then there is neither a rush nor a lump-sum burden.',
          'If March has already arrived and something must be done, choose the simplest option with the least commitment. A plain one-year thing beats a complicated fifteen-year thing when there is no time to think.',
        ],
      },
      {
        hHi: '80C के बाहर की कटौतियाँ, जो अक्सर छूट जाती हैं',
        hEn: 'Deductions outside 80C, which often get missed',
        pHi: [
          '80C पर इतना ध्यान जाता है कि उसके बाहर की कटौतियाँ अक्सर छूट जाती हैं, हालाँकि उनमें से कई बिना कोई नया निवेश किए ही मिल जाती हैं।',
          'स्वास्थ्य बीमा का प्रीमियम एक अलग धारा में आता है, और माता-पिता के लिए लिया गया बीमा उसमें अतिरिक्त जगह देता है। यह वह कटौती है जो सबसे ज़्यादा छोड़ी जाती है और जिसका असल जीवन में सबसे ज़्यादा उपयोग है।',
          'शिक्षा ऋण का ब्याज, कुछ शर्तों के साथ दान, और गृह ऋण का ब्याज — ये सब अलग-अलग धाराओं में आते हैं और 80C की सीमा से अलग गिने जाते हैं।',
          'राष्ट्रीय पेंशन प्रणाली में एक अतिरिक्त कटौती भी उपलब्ध है जो 80C से ऊपर है। जिनकी 80C की सीमा पहले ही भर चुकी है, उनके लिए यह अक्सर अगला तार्किक क़दम होता है।',
          'यहाँ सलाह देने का इरादा नहीं है, केवल यह याद दिलाने का कि कर-बचत की तस्वीर 80C से बहुत बड़ी है। अपनी स्थिति के हिसाब से कौन सी धारा लागू होती है, यह किसी योग्य कर सलाहकार से एक बार जाँच लेना समझदारी है।',
        ],
        pEn: [
          'So much attention goes to 80C that the deductions outside it get missed, even though several of them are available without making any new investment at all.',
          'Health-insurance premiums sit in a separate section, and cover taken for parents adds further room inside it. This is the most commonly skipped deduction and the one with the most real-life use.',
          'Interest on an education loan, donations under certain conditions, and interest on a home loan all fall under their own sections and are counted separately from the 80C limit.',
          'An additional deduction is also available for the National Pension System, over and above 80C. For anyone whose 80C limit is already full, that is often the next logical step.',
          'The intention here is not to advise but to point out that the tax-saving picture is much larger than 80C. Which sections apply to your own situation is worth checking once with a qualified tax adviser.',
        ],
      },
      {
        hHi: 'नई और पुरानी व्यवस्था का चुनाव',
        hEn: 'Choosing between the new regime and the old',
        pHi: [
          'अब दो कर व्यवस्थाएँ मौजूद हैं। एक में दरें कम हैं पर अधिकांश कटौतियाँ उपलब्ध नहीं; दूसरी में दरें ऊँची हैं पर कटौतियाँ ली जा सकती हैं। इस चुनाव के बिना 80C की पूरी चर्चा अधूरी है।',
          'सामान्य नियम यह है कि जिस व्यक्ति की कटौतियाँ पहले से बड़ी हैं — गृह ऋण, बीमा, भविष्य निधि, किराया — उसके लिए पुरानी व्यवस्था अक्सर बेहतर बैठती है। जिसकी कटौतियाँ कम हैं, उसके लिए नई।',
          'पर यह नियम अनुमान है, गणना नहीं। सही तरीक़ा यह है कि दोनों व्यवस्थाओं में अपनी असली संख्याएँ डालकर कर की राशि निकाली जाए। यह काम एक बार करने में आधा घंटा लगता है और साल भर की उलझन ख़त्म कर देता है।',
          'यहाँ एक बड़ा ख़तरा भी है: सिर्फ़ कर बचाने के लिए ऐसा निवेश करना जिसकी ज़रूरत नहीं। अगर नई व्यवस्था में आपका कर कम बैठता है, तो 80C के लिए कुछ ख़रीदने की कोई वजह नहीं बचती।',
          'और नियम बदलते रहते हैं — दरें, सीमाएँ, और कौन सी कटौती किस व्यवस्था में उपलब्ध है, यह सब। इसलिए हर साल यह तुलना दोबारा करनी चाहिए, पिछले साल का निर्णय दोहराना नहीं चाहिए।',
        ],
        pEn: [
          'There are now two tax regimes. One has lower rates but does not allow most deductions; the other has higher rates but allows them. Without settling this choice, any discussion of 80C is incomplete.',
          'The general rule is that somebody whose deductions are already large — home loan, insurance, provident fund, rent — usually fits the old regime better, and somebody with few deductions fits the new one.',
          'But that rule is a guess, not a calculation. The correct method is to put your own real numbers into both regimes and work out the tax each way. It takes half an hour once and removes a year of confusion.',
          'There is a real danger here too: making an investment you do not need purely to save tax. If your tax works out lower under the new regime, there is no longer any reason to buy anything for 80C.',
          'And the rules keep changing — the rates, the limits, and which deduction survives in which regime. So this comparison should be redone every year rather than repeating last year’s decision.',
        ],
      },
    ],
  },
  {
    slug: 'savings-account-smart-use',
    sections: [
      {
        hHi: 'खाता बदलने का सही समय',
        hEn: 'When it is time to change the account',
        pHi: [
          'बचत खाता बदलना झंझट लगता है, इसलिए लोग वर्षों तक ऐसा खाता चलाते रहते हैं जो उनके लिए अब उपयुक्त नहीं है। कुछ स्पष्ट संकेत बता देते हैं कि समय आ गया है।',
          'पहला संकेत: साल भर के शुल्क उस साल के ब्याज से ज़्यादा हैं। यह गणना विवरण से पंद्रह मिनट में हो जाती है और यह सबसे साफ़ जवाब देती है।',
          'दूसरा: न्यूनतम शेष की शर्त इतनी ऊँची है कि उसे बनाए रखने के लिए आपको ऐसा पैसा खाते में रोकना पड़ता है जो कहीं और बेहतर काम कर सकता था।',
          'तीसरा: शाखा या एटीएम अब पहुँच में नहीं, क्योंकि आप शहर बदल चुके हैं। चौथा: नेट बैंकिंग या ऐप इतना असुविधाजनक है कि आप ज़रूरी काम टालने लगे हैं।',
          'बदलते समय पुराना खाता तुरंत बंद मत कीजिए। पहले नया खाता चालू कीजिए, फिर वेतन और सारे स्वचालित निर्देश एक-एक करके स्थानांतरित कीजिए, दो महीने दोनों साथ चलने दीजिए, और उसके बाद ही पुराना बंद कीजिए।',
        ],
        pEn: [
          'Changing a savings account feels like a hassle, so people carry an account for years after it has stopped suiting them. A few plain signals say the time has come.',
          'The first signal: the year’s charges exceed the year’s interest. That calculation takes fifteen minutes from the statement, and it gives the clearest possible answer.',
          'The second: the minimum-balance condition is high enough that meeting it forces you to hold money in the account which could be doing better work somewhere else.',
          'The third: the branch or the ATM is no longer within reach because you have moved city. The fourth: the net banking or the app is awkward enough that you have started postponing things you need to do.',
          'When you do change, do not close the old account straight away. Open the new one, move the salary and the standing instructions across one at a time, let both run side by side for two months, and only then close the old one.',
        ],
      },
    ],
  },
  {
    slug: 'tax-saving-80c-options',
    sections: [
      {
        hHi: 'साल भर का एक सरल कैलेंडर',
        hEn: 'A simple calendar for the year',
        pHi: [
          'कर-बचत को एक घटना की तरह देखने के बजाय एक कैलेंडर की तरह देखना ही सबसे बड़ा सुधार है। इसमें चार तारीख़ें काफ़ी हैं और हर एक पर काम आधे घंटे से ज़्यादा का नहीं।',
          'अप्रैल में: पिछले साल की कटौतियों की सूची लीजिए, देखिए कि इस साल कौन सी अपने-आप चलती रहेंगी, और बची हुई जगह के लिए मासिक राशि तय कीजिए। यही वह फ़ैसला है जो मार्च की जल्दबाज़ी ख़त्म कर देता है।',
          'जुलाई में: पिछले वित्तीय वर्ष का रिटर्न भरते समय अपने असली आँकड़े देखिए और अनुमान से तुलना कीजिए। यहीं पता चलता है कि पिछली योजना कहाँ चूकी थी।',
          'नवंबर में: नियोक्ता को प्रमाण देने का समय आता है। तब तक आपके पास दस्तावेज़ तैयार होने चाहिए, ताकि अंतिम महीनों में वेतन से बड़ी कटौती न हो।',
          'और जनवरी में: बची हुई जगह की आख़िरी जाँच। अगर योजना अप्रैल में बनी थी तो इस समय करने को लगभग कुछ नहीं बचता, और यही इस पूरे कैलेंडर का उद्देश्य है।',
        ],
        pEn: [
          'Treating tax saving as a calendar rather than an event is the single biggest improvement available. Four dates are enough, and none of them takes more than half an hour.',
          'In April: take last year’s list of deductions, see which of them will keep running on their own this year, and fix a monthly amount for whatever space is left. This is the decision that removes the March rush entirely.',
          'In July: while filing the previous year’s return, look at your real figures and compare them with the estimate. This is where you find out where last year’s plan went wrong.',
          'In November: proofs are due to the employer. By then the documents should be ready, so that the last months of the year do not carry a heavy deduction from salary.',
          'And in January: a final check on the space that remains. If the plan was made in April there is almost nothing left to do at this point, and that is exactly what the calendar is for.',
        ],
      },
    ],
  },
  {
    slug: 'hra-exemption-guide',
    sections: [
      {
        hHi: 'तीन में से सबसे कम वाला नियम',
        hEn: 'The lowest-of-three rule',
        pHi: [
          'एचआरए की छूट का हिसाब एक जैसा नहीं होता; वह तीन अलग-अलग संख्याओं में से सबसे कम वाली होती है। यही एक बात समझ लेने से अधिकांश उलझन ख़त्म हो जाती है।',
          'पहली संख्या है वह एचआरए जो आपको वेतन में वास्तव में मिला। दूसरी है चुकाया गया वास्तविक किराया घटाकर मूल वेतन का दस प्रतिशत। तीसरी है मूल वेतन का एक तय प्रतिशत, जो महानगर और ग़ैर-महानगर में अलग होता है।',
          'तीनों निकालकर सबसे छोटी वाली छूट के रूप में मिलती है। इसका सीधा मतलब यह है कि सिर्फ़ ज़्यादा एचआरए मिलने से छूट नहीं बढ़ती — अगर किराया कम है तो दूसरी संख्या छोटी रहेगी और वही लागू होगी।',
          'यही कारण है कि जो लोग अपने ही घर में रहते हैं और किराया नहीं देते, उन्हें एचआरए मिलने के बावजूद कोई छूट नहीं मिलती। छूट किराया देने पर है, एचआरए मिलने पर नहीं।',
          'यह भी ध्यान रखिए कि यहाँ "मूल वेतन" का अर्थ आमतौर पर मूल वेतन और महँगाई भत्ते का वह हिस्सा है जो सेवानिवृत्ति लाभ में गिना जाता है — कुल वेतन नहीं। यह छोटी-सी बात गणना को काफ़ी बदल देती है।',
          'इसलिए साल की शुरुआत में एक बार तीनों संख्याएँ निकालकर देख लेना उपयोगी है। तब आपको पता होता है कि आपकी असली छूट किस संख्या से बँधी है और उसे बढ़ाने की गुंजाइश है भी या नहीं।',
        ],
        pEn: [
          'The HRA exemption is not a single figure; it is the smallest of three separate numbers. Understanding that one point removes most of the confusion around it.',
          'The first number is the HRA actually received in your salary. The second is the rent actually paid, minus ten per cent of basic salary. The third is a fixed percentage of basic salary, which differs between metro and non-metro cities.',
          'Work out all three and the smallest becomes the exemption. The direct meaning is that a larger HRA alone does not increase the exemption — if the rent is low, the second number stays small and that is what applies.',
          'This is why people living in their own homes and paying no rent receive no exemption despite having HRA in their salary. The relief is for paying rent, not for receiving an allowance.',
          'Note also that "basic salary" here usually means basic pay plus the part of dearness allowance that counts towards retirement benefits — not total salary. That small distinction changes the calculation considerably.',
          'So it is worth working out all three numbers once at the start of the year. You then know which number your exemption is actually bound to, and whether there is any room to change it.',
        ],
      },
      {
        hHi: 'माता-पिता को किराया देना',
        hEn: 'Paying rent to parents',
        pHi: [
          'जो व्यक्ति माता-पिता के घर में रहता है, वह उन्हें किराया देकर एचआरए की छूट ले सकता है। यह क़ानूनी है, पर इसके लिए व्यवस्था वास्तविक होनी चाहिए, केवल काग़ज़ी नहीं।',
          'पहली शर्त: घर के मालिक माता-पिता होने चाहिए, आप नहीं। अगर आप उस संपत्ति के सह-मालिक हैं तो व्यवस्था कमज़ोर पड़ जाती है, क्योंकि कोई अपने ही हिस्से का किराया ख़ुद को नहीं दे सकता।',
          'दूसरी: किराया वास्तव में चुकाया जाना चाहिए, और बैंक से चुकाया जाना चाहिए। नक़द में दिया गया किराया साबित करना कठिन होता है, और जाँच के समय यही सबसे पहले पूछा जाता है।',
          'तीसरी: किराया बाज़ार दर के आसपास होना चाहिए। उसी इलाक़े के सामान्य किराए से कई गुना ज़्यादा राशि दिखाना संदेह पैदा करता है और छूट अस्वीकार हो सकती है।',
          'चौथी, और यह सबसे ज़्यादा भुलाई जाती है: वह किराया माता-पिता की आय में जुड़ता है और उन्हें अपने रिटर्न में दिखाना पड़ता है। अगर उनकी आय कर योग्य सीमा से नीचे है तो कुल मिलाकर परिवार को लाभ होता है; अगर वे ऊँचे स्लैब में हैं तो पूरी क़वायद बेमानी हो सकती है।',
          'इसलिए यह फ़ैसला अकेले अपने कर को देखकर नहीं, परिवार के दोनों पक्षों को एक साथ देखकर लेना चाहिए। एक किराया-नामा और बैंक से भुगतान — ये दो चीज़ें व्यवस्था को साफ़ रखती हैं।',
        ],
        pEn: [
          'Somebody living in a parent’s house can pay them rent and claim the HRA exemption. This is legitimate, but the arrangement has to be real rather than merely written down.',
          'First condition: the parents must own the house, and you must not. If you are a co-owner of the property the arrangement weakens, because nobody can pay rent to themselves on their own share.',
          'Second: the rent must actually be paid, and paid through a bank. Rent handed over in cash is hard to establish, and it is the first thing asked about if the claim is examined.',
          'Third: the rent should be somewhere near the market rate. Showing an amount several times the ordinary rent of that locality invites suspicion and the exemption can be disallowed.',
          'Fourth, and this is the most forgotten: that rent becomes income in the parents’ hands and has to be shown in their return. If their income is below the taxable limit the family gains overall; if they are in a higher slab the whole exercise can be pointless.',
          'So the decision should be taken by looking at both sides of the family together, not at your own tax alone. A rent agreement and payment through a bank are the two things that keep the arrangement clean.',
        ],
      },
      {
        hHi: 'मकान मालिक का पैन और किराए की रसीदें',
        hEn: 'The landlord’s PAN and the rent receipts',
        pHi: [
          'अगर साल भर का किराया एक तय सीमा से ऊपर है, तो नियोक्ता को मकान मालिक का पैन देना पड़ता है। यह वह जगह है जहाँ हर साल सबसे ज़्यादा दावे अटकते हैं।',
          'कई मकान मालिक पैन देने में हिचकते हैं। ऐसी स्थिति में एकमात्र वैध रास्ता यह है कि मकान मालिक एक घोषणा-पत्र दे जिसमें उसका पैन न होने का कारण लिखा हो, साथ में नाम और पता। किसी और का पैन लगाना, या मनगढ़ंत संख्या डालना, इसका विकल्प नहीं है।',
          'रसीदों के मामले में लोग दो ग़लतियाँ करते हैं। पहली, साल के अंत में एक साथ बारह रसीदें बनवाना, जो सब एक ही क़लम और एक ही दिन की दिखती हैं। दूसरी, रसीद पर तारीख़ और अवधि न लिखवाना।',
          'सही तरीक़ा यह है कि हर महीने या हर तिमाही रसीद ली जाए, उस पर किराए की राशि, अवधि, मकान का पता, मकान मालिक का नाम और हस्ताक्षर हो। जहाँ लागू हो वहाँ राजस्व टिकट भी।',
          'सबसे मज़बूत सबूत रसीद नहीं, बैंक विवरण है। हर महीने उसी तारीख़ के आसपास उसी खाते में गया भुगतान अपने-आप में एक कहानी कहता है जिस पर सवाल कम उठते हैं।',
          'और किराया-नामा रखिए, भले ही मकान मालिक परिचित हो। यह क़ानूनी सुरक्षा भी है और कर के लिए दस्तावेज़ भी, और दोनों काम एक ही काग़ज़ से हो जाते हैं।',
        ],
        pEn: [
          'If the year’s rent crosses a set threshold, the landlord’s PAN has to be given to the employer. This is where most claims get stuck, year after year.',
          'Many landlords hesitate to share a PAN. In that situation the only valid route is a declaration from the landlord stating the reason for not having one, along with name and address. Using somebody else’s PAN, or inventing a number, is not an alternative.',
          'With receipts, people make two mistakes. The first is getting twelve receipts written at the end of the year, all in the same pen on the same day. The second is leaving the date and the period off them.',
          'The right way is to take a receipt every month or every quarter, carrying the rent amount, the period, the address of the house, the landlord’s name and a signature — with a revenue stamp where that applies.',
          'The strongest evidence is not the receipt but the bank statement. A payment made to the same account around the same date every month tells its own story, and that story attracts few questions.',
          'And keep a rent agreement even when the landlord is somebody you know. It is legal protection and tax documentation at once, and one piece of paper does both jobs.',
        ],
      },
      {
        hHi: 'एचआरए और गृह ऋण एक साथ',
        hEn: 'HRA and a home loan together',
        pHi: [
          'एक आम धारणा यह है कि जिसके पास अपना घर है और गृह ऋण चल रहा है, वह एचआरए की छूट नहीं ले सकता। यह हमेशा सही नहीं है, और यह ग़लतफ़हमी बहुत लोगों को अनावश्यक नुक़सान पहुँचाती है।',
          'दोनों लाभ एक साथ लिए जा सकते हैं, बशर्ते परिस्थिति वास्तविक हो। सबसे स्पष्ट स्थिति यह है कि आपका अपना घर एक शहर में है और नौकरी के कारण आप दूसरे शहर में किराए पर रहते हैं।',
          'दूसरी स्थिति, जो कम स्पष्ट है: अपना घर उसी शहर में है पर उसमें रहना व्यावहारिक नहीं — जैसे वह बहुत दूर है, या अभी निर्माणाधीन है, या किराए पर चढ़ा हुआ है। ऐसे मामलों में कारण दस्तावेज़ों से स्पष्ट होना चाहिए।',
          'जहाँ अपना घर किराए पर चढ़ा है, वहाँ उससे मिलने वाला किराया आपकी आय में जुड़ेगा, और उस संपत्ति पर ब्याज की कटौती के अलग नियम लागू होंगे। दोनों पक्ष रिटर्न में दिखाने होंगे।',
          'जो स्थिति काम नहीं करती वह यह है कि आप अपने ही घर में रहें, उसी पर ऋण चल रहा हो, और साथ में एचआरए की छूट भी माँगें। वहाँ किराया दिया ही नहीं जा रहा, इसलिए छूट का आधार ही नहीं बनता।',
          'चूँकि यहाँ हर मामला परिस्थिति पर निर्भर है, इसलिए दोनों लाभ एक साथ लेने से पहले एक बार किसी योग्य कर सलाहकार से जाँच करा लेना ठीक रहता है। दस्तावेज़ पहले से तैयार रखना उस जाँच को आसान बना देता है।',
        ],
        pEn: [
          'A common belief is that somebody who owns a house and is repaying a home loan cannot claim HRA. This is not always true, and the misunderstanding costs a great many people money for no reason.',
          'Both benefits can be taken together, provided the circumstances are genuine. The clearest case is owning a house in one city while living in rented accommodation in another because of work.',
          'A second, less obvious case: the owned house is in the same city but living in it is not practical — it is too far, or still under construction, or let out to a tenant. In such cases the reason should be evident from the documents.',
          'Where the owned house is let out, the rent it earns becomes part of your income, and separate rules govern the interest deduction on that property. Both sides have to appear in the return.',
          'The situation that does not work is living in your own house, repaying the loan on it, and claiming HRA as well. No rent is being paid there, so there is no basis for the exemption at all.',
          'Because every case here turns on circumstance, it is sensible to have a qualified tax adviser check the position once before claiming both together. Having the documents ready in advance makes that check straightforward.',
        ],
      },
      {
        hHi: 'अगर नियोक्ता से छूट न मिल सकी',
        hEn: 'If the employer did not give the exemption',
        pHi: [
          'कई बार प्रमाण समय पर जमा नहीं हो पाते, या कोई दस्तावेज़ अधूरा रह जाता है, और नियोक्ता एचआरए की छूट दिए बिना कर काट लेता है। यह अंत नहीं है।',
          'ऐसी स्थिति में छूट रिटर्न भरते समय ख़ुद दावा की जा सकती है। तब गणना आप करते हैं, और अतिरिक्त कटा हुआ कर वापसी के रूप में आता है।',
          'इसके लिए तीन चीज़ें तैयार रखिए: किराए के भुगतान का बैंक रिकॉर्ड, रसीदें या किराया-नामा, और फ़ॉर्म 16 जिसमें दिखे कि छूट नहीं दी गई। दावा इन्हीं पर टिकता है।',
          'यह भी देख लीजिए कि आपका फ़ॉर्म 26AS और वार्षिक सूचना विवरण आपके अपने आँकड़ों से मेल खाते हैं। बेमेल आँकड़े ही अधिकांश नोटिस की जड़ होते हैं, ग़लत दावा नहीं।',
          'दस्तावेज़ रिटर्न के साथ भेजने नहीं होते, पर उन्हें कई साल तक सँभालकर रखना पड़ता है। जाँच किसी भी साल आ सकती है, और तब तक रसीदें अक्सर कहीं खो चुकी होती हैं।',
          'सबसे अच्छा तरीक़ा यह है कि हर वित्तीय वर्ष के लिए एक फ़ोल्डर बना लिया जाए — भौतिक या डिजिटल — जिसमें उस साल के किराए के सारे काग़ज़ एक जगह रहें। यह पंद्रह मिनट की आदत बाद में घंटों बचाती है।',
        ],
        pEn: [
          'Sometimes the proofs are not submitted in time, or a document is incomplete, and the employer deducts tax without giving the HRA exemption. That is not the end of the matter.',
          'In that situation the exemption can be claimed directly while filing the return. You do the calculation yourself, and the excess tax deducted comes back as a refund.',
          'Keep three things ready for it: the bank record of the rent payments, the receipts or the rent agreement, and the Form 16 showing that the exemption was not granted. The claim rests on these.',
          'Check also that your Form 26AS and Annual Information Statement agree with your own figures. Mismatched data, rather than a wrong claim, is the root of most notices.',
          'The documents are not sent with the return, but they have to be kept for several years. A query can arrive in any of those years, and by then the receipts have usually gone missing.',
          'The best approach is to keep one folder per financial year — physical or digital — holding all of that year’s rent papers in one place. Fifteen minutes of habit saves hours later.',
        ],
      },
    ],
  },
  {
    slug: 'capital-gains-basics-india',
    sections: [
      {
        hHi: 'ख़रीद मूल्य में क्या-क्या जुड़ता है',
        hEn: 'What counts as the purchase cost',
        pHi: [
          'पूँजीगत लाभ बिक्री मूल्य में से ख़रीद मूल्य घटाकर निकलता है, और यहीं पर लोग सबसे ज़्यादा पैसा छोड़ देते हैं — क्योंकि ख़रीद मूल्य केवल वह राशि नहीं है जो चुकाई गई थी।',
          'ख़रीदते समय दिए गए ब्रोकरेज, स्टांप ड्यूटी, पंजीकरण शुल्क और अन्य वैध ख़र्च ख़रीद मूल्य में जुड़ते हैं। बिक्री के समय दिया गया ब्रोकरेज बिक्री मूल्य में से घटता है।',
          'संपत्ति के मामले में एक और मद है: सुधार पर हुआ ख़र्च। जो निर्माण या बड़ा सुधार आपने बाद में कराया, वह भी लागत में जुड़ता है — बशर्ते उसके बिल और भुगतान का रिकॉर्ड मौजूद हो।',
          'यही वजह है कि पुराने काग़ज़ फेंकने से पहले सोचना चाहिए। जिस व्यक्ति के पास बीस साल पुराने सुधार के बिल हैं, उसका कर योग्य लाभ उससे काफ़ी कम बैठेगा जिसके पास केवल याददाश्त है।',
          'कुछ श्रेणियों की परिसंपत्तियों में लंबी अवधि पर लागत को महँगाई के हिसाब से समायोजित करने की व्यवस्था रही है, हालाँकि यह प्रावधान समय-समय पर बदलता रहा है और हर परिसंपत्ति पर एक जैसा लागू नहीं होता।',
          'इसलिए बेचने से पहले लागत का हिसाब काग़ज़ पर बनाइए, बिक्री के बाद नहीं। बाद में बनाया गया हिसाब हमेशा उन दस्तावेज़ों तक सीमित रह जाता है जो संयोग से बच गए।',
        ],
        pEn: [
          'A capital gain is the sale price minus the purchase cost, and this is exactly where people leave the most money behind — because the purchase cost is not only the amount that was paid.',
          'Brokerage paid at the time of buying, stamp duty, registration charges and other legitimate expenses are added to the cost. Brokerage paid on the sale is deducted from the sale price.',
          'With property there is a further item: money spent on improvement. Construction or a substantial improvement carried out later also adds to the cost — provided the bills and payment records still exist.',
          'This is why old papers deserve a thought before they are thrown away. Somebody holding twenty-year-old improvement bills will show a considerably smaller taxable gain than somebody holding only a memory.',
          'For some categories of asset there has been a mechanism to adjust the cost for inflation over long holding periods, though that provision has changed from time to time and does not apply identically to every asset.',
          'So build the cost working on paper before selling, not after. A working built afterwards is always limited to whichever documents happened to survive.',
        ],
      },
      {
        hHi: 'होल्डिंग अवधि की गिनती',
        hEn: 'Counting the holding period',
        pHi: [
          'लंबी अवधि और छोटी अवधि का अंतर कर की राशि पर सबसे बड़ा असर डालता है, और उसकी गिनती उतनी सीधी नहीं है जितनी लगती है।',
          'गिनती ख़रीद की तारीख़ से बिक्री की तारीख़ तक होती है, न कि वित्तीय वर्ष के हिसाब से। एक ही महीने में ख़रीदी और बेची गई चीज़ पर वही नियम लगता है चाहे बीच में मार्च आया हो या नहीं।',
          'अलग-अलग परिसंपत्तियों के लिए लंबी अवधि की सीमा अलग होती है। यही वह जगह है जहाँ अनुमान लगाना सबसे महँगा पड़ता है, क्योंकि एक श्रेणी का नियम दूसरी पर लागू कर देना बहुत आसान है।',
          'अगर एक ही चीज़ अलग-अलग समय पर टुकड़ों में ख़रीदी गई है, तो हर टुकड़े की अपनी तारीख़ होती है। बेचते समय आमतौर पर पहले ख़रीदी गई इकाइयाँ पहले बिकी मानी जाती हैं।',
          'इसका व्यावहारिक अर्थ यह है कि कभी-कभी बिक्री कुछ हफ़्ते टाल देने से श्रेणी बदल जाती है और कर की राशि काफ़ी घट जाती है। यह अकेला विचार बड़ी बिक्री में सबसे ज़्यादा फ़र्क़ लाता है।',
          'पर इसका उल्टा भी सच है: सिर्फ़ कर बचाने के लिए किसी गिरती हुई परिसंपत्ति को महीनों पकड़े रहना अक्सर उतना ही नुक़सान करता है जितना कर बचाता है। तारीख़ का फ़ैसला निवेश के फ़ैसले के बाद आना चाहिए, पहले नहीं।',
        ],
        pEn: [
          'The difference between long-term and short-term has the largest single effect on the tax, and counting it is less straightforward than it looks.',
          'The count runs from the date of purchase to the date of sale, not by financial year. Something bought and sold within one month is treated the same way whether or not March fell in between.',
          'The threshold for "long-term" differs from one class of asset to another. This is where guessing costs the most, because it is very easy to apply one category’s rule to another.',
          'If the same holding was bought in pieces at different times, each piece has its own date. On a sale, the units bought first are normally treated as the ones sold first.',
          'The practical meaning is that postponing a sale by a few weeks sometimes moves it into the other category and reduces the tax considerably. On a large sale, this single consideration makes the biggest difference of all.',
          'But the reverse is also true: holding a falling asset for months purely to save tax often costs as much as the tax saved. The date decision should follow the investment decision, not lead it.',
        ],
      },
      {
        hHi: 'नुक़सान की भरपाई और आगे ले जाना',
        hEn: 'Setting off losses and carrying them forward',
        pHi: [
          'पूँजीगत नुक़सान का भी कर में उपयोग होता है, और यह उन कुछ प्रावधानों में है जिन्हें सबसे ज़्यादा लोग नहीं जानते या जानकर भी इस्तेमाल नहीं करते।',
          'सामान्य ढाँचा यह है कि एक ही श्रेणी के लाभ और नुक़सान आपस में समायोजित होते हैं, और कुछ मामलों में एक श्रेणी का नुक़सान दूसरी के लाभ से भी समायोजित हो सकता है। किस दिशा में यह अनुमति है, यह नियमों में स्पष्ट रूप से तय है।',
          'अगर उस साल पर्याप्त लाभ नहीं है, तो बचा हुआ नुक़सान आगे के वर्षों में ले जाया जा सकता है और वहाँ के लाभ से समायोजित किया जा सकता है। यह सुविधा कई वर्षों तक उपलब्ध रहती है।',
          'पर इसकी एक शर्त है जो सबसे ज़्यादा तोड़ी जाती है: नुक़सान को आगे ले जाने के लिए उस वर्ष का रिटर्न नियत तारीख़ तक भरना ज़रूरी है। देर से भरा गया रिटर्न यह अधिकार समाप्त कर देता है।',
          'यही कारण है कि जिस साल भारी नुक़सान हुआ हो, उस साल रिटर्न भरना और भी ज़रूरी हो जाता है — भले ही कर देना न बनता हो। बहुत से लोग "कर नहीं बनता तो रिटर्न क्यों" सोचकर यह अधिकार गँवा देते हैं।',
          'नुक़सान का रिकॉर्ड भी साल-दर-साल सँभालना पड़ता है, क्योंकि जिस साल आप उसे इस्तेमाल करेंगे उस साल आपको दिखाना होगा कि वह कहाँ से आया था।',
        ],
        pEn: [
          'Capital losses have a use in tax too, and this is among the provisions that most people either do not know about or know about and never use.',
          'The general structure is that gains and losses within the same category are set off against each other, and in some cases a loss in one category may be set off against a gain in another. Which direction is permitted is set out clearly in the rules.',
          'If there is not enough gain in that year, the remaining loss can be carried forward into later years and set off against gains there. That facility remains available for several years.',
          'But it carries a condition that is broken more often than any other: to carry a loss forward, that year’s return must be filed by the due date. A late return extinguishes the right.',
          'This is why filing a return matters even more in a year of heavy losses — even when no tax is payable. Many people think "no tax, so why file" and lose the entitlement.',
          'The record of the loss has to be maintained year after year as well, because in the year you finally use it you will have to show where it came from.',
        ],
      },
      {
        hHi: 'बिक्री से पहले की तैयारी',
        hEn: 'Preparing before a sale',
        pHi: [
          'बड़ी बिक्री से पहले की एक शाम अक्सर बाद के कई महीनों की परेशानी बचा लेती है। तैयारी में केवल चार काम हैं और उनमें से कोई भी कठिन नहीं।',
          'पहला: सारे ख़रीद दस्तावेज़ एक जगह इकट्ठा कीजिए — बिल, अनुबंध, भुगतान का रिकॉर्ड, और सुधार पर हुए ख़र्च के काग़ज़। जो नहीं मिल रहे उन्हें अभी खोजिए, बिक्री के बाद नहीं।',
          'दूसरा: लाभ का मोटा अनुमान लगाइए और उस पर संभावित कर की राशि निकालिए। यह संख्या पहले से जानना इसलिए ज़रूरी है क्योंकि बिक्री की पूरी रक़म आपकी नहीं होती, और लोग अक्सर उसी दिन उसे किसी और जगह लगा देते हैं।',
          'तीसरा: देखिए कि क्या अग्रिम कर की किस्त लागू होगी। बड़ी बिक्री उस तिमाही के अग्रिम कर को बदल देती है, और चूकने पर ब्याज लगता है।',
          'चौथा: अगर बिक्री के बाद उसी श्रेणी में दोबारा निवेश करने की सोच रहे हैं, तो नियमों में उपलब्ध छूटों की समय-सीमाएँ पहले पढ़ लीजिए। ये समय-सीमाएँ सख़्त होती हैं और चूक जाने पर वापस नहीं आतीं।',
          'यह लेख सामान्य जानकारी है और आपकी परिस्थिति पर लागू नियम अलग हो सकते हैं; इसलिए बड़ी बिक्री से पहले एक बार किसी योग्य कर सलाहकार से बात कर लेना उचित है।',
        ],
        pEn: [
          'One evening of preparation before a large sale usually saves several months of trouble afterwards. There are only four tasks and none of them is difficult.',
          'First: gather every purchase document in one place — bills, agreements, payment records, and the papers for money spent on improvement. Look now for whatever is missing, not after the sale.',
          'Second: make a rough estimate of the gain and work out the likely tax on it. Knowing that figure in advance matters because the whole sale amount is not yours, and people commonly commit it elsewhere on the same day.',
          'Third: check whether an advance-tax instalment will apply. A large sale changes that quarter’s advance tax, and missing it attracts interest.',
          'Fourth: if you are considering reinvesting in the same class after the sale, read the time limits attached to the available exemptions first. Those limits are strict and do not come back once missed.',
          'This article is general information and the rules applying to your own circumstances may differ; before a large sale it is sensible to speak once to a qualified tax adviser.',
        ],
      },
      {
        hHi: 'रिकॉर्ड रखने की एक सरल व्यवस्था',
        hEn: 'A simple system for keeping records',
        pHi: [
          'पूँजीगत लाभ का सारा गणित दस्तावेज़ों पर टिका है, और दस्तावेज़ वहीं टूटते हैं जहाँ कोई व्यवस्था नहीं होती। एक साधारण व्यवस्था कई वर्षों तक काम करती है।',
          'हर परिसंपत्ति के लिए एक फ़ोल्डर बनाइए, फ़ोल्डर का नाम परिसंपत्ति और ख़रीद वर्ष के साथ रखिए। उसमें ख़रीद के काग़ज़, भुगतान का प्रमाण, और उसके बाद हर बड़ा ख़र्च क्रम से जोड़ते जाइए।',
          'एक अलग सूची भी रखिए जिसमें हर परिसंपत्ति के सामने ख़रीद की तारीख़, राशि और मात्रा लिखी हो। यह एक पन्ना बेचते समय सबसे काम आता है, क्योंकि तब सारी तारीख़ें एक साथ सामने होती हैं।',
          'डिजिटल प्रति ज़रूर रखिए, पर केवल डिजिटल पर निर्भर मत रहिए। पुराने ईमेल खाते बंद हो जाते हैं और ब्रोकर बदल जाते हैं; जो फ़ाइल आपके अपने पास है वही दस साल बाद मिलेगी।',
          'हर साल रिटर्न भरते समय उस साल की पूरी लेन-देन विवरणी डाउनलोड करके इसी फ़ोल्डर में रख दीजिए। यह पाँच मिनट का काम है और यह वह इकलौती चीज़ है जो बाद में सबसे ज़्यादा काम आती है।',
          'और जब कोई परिसंपत्ति बिक जाए, तो फ़ोल्डर फेंकिए मत। बिक्री के बाद भी उन काग़ज़ों की ज़रूरत कई वर्षों तक पड़ सकती है, ख़ासकर अगर उस साल का नुक़सान आगे ले जाया गया हो।',
        ],
        pEn: [
          'The whole arithmetic of capital gains rests on documents, and documents fail wherever there is no system. A very simple system holds up for years.',
          'Keep one folder per asset, named for the asset and the year of purchase. Put the purchase papers and proof of payment in it, and add every significant expense after that in order.',
          'Keep a separate list as well, showing the purchase date, amount and quantity against each asset. That single page is what helps most at the time of selling, because every date is then in front of you at once.',
          'Do keep digital copies, but do not depend on digital alone. Old email accounts get closed and brokers change; the file you hold yourself is the one that will still be there in ten years.',
          'Each year while filing the return, download the full transaction statement for that year and drop it into the same folder. It is five minutes of work and it is the single thing that helps most later.',
          'And when an asset is sold, do not throw the folder away. Those papers can be needed for several years afterwards, particularly if a loss from that year has been carried forward.',
        ],
      },
    ],
  },
  {
    slug: 'form-26as-and-ais',
    sections: [
      {
        hHi: 'दोनों दस्तावेज़ अलग क्यों हैं',
        hEn: 'Why the two documents are different',
        pHi: [
          'बहुत से लोग इन्हें एक ही चीज़ के दो नाम समझते हैं। ऐसा नहीं है, और अंतर समझ लेने से रिटर्न भरना काफ़ी आसान हो जाता है।',
          'फ़ॉर्म 26AS मुख्य रूप से कर का लेखा-जोखा है — आपकी आय पर किसने कितना कर काटा, आपने ख़ुद कितना जमा किया, और वह सब सरकार के खाते में पहुँचा या नहीं।',
          'वार्षिक सूचना विवरण, यानी एआईएस, इससे कहीं ज़्यादा विस्तृत है। उसमें केवल कटा हुआ कर नहीं, बल्कि वे लेन-देन भी दिखते हैं जिनकी सूचना विभिन्न संस्थाओं ने विभाग को दी है — ब्याज, लाभांश, प्रतिभूतियों की ख़रीद-बिक्री, बड़ी जमा, और कुछ बड़े ख़र्च।',
          'इसलिए 26AS छोटा और साफ़ होता है, जबकि एआईएस लंबा होता है और उसमें दोहराव भी दिख सकता है — क्योंकि एक ही लेन-देन दो अलग स्रोतों से सूचित हो सकता है।',
          'रिटर्न भरने से पहले दोनों देखने चाहिए। 26AS यह बताता है कि आपके नाम कितना कर पहले से जमा है; एआईएस यह बताता है कि विभाग आपकी आय के बारे में क्या-क्या जानता है।',
          'इन दोनों को न देखकर रिटर्न भरना अंधेरे में तीर चलाने जैसा है। जो बेमेल आप आज दस मिनट में सुधार सकते थे, वही बाद में एक नोटिस बनकर लौटता है।',
        ],
        pEn: [
          'Many people treat these as two names for the same thing. They are not, and understanding the difference makes filing a return considerably easier.',
          'Form 26AS is primarily a tax ledger — who deducted how much tax on your income, how much you paid yourself, and whether all of it reached the government’s account.',
          'The Annual Information Statement, the AIS, is far broader. It shows not only tax deducted but also the transactions various institutions have reported about you — interest, dividends, purchases and sales of securities, large deposits and certain large expenses.',
          'So 26AS is short and clean, while the AIS is long and may even show duplication, because the same transaction can be reported by two different sources.',
          'Both should be looked at before filing. 26AS tells you how much tax already stands to your credit; the AIS tells you what the department knows about your income.',
          'Filing without reading either is shooting in the dark. A mismatch you could have corrected in ten minutes today comes back later as a notice.',
        ],
      },
      {
        hHi: 'बेमेल कहाँ-कहाँ से आते हैं',
        hEn: 'Where mismatches come from',
        pHi: [
          'अधिकांश बेमेल बेईमानी से नहीं, साधारण चूक से पैदा होते हैं। यह जान लेना राहत देता है, क्योंकि तब उन्हें ठीक करना एक प्रक्रिया बन जाती है, घबराहट नहीं।',
          'सबसे आम कारण है ग़लत पैन। किसी बैंक या नियोक्ता ने कर काटते समय पैन में एक अक्षर ग़लत दर्ज कर दिया, और वह कर आपके खाते में दिखा ही नहीं।',
          'दूसरा कारण है समय। कर काटा गया मार्च में, पर उसकी विवरणी दाख़िल हुई अप्रैल या मई में। जो व्यक्ति अप्रैल में 26AS देखता है उसे वह प्रविष्टि नहीं मिलती, और वह मान लेता है कि कर कटा ही नहीं।',
          'तीसरा, संयुक्त खातों का। संयुक्त खाते का पूरा ब्याज कभी-कभी दोनों धारकों के एआईएस में पूरा-पूरा दिखता है, जबकि असल में उसे हिस्सों में बाँटा जाना है।',
          'चौथा, वे लेन-देन जो आपके नाम पर हुए ही नहीं — पैन के दुरुपयोग या किसी संस्था की ग़लत प्रविष्टि के कारण। यह दुर्लभ है, पर होता है, और इसे नज़रअंदाज़ करना सबसे ख़तरनाक है।',
          'पाँचवाँ, वही आय दो स्रोतों से सूचित होना, जिससे एआईएस में वह दोगुनी दिखती है। यह सबसे आम है और आमतौर पर सबसे आसानी से ठीक होता है।',
        ],
        pEn: [
          'Most mismatches arise from ordinary slips rather than dishonesty. Knowing that is a relief, because it turns the problem into a procedure instead of a panic.',
          'The commonest cause is a wrong PAN. A bank or an employer recorded one character incorrectly while deducting tax, and that tax never appeared against your name at all.',
          'The second cause is timing. Tax was deducted in March but the statement for it was filed in April or May. Somebody checking 26AS in April does not find the entry and concludes no tax was deducted.',
          'The third involves joint accounts. The full interest on a joint account sometimes appears in full in both holders’ statements, when in reality it has to be divided between them.',
          'The fourth is transactions that were never yours at all — through misuse of a PAN or a wrong entry by an institution. This is rare, but it happens, and ignoring it is the most dangerous option.',
          'The fifth is the same income being reported by two sources, so the AIS shows it twice. This is the most common of all and usually the easiest to resolve.',
        ],
      },
      {
        hHi: 'प्रतिक्रिया कैसे दर्ज करें',
        hEn: 'How to record a response',
        pHi: [
          'एआईएस की सबसे उपयोगी सुविधा यह है कि हर प्रविष्टि पर आप अपनी प्रतिक्रिया दर्ज कर सकते हैं। यह विकल्प मौजूद है और बहुत कम लोग उसका उपयोग करते हैं।',
          'विकल्प सीधे हैं: सूचना सही है; आंशिक रूप से सही है; यह मेरी नहीं है; यह दोहराव है; या यह किसी और व्यक्ति से संबंधित है। हर प्रविष्टि के लिए अलग प्रतिक्रिया दी जा सकती है।',
          'प्रतिक्रिया दर्ज करने से प्रविष्टि अपने-आप हट नहीं जाती, पर वह रिकॉर्ड में आ जाती है, और यही रिकॉर्ड बाद में आपके पक्ष का पहला दस्तावेज़ बनता है।',
          'जहाँ प्रविष्टि सचमुच ग़लत है, वहाँ प्रतिक्रिया के साथ-साथ स्रोत को भी सुधारने के लिए कहना चाहिए — बैंक, नियोक्ता या जिस भी संस्था ने वह सूचना भेजी है। स्रोत सुधरे बिना अगली बार वही प्रविष्टि दोबारा आ जाएगी।',
          'और जहाँ प्रविष्टि सही है पर आपने उसे अपने हिसाब में शामिल नहीं किया था, वहाँ सही रास्ता प्रतिक्रिया नहीं, बल्कि अपने रिटर्न में उसे शामिल करना है। एआईएस का उद्देश्य यही है कि छूट गई आय याद आ जाए।',
          'यह पूरा काम रिटर्न भरने से पहले होना चाहिए, बाद में नहीं। रिटर्न भरने के बाद दर्ज की गई प्रतिक्रिया उस साल की गड़बड़ी को नहीं सुधारती।',
        ],
        pEn: [
          'The most useful feature of the AIS is that you can record a response against every entry. The option is there and very few people use it.',
          'The choices are plain: the information is correct; it is partly correct; it is not mine; it is a duplicate; or it relates to another person. A separate response can be given for each entry.',
          'Recording a response does not delete the entry by itself, but it goes on record, and that record becomes the first document on your side later.',
          'Where an entry is genuinely wrong, ask the source to correct it as well as recording the response — the bank, the employer, or whichever institution reported it. Without fixing the source the same entry returns next year.',
          'And where an entry is correct but you had left it out of your own working, the right course is not a response but including it in your return. Reminding you of missed income is exactly what the AIS is for.',
          'All of this belongs before filing, not after. A response recorded after the return is filed does not repair that year’s problem.',
        ],
      },
      {
        hHi: 'रिटर्न भरने से पहले की जाँच-सूची',
        hEn: 'A checklist before filing',
        pHi: [
          'यह पूरा विषय एक छोटी सूची में समा जाता है, और वह सूची हर साल वही रहती है। एक बार बना लीजिए और हर जून में दोहरा दीजिए।',
          'पहले 26AS डाउनलोड कीजिए और उसमें कटे हुए कर का कुल जोड़ अपने फ़ॉर्म 16 और अन्य प्रमाणपत्रों से मिलाइए। जो अंतर मिले, उसे लिख लीजिए।',
          'फिर एआईएस डाउनलोड कीजिए और उसे श्रेणीवार पढ़िए — वेतन, ब्याज, लाभांश, प्रतिभूतियाँ, संपत्ति। हर श्रेणी में एक ही सवाल पूछिए: क्या यह संख्या मेरे अपने रिकॉर्ड से मेल खाती है?',
          'तीसरा, बैंक विवरणों से साल भर का ब्याज ख़ुद जोड़िए। बचत खाते का छोटा ब्याज सबसे ज़्यादा भुलाया जाता है और एआईएस में वह दिखता है।',
          'चौथा, जो प्रविष्टियाँ ग़लत हैं उन पर प्रतिक्रिया दर्ज कीजिए और स्रोत को सूचित कीजिए। पाँचवाँ, जो आय छूट गई थी उसे अपने हिसाब में जोड़िए।',
          'और अंत में, इन दोनों दस्तावेज़ों की प्रतियाँ उस साल के फ़ोल्डर में रख दीजिए। जिस दिन कोई सवाल आएगा, उस दिन यही फ़ोल्डर सबसे पहले खोला जाएगा।',
        ],
        pEn: [
          'The whole subject fits into a short checklist, and the checklist stays the same every year. Make it once and repeat it each June.',
          'Download 26AS first and match the total tax deducted in it against your Form 16 and other certificates. Write down whatever difference appears.',
          'Then download the AIS and read it category by category — salary, interest, dividends, securities, property. In each category ask the one question: does this figure agree with my own record?',
          'Third, add up the year’s interest yourself from the bank statements. Small savings-account interest is the most forgotten item of all, and it shows up in the AIS.',
          'Fourth, record a response against the entries that are wrong and inform the source. Fifth, add the income you had missed into your own working.',
          'And finally, put copies of both documents into that year’s folder. On the day a question arrives, that folder is the first thing that will be opened.',
        ],
      },
      {
        hHi: 'अगर नोटिस आ ही जाए',
        hEn: 'If a notice does arrive',
        pHi: [
          'नोटिस आना अपने-आप में आरोप नहीं है। अधिकांश नोटिस केवल यह कहते हैं कि दो आँकड़े आपस में मेल नहीं खा रहे, और उनका जवाब देना एक सामान्य प्रक्रिया है।',
          'पहला क़दम है नोटिस को शांति से पढ़ना और यह देखना कि वह किस धारा के तहत है, किस वर्ष का है, और जवाब की अंतिम तारीख़ क्या है। यही तीन बातें आगे का पूरा रास्ता तय करती हैं।',
          'दूसरा क़दम है नोटिस में लिखी संख्या को अपने रिकॉर्ड से मिलाना। अक्सर पता चलता है कि अंतर किसी एक प्रविष्टि से आ रहा है, और उसका स्पष्टीकरण एक दस्तावेज़ से हो जाता है।',
          'अगर विभाग सही है और आय सचमुच छूट गई थी, तो सही रास्ता उसे स्वीकार करके कर और लागू ब्याज चुका देना है। जितनी जल्दी यह हो, उतना कम ब्याज लगता है।',
          'अगर आप सही हैं, तो जवाब में दस्तावेज़ लगाइए — बैंक विवरण, प्रमाणपत्र, अनुबंध — और संक्षेप में लिखिए कि अंतर क्यों है। लंबे पत्र की ज़रूरत नहीं; दस्तावेज़ ही तर्क होते हैं।',
          'और जहाँ राशि बड़ी हो या मामला जटिल, वहाँ किसी योग्य कर सलाहकार की मदद लेना समझदारी है। नोटिस का जवाब समय पर देना उसकी विषय-वस्तु से भी ज़्यादा महत्वपूर्ण है।',
        ],
        pEn: [
          'A notice is not an accusation in itself. Most notices only say that two figures do not agree, and answering one is an ordinary procedure.',
          'The first step is to read it calmly and note which section it is under, which year it concerns, and the last date for a reply. Those three facts shape everything that follows.',
          'The second step is to match the figure in the notice against your own records. It usually turns out that the difference comes from a single entry, and one document explains it.',
          'If the department is right and income really was left out, the correct course is to accept it and pay the tax with whatever interest applies. The sooner that happens, the less the interest.',
          'If you are right, attach the documents to your reply — bank statements, certificates, agreements — and state briefly why the difference exists. A long letter is not needed; the documents are the argument.',
          'And where the amount is large or the matter complicated, taking help from a qualified tax adviser is sensible. Replying on time matters even more than what the reply contains.',
        ],
      },
    ],
  },
  {
    slug: 'form-26as-and-ais',
    sections: [
      {
        hHi: 'एक बात जो याद रखने लायक़ है',
        hEn: 'One point worth remembering',
        pHi: [
          'इन दोनों दस्तावेज़ों को विभाग की निगरानी के रूप में देखना स्वाभाविक है, पर व्यवहार में ये करदाता के अपने काम की चीज़ें हैं। ये आपको वह याद दिला देते हैं जो आप भूल चुके थे।',
          'साल भर में कितने खातों से ब्याज आया, कौन सा पुराना निवेश अब भी चल रहा है, किस संस्था ने आपके नाम पर क्या दर्ज किया — यह सब एक जगह और कहीं नहीं मिलता।',
          'इसलिए इन्हें साल में एक बार, रिटर्न से पहले, इत्मीनान से पढ़िए। यह जाँच नहीं, अपनी ही वित्तीय स्थिति की एक साफ़ तस्वीर है।',
        ],
        pEn: [
          'It is natural to see these two documents as the department watching you, but in practice they are tools for the taxpayer. They remind you of what you had forgotten.',
          'How many accounts paid interest during the year, which old investment is still running, what each institution has recorded against your name — there is nowhere else this appears together in one place.',
          'So read them unhurriedly once a year, before filing. It is not an examination; it is a clear picture of your own financial position.',
        ],
      },
    ],
  },
  {
    slug: 'insurance-vs-investment',
    sections: [
      {
        hHi: 'मिश्रित उत्पाद बिकते क्यों हैं',
        hEn: 'Why mixed products get sold',
        pHi: [
          'अगर बीमा और निवेश को अलग रखना बेहतर है, तो सवाल उठता है कि फिर मिश्रित उत्पाद इतने ज़्यादा क्यों बिकते हैं। इसका जवाब ख़रीदार में नहीं, व्यवस्था में है।',
          'पहला कारण प्रोत्साहन है। सादे सुरक्षा-बीमा पर बेचने वाले को बहुत कम कमीशन मिलता है, क्योंकि उसका प्रीमियम कम होता है। मिश्रित उत्पाद पर प्रीमियम कई गुना होता है, और कमीशन भी।',
          'दूसरा कारण भाषा है। "पैसा वापस मिलेगा" सुनने में "पैसा डूब जाएगा अगर कुछ न हुआ" से कहीं बेहतर लगता है, हालाँकि सुरक्षा-बीमा में पैसा डूबता नहीं — वह जोख़िम की क़ीमत होती है, ठीक वैसे ही जैसे गाड़ी के बीमे में।',
          'तीसरा कारण भरोसे का है। ये उत्पाद अक्सर उसी बैंक या उसी परिचित व्यक्ति के ज़रिए आते हैं जिस पर परिवार वर्षों से भरोसा करता आया है, और उस भरोसे के सामने कोई तुलना नहीं की जाती।',
          'चौथा कारण समय है। अधिकांश बिक्री मार्च में होती है, जब सोचने का समय नहीं होता और कर बचाना ही एकमात्र लक्ष्य दिखता है।',
          'यह समझना दोषारोपण के लिए नहीं है। यह इसलिए ज़रूरी है क्योंकि जो ख़रीदार जानता है कि दबाव कहाँ से आ रहा है, वह उस दबाव में अच्छे फ़ैसले ले पाता है।',
        ],
        pEn: [
          'If keeping insurance and investment apart is better, the question arises why mixed products sell so heavily. The answer lies in the system rather than in the buyer.',
          'The first reason is incentives. Plain protection insurance pays the seller very little commission, because its premium is small. A mixed product carries a premium several times larger, and commission to match.',
          'The second is language. "You get your money back" sounds far better than "the money is gone if nothing happens", although nothing is lost in protection cover — the premium is the price of the risk, exactly as it is in motor insurance.',
          'The third is trust. These products usually arrive through the same bank or the same familiar person the family has relied on for years, and in the face of that trust no comparison gets made.',
          'The fourth is timing. Most of the selling happens in March, when there is no time to think and saving tax looks like the only objective.',
          'Understanding this is not about blaming anyone. It matters because a buyer who knows where the pressure is coming from can make good decisions under it.',
        ],
      },
      {
        hHi: 'कवर की राशि कैसे तय होती है',
        hEn: 'How the amount of cover is decided',
        pHi: [
          'बीमा का असली सवाल यह नहीं है कि कौन सी पॉलिसी लें, बल्कि यह है कि कितने का कवर लें। ग़लत राशि वाली सही पॉलिसी भी परिवार के काम नहीं आती।',
          'एक सरल शुरुआत यह है: परिवार का सालाना ख़र्च निकालिए, उसे उन वर्षों से गुणा कीजिए जब तक परिवार को सहारे की ज़रूरत है, और उसमें बचा हुआ क़र्ज़ जोड़ दीजिए।',
          'फिर उसमें से वह घटाइए जो पहले से मौजूद है — बचत, निवेश, नियोक्ता का समूह बीमा। जो शेष बचे, वही वह राशि है जिसका बीमा चाहिए।',
          'यहाँ बड़े बच्चों की शिक्षा और किसी बुज़ुर्ग की देखभाल जैसी बड़ी भविष्य की ज़िम्मेदारियाँ भी जोड़नी चाहिए, क्योंकि वे रोज़मर्रा के ख़र्च में नहीं दिखतीं।',
          'नियोक्ता के समूह बीमा पर पूरी तरह निर्भर रहना एक आम भूल है। वह नौकरी के साथ ख़त्म हो जाता है, और नौकरी बदलने या छूटने का समय अक्सर वही होता है जब सुरक्षा की सबसे ज़्यादा ज़रूरत होती है।',
          'और कवर की राशि हर बड़े बदलाव पर दोबारा देखनी चाहिए — विवाह, बच्चे का जन्म, गृह ऋण, या आय में बड़ा बदलाव। जो राशि दस साल पहले पर्याप्त थी, वह आज अक्सर आधी पड़ती है।',
        ],
        pEn: [
          'The real question in insurance is not which policy to buy but how much cover to take. Even the right policy for the wrong amount does not help the family.',
          'A simple starting point: work out the family’s annual expenses, multiply by the number of years the family would need support, and add whatever debt is outstanding.',
          'Then subtract what already exists — savings, investments, the employer’s group cover. What remains is the amount that needs insuring.',
          'Large future responsibilities such as children’s education or the care of an elderly parent belong in this sum too, because they do not show up in everyday expenses.',
          'Depending entirely on the employer’s group cover is a common error. It ends with the job, and the time of changing or losing a job is often exactly when protection is needed most.',
          'And the amount should be revisited at every large change — marriage, a birth, a home loan, or a significant change in income. A figure that was adequate ten years ago is usually half of what is needed today.',
        ],
      },
      {
        hHi: 'पहले से चल रही मिश्रित पॉलिसी का क्या करें',
        hEn: 'What to do with a mixed policy already running',
        pHi: [
          'यह वह सवाल है जो लेख पढ़ने के बाद सबसे पहले उठता है, और इसका जवाब "तुरंत बंद कर दीजिए" नहीं है। जल्दबाज़ी में बंद करना अक्सर पहले से हुए नुक़सान को बढ़ा देता है।',
          'पहले तीन तथ्य निकालिए: अब तक कुल कितना प्रीमियम दिया जा चुका है, आज सरेंडर करने पर कितना मिलेगा, और पॉलिसी पूरी करने पर अनुमानित रूप से कितना मिलेगा।',
          'अगर पॉलिसी को शुरू हुए बहुत कम समय हुआ है, तो सरेंडर मूल्य आमतौर पर बहुत कम या शून्य होता है। अगर वह लगभग पूरी हो चुकी है, तो उसे चलाकर पूरा कर लेना अक्सर बेहतर होता है।',
          'बीच की स्थिति सबसे कठिन है। वहाँ एक तीसरा रास्ता भी होता है: पॉलिसी को "पेड-अप" कर देना, यानी आगे प्रीमियम न देना पर पॉलिसी को समाप्त भी न करना। कवर घट जाता है, पर जो दिया जा चुका है वह पूरी तरह नहीं जाता।',
          'जो भी फ़ैसला हो, एक क्रम ज़रूरी है: पहले पर्याप्त सुरक्षा-बीमा और स्वास्थ्य बीमा ले लीजिए, और उसके चालू हो जाने के बाद ही पुरानी पॉलिसी के बारे में कोई क़दम उठाइए।',
          'सबसे बुरा परिणाम यह होता है कि पुरानी पॉलिसी बंद कर दी जाए और नई सुरक्षा लेने में देर हो जाए, और उसी बीच में परिवार बिना किसी कवर के रह जाए।',
        ],
        pEn: [
          'This is the first question that arises after reading such an article, and the answer is not "close it at once". Closing in haste usually adds to the loss already incurred.',
          'Establish three facts first: how much premium has been paid so far, what surrender today would pay, and what completing the policy is projected to pay.',
          'If the policy started very recently, the surrender value is usually tiny or nil. If it is nearly complete, running it to the end is often the better course.',
          'The middle case is the hardest. There a third route exists: making the policy "paid-up", which means paying no further premium without terminating it. The cover falls, but what has been paid is not entirely lost.',
          'Whatever the decision, one order matters: put adequate protection cover and health cover in place first, and only once those are active take any step on the old policy.',
          'The worst outcome of all is closing the old policy, being delayed in taking the new protection, and leaving the family with no cover at all in between.',
        ],
      },
      {
        hHi: 'स्वास्थ्य बीमा अलग विषय है',
        hEn: 'Health insurance is a separate subject',
        pHi: [
          'जीवन बीमा और निवेश को अलग रखने की बात करते हुए स्वास्थ्य बीमा अक्सर चर्चा से बाहर रह जाता है, जबकि भारतीय परिवारों की बचत सबसे ज़्यादा वहीं ख़त्म होती है।',
          'एक बड़ा अस्पताल का बिल वर्षों की बचत एक हफ़्ते में मिटा सकता है, और यह जीवन बीमा से नहीं बचता। जीवन बीमा मृत्यु पर काम आता है; अस्पताल का ख़र्च जीते-जी आता है।',
          'यहाँ भी वही सिद्धांत लागू होता है: सुरक्षा को निवेश से मत मिलाइए। सादा स्वास्थ्य बीमा, पर्याप्त राशि का, उस उत्पाद से बेहतर है जो कवर के साथ पैसा लौटाने का वादा करता है।',
          'नियोक्ता का स्वास्थ्य कवर उपयोगी है पर पर्याप्त नहीं। वह नौकरी के साथ ख़त्म होता है, उसकी राशि अक्सर छोटी होती है, और उसमें माता-पिता शामिल हों यह ज़रूरी नहीं।',
          'अपनी अलग पॉलिसी जितनी कम उम्र में ली जाए उतना अच्छा, क्योंकि प्रतीक्षा अवधि तभी से चलनी शुरू होती है। जो व्यक्ति बीमारी का पता चलने के बाद पॉलिसी लेने जाता है, उसे वही बीमारी अक्सर कवर में नहीं मिलती।',
          'पॉलिसी लेते समय पूरी और सच्ची जानकारी देना यहाँ और भी ज़रूरी है, क्योंकि दावा अस्वीकार होने का सबसे बड़ा कारण छिपाई गई जानकारी होती है, न कि पॉलिसी की शर्तें।',
        ],
        pEn: [
          'While discussing the separation of life insurance and investment, health insurance often stays out of the conversation — although this is where Indian families’ savings most often disappear.',
          'One large hospital bill can erase years of saving in a week, and life insurance does not prevent that. Life cover pays on death; hospital costs arrive while you are alive.',
          'The same principle applies here: do not mix protection with investment. Plain health cover of an adequate amount is better than a product promising cover along with money back.',
          'The employer’s health cover is useful but not sufficient. It ends with the job, the sum is often small, and it does not necessarily include parents.',
          'Take your own separate policy as young as possible, because the waiting periods begin from then. Somebody who goes to buy cover after a diagnosis usually finds that condition excluded.',
          'Giving complete and truthful information at the time of buying matters even more here, because the biggest single cause of rejected claims is concealed information, not the terms of the policy.',
        ],
      },
      {
        hHi: 'एक सादा ढाँचा जिससे शुरुआत हो',
        hEn: 'A plain structure to start from',
        pHi: [
          'पूरे विषय को चार पंक्तियों में रखा जा सकता है, और अधिकांश परिवारों के लिए इतना ही काफ़ी है। जटिलता तब जोड़िए जब उसकी वजह हो, पहले से नहीं।',
          'पहली पंक्ति: पर्याप्त राशि का सुरक्षा-बीमा, तब तक के लिए जब तक परिवार आर्थिक रूप से आप पर निर्भर है। यह सबसे सस्ता और सबसे ज़रूरी हिस्सा है।',
          'दूसरी: परिवार के लिए स्वास्थ्य बीमा, अलग से, और जहाँ ज़रूरत हो वहाँ माता-पिता के लिए भी अलग।',
          'तीसरी: आपातकालीन कोष, तीन से छह महीने के ख़र्च के बराबर, ऐसी जगह जहाँ से वह उसी दिन निकल सके।',
          'चौथी: निवेश, लक्ष्य के हिसाब से चुना हुआ, बीमा से पूरी तरह अलग रखा हुआ, और इतना सरल कि आप उसे किसी और को दो मिनट में समझा सकें।',
          'इन चारों में से पहली तीन पूरी हुए बिना चौथी पर बहुत ध्यान देना उल्टा क्रम है। एक अच्छा निवेश एक अच्छे बीमे की जगह नहीं ले सकता, और एक बड़ा बीमा एक अच्छे निवेश की जगह नहीं ले सकता।',
        ],
        pEn: [
          'The whole subject fits into four lines, and for most families that is enough. Add complexity when there is a reason for it, not in advance.',
          'First line: protection cover of an adequate amount, for as long as the family depends on you financially. This is the cheapest and the most necessary part.',
          'Second: health cover for the family, taken separately, and separately again for parents where that is needed.',
          'Third: an emergency fund equal to three to six months of expenses, kept somewhere it can be withdrawn the same day.',
          'Fourth: investments, chosen for the goal, kept entirely apart from insurance, and simple enough that you could explain them to somebody else in two minutes.',
          'Paying great attention to the fourth before the first three exist is the wrong order. A good investment cannot substitute for good cover, and a large policy cannot substitute for good investing.',
        ],
      },
    ],
  },
  {
    slug: 'motor-insurance-basics',
    sections: [
      {
        hHi: 'आईडीवी क्या है और वह क्यों मायने रखती है',
        hEn: 'What IDV is and why it matters',
        pHi: [
          'गाड़ी के बीमे में सबसे कम समझा जाने वाला शब्द आईडीवी है — बीमित घोषित मूल्य। यह वह राशि है जो गाड़ी के पूरी तरह नष्ट हो जाने या चोरी हो जाने पर अधिकतम मिल सकती है।',
          'यह गाड़ी का बाज़ार मूल्य नहीं है और न ही वह क़ीमत जो आपने चुकाई थी। यह निर्माता के सूचीबद्ध मूल्य में से उम्र के हिसाब से मूल्यह्रास घटाकर निकाली जाती है।',
          'नवीनीकरण के समय आईडीवी घटाने से प्रीमियम कम हो जाता है, और यही कारण है कि बहुत से लोग बिना सोचे उसे घटा देते हैं। इसका असर सामान्य दावों में नहीं दिखता, पर कुल हानि वाले दावे में सीधा दिखता है।',
          'दूसरी तरफ़, आईडीवी को बहुत ऊँचा रखना भी उपयोगी नहीं, क्योंकि भुगतान वास्तविक हानि से अधिक नहीं होता और प्रीमियम बेवजह बढ़ जाता है।',
          'सही तरीक़ा यह है कि आईडीवी उसी के आसपास रखी जाए जितने में उसी उम्र और स्थिति की गाड़ी आज बाज़ार में मिल सकती है। यह एक बार देख लेने का काम है।',
          'नवीनीकरण से पहले पॉलिसी में लिखी आईडीवी ज़रूर पढ़िए। यह एक संख्या है जो पाँच सेकंड में दिखती है और जिसका महत्व केवल उस दिन पता चलता है जिस दिन गाड़ी वापस नहीं आती।',
        ],
        pEn: [
          'The least understood term in motor insurance is IDV — insured declared value. It is the maximum amount payable if the vehicle is destroyed completely or stolen.',
          'It is not the market price of the vehicle and not the price you paid. It is worked out from the manufacturer’s listed price after deducting depreciation for the vehicle’s age.',
          'Lowering the IDV at renewal lowers the premium, which is why many people reduce it without thinking. The effect is invisible in ordinary claims and immediate in a total-loss claim.',
          'On the other side, setting the IDV very high is not useful either, because the payout cannot exceed the actual loss and the premium rises for nothing.',
          'The right approach is to keep the IDV close to what a vehicle of the same age and condition would cost in the market today. It is a one-time check.',
          'Do read the IDV written in the policy before renewing. It is a number visible in five seconds whose importance becomes clear only on the day the vehicle does not come back.',
        ],
      },
      {
        hHi: 'नो-क्लेम बोनस, जो चुपचाप जमा होता है',
        hEn: 'The no-claim bonus, which accumulates quietly',
        pHi: [
          'हर साल जिसमें कोई दावा नहीं किया गया, उसके बदले नवीनीकरण पर छूट मिलती है। यह छूट साल-दर-साल बढ़ती है और कई वर्षों बाद प्रीमियम के एक बड़े हिस्से के बराबर हो जाती है।',
          'इसका एक सीधा नतीजा यह है कि छोटे दावे करना अक्सर घाटे का सौदा होता है। जिस खरोंच की मरम्मत पर थोड़ा ख़र्च आता है, उसका दावा करने से कई वर्षों की जमा छूट एक झटके में चली जाती है।',
          'दावा करने से पहले एक साधारण गणना कीजिए: मरम्मत का ख़र्च, घटाकर वह हिस्सा जो आपको वैसे भी ख़ुद देना है, और उसकी तुलना कीजिए अगले कुछ वर्षों में खोने वाली छूट से।',
          'यह बोनस गाड़ी से नहीं, चालक से जुड़ा होता है। गाड़ी बेचकर नई लेने पर उसे नई पॉलिसी में स्थानांतरित कराया जा सकता है, और यह अधिकार बहुत से लोग जानते ही नहीं।',
          'बीमा कंपनी बदलने पर भी यह बोनस साथ जाता है, बशर्ते पुरानी कंपनी से प्रमाण-पत्र लिया जाए और नवीनीकरण में बहुत देर न हो।',
          'यहीं एक चेतावनी भी है: पॉलिसी समाप्त होने के बाद तय अवधि से ज़्यादा देर हो जाए तो जमा हुआ बोनस समाप्त हो सकता है। नवीनीकरण की तारीख़ फ़ोन में दर्ज करना उसे बचाने का सबसे सस्ता तरीक़ा है।',
        ],
        pEn: [
          'Every year without a claim earns a discount at renewal. That discount grows year on year and after several years amounts to a large part of the premium.',
          'One direct consequence is that small claims are usually a bad bargain. Claiming for a scratch that costs a modest amount to repair wipes out several years of accumulated discount in one stroke.',
          'Before making a claim, do a simple sum: the repair cost, minus the part you have to bear yourself in any case, against the discount you would lose over the next few years.',
          'This bonus attaches to the driver, not to the vehicle. On selling one vehicle and buying another it can be transferred to the new policy, and a great many people do not know they have that right.',
          'It also travels when you change insurers, provided a certificate is taken from the previous company and the renewal is not left too late.',
          'And here is the warning: if the gap after expiry exceeds the permitted period, the accumulated bonus can lapse. Putting the renewal date into your phone is the cheapest way to protect it.',
        ],
      },
      {
        hHi: 'ऐड-ऑन जो सचमुच काम आते हैं',
        hEn: 'The add-ons that genuinely earn their place',
        pHi: [
          'बीमा बेचते समय ऐड-ऑन की लंबी सूची सामने रखी जाती है और सब ज़रूरी लगते हैं। असल में उनमें से कुछ ही अधिकांश लोगों के लिए सार्थक हैं।',
          'शून्य-मूल्यह्रास कवर आमतौर पर सबसे उपयोगी है, ख़ासकर नई गाड़ी में। इसके बिना दावे के समय पुर्ज़ों पर मूल्यह्रास घटा दिया जाता है, और प्लास्टिक तथा रबर के हिस्सों में यह कटौती काफ़ी बड़ी होती है।',
          'इंजन सुरक्षा कवर उन इलाक़ों में सार्थक है जहाँ पानी भरता है, क्योंकि पानी में बंद हुए इंजन का नुक़सान सामान्य पॉलिसी में अक्सर शामिल नहीं होता और मरम्मत महँगी होती है।',
          'सड़क किनारे सहायता कवर सस्ता होता है और उसका मूल्य पैसे में नहीं, रात के दो बजे राजमार्ग पर खड़े होने की स्थिति में समझ आता है।',
          'चालक और सवारियों के लिए व्यक्तिगत दुर्घटना कवर भी देखने योग्य है, हालाँकि अगर आपके पास पहले से पर्याप्त जीवन और स्वास्थ्य बीमा है तो उसका महत्व कम हो जाता है।',
          'बाक़ी ऐड-ऑन को एक ही कसौटी पर परखिए: यह किस स्थिति में काम आएगा, वह स्थिति मेरे साथ कितनी बार आ सकती है, और तब मुझे कितना बचेगा। जिस ऐड-ऑन का जवाब स्पष्ट न हो, वह ज़रूरी नहीं है।',
        ],
        pEn: [
          'A long list of add-ons is put in front of you at the time of buying, and all of them sound essential. In reality only a few are meaningful for most people.',
          'Zero-depreciation cover is usually the most useful, particularly on a new vehicle. Without it, depreciation is deducted from the value of parts at claim time, and on plastic and rubber components that deduction is substantial.',
          'Engine protection cover is worth it in areas that flood, because damage to an engine stopped by water is often excluded from an ordinary policy and the repair is expensive.',
          'Roadside assistance is inexpensive and its value is understood not in money but while standing on a highway at two in the morning.',
          'Personal accident cover for the driver and passengers is worth a look too, although its importance falls if you already hold adequate life and health cover.',
          'Test every other add-on against one question: in what situation will this help, how often can that situation arise for me, and how much would it save then. Any add-on without a clear answer is not necessary.',
        ],
      },
      {
        hHi: 'दुर्घटना के बाद के पहले तीस मिनट',
        hEn: 'The first thirty minutes after an accident',
        pHi: [
          'दावे की सफलता का बड़ा हिस्सा उन तीस मिनटों में तय होता है जो दुर्घटना के तुरंत बाद आते हैं, और वे ठीक वे मिनट होते हैं जब कोई शांत नहीं रह पाता।',
          'पहला काम सुरक्षा है — गाड़ी को सड़क से हटाना अगर संभव हो, चेतावनी रोशनी जलाना, और किसी को चोट लगी हो तो चिकित्सा सहायता। बीमा इसके बाद आता है।',
          'दूसरा काम तस्वीरें हैं। गाड़ी की, दूसरी गाड़ी की, दोनों नंबर प्लेटों की, सड़क की, और आसपास के चिह्नों की। ये तस्वीरें बाद में सबसे मज़बूत दस्तावेज़ बनती हैं और उन्हें दोबारा नहीं लिया जा सकता।',
          'तीसरा, बीमा कंपनी को तुरंत सूचित कीजिए, भले ही आप अभी तय न कर पाए हों कि दावा करना है या नहीं। अधिकांश पॉलिसियों में सूचना की समय-सीमा होती है, और देर से दी गई सूचना दावे को कमज़ोर करती है।',
          'चौथा, जहाँ चोट, तीसरे पक्ष का नुक़सान या चोरी शामिल हो, वहाँ पुलिस को सूचित करना ज़रूरी है। ऐसे मामलों में प्राथमिकी के बिना दावा आगे नहीं बढ़ता।',
          'और पाँचवाँ: घटनास्थल पर कोई समझौता या भुगतान करने से पहले बीमा कंपनी से बात कर लीजिए। मौक़े पर किया गया समझौता बाद में दावे के रास्ते में आ सकता है।',
        ],
        pEn: [
          'A large part of whether a claim succeeds is decided in the thirty minutes immediately after an accident — which are precisely the minutes when nobody is calm.',
          'The first task is safety: move the vehicle off the road if possible, put the hazard lights on, and get medical help for anyone injured. Insurance comes after that.',
          'The second task is photographs. Of your vehicle, the other vehicle, both number plates, the road, and the surrounding landmarks. These become the strongest documents later and cannot be taken again.',
          'Third, inform the insurer straight away, even if you have not yet decided whether to claim. Most policies carry a time limit for notification, and late notice weakens a claim.',
          'Fourth, where injury, third-party damage or theft is involved, informing the police is necessary. In such cases a claim does not move forward without an FIR.',
          'And fifth: speak to the insurer before agreeing to any settlement or making a payment at the scene. An arrangement made on the spot can get in the way of the claim afterwards.',
        ],
      },
      {
        hHi: 'नवीनीकरण के समय क्या देखें',
        hEn: 'What to look at when renewing',
        pHi: [
          'नवीनीकरण अक्सर एक बटन दबाने का काम बन गया है, और यही वह जगह है जहाँ हर साल चुपचाप कवर घटता जाता है। दस मिनट की जाँच पूरे साल के लिए काफ़ी है।',
          'पहला, आईडीवी देखिए और उसकी तुलना पिछले साल से कीजिए। दूसरा, देखिए कि नो-क्लेम बोनस लागू हुआ है या नहीं — यह प्रीमियम की गणना में अलग पंक्ति के रूप में दिखता है।',
          'तीसरा, देखिए कि जो ऐड-ऑन पिछले साल थे वे इस साल भी हैं। नवीनीकरण के सस्ते विकल्प अक्सर उन्हें चुपचाप हटा देते हैं, और अंतर केवल दावे के समय पता चलता है।',
          'चौथा, अपनी ओर से देय हिस्से यानी डिडक्टिबल की राशि जाँचिए। इसे बढ़ाने से प्रीमियम घटता है, पर हर दावे में आपकी जेब से ज़्यादा जाता है।',
          'पाँचवाँ, पॉलिसी में दर्ज पता, फ़ोन नंबर और गाड़ी का विवरण सही है या नहीं। ग़लत विवरण दावे के समय सबसे आम रुकावट है।',
          'और छठा, तारीख़। पॉलिसी एक दिन के लिए भी समाप्त हो जाए तो उस दिन गाड़ी चलाना क़ानूनन ग़लत है, और उस अंतराल में हुई कोई घटना पूरी तरह आपकी अपनी ज़िम्मेदारी बन जाती है।',
        ],
        pEn: [
          'Renewal has largely become a matter of pressing one button, and that is exactly where cover quietly shrinks year after year. Ten minutes of checking is enough for the whole year.',
          'First, look at the IDV and compare it with last year’s. Second, check whether the no-claim bonus has been applied — it appears as a separate line in the premium working.',
          'Third, check that the add-ons you had last year are still there. Cheaper renewal options often drop them silently, and the difference only surfaces at claim time.',
          'Fourth, check the deductible, the part you pay yourself. Raising it lowers the premium, but it takes more out of your pocket on every claim.',
          'Fifth, verify the address, phone number and vehicle details recorded in the policy. Incorrect details are the commonest obstacle when a claim is made.',
          'And sixth, the date. If the policy lapses even for a day, driving on that day is unlawful, and anything that happens in that gap becomes entirely your own responsibility.',
        ],
      },
    ],
  },
  {
    slug: 'insurance-vs-investment',
    sections: [
      {
        hHi: 'एक वाक्य में पूरी बात',
        hEn: 'The whole point in one sentence',
        pHi: [
          'बीमा का काम है उस नुक़सान को सँभालना जिसे आप अकेले नहीं सँभाल सकते; निवेश का काम है समय के साथ पैसे को बढ़ाना। ये दो अलग काम हैं और अच्छे औज़ार आमतौर पर एक ही काम अच्छा करते हैं।',
          'जब दोनों को एक उत्पाद में मिलाया जाता है, तो कवर आमतौर पर ज़रूरत से बहुत कम रह जाता है और रिटर्न आमतौर पर सादे विकल्पों से कम, जबकि प्रतिबद्धता कई वर्षों की हो जाती है।',
          'इसलिए यह लेख किसी उत्पाद के ख़िलाफ़ नहीं है। यह केवल यह कहता है कि हर उत्पाद से एक ही काम माँगिए, और उस काम में उसे तौलिए।',
        ],
        pEn: [
          'The job of insurance is to absorb a loss you could not absorb alone; the job of investing is to grow money over time. These are two different jobs, and good tools usually do one job well.',
          'When both are combined into a single product, the cover typically falls far short of what is needed and the return typically falls short of plain alternatives, while the commitment stretches over many years.',
          'So this article is not against any product. It only says: ask one job of each product, and weigh it on that job.',
        ],
      },
    ],
  },
  {
    slug: 'insurance-claim-process-tips',
    sections: [
      {
        hHi: 'दावा अस्वीकार होने के असली कारण',
        hEn: 'The real reasons claims get rejected',
        pHi: [
          'दावा अस्वीकार होने की कहानियाँ बहुत सुनी जाती हैं और उनसे यह धारणा बन जाती है कि बीमा कंपनियाँ बहाने ढूँढ़ती हैं। आँकड़े कुछ और बताते हैं: अधिकांश अस्वीकृतियाँ कुछ गिनी-चुनी वजहों से होती हैं, और उनमें से ज़्यादातर से बचा जा सकता था।',
          'सबसे बड़ी वजह है प्रस्ताव फ़ॉर्म में अधूरी या ग़लत जानकारी। पुरानी बीमारी, धूम्रपान की आदत, कोई पहले हुई सर्जरी — जो बात उस समय छोटी लगी थी, वही दावे के समय पूरे अनुबंध को कमज़ोर कर देती है।',
          'दूसरी वजह है प्रतीक्षा अवधि। हर स्वास्थ्य पॉलिसी में कुछ स्थितियों के लिए एक निश्चित अवधि होती है जिसके भीतर वे कवर नहीं होतीं। यह छिपाया नहीं जाता; यह पॉलिसी में लिखा होता है और पढ़ा नहीं जाता।',
          'तीसरी वजह है अपवाद। हर पॉलिसी में कुछ चीज़ें साफ़-साफ़ बाहर रखी जाती हैं, और वह सूची आमतौर पर दो पन्नों की होती है। जो व्यक्ति वह सूची एक बार पढ़ लेता है, उसे कभी हैरानी नहीं होती।',
          'चौथी वजह है समय पर सूचना न देना। कई पॉलिसियों में अस्पताल में भर्ती होने या दुर्घटना के बाद एक निश्चित समय के भीतर सूचित करना ज़रूरी होता है।',
          'और पाँचवीं: पॉलिसी का समाप्त हो जाना। जिस दिन प्रीमियम की तारीख़ निकल गई और छूट अवधि भी बीत गई, उस दिन के बाद कोई कवर नहीं रहता, चाहे आपने वर्षों प्रीमियम भरा हो।',
        ],
        pEn: [
          'Stories of rejected claims are widely told, and they create an impression that insurers hunt for excuses. The figures say something else: most rejections come from a handful of causes, and most of those were avoidable.',
          'The largest cause is incomplete or incorrect information in the proposal form. An existing condition, a smoking habit, a surgery long ago — whatever seemed minor then is exactly what weakens the whole contract at claim time.',
          'The second cause is waiting periods. Every health policy sets a period during which certain conditions are not covered. This is not hidden; it is written in the policy and it goes unread.',
          'The third is exclusions. Every policy places some things plainly outside the cover, and that list usually runs to two pages. Anyone who reads it once is never surprised later.',
          'The fourth is failing to notify in time. Many policies require intimation within a set period after hospitalisation or an accident.',
          'And the fifth: a lapsed policy. Once the premium date has passed and the grace period is over, there is no cover from that day, however many years of premium came before.',
        ],
      },
      {
        hHi: 'प्रस्ताव फ़ॉर्म ख़ुद भरिए',
        hEn: 'Fill the proposal form yourself',
        pHi: [
          'यह इस पूरे विषय की सबसे व्यावहारिक सलाह है और सबसे कम मानी जाने वाली। प्रस्ताव फ़ॉर्म वह दस्तावेज़ है जिस पर पूरा अनुबंध टिका है, और उसे अक्सर बेचने वाला भर देता है।',
          'ऐसा सुविधा के लिए होता है और नीयत आमतौर पर बुरी नहीं होती। पर जो व्यक्ति फ़ॉर्म भर रहा है वह आपका चिकित्सा इतिहास नहीं जानता, और वह जल्दी में "नहीं" लिख देता है जहाँ जवाब "हाँ, दस साल पहले" था।',
          'उस एक शब्द का असर वर्षों बाद दिखता है। हस्ताक्षर आपके होते हैं, इसलिए ज़िम्मेदारी भी आपकी होती है, चाहे लिखा किसी और ने हो।',
          'इसलिए हर सवाल ख़ुद पढ़िए और ख़ुद जवाब दीजिए। जहाँ याद न हो, वहाँ अनुमान लगाने के बजाय पुराने दस्तावेज़ देख लीजिए या डॉक्टर से पूछ लीजिए।',
          'और जो जानकारी दी है उसका एक रिकॉर्ड अपने पास रखिए — भरे हुए फ़ॉर्म की एक प्रति। दावे के समय यह प्रति सबसे उपयोगी दस्तावेज़ साबित होती है।',
          'सच बताने से प्रीमियम बढ़ सकता है, और यह असुविधाजनक है। पर बढ़ा हुआ प्रीमियम एक ज्ञात लागत है, जबकि छिपाई गई जानकारी एक अज्ञात जोखिम है जो ठीक उसी दिन सामने आता है जिस दिन पैसे की सबसे ज़्यादा ज़रूरत होती है।',
        ],
        pEn: [
          'This is the most practical advice in the whole subject and the least followed. The proposal form is the document the entire contract rests on, and it is usually filled in by the seller.',
          'That happens for convenience and the intention is generally not bad. But the person filling it does not know your medical history, and in a hurry writes "no" where the answer was "yes, ten years ago".',
          'The effect of that one word appears years later. The signature is yours, so the responsibility is yours too, whoever did the writing.',
          'So read every question yourself and answer it yourself. Where you cannot remember, look at old records or ask a doctor instead of guessing.',
          'And keep a record of what was declared — a copy of the completed form. At claim time that copy turns out to be the most useful document you have.',
          'Telling the truth may raise the premium, and that is uncomfortable. But a higher premium is a known cost, while concealed information is an unknown risk that surfaces on exactly the day the money is needed most.',
        ],
      },
      {
        hHi: 'कैशलेस और प्रतिपूर्ति में अंतर',
        hEn: 'The difference between cashless and reimbursement',
        pHi: [
          'स्वास्थ्य दावे दो रास्तों से निपटते हैं और दोनों की तैयारी अलग है। यह जानना अस्पताल पहुँचने से पहले काम आता है, बाद में नहीं।',
          'कैशलेस में बीमा कंपनी सीधे अस्पताल को भुगतान करती है, पर यह केवल उन अस्पतालों में संभव है जो कंपनी के नेटवर्क में हैं। नियोजित भर्ती के लिए पहले से मंज़ूरी लेनी होती है; आपात स्थिति में भर्ती के तुरंत बाद।',
          'प्रतिपूर्ति में आप पहले ख़ुद भुगतान करते हैं और बाद में दस्तावेज़ जमा करके पैसा वापस लेते हैं। यह किसी भी अस्पताल में संभव है, पर इसके लिए उतनी राशि तुरंत जुटानी पड़ती है।',
          'इसीलिए पर्याप्त स्वास्थ्य बीमा होने के बावजूद एक आपातकालीन कोष ज़रूरी है। बीमा पैसा लौटाता है, पर वह उसी दिन नहीं लौटाता।',
          'नेटवर्क अस्पतालों की सूची समय-समय पर बदलती है। साल में एक बार यह देख लेना कि आपके घर के पास वाला अस्पताल अब भी सूची में है या नहीं, दस मिनट का काम है और आपात स्थिति में बड़ा अंतर लाता है।',
          'कैशलेस में भी सब कुछ कवर नहीं होता। उपभोग्य सामग्री, कुछ शुल्क और कमरे की श्रेणी से जुड़ी सीमाएँ अक्सर आपकी जेब से जाती हैं, और यही वह हिस्सा है जो अंतिम बिल में चौंकाता है।',
        ],
        pEn: [
          'Health claims are settled by two routes, and each needs different preparation. Knowing this before reaching the hospital helps; knowing it afterwards does not.',
          'In a cashless claim the insurer pays the hospital directly, but this is only possible at hospitals inside the insurer’s network. Planned admissions need approval in advance; emergencies need it immediately after admission.',
          'In reimbursement you pay first and get the money back later by submitting documents. It works at any hospital, but it requires you to raise that amount straight away.',
          'This is why an emergency fund is necessary even with adequate health cover. Insurance returns the money, but it does not return it the same day.',
          'The network hospital list changes from time to time. Checking once a year whether the hospital near your home is still on it takes ten minutes and makes a large difference in an emergency.',
          'Not everything is covered even in a cashless claim. Consumables, certain charges and limits tied to the room category usually come out of your own pocket, and that is the part that surprises people in the final bill.',
        ],
      },
      {
        hHi: 'दस्तावेज़ों की सूची जो पहले से तैयार रखी जा सकती है',
        hEn: 'The document list you can keep ready in advance',
        pHi: [
          'दावे के समय दस्तावेज़ जुटाना सबसे थकाने वाला हिस्सा होता है, और वह ठीक तब आता है जब परिवार पहले से परेशान होता है। इसका बड़ा हिस्सा पहले से तैयार रखा जा सकता है।',
          'स्थायी दस्तावेज़: पॉलिसी की प्रति, प्रस्ताव फ़ॉर्म की प्रति, पहचान और पते का प्रमाण, और बैंक खाते का विवरण जिसमें भुगतान आना है। ये सब आज ही एक फ़ोल्डर में रखे जा सकते हैं।',
          'घटना से जुड़े दस्तावेज़ बाद में जुड़ते हैं: अस्पताल का बिल और भुगतान की रसीद, छुट्टी का सारांश, जाँच रिपोर्ट, डॉक्टर का पर्चा, और जहाँ लागू हो वहाँ पुलिस रिपोर्ट।',
          'एक व्यावहारिक आदत: अस्पताल छोड़ने से पहले हर काग़ज़ की एक प्रति माँग लीजिए, और मूल दस्तावेज़ों को क्रम से रखिए। बाद में अस्पताल से दोबारा काग़ज़ लेना कहीं ज़्यादा कठिन होता है।',
          'हर काग़ज़ की तस्वीर भी उसी दिन ले लीजिए। मूल दस्तावेज़ बीमा कंपनी को भेजने पड़ सकते हैं, और तब आपके पास कुछ नहीं बचता।',
          'और परिवार के कम से कम एक और व्यक्ति को यह पता होना चाहिए कि पॉलिसी कहाँ रखी है और उसका नंबर क्या है। जो पॉलिसी परिवार को नहीं मिलती, वह होते हुए भी नहीं होती।',
        ],
        pEn: [
          'Gathering documents is the most exhausting part of a claim, and it arrives precisely when the family is already under strain. A large part of it can be prepared in advance.',
          'Permanent documents: a copy of the policy, a copy of the proposal form, identity and address proof, and the details of the bank account the payment should reach. All of these can go into a folder today.',
          'The event-related documents are added later: hospital bills and payment receipts, the discharge summary, test reports, the doctor’s prescription, and a police report where that applies.',
          'One practical habit: ask for a copy of every paper before leaving the hospital, and keep the originals in order. Getting papers again from a hospital afterwards is considerably harder.',
          'Photograph every paper the same day as well. Originals may have to be sent to the insurer, and then you are left with nothing.',
          'And at least one other person in the family should know where the policy is kept and what its number is. A policy the family cannot find is a policy that does not exist.',
        ],
      },
      {
        hHi: 'अगर दावा अस्वीकार हो जाए',
        hEn: 'If a claim is rejected',
        pHi: [
          'अस्वीकृति अंतिम नहीं होती। इसके आगे एक स्पष्ट रास्ता है, और वह रास्ता मुफ़्त है, पर उसे बहुत कम लोग जानते हैं।',
          'पहला क़दम: अस्वीकृति का कारण लिखित में माँगिए। मौखिक कारण पर आगे कुछ नहीं बढ़ता। लिखित कारण मिलते ही यह स्पष्ट हो जाता है कि मामला तथ्य का है या व्याख्या का।',
          'दूसरा: उसी बीमा कंपनी के शिकायत निवारण अधिकारी को लिखिए। हर कंपनी में यह पद होता है और जवाब देने की एक निर्धारित समय-सीमा होती है।',
          'तीसरा: अगर वहाँ से संतोषजनक उत्तर न मिले, तो बीमा लोकपाल के पास जाया जा सकता है। यह प्रक्रिया निःशुल्क है, इसमें वकील ज़रूरी नहीं, और यह उपभोक्ता की पहुँच में है।',
          'हर चरण में वही चीज़ काम आती है जो शुरू से कही जा रही है: दस्तावेज़। पॉलिसी की प्रति, प्रस्ताव फ़ॉर्म, अस्पताल के काग़ज़, और कंपनी के साथ हुआ सारा पत्राचार तारीख़ के क्रम में।',
          'और एक बात जो कड़वी है पर सच है: जहाँ जानकारी सचमुच छिपाई गई थी, वहाँ अपील का कोई चरण मदद नहीं करता। यही कारण है कि इस लेख का सबसे ज़रूरी हिस्सा दावे के बारे में नहीं, प्रस्ताव फ़ॉर्म के बारे में है।',
        ],
        pEn: [
          'A rejection is not the end. There is a clear route beyond it, that route is free, and very few people know about it.',
          'First step: ask for the reason for rejection in writing. Nothing moves forward on a verbal reason. Once the written reason arrives it becomes clear whether the dispute is about facts or about interpretation.',
          'Second: write to the insurer’s own grievance redressal officer. Every company has that position and a prescribed time limit for replying.',
          'Third: if the answer there is unsatisfactory, the matter can go to the Insurance Ombudsman. The process is free, a lawyer is not required, and it is within an ordinary consumer’s reach.',
          'At every stage the same thing does the work: documents. The policy copy, the proposal form, the hospital papers, and all correspondence with the company in date order.',
          'And one point that is bitter but true: where information really was concealed, no stage of appeal helps. That is exactly why the most important part of this article is not about claims but about the proposal form.',
        ],
      },
    ],
  },
  {
    slug: 'credit-card-smart-use',
    sections: [
      {
        hHi: 'बिलिंग चक्र को समझना',
        hEn: 'Understanding the billing cycle',
        pHi: [
          'क्रेडिट कार्ड का सबसे उपयोगी हिस्सा वह ब्याज-मुक्त अवधि है जो ख़र्च और भुगतान के बीच मिलती है, और उसे समझे बिना कार्ड का आधा लाभ ही मिलता है।',
          'हर कार्ड की एक बिलिंग तारीख़ होती है और उसके कुछ हफ़्ते बाद भुगतान की तारीख़। जो ख़र्च बिलिंग तारीख़ के ठीक बाद होता है, उसे चुकाने के लिए सबसे लंबा समय मिलता है — अक्सर पचास दिन से ज़्यादा।',
          'और जो ख़र्च बिलिंग तारीख़ से एक दिन पहले होता है, वह उसी बिल में आ जाता है और उसे चुकाने के लिए सबसे कम समय मिलता है।',
          'इसका व्यावहारिक उपयोग यह है कि बड़ी नियोजित ख़रीद, जहाँ तारीख़ आपके हाथ में हो, बिलिंग तारीख़ के तुरंत बाद की जाए। यह कोई चालाकी नहीं, यह कार्ड की बनावट का सही उपयोग है।',
          'यह ब्याज-मुक्त अवधि केवल ख़रीद पर लागू होती है। कार्ड से नक़द निकालने पर ब्याज पहले दिन से शुरू हो जाता है और साथ में एक अलग शुल्क भी लगता है।',
          'यही कारण है कि क्रेडिट कार्ड से नक़द निकालना लगभग हमेशा सबसे महँगा विकल्प होता है — उससे महँगा जो अधिकांश लोग कल्पना करते हैं।',
        ],
        pEn: [
          'The most useful part of a credit card is the interest-free period between spending and payment, and without understanding it you get only half the benefit of the card.',
          'Every card has a billing date and a payment date some weeks after it. Spending done just after the billing date gets the longest time to be repaid — often more than fifty days.',
          'Spending done a day before the billing date falls into that same bill and gets the shortest time of all.',
          'The practical use is to make a large planned purchase, where the date is in your hands, immediately after the billing date. This is not a trick; it is using the card exactly as it is built.',
          'That interest-free period applies only to purchases. Withdrawing cash on a card starts interest from day one and carries a separate fee as well.',
          'This is why taking cash on a credit card is almost always the most expensive option available — more expensive than most people imagine.',
        ],
      },
      {
        hHi: 'क्रेडिट उपयोग अनुपात',
        hEn: 'The credit utilisation ratio',
        pHi: [
          'यह एक साधारण संख्या है जिसका आपके क्रेडिट स्कोर पर बड़ा असर होता है, और अधिकांश लोगों ने इसका नाम तक नहीं सुना।',
          'यह बताती है कि आपकी कुल उपलब्ध सीमा में से आप कितनी इस्तेमाल कर रहे हैं। एक लाख की सीमा में से चालीस हज़ार का इस्तेमाल यानी चालीस प्रतिशत।',
          'सामान्य मार्गदर्शन यह है कि इसे तीस प्रतिशत के आसपास या उससे नीचे रखा जाए। इससे ऊपर लगातार बना रहना यह संकेत देता है कि व्यक्ति उपलब्ध ऋण पर बहुत अधिक निर्भर है, भले ही वह हर महीने पूरा बिल चुका देता हो।',
          'यहाँ एक बात चौंकाने वाली है: यह संख्या आमतौर पर बिलिंग तारीख़ पर दर्ज होती है, भुगतान के बाद नहीं। यानी जो व्यक्ति हर महीने पूरा बिल चुकाता है पर सीमा के क़रीब ख़र्च करता है, उसका अनुपात फिर भी ऊँचा दर्ज हो सकता है।',
          'इसका सरल उपाय यह है कि बड़े ख़र्च के बाद बिलिंग तारीख़ से पहले ही आंशिक भुगतान कर दिया जाए। इससे दर्ज होने वाला शेष कम हो जाता है।',
          'और यही कारण है कि पुराना कार्ड बंद करना अक्सर उल्टा पड़ता है: कार्ड बंद होते ही कुल उपलब्ध सीमा घट जाती है, और उतने ही ख़र्च पर अनुपात अपने-आप बढ़ जाता है।',
        ],
        pEn: [
          'This is a simple number with a large effect on your credit score, and most people have never heard its name.',
          'It measures how much of your total available limit you are using. Forty thousand used against a limit of one lakh is forty per cent.',
          'The general guidance is to keep it around thirty per cent or below. Staying above that consistently signals heavy dependence on available credit, even for somebody who clears the full bill every month.',
          'One point here is surprising: the number is usually recorded on the billing date, not after payment. So somebody who pays the full bill each month but spends close to the limit can still be recorded with a high ratio.',
          'The simple remedy is to make a part payment before the billing date after a large purchase. That lowers the balance that gets recorded.',
          'And this is why closing an old card often backfires: the moment it closes, the total available limit falls, and the same spending automatically produces a higher ratio.',
        ],
      },
      {
        hHi: 'रिवॉर्ड का असली हिसाब',
        hEn: 'The real arithmetic of rewards',
        pHi: [
          'कार्ड बेचते समय रिवॉर्ड सबसे आगे रखे जाते हैं, और वे सबसे कम मायने रखने वाली चीज़ हैं। इसका कारण गणित है, राय नहीं।',
          'अधिकांश कार्डों पर रिवॉर्ड का मूल्य ख़र्च के एक छोटे प्रतिशत के बराबर होता है। एक महीने का ब्याज, अगर बिल पूरा न चुके, उस पूरे साल के रिवॉर्ड से बड़ा हो सकता है।',
          'इसलिए क्रम स्पष्ट है: पहले पूरा बिल हर महीने चुकाइए, फिर वार्षिक शुल्क देखिए, और उसके बाद ही रिवॉर्ड की बात कीजिए। उल्टे क्रम में सोचने से महँगे कार्ड ख़रीदे जाते हैं।',
          'वार्षिक शुल्क वाले कार्ड बुरे नहीं होते, पर उन्हें एक ही कसौटी पर परखा जाना चाहिए: क्या इस कार्ड से मिलने वाला लाभ, मेरे अपने वास्तविक ख़र्च पर, शुल्क से ज़्यादा है? अनुमानित ख़र्च पर नहीं, पिछले साल के असली ख़र्च पर।',
          'रिवॉर्ड अंकों की एक समाप्ति तिथि भी होती है, और उनका मूल्य कंपनी बदल सकती है। जो अंक इस्तेमाल नहीं हुए, वे अक्सर चुपचाप ख़त्म हो जाते हैं।',
          'और सबसे महँगी बात: रिवॉर्ड पाने के लिए वह ख़रीदना जो वैसे नहीं ख़रीदते। दो प्रतिशत का लाभ पाने के लिए सौ प्रतिशत ख़र्च करना कोई बचत नहीं है।',
        ],
        pEn: [
          'Rewards are put at the front while a card is being sold, and they are the least important thing about it. The reason is arithmetic, not opinion.',
          'On most cards the value of rewards works out to a small percentage of spending. One month of interest, if the bill is not cleared, can exceed a whole year of rewards.',
          'So the order is plain: first clear the full bill every month, then look at the annual fee, and only then discuss rewards. Thinking in the reverse order is how expensive cards get bought.',
          'Cards with an annual fee are not bad, but they should be tested on one question: does the benefit from this card, on my own actual spending, exceed the fee? On last year’s real spending, not on imagined spending.',
          'Reward points also carry an expiry, and the company can change what they are worth. Points that go unused very often disappear quietly.',
          'And the most expensive habit of all: buying something you would not otherwise buy in order to earn rewards. Spending a hundred per cent to gain two per cent is not a saving.',
        ],
      },
      {
        hHi: 'ऑटो-डेबिट और स्टेटमेंट पढ़ने की आदत',
        hEn: 'Auto-debit and the habit of reading the statement',
        pHi: [
          'पूरा बिल चुकाने के लिए ऑटो-डेबिट लगाना सबसे उपयोगी एक क़दम है। यह भूलने की संभावना ख़त्म कर देता है, और भूलना ही वह जगह है जहाँ सबसे ज़्यादा नुक़सान होता है।',
          'यहाँ एक विवरण महत्वपूर्ण है: ऑटो-डेबिट "कुल देय राशि" पर लगाइए, "न्यूनतम देय राशि" पर नहीं। दूसरा विकल्प चुपचाप वही जाल बना देता है जिससे बचने की कोशिश की जा रही थी।',
          'ऑटो-डेबिट लगाने के बाद भी स्टेटमेंट पढ़ना बंद मत कीजिए। स्वचालित भुगतान का सबसे बड़ा जोखिम यही है कि ग़लत शुल्क भी अपने-आप चुक जाते हैं और किसी को पता नहीं चलता।',
          'हर महीने पाँच मिनट में तीन चीज़ें देखिए: कोई ऐसा लेन-देन जो आपने नहीं किया, कोई ऐसी सदस्यता जो अब नहीं चाहिए, और कोई ऐसा शुल्क जिसका कारण समझ न आए।',
          'जो लेन-देन आपका नहीं है, उसकी शिकायत तुरंत कीजिए। अधिकांश कार्डों में अनधिकृत लेन-देन की सूचना देने की समय-सीमा होती है, और उस सीमा के भीतर दी गई सूचना पर उपभोक्ता की देनदारी सीमित होती है।',
          'और अगर किसी महीने पूरा बिल चुकाना संभव न हो, तो न्यूनतम राशि चुकाकर चुप मत बैठिए। कार्ड कंपनी से बात कीजिए — शेष राशि को किश्तों में बदलने का विकल्प आमतौर पर उपलब्ध होता है और उसकी दर कार्ड के सामान्य ब्याज से काफ़ी कम होती है।',
        ],
        pEn: [
          'Setting up an auto-debit for the full bill is the single most useful step available. It removes the possibility of forgetting, and forgetting is where most of the damage happens.',
          'One detail matters here: set the auto-debit on the "total amount due", not the "minimum amount due". The second option quietly builds the very trap you were trying to avoid.',
          'Do not stop reading the statement after setting up auto-debit. The biggest risk of automatic payment is that wrong charges get paid automatically too, and nobody notices.',
          'Every month, in five minutes, look for three things: a transaction you did not make, a subscription you no longer want, and a charge whose reason you cannot explain.',
          'Report any transaction that is not yours immediately. Most cards set a time limit for reporting unauthorised transactions, and within that limit the customer’s liability is restricted.',
          'And if the full bill cannot be cleared in some month, do not pay the minimum and go quiet. Speak to the card company — converting the balance into instalments is usually available, and that rate is considerably lower than the card’s ordinary interest.',
        ],
      },
      {
        hHi: 'कार्ड बंद करने से पहले',
        hEn: 'Before closing a card',
        pHi: [
          'कार्ड बंद करना कभी-कभी सही होता है — जब उसका शुल्क उसके उपयोग से ज़्यादा हो, या जब वह ख़र्च को बढ़ा रहा हो। पर इसे सोचकर करना चाहिए, क्योंकि इसके कुछ परिणाम तुरंत दिखते नहीं।',
          'पहला परिणाम: कुल उपलब्ध सीमा घट जाती है, और उपयोग अनुपात बढ़ जाता है। दूसरा: आपके क्रेडिट इतिहास की औसत आयु घट सकती है, और पुराना खाता बंद करने पर यह असर सबसे ज़्यादा होता है।',
          'इसलिए सामान्य नियम यह है कि जिन कार्डों पर कोई वार्षिक शुल्क नहीं है, उन्हें खुला रहने देना अक्सर बेहतर है — भले ही उनका उपयोग न हो, बशर्ते साल में एक बार कोई छोटा लेन-देन कर दिया जाए ताकि वे निष्क्रिय होकर बंद न हो जाएँ।',
          'शुल्क वाले कार्ड के मामले में बंद करने से पहले कंपनी से बात करने का विकल्प भी है। कई बार शुल्क माफ़ हो जाता है, या कार्ड को बिना शुल्क वाले संस्करण में बदला जा सकता है, जिससे खाता और उसका इतिहास बना रहता है।',
          'बंद करते समय तीन काम ज़रूरी हैं: सारे स्वचालित भुगतान दूसरे कार्ड पर स्थानांतरित कीजिए, बचे हुए रिवॉर्ड अंक इस्तेमाल कर लीजिए, और शेष राशि पूरी चुका दीजिए।',
          'और अंत में, बंद होने की लिखित पुष्टि लीजिए और कुछ महीनों बाद अपनी क्रेडिट रिपोर्ट में जाँचिए कि वह खाता वाक़ई बंद दिखाया गया है। बिना पुष्टि के बंद किया गया कार्ड कभी-कभी रिपोर्ट में खुला ही रह जाता है।',
        ],
        pEn: [
          'Closing a card is sometimes right — when its fee exceeds its use, or when it is driving spending. But it deserves thought, because some of its consequences are not immediately visible.',
          'The first consequence: the total available limit falls and the utilisation ratio rises. The second: the average age of your credit history can drop, and closing an old account has the largest effect there.',
          'So the general rule is that cards with no annual fee are usually better left open — even unused, provided one small transaction is made each year so that they are not closed for inactivity.',
          'With a fee-carrying card, speaking to the company before closing is worth doing. Fees are often waived, or the card can be converted to a no-fee version, which keeps the account and its history alive.',
          'When closing, three tasks matter: move every automatic payment to another card, use up the remaining reward points, and clear the outstanding balance in full.',
          'And finally, get written confirmation of closure and check your credit report a few months later to see the account actually shown as closed. A card closed without confirmation sometimes stays open on the report.',
        ],
      },
    ],
  },
  {
    slug: 'minimum-due-trap',
    sections: [
      {
        hHi: 'न्यूनतम राशि गिनी कैसे जाती है',
        hEn: 'How the minimum amount is calculated',
        pHi: [
          'न्यूनतम देय राशि आमतौर पर कुल बिल का एक छोटा प्रतिशत होती है, और उसमें उस महीने का ब्याज, शुल्क और किसी चालू किश्त की राशि भी जुड़ी होती है।',
          'इसका मतलब यह है कि जो व्यक्ति केवल न्यूनतम चुकाता है, वह अक्सर मूलधन का बहुत छोटा हिस्सा ही चुका रहा होता है। बाक़ी सब ब्याज और शुल्क में चला जाता है।',
          'यह संख्या हर महीने बदलती है, और वह भी एक भ्रम पैदा करती है। बिल बढ़ने पर न्यूनतम भी बढ़ता है, इसलिए भुगतान बड़ा महसूस होता है जबकि क़र्ज़ उतना ही या उससे ज़्यादा बना रहता है।',
          'बिल पर यह राशि सबसे प्रमुखता से छपती है और कुल देय राशि उसके पास छोटे अक्षरों में। यह डिज़ाइन का चुनाव है, और वह चुनाव किसके पक्ष में है यह समझना ज़रूरी है।',
          'सबसे महत्वपूर्ण बात जो बिल पर नहीं लिखी होती: न्यूनतम चुकाते ही उस महीने की पूरी ब्याज-मुक्त अवधि समाप्त हो जाती है। उसके बाद हर नई ख़रीद पर ब्याज पहले दिन से लगना शुरू हो जाता है।',
          'यानी न्यूनतम चुकाना केवल पुराने शेष को महँगा नहीं करता, वह आने वाले हर लेन-देन को भी महँगा कर देता है। यही वह बात है जो इस जाल को इतना गहरा बनाती है।',
        ],
        pEn: [
          'The minimum amount due is usually a small percentage of the total bill, with that month’s interest, fees and any running instalment added on top.',
          'This means somebody paying only the minimum is often repaying a very small part of the principal. The rest disappears into interest and charges.',
          'The figure changes every month, and that creates an illusion of its own. As the bill grows the minimum grows too, so the payment feels larger while the debt stays the same or grows.',
          'On the statement this number is printed most prominently, with the total amount due beside it in smaller type. That is a design choice, and it is worth understanding whose side the choice is on.',
          'The most important point is not printed on the bill at all: paying the minimum ends that month’s interest-free period entirely. From then on, every new purchase attracts interest from day one.',
          'So paying the minimum does not merely make the old balance expensive; it makes every future transaction expensive as well. That is what makes this trap so deep.',
        ],
      },
      {
        hHi: 'एक साधारण उदाहरण, संख्याओं के साथ',
        hEn: 'A plain example, with numbers',
        pHi: [
          'मान लीजिए एक बिल है और उस पर मासिक ब्याज दर वह है जो अधिकांश कार्डों पर लागू होती है। व्यक्ति हर महीने केवल न्यूनतम चुकाता है और कोई नई ख़रीद नहीं करता।',
          'पहले महीने में जो चुकाया गया, उसका बड़ा हिस्सा ब्याज में गया और मूलधन में बहुत कम कमी आई। दूसरे महीने ब्याज थोड़ा कम हुआ, पर न्यूनतम राशि भी कम हो गई, इसलिए मूलधन में कमी फिर भी धीमी रही।',
          'इस गति से चलते हुए शेष राशि को शून्य होने में वर्षों लग जाते हैं, और उन वर्षों में चुकाया गया कुल ब्याज मूल बिल के बराबर या उससे अधिक हो सकता है।',
          'अब उसी बिल पर एक अलग व्यवहार रखिए: हर महीने न्यूनतम के बजाय एक निश्चित, बड़ी राशि चुकाई जाए — जो भी संभव हो, पर वही राशि हर महीने। यहाँ अवधि नाटकीय रूप से घट जाती है।',
          'कारण सरल है: चूँकि ब्याज शेष राशि पर लगता है, इसलिए शेष जितनी तेज़ी से घटेगा, ब्याज उतनी ही तेज़ी से घटेगा, और अगली किश्त का उतना ही बड़ा हिस्सा मूलधन में जाएगा।',
          'यह गणना अपने असली बिल पर एक बार ख़ुद करके देखिए। जो चीज़ सलाह के रूप में उबाऊ लगती है, वह अपनी ही संख्याओं में देखने पर व्यवहार बदल देती है।',
        ],
        pEn: [
          'Suppose there is a bill carrying the monthly interest rate that applies on most cards. The person pays only the minimum each month and makes no new purchases.',
          'In the first month, most of what was paid went to interest and the principal fell very little. In the second month the interest was slightly lower, but the minimum was lower too, so the principal still fell slowly.',
          'At that pace the balance takes years to reach zero, and the total interest paid over those years can equal or exceed the original bill.',
          'Now take the same bill with different behaviour: instead of the minimum, a fixed and larger amount is paid every month — whatever is possible, but the same amount each time. The period shortens dramatically.',
          'The reason is simple: interest is charged on the balance, so the faster the balance falls, the faster the interest falls, and the larger the share of the next payment that reaches the principal.',
          'Do this calculation once on your own real bill. What sounds dull as advice changes behaviour when it is seen in your own numbers.',
        ],
      },
      {
        hHi: 'जाल से निकलने का क्रम',
        hEn: 'The order for getting out',
        pHi: [
          'अगर यह स्थिति पहले से बनी हुई है, तो शर्मिंदगी किसी काम की नहीं। एक क्रम है, और वह क्रम काम करता है।',
          'पहला क़दम: नई ख़रीद रोक दीजिए। जब तक कार्ड पर शेष है, हर नई ख़रीद पर ब्याज पहले दिन से लगता है, इसलिए कार्ड को कुछ महीनों के लिए किनारे रख देना ही सबसे तेज़ राहत है।',
          'दूसरा: कुल तस्वीर काग़ज़ पर लाइए — हर कार्ड और हर ऋण का शेष, ब्याज दर, और न्यूनतम राशि। जब तक पूरी संख्या सामने नहीं आती, कोई योजना नहीं बनती।',
          'तीसरा: कार्ड कंपनी से बात कीजिए और शेष राशि को निश्चित किश्तों में बदलने का विकल्प पूछिए। उसकी दर आमतौर पर कार्ड के सामान्य ब्याज से काफ़ी कम होती है, और किश्त की समाप्ति तिथि निश्चित होती है।',
          'चौथा: अगर कई कार्ड हैं, तो सबसे ऊँची ब्याज दर वाले पर अतिरिक्त राशि डालिए और बाक़ी पर न्यूनतम चलाइए। यह गणितीय रूप से सबसे सस्ता रास्ता है।',
          'पाँचवाँ: एक छोटा आपातकालीन कोष साथ-साथ बनाइए, भले ही वह बहुत छोटा हो। इसके बिना पहली अप्रत्याशित ज़रूरत फिर से कार्ड पर आ जाती है, और सारी प्रगति मिट जाती है।',
        ],
        pEn: [
          'If this situation already exists, embarrassment is of no use. There is an order, and the order works.',
          'First step: stop new purchases. While a balance is outstanding, every new purchase attracts interest from day one, so putting the card aside for a few months is the fastest relief available.',
          'Second: put the full picture on paper — every card and every loan with its balance, its rate and its minimum. No plan can be made until the whole number is visible.',
          'Third: speak to the card company and ask about converting the balance into fixed instalments. That rate is usually well below the card’s ordinary interest, and the instalment has a definite end date.',
          'Fourth: if there are several cards, put the extra money on the highest rate and keep the others at the minimum. That is mathematically the cheapest route.',
          'Fifth: build a small emergency fund alongside, however small. Without one, the first unexpected need lands back on the card and all the progress disappears.',
        ],
      },
      {
        hHi: 'भावनात्मक हिस्सा, जिसकी चर्चा कम होती है',
        hEn: 'The emotional part, which is discussed less',
        pHi: [
          'यह जाल केवल गणित का नहीं है। जो व्यक्ति इसमें फँसा होता है, वह अक्सर बिल खोलना ही बंद कर देता है, और यही सबसे महँगा व्यवहार है।',
          'बिल न देखने से शेष कम नहीं होता, पर देर से भुगतान का शुल्क और ब्याज ज़रूर जुड़ता रहता है। जिस दिन बिल खोला जाता है, संख्या पहले से बड़ी हो चुकी होती है, और डर और बढ़ जाता है।',
          'इस चक्र को तोड़ने का सबसे सरल तरीक़ा यह है कि महीने में एक निश्चित दिन तय कर लिया जाए जब सारे बिल देखे जाएँगे — चाहे उस दिन कुछ चुकाया जाए या नहीं। देखना और चुकाना दो अलग काम हैं।',
          'दूसरी बात: परिवार में किसी एक व्यक्ति को स्थिति बता देना अक्सर उतना ही उपयोगी होता है जितना कोई वित्तीय उपाय। अकेले सँभाला गया क़र्ज़ लगभग हमेशा ज़्यादा समय लेता है।',
          'तीसरी: प्रगति को दिखने लायक़ बनाइए। एक काग़ज़ पर शेष राशि हर महीने लिखिए। घटती हुई संख्या देखना उस अनुशासन को बनाए रखता है जिसे केवल इच्छाशक्ति नहीं बनाए रख पाती।',
          'और चौथी: बाहर निकलने के बाद कार्ड को दुश्मन मत मानिए। समस्या कार्ड में नहीं थी, न्यूनतम राशि वाले उस एक विकल्प में थी। पूरा बिल हर महीने चुकाने वाला वही कार्ड एक उपयोगी औज़ार है।',
        ],
        pEn: [
          'This trap is not only arithmetic. Somebody caught in it very often stops opening the bill altogether, and that is the most expensive behaviour of all.',
          'Not looking does not reduce the balance, but late-payment fees and interest keep accumulating. On the day the bill is finally opened the number has grown, and the fear grows with it.',
          'The simplest way to break that cycle is to fix one day a month when every bill is looked at — whether or not anything is paid that day. Looking and paying are two different tasks.',
          'Second: telling one person in the family is often as useful as any financial measure. Debt handled alone almost always takes longer.',
          'Third: make the progress visible. Write the balance on a sheet of paper every month. Watching the number fall sustains a discipline that willpower alone does not.',
          'And fourth: once you are out, do not treat the card as an enemy. The problem was never the card; it was that one option marked "minimum due". The same card, with the full bill paid monthly, is a useful tool.',
        ],
      },
      {
        hHi: 'बचाव के लिए तीन स्थायी आदतें',
        hEn: 'Three permanent habits that prevent it',
        pHi: [
          'इस जाल में दोबारा न फँसने के लिए तीन आदतें काफ़ी हैं, और तीनों एक-एक बार लगाकर छोड़ी जा सकती हैं।',
          'पहली: कुल देय राशि पर ऑटो-डेबिट। यह अकेली चीज़ इस पूरे लेख की समस्या को जड़ से हटा देती है, क्योंकि तब न्यूनतम राशि का विकल्प कभी सामने आता ही नहीं।',
          'दूसरी: कार्ड की सीमा उतनी रखिए जितनी आपके एक महीने के ख़र्च के हिसाब से उचित हो। बहुत ऊँची सीमा कोई सम्मान नहीं है; वह केवल ग़लती की गुंजाइश बढ़ाती है।',
          'तीसरी: एक आपातकालीन कोष। कार्ड पर शेष जमा होने का सबसे आम कारण फ़िज़ूलख़र्ची नहीं, बल्कि कोई एक अप्रत्याशित ख़र्च होता है जिसके लिए कहीं और पैसा नहीं था।',
          'इन तीनों के साथ एक चौथी चीज़ जोड़ी जा सकती है, हालाँकि वह आदत नहीं बल्कि एक नियम है: कार्ड से नक़द कभी मत निकालिए। उस पर ब्याज पहले दिन से लगता है और अलग शुल्क भी।',
          'ये चारों मिलकर उस स्थिति को लगभग असंभव बना देते हैं जिसका वर्णन इस लेख में है — और वे सब एक ही दोपहर में लागू किए जा सकते हैं।',
        ],
        pEn: [
          'Three habits are enough to keep out of this trap for good, and each can be set up once and left alone.',
          'First: an auto-debit on the total amount due. This single thing removes the problem of this whole article at the root, because the minimum-due option never comes up again.',
          'Second: keep the card limit at a level appropriate to one month of your spending. A very high limit is not a compliment; it only widens the room for error.',
          'Third: an emergency fund. The commonest cause of a balance building up on a card is not extravagance but one unexpected expense with no money set aside anywhere else.',
          'A fourth thing can be added, though it is a rule rather than a habit: never take cash on a credit card. Interest starts on day one and a separate fee applies as well.',
          'Together these four make the situation described in this article very nearly impossible — and all of them can be put in place in a single afternoon.',
        ],
      },
    ],
  },
  {
    slug: 'personal-loan-emi-basics',
    sections: [
      {
        hHi: 'ईएमआई के अंदर क्या-क्या है',
        hEn: 'What is inside an EMI',
        pHi: [
          'हर किश्त दो हिस्सों से बनी होती है: ब्याज और मूलधन। किश्त की राशि पूरी अवधि में एक जैसी रहती है, पर इन दोनों का अनुपात हर महीने बदलता रहता है।',
          'शुरुआती महीनों में ब्याज का हिस्सा सबसे बड़ा होता है, क्योंकि शेष मूलधन सबसे ज़्यादा होता है। अंतिम महीनों में लगभग पूरी किश्त मूलधन में जाती है।',
          'इसका एक महत्वपूर्ण नतीजा है: ऋण के पहले तिहाई हिस्से में जितनी किश्तें चुकाई गईं, उनसे शेष मूलधन उतना नहीं घटा जितना लोग मान लेते हैं।',
          'यही कारण है कि दो साल किश्त चुकाने के बाद जब कोई अपना बकाया देखता है, तो वह संख्या अक्सर अपेक्षा से बड़ी निकलती है, और यह अनुभव निराश करता है।',
          'हर बैंक परिशोधन तालिका देता है, जिसमें हर महीने का ब्याज और मूलधन अलग-अलग लिखा होता है। ऋण लेते समय यह तालिका माँगिए और एक बार पूरी देख लीजिए।',
          'वह एक पन्ना ऋण के बारे में उतना बता देता है जितना बाक़ी सारे दस्तावेज़ मिलकर नहीं बताते, और उसे पढ़ने में पाँच मिनट लगते हैं।',
        ],
        pEn: [
          'Every instalment is made of two parts: interest and principal. The instalment amount stays the same across the term, but the proportion between the two changes every month.',
          'In the early months the interest portion is largest, because the outstanding principal is at its highest. In the final months almost the whole instalment goes to principal.',
          'This has an important consequence: in the first third of a loan, the instalments paid reduce the outstanding principal much less than people assume.',
          'That is why somebody who checks the outstanding balance after two years of paying often finds a larger number than expected, and the experience is discouraging.',
          'Every lender provides an amortisation table setting out the interest and principal for each month separately. Ask for it while taking the loan and read it through once.',
          'That single page says more about the loan than all the other documents combined, and it takes five minutes to read.',
        ],
      },
      {
        hHi: 'फ़्लैट दर और घटती शेष दर',
        hEn: 'Flat rate versus reducing balance',
        pHi: [
          'यह वह जगह है जहाँ सबसे ज़्यादा भ्रम पैदा होता है, और यह भ्रम हमेशा उधार लेने वाले के ख़िलाफ़ जाता है।',
          'घटती शेष दर में ब्याज उस राशि पर लगता है जो अभी बाक़ी है। जैसे-जैसे मूलधन घटता है, ब्याज भी घटता है। यह वह तरीक़ा है जिसे अधिकांश लोग स्वाभाविक मानते हैं।',
          'फ़्लैट दर में ब्याज पूरी मूल राशि पर, पूरी अवधि के लिए गिना जाता है — चाहे आपने आधा मूलधन चुका दिया हो। इसमें दर संख्या के रूप में छोटी दिखती है पर असल लागत काफ़ी ऊँची होती है।',
          'दोनों की तुलना करने का एक ही सही तरीक़ा है: वार्षिक प्रतिशत दर पूछिए, जो सभी शुल्कों को मिलाकर वास्तविक वार्षिक लागत बताती है। दो प्रस्तावों की तुलना केवल इसी संख्या पर की जा सकती है।',
          'यही कारण है कि "केवल इतने प्रतिशत" वाले विज्ञापन को अकेले देखकर फ़ैसला नहीं करना चाहिए। सवाल यह नहीं है कि दर क्या है, सवाल यह है कि दर किस पर लग रही है।',
          'और हमेशा कुल चुकाई जाने वाली राशि पूछिए — किश्त गुणा महीनों की संख्या। वह एक संख्या पूरे प्रस्ताव को सबसे ईमानदारी से बता देती है।',
        ],
        pEn: [
          'This is where the most confusion arises, and the confusion always runs against the borrower.',
          'On a reducing-balance rate, interest is charged on what is still outstanding. As the principal falls the interest falls with it. This is the method most people assume is being used.',
          'On a flat rate, interest is calculated on the full original amount for the full term — even after half the principal has been repaid. The rate looks smaller as a number while the real cost is considerably higher.',
          'There is only one correct way to compare the two: ask for the annual percentage rate, which expresses the real annual cost with all charges included. Two offers can only be compared on that number.',
          'This is why an advertisement quoting "only this much per cent" should never decide anything on its own. The question is not what the rate is; the question is what the rate is charged on.',
          'And always ask for the total amount repayable — the instalment multiplied by the number of months. That single figure describes the offer more honestly than anything else.',
        ],
      },
      {
        hHi: 'वे शुल्क जो दर में नहीं दिखते',
        hEn: 'The charges that do not appear in the rate',
        pHi: [
          'ऋण की असली लागत केवल ब्याज नहीं है। कई शुल्क ऐसे हैं जो दर में शामिल नहीं दिखते पर जेब से जाते हैं, और उन्हें पहले से पूछना पड़ता है क्योंकि वे स्वयं नहीं बताए जाते।',
          'प्रसंस्करण शुल्क सबसे आम है और वह आमतौर पर वितरण के समय ही काट लिया जाता है। यानी जितना ऋण स्वीकृत हुआ, हाथ में उससे कम आता है, जबकि ब्याज पूरी स्वीकृत राशि पर लगता है।',
          'दूसरा है समय से पहले भुगतान पर लगने वाला शुल्क। यह सबसे महत्वपूर्ण है क्योंकि यह भविष्य में आपके विकल्प तय करता है। ऋण लेने से पहले यह ज़रूर पूछिए कि आंशिक और पूर्ण, दोनों तरह के पूर्व-भुगतान पर क्या नियम लागू होंगे।',
          'तीसरा है देर से भुगतान पर शुल्क और उस पर लगने वाला अतिरिक्त ब्याज, जो सामान्य दर से ऊँचा होता है।',
          'चौथा, कभी-कभी ऋण के साथ एक बीमा पॉलिसी भी जोड़ दी जाती है। वह उपयोगी हो सकती है, पर वह एक अलग उत्पाद है और उसका प्रीमियम ऋण की लागत में जुड़ता है। यह वैकल्पिक है या अनिवार्य, यह पूछना आपका अधिकार है।',
          'सारे शुल्क स्वीकृति पत्र में लिखे होते हैं। हस्ताक्षर से पहले उस पत्र को पूरा पढ़ने का समय माँगना पूरी तरह उचित है, और कोई भी अच्छा ऋणदाता उससे मना नहीं करेगा।',
        ],
        pEn: [
          'The real cost of a loan is not only the interest. Several charges sit outside the rate but come out of your pocket, and they have to be asked about because they are not volunteered.',
          'The processing fee is the commonest and is usually deducted at disbursal. So the amount that reaches your hands is less than the amount sanctioned, while interest is charged on the full sanctioned sum.',
          'The second is the prepayment charge. This is the most important one because it decides your future options. Before taking the loan, ask what rules apply to both part-prepayment and full closure.',
          'The third is the late-payment fee together with the penal interest charged on it, which is higher than the ordinary rate.',
          'Fourth, an insurance policy is sometimes attached to the loan. It may be useful, but it is a separate product and its premium adds to the cost of borrowing. Whether it is optional or mandatory is a question you are entitled to ask.',
          'Every charge is written in the sanction letter. Asking for time to read that letter fully before signing is entirely reasonable, and no good lender will refuse.',
        ],
      },
      {
        hHi: 'पूर्व-भुगतान का गणित',
        hEn: 'The arithmetic of prepayment',
        pHi: [
          'ऋण की लागत घटाने का सबसे प्रभावी तरीक़ा पूर्व-भुगतान है, और उसमें समय का महत्व राशि से भी ज़्यादा होता है।',
          'शुरुआती वर्षों में किया गया पूर्व-भुगतान सबसे ज़्यादा ब्याज बचाता है, क्योंकि उस समय शेष मूलधन सबसे बड़ा होता है और उसी पर ब्याज लग रहा होता है।',
          'पूर्व-भुगतान करते समय एक विकल्प चुनना होता है: किश्त वही रखकर अवधि घटा दी जाए, या अवधि वही रखकर किश्त घटा दी जाए। पहला विकल्प कहीं ज़्यादा ब्याज बचाता है।',
          'दूसरा विकल्प तब उपयुक्त है जब मासिक नक़दी का दबाव हो। यह चुनाव आपका है और ऋणदाता से स्पष्ट रूप से कहना पड़ता है, क्योंकि डिफ़ॉल्ट व्यवस्था हर जगह एक जैसी नहीं होती।',
          'हर पूर्व-भुगतान के बाद नई परिशोधन तालिका माँगिए और यह पुष्टि कीजिए कि राशि मूलधन में समायोजित हुई है, ब्याज में नहीं। यह जाँच एक मिनट की है और बहुत ज़रूरी है।',
          'और ऋण पूरा चुक जाने के बाद अनापत्ति प्रमाणपत्र ज़रूर लीजिए, और कुछ महीनों बाद अपनी क्रेडिट रिपोर्ट में देख लीजिए कि खाता बंद दिखाया गया है। बंद न दिखाया गया ऋण भविष्य में उधार लेने में बाधा बनता है।',
        ],
        pEn: [
          'Prepayment is the most effective way to reduce the cost of a loan, and in it timing matters even more than the amount.',
          'A prepayment made in the early years saves the most interest, because the outstanding principal is largest then and that is what the interest is running on.',
          'When prepaying, a choice has to be made: keep the instalment and shorten the term, or keep the term and reduce the instalment. The first saves far more interest.',
          'The second suits somebody under monthly cash pressure. The choice is yours and has to be stated clearly to the lender, because the default arrangement is not the same everywhere.',
          'After each prepayment, ask for a fresh amortisation table and confirm that the amount was applied to principal and not to interest. That check takes a minute and matters a great deal.',
          'And once the loan is fully repaid, take the no-objection certificate, then check your credit report a few months later to see the account shown as closed. A loan not shown as closed becomes an obstacle to borrowing later.',
        ],
      },
      {
        hHi: 'उधार लेने से पहले के चार सवाल',
        hEn: 'Four questions before borrowing',
        pHi: [
          'व्यक्तिगत ऋण अपने-आप में बुरा नहीं है। वह एक औज़ार है, और उसकी उपयुक्तता चार सवालों के जवाब से तय होती है।',
          'पहला: यह ख़र्च टाला जा सकता है या नहीं? अगर टाला जा सकता है, तो अक्सर बचत करके बाद में करना सस्ता पड़ता है। अगर नहीं टाला जा सकता — इलाज, कोई ज़रूरी मरम्मत — तो ऋण उचित है।',
          'दूसरा: क्या इससे सस्ता कोई विकल्प मौजूद है? किसी परिसंपत्ति पर लिया गया ऋण आमतौर पर व्यक्तिगत ऋण से सस्ता होता है, और कार्ड की बकाया राशि को किश्तों में बदलना भी अक्सर सस्ता पड़ता है।',
          'तीसरा: क्या किश्त मेरी मासिक आय में आराम से बैठती है? एक सामान्य मार्गदर्शन यह है कि सभी ऋणों की कुल किश्तें आय के एक तिहाई से अधिक न हों, और यह सीमा आपके अपने ख़र्चों के हिसाब से और कम भी हो सकती है।',
          'चौथा: अगर तीन महीने आय रुक जाए तो क्या होगा? इसका जवाब आपातकालीन कोष है। बिना उसके लिया गया ऋण पहली रुकावट पर ही चूक में बदल जाता है।',
          'इन चार सवालों के जवाब लिखकर देखिए, सोचकर नहीं। लिखा हुआ जवाब उस दबाव में भी टिकता है जो ऋण स्वीकृत हो जाने के उत्साह में पैदा होता है।',
        ],
        pEn: [
          'A personal loan is not bad in itself. It is a tool, and its suitability is settled by the answers to four questions.',
          'First: can this expense be postponed? If it can, saving up and doing it later is usually cheaper. If it cannot — treatment, an essential repair — then borrowing is reasonable.',
          'Second: does a cheaper option exist? A loan secured against an asset is usually cheaper than a personal loan, and converting a card balance into instalments is often cheaper too.',
          'Third: does the instalment sit comfortably inside my monthly income? A common guideline is that all loan instalments together should not exceed a third of income, and depending on your own expenses that limit may need to be lower.',
          'Fourth: what happens if income stops for three months? The answer to that is an emergency fund. A loan taken without one turns into a default at the first interruption.',
          'Write the answers to these four down rather than merely thinking them. A written answer survives the pressure created by the excitement of an approved loan.',
        ],
      },
    ],
  },
  {
    slug: 'cibil-score-myths',
    sections: [
      {
        hHi: 'स्कोर बनता किन हिस्सों से है',
        hEn: 'What the score is actually built from',
        pHi: [
          'भ्रम इसलिए फैलते हैं क्योंकि लोग यह नहीं जानते कि स्कोर किन चीज़ों से बनता है। ढाँचा कुछ मुख्य हिस्सों में बँटा होता है और उनका भार बराबर नहीं होता।',
          'सबसे बड़ा भार भुगतान के इतिहास का होता है — क्या किश्तें और बिल समय पर चुके। एक भी चूक कई महीनों तक दिखती है, और लगातार चूक सबसे तेज़ी से स्कोर गिराती है।',
          'दूसरा बड़ा हिस्सा उपयोग अनुपात है, यानी उपलब्ध सीमा में से कितनी इस्तेमाल हो रही है। यह वह हिस्सा है जिसे सबसे जल्दी सुधारा जा सकता है, अक्सर एक ही महीने में।',
          'तीसरा है क्रेडिट इतिहास की आयु। पुराना खाता, जो नियमित चलता रहा हो, स्कोर को स्थिरता देता है। यही कारण है कि पुराना कार्ड बंद करना अक्सर उल्टा पड़ता है।',
          'चौथा है ऋणों का मिश्रण — केवल असुरक्षित ऋण होने के बजाय सुरक्षित और असुरक्षित दोनों का अनुभव होना बेहतर माना जाता है।',
          'और पाँचवाँ, हाल में की गई पूछताछ की संख्या। कम समय में कई जगह आवेदन करना यह संकेत देता है कि व्यक्ति ऋण के लिए बेचैन है, और यह संकेत स्कोर पर असर डालता है।',
        ],
        pEn: [
          'Myths spread because people do not know what the score is built from. The structure divides into a few main parts, and they do not carry equal weight.',
          'The largest weight goes to payment history — whether instalments and bills were paid on time. A single miss stays visible for months, and repeated misses bring the score down fastest of all.',
          'The second large component is the utilisation ratio, how much of the available limit is in use. This is the part that can be improved quickest, often within a single month.',
          'Third is the age of the credit history. An old account that has run regularly gives the score stability. This is exactly why closing an old card usually backfires.',
          'Fourth is the mix of credit — having experience of both secured and unsecured borrowing is regarded better than unsecured alone.',
          'And fifth, the number of recent enquiries. Applying in several places within a short period signals that somebody is anxious for credit, and that signal affects the score.',
        ],
      },
      {
        hHi: 'कठोर और नरम पूछताछ',
        hEn: 'Hard enquiries and soft enquiries',
        pHi: [
          'यह वह अंतर है जिससे सबसे लोकप्रिय भ्रम पैदा होता है — कि अपना स्कोर देखने से स्कोर गिर जाता है।',
          'जब आप ख़ुद अपनी रिपोर्ट देखते हैं, वह नरम पूछताछ होती है। वह रिपोर्ट में दर्ज होती है पर स्कोर पर कोई असर नहीं डालती, चाहे आप हर महीने देखें।',
          'जब कोई ऋणदाता आपके आवेदन पर आपकी रिपोर्ट माँगता है, वह कठोर पूछताछ होती है। वह स्कोर पर थोड़ा असर डालती है और कुछ समय तक रिपोर्ट में दिखती है।',
          'इसका व्यावहारिक अर्थ यह है कि एक साथ चार-पाँच जगह ऋण के लिए आवेदन करना स्कोर के लिए अच्छा नहीं। तुलना करनी है तो पहले दरों के बारे में पूछताछ कीजिए, आवेदन बाद में एक या दो जगह ही कीजिए।',
          'और अपनी रिपोर्ट नियमित रूप से देखना न केवल सुरक्षित है, बल्कि ज़रूरी है — क्योंकि रिपोर्ट में दर्ज ग़लतियाँ उसी तरह मिलती हैं।',
          'साल में कम से कम एक बार अपनी पूरी क्रेडिट रिपोर्ट पढ़िए, केवल स्कोर की संख्या नहीं। संख्या बताती है कि क्या हुआ; रिपोर्ट बताती है कि क्यों हुआ।',
        ],
        pEn: [
          'This is the distinction behind the most popular myth of all — that checking your own score lowers it.',
          'When you look at your own report, that is a soft enquiry. It is recorded in the report but has no effect on the score, however often you look.',
          'When a lender pulls your report against an application, that is a hard enquiry. It has a small effect on the score and remains visible in the report for a while.',
          'The practical meaning is that applying to four or five lenders at once is not good for the score. If you want to compare, enquire about rates first and apply in only one or two places afterwards.',
          'And checking your own report regularly is not merely safe, it is necessary — because errors recorded in the report are found exactly that way.',
          'Read your full credit report at least once a year, not only the score. The number tells you what happened; the report tells you why.',
        ],
      },
      {
        hHi: 'रिपोर्ट में ग़लतियाँ और उन्हें सुधारना',
        hEn: 'Errors in the report and getting them corrected',
        pHi: [
          'क्रेडिट रिपोर्ट में ग़लतियाँ असामान्य नहीं हैं, और वे अक्सर वहीं पकड़ी जाती हैं जहाँ किसी का आवेदन बिना स्पष्ट कारण के अस्वीकार हो जाता है।',
          'सबसे आम ग़लती है वह ऋण या कार्ड दिखना जो चुकाया जा चुका है पर बंद के रूप में दर्ज नहीं हुआ। दूसरी, किसी और के नाम का खाता आपकी रिपोर्ट में जुड़ जाना, आमतौर पर नाम मिलने के कारण।',
          'तीसरी, समय पर चुकाई गई किश्त का देर से दर्ज होना। चौथी, वही ऋण दो बार दिखना — एक बार मूल ऋणदाता से और एक बार उस संस्था से जिसे वह हस्तांतरित हुआ।',
          'सुधार की प्रक्रिया निःशुल्क है। ब्यूरो की वेबसाइट पर विवाद दर्ज कीजिए, संबंधित प्रविष्टि चिह्नित कीजिए, और अपने पास मौजूद प्रमाण संलग्न कीजिए — अनापत्ति प्रमाणपत्र, भुगतान की रसीद, या खाता बंद होने का पत्र।',
          'ब्यूरो उस प्रविष्टि की पुष्टि उस बैंक या संस्था से करता है जिसने वह सूचना भेजी थी। इसीलिए समानांतर रूप से उस संस्था को भी लिखना उपयोगी होता है; स्रोत सुधरे बिना प्रविष्टि लौट सकती है।',
          'इस प्रक्रिया में कुछ हफ़्ते लगते हैं, इसलिए इसे किसी बड़े ऋण के आवेदन से पहले शुरू करना चाहिए, आवेदन अस्वीकार होने के बाद नहीं।',
        ],
        pEn: [
          'Errors in a credit report are not unusual, and they are most often discovered when somebody’s application is rejected for no obvious reason.',
          'The commonest error is a loan or card that has been repaid still not being recorded as closed. The second is somebody else’s account attaching to your report, usually through a similarity of names.',
          'The third is an instalment paid on time being recorded as late. The fourth is the same loan appearing twice — once from the original lender and once from the institution it was transferred to.',
          'The correction process is free. Raise a dispute on the bureau’s website, mark the entry concerned, and attach whatever proof you hold — a no-objection certificate, a payment receipt, or a closure letter.',
          'The bureau verifies the entry with the bank or institution that reported it. This is why writing to that institution in parallel helps; without fixing the source the entry can return.',
          'The process takes some weeks, so it should be started before applying for a large loan, not after an application has been refused.',
        ],
      },
      {
        hHi: 'स्कोर सुधारने में कितना समय लगता है',
        hEn: 'How long improving a score takes',
        pHi: [
          'यह वह सवाल है जो सबसे ज़्यादा पूछा जाता है और जिसका जवाब सबसे कम पसंद आता है: कोई तुरंत उपाय नहीं है, और जो सेवा तुरंत सुधार का वादा करे उससे दूर रहना चाहिए।',
          'सबसे तेज़ असर उपयोग अनुपात घटाने का होता है। बिलिंग तारीख़ से पहले शेष कम कर देने का प्रभाव अगली रिपोर्ट में ही दिख सकता है — यानी एक से दो महीने में।',
          'भुगतान के इतिहास का सुधार धीमा है। हर समय पर चुकाई गई किश्त धीरे-धीरे भरोसा बनाती है, और पुरानी चूक का असर कई महीनों में घटता है, मिटता नहीं।',
          'इतिहास की आयु समय के साथ अपने-आप बढ़ती है, बशर्ते पुराने खाते खुले रहें और नियमित चलते रहें। यह वह हिस्सा है जिसे केवल धैर्य सुधारता है।',
          'इसलिए एक व्यावहारिक अपेक्षा यह है: अनुशासित व्यवहार से कुछ महीनों में दिखने लायक़ सुधार आता है, और बड़ी क्षति की भरपाई में एक से दो साल लग सकते हैं।',
          'और सबसे ज़रूरी बात: स्कोर लक्ष्य नहीं है। वह उन आदतों का पैमाना है जो आप वैसे भी अपनाना चाहते हैं। जो व्यक्ति समय पर चुकाता है और सीमा के भीतर रहता है, उसका स्कोर अपने-आप ठीक हो जाता है।',
        ],
        pEn: [
          'This is the most-asked question with the least-liked answer: there is no instant fix, and any service promising an immediate improvement should be avoided.',
          'The fastest effect comes from reducing the utilisation ratio. Bringing the balance down before the billing date can show up in the very next report — within one to two months.',
          'Improving payment history is slow. Each instalment paid on time builds trust gradually, and the effect of an old miss fades over many months rather than disappearing.',
          'The age of the history grows by itself with time, provided old accounts stay open and keep running. This is the part that only patience improves.',
          'So a realistic expectation is this: disciplined behaviour produces a visible improvement within a few months, and repairing serious damage can take one to two years.',
          'And the most important point: the score is not the goal. It is a measure of habits you would want anyway. Somebody who pays on time and stays within limits ends up with a good score without aiming at one.',
        ],
      },
      {
        hHi: 'बिना स्कोर वाले लोग',
        hEn: 'People with no score at all',
        pHi: [
          'एक श्रेणी ऐसी है जिसकी चर्चा कम होती है: वे लोग जिन्होंने कभी कोई कार्ड या ऋण नहीं लिया, और इसलिए जिनका कोई स्कोर ही नहीं है।',
          'ऐसे व्यक्ति की रिपोर्ट में स्कोर के बजाय एक विशेष संकेत होता है जिसका अर्थ है "पर्याप्त इतिहास नहीं"। यह ख़राब स्कोर नहीं है, पर ऋणदाता के लिए यह भी एक अनिश्चितता है।',
          'इसका नतीजा यह होता है कि जिस व्यक्ति ने जीवन भर कभी उधार नहीं लिया, उसे पहला गृह ऋण लेते समय अक्सर कठिनाई होती है — जो अन्यायपूर्ण लगता है और तकनीकी रूप से समझ में आता है।',
          'इसका सामान्य समाधान यह है कि एक छोटी शुरुआत की जाए: एक सादा कार्ड या किसी जमा के विरुद्ध लिया गया कार्ड, जिसका उपयोग छोटे नियमित ख़र्चों के लिए हो और जिसका बिल हर महीने पूरा चुकाया जाए।',
          'कुछ महीनों के नियमित व्यवहार के बाद इतिहास बनना शुरू हो जाता है। यहाँ उद्देश्य उधार लेना नहीं है; उद्देश्य यह दर्ज कराना है कि आप समय पर चुकाते हैं।',
          'और जिनका स्कोर पहले अच्छा था पर अब कोई सक्रिय खाता नहीं है, उनके लिए भी यही बात लागू होती है। इतिहास पुराना होकर धुँधला पड़ जाता है, इसलिए एक छोटा सक्रिय खाता बनाए रखना उपयोगी है।',
        ],
        pEn: [
          'There is a category that gets discussed less: people who have never taken a card or a loan, and therefore have no score at all.',
          'Such a person’s report carries a particular marker instead of a score, meaning "insufficient history". That is not a poor score, but for a lender it is still an uncertainty.',
          'The result is that somebody who has never borrowed in their life often finds the first home loan difficult — which feels unfair and makes technical sense at the same time.',
          'The usual remedy is to start small: a plain card, or a card taken against a deposit, used for small regular expenses with the bill cleared in full every month.',
          'After a few months of steady behaviour a history begins to form. The purpose here is not to borrow; the purpose is to have it recorded that you repay on time.',
          'The same applies to somebody whose score was once good but who now has no active account. History fades as it ages, so keeping one small active account is worthwhile.',
        ],
      },
    ],
  },
  {
    slug: 'getting-out-of-debt-trap',
    sections: [
      {
        hHi: 'दो तरीक़े: सबसे महँगा पहले या सबसे छोटा पहले',
        hEn: 'Two methods: costliest first or smallest first',
        pHi: [
          'क़र्ज़ चुकाने के दो प्रचलित तरीक़े हैं और दोनों काम करते हैं, पर वे अलग-अलग चीज़ों को महत्व देते हैं। यह जान लेना ज़रूरी है कि आपके लिए कौन सा उपयुक्त है।',
          'पहला तरीक़ा: सबसे ऊँची ब्याज दर वाले क़र्ज़ पर सारी अतिरिक्त राशि डालिए, बाक़ी पर केवल न्यूनतम चलाइए। जब वह चुक जाए, तो अगली सबसे ऊँची दर पर वही राशि डालिए।',
          'यह गणितीय रूप से सबसे सस्ता रास्ता है — इसमें कुल ब्याज सबसे कम चुकाना पड़ता है, और अवधि भी सबसे कम रहती है।',
          'दूसरा तरीक़ा: सबसे छोटी राशि वाला क़र्ज़ पहले पूरा कीजिए, चाहे उसकी दर कम ही क्यों न हो। फिर अगला सबसे छोटा। इसमें कुछ ज़्यादा ब्याज लगता है।',
          'तो दूसरा तरीक़ा क्यों? क्योंकि पहला क़र्ज़ पूरी तरह ख़त्म होना एक ऐसा अनुभव है जो हौसला देता है, और क़र्ज़ चुकाना जितना गणित है उतना ही धैर्य का काम है।',
          'जिस व्यक्ति ने पहले कई बार कोशिश करके छोड़ दिया है, उसके लिए दूसरा तरीक़ा अक्सर बेहतर है। जो व्यक्ति संख्याओं से प्रेरित होता है, उसके लिए पहला। दोनों में से जो आप पूरा कर पाएँगे, वही आपके लिए सही है।',
        ],
        pEn: [
          'There are two well-known methods for clearing debt and both work, but they value different things. It is worth knowing which one suits you.',
          'The first: put every spare rupee on the debt with the highest interest rate and keep the others at their minimum. When it clears, move that same amount to the next highest rate.',
          'This is mathematically the cheapest route — the least total interest and the shortest overall period.',
          'The second: clear the smallest balance first, even if its rate is low. Then the next smallest. This costs somewhat more in interest.',
          'So why the second? Because finishing one debt completely is an experience that builds momentum, and clearing debt is as much a matter of persistence as of arithmetic.',
          'For somebody who has tried and abandoned the effort several times before, the second method is often better. For somebody driven by numbers, the first. Whichever one you will actually finish is the right one for you.',
        ],
      },
      {
        hHi: 'ऋणदाता से बात करना',
        hEn: 'Talking to the lender',
        pHi: [
          'जब चुकाना कठिन हो जाए, तो सबसे आम प्रतिक्रिया यह होती है कि फ़ोन उठाना बंद कर दिया जाए। यह पूरी स्थिति में सबसे महँगा क़दम है।',
          'ऋणदाता के पास चूक की स्थिति के लिए विकल्प होते हैं, और वे विकल्प तभी काम आते हैं जब बातचीत जल्दी हो। चूक हो जाने के बाद गुंजाइश कम हो जाती है।',
          'उपलब्ध विकल्पों में आमतौर पर ये होते हैं: अवधि बढ़ाकर किश्त घटाना, कुछ महीनों के लिए भुगतान रोकना, या शेष राशि को कम दर वाली किश्तों में बदलना।',
          'बात करने से पहले तैयारी कीजिए: कुल आय, कुल आवश्यक ख़र्च, और वह राशि जो आप वास्तव में हर महीने चुका सकते हैं। एक ठोस प्रस्ताव लेकर जाना केवल कठिनाई बताने से कहीं बेहतर काम करता है।',
          'जो भी तय हो, उसे लिखित में लीजिए। मौखिक आश्वासन उस कर्मचारी के तबादले के साथ ख़त्म हो जाते हैं, और बाद में उनका कोई प्रमाण नहीं होता।',
          'और यह भी समझ लीजिए कि पुनर्गठन का असर क्रेडिट रिपोर्ट पर पड़ता है। यह असर चूक के असर से बहुत कम होता है, पर शून्य नहीं होता, और इसे पहले से जानना बेहतर है।',
        ],
        pEn: [
          'When repayment becomes difficult, the commonest reaction is to stop answering the phone. That is the most expensive step in the whole situation.',
          'Lenders have options for cases of difficulty, and those options only work if the conversation happens early. Once a default has occurred the room narrows.',
          'The options usually available include extending the term to reduce the instalment, pausing payments for some months, or converting the balance into instalments at a lower rate.',
          'Prepare before calling: total income, total essential expenses, and the amount you can genuinely pay each month. Arriving with a concrete proposal works far better than describing the difficulty alone.',
          'Whatever is agreed, get it in writing. Verbal assurances end with the transfer of the employee who gave them, and afterwards there is no proof.',
          'And understand that restructuring does affect the credit report. That effect is much smaller than the effect of a default, but it is not nil, and it is better known in advance.',
        ],
      },
      {
        hHi: 'वसूली एजेंटों के सामने आपके अधिकार',
        hEn: 'Your rights in front of recovery agents',
        pHi: [
          'यह वह विषय है जिस पर सबसे कम जानकारी है और सबसे ज़्यादा डर। स्थिति चाहे जितनी कठिन हो, उधार लेने वाले के कुछ स्पष्ट अधिकार हैं।',
          'वसूली के लिए संपर्क करने का समय निर्धारित है — रात में या बहुत सुबह संपर्क करना नियमों के विरुद्ध है। धमकी, अपमान, या परिवार और पड़ोसियों के सामने शर्मिंदा करना किसी भी स्थिति में स्वीकार्य नहीं है।',
          'कार्यस्थल पर जाकर सहकर्मियों के सामने बात करना, या रिश्तेदारों को फ़ोन करके क़र्ज़ की जानकारी देना भी उचित आचरण के दायरे से बाहर है।',
          'अगर ऐसा हो रहा है, तो पहला क़दम है लिखित शिकायत — उसी ऋणदाता के शिकायत निवारण अधिकारी को, तारीख़ और घटना के विवरण के साथ। दूसरा क़दम, संतोषजनक उत्तर न मिलने पर, संबंधित नियामक के लोकपाल के पास जाना।',
          'हर बातचीत का रिकॉर्ड रखिए — तारीख़, समय, नाम, और क्या कहा गया। यह रिकॉर्ड बाद में सबसे मज़बूत दस्तावेज़ बनता है।',
          'और यह याद रखिए कि क़र्ज़ चुकाना एक वित्तीय दायित्व है, अपमान सहने का कारण नहीं। जो व्यक्ति अपने अधिकार जानता है, वह बातचीत में कहीं बेहतर स्थिति में होता है।',
        ],
        pEn: [
          'This is the subject with the least information and the most fear attached to it. However difficult the situation, a borrower has clear rights.',
          'The hours during which recovery contact may be made are prescribed — contacting late at night or very early is against the rules. Threats, abuse, or humiliating somebody in front of family and neighbours are not acceptable in any circumstances.',
          'Turning up at a workplace and discussing the matter in front of colleagues, or telephoning relatives to tell them about the debt, also falls outside proper conduct.',
          'If this is happening, the first step is a written complaint to the lender’s own grievance redressal officer, with dates and details of what occurred. The second step, if the answer is unsatisfactory, is the relevant regulator’s ombudsman.',
          'Keep a record of every conversation — date, time, name, and what was said. That record becomes the strongest document later.',
          'And remember that repaying a debt is a financial obligation, not a reason to accept humiliation. Somebody who knows their rights is in a far better position in the conversation.',
        ],
      },
      {
        hHi: 'निकलने के बाद की व्यवस्था',
        hEn: 'The arrangement after getting out',
        pHi: [
          'क़र्ज़ चुक जाने का दिन महत्वपूर्ण है, पर उससे भी महत्वपूर्ण वह है जो उसके अगले महीने से होता है। बहुत से लोग दो साल में वापस उसी जगह पहुँच जाते हैं।',
          'सबसे उपयोगी क़दम यह है कि जो राशि हर महीने किश्त में जा रही थी, उसे बंद मत होने दीजिए। उसी राशि का स्वचालित निर्देश अब बचत की ओर मोड़ दीजिए, उसी तारीख़ को।',
          'यह क़दम इसलिए काम करता है क्योंकि आपका बजट पहले से उस राशि के बिना चलने का आदी है। जो पैसा आपने पिछले तीन साल नहीं देखा, उसे बचत में डालना कोई कटौती महसूस नहीं होती।',
          'दूसरा क़दम: पहले तीन से छह महीने का आपातकालीन कोष बनाइए, उसके बाद ही किसी और लक्ष्य की ओर बढ़िए। यही वह चीज़ है जिसकी कमी ने पहली बार क़र्ज़ को जन्म दिया था।',
          'तीसरा: सारे अनापत्ति प्रमाणपत्र सँभालकर रखिए और कुछ महीनों बाद क्रेडिट रिपोर्ट में जाँचिए कि सब खाते बंद दिखाए गए हैं।',
          'और चौथा, जो सबसे कम तकनीकी है: यह लिख लीजिए कि क़र्ज़ किस कारण बना था। वही कारण दोबारा आएगा, किसी और रूप में, और लिखा हुआ अनुभव उसे पहचानने में मदद करता है।',
        ],
        pEn: [
          'The day the debt clears matters, but what happens from the following month matters more. A great many people are back in the same place within two years.',
          'The most useful step is not to let the instalment amount disappear. Redirect a standing instruction for the same amount into savings, on the same date.',
          'This works because your budget is already used to running without that money. Money you have not seen for three years does not feel like a cut when it goes into savings.',
          'Second step: build three to six months of emergency fund before moving towards any other goal. The absence of exactly that is what created the debt in the first place.',
          'Third: keep every no-objection certificate, and check the credit report a few months later to see all the accounts shown as closed.',
          'And fourth, the least technical of all: write down why the debt happened. The same cause will return in some other form, and a written account of it helps you recognise it.',
        ],
      },
      {
        hHi: 'क्या नहीं करना चाहिए',
        hEn: 'What not to do',
        pHi: [
          'क़र्ज़ से निकलने की कोशिश में कुछ रास्ते ऐसे हैं जो राहत जैसे लगते हैं और स्थिति को गहरा कर देते हैं। इन्हें पहचान लेना उतना ही ज़रूरी है जितना सही रास्ता जानना।',
          'पहला: एक क़र्ज़ चुकाने के लिए दूसरा क़र्ज़ लेना, बिना कुल लागत की तुलना किए। समेकन तभी सार्थक है जब नई दर वास्तव में कम हो और शुल्क मिलाकर भी कुल राशि घटे।',
          'दूसरा: सेवानिवृत्ति के लिए रखी गई राशि निकाल लेना। वह पैसा वापस नहीं आता, और उसका असल नुक़सान आज की राशि नहीं बल्कि वह वृद्धि है जो अगले बीस साल में होती।',
          'तीसरा: उन सेवाओं के पास जाना जो शुल्क लेकर स्कोर सुधारने या क़र्ज़ मिटाने का वादा करती हैं। जो काम वे करती हैं वह आप ख़ुद निःशुल्क कर सकते हैं, और जो वे वादा करती हैं वह अक्सर संभव ही नहीं।',
          'चौथा: बिना दस्तावेज़ के अनौपचारिक स्रोतों से उधार लेना। वहाँ दर अक्सर बहुत ऊँची होती है और कोई नियामक संरक्षण नहीं होता।',
          'और पाँचवाँ: स्थिति को परिवार से पूरी तरह छिपाना। छिपाने से ख़र्च की आदतें नहीं बदलतीं, और अकेले लिया गया हर फ़ैसला जल्दबाज़ी में लिया गया फ़ैसला बन जाता है।',
        ],
        pEn: [
          'In the effort to get out of debt some routes feel like relief and make the situation deeper. Recognising them matters as much as knowing the right route.',
          'First: taking a second loan to pay off the first without comparing total cost. Consolidation only makes sense when the new rate is genuinely lower and the total, fees included, actually falls.',
          'Second: withdrawing money set aside for retirement. That money does not come back, and the real loss is not today’s amount but the growth it would have produced over the next twenty years.',
          'Third: going to services that charge a fee and promise to repair a score or erase a debt. What they do you can do yourself for nothing, and what they promise is frequently not possible at all.',
          'Fourth: borrowing from informal sources without documentation. The rate there is usually very high and there is no regulatory protection.',
          'And fifth: hiding the situation entirely from the family. Hiding it does not change spending habits, and every decision taken alone becomes a decision taken in haste.',
        ],
      },
    ],
  },
  {
    slug: 'cibil-score-myths',
    sections: [
      {
        hHi: 'अंत में एक बात',
        hEn: 'One closing point',
        pHi: [
          'स्कोर एक चरित्र प्रमाणपत्र नहीं है। वह केवल यह दर्ज करता है कि आपने उधार लेकर उसे कैसे चुकाया, और उससे ज़्यादा कुछ नहीं कहता।',
          'जिसका स्कोर कम है वह बुरा व्यक्ति नहीं है, और जिसका ऊँचा है वह अमीर नहीं है। दोनों बातें अलग-अलग हैं और उन्हें अलग ही रखना चाहिए।',
        ],
        pEn: [
          'A credit score is not a character certificate. It records only how you repaid what you borrowed, and it says nothing beyond that.',
          'Somebody with a low score is not a bad person, and somebody with a high one is not wealthy. Those are separate things and should be kept separate.',
        ],
      },
    ],
  },
  {
    slug: 'getting-out-of-debt-trap',
    sections: [
      {
        hHi: 'पहला महीना कैसा दिखता है',
        hEn: 'What the first month looks like',
        pHi: [
          'योजना बनाने और योजना चलने के बीच एक महीना होता है, और वह महीना सबसे कठिन होता है क्योंकि उसमें कुछ भी बदला हुआ नहीं दिखता।',
          'शेष राशि लगभग वही रहती है, फ़ोन आना बंद नहीं होते, और मेहनत का कोई नतीजा नज़र नहीं आता। यही वह जगह है जहाँ अधिकांश लोग रुक जाते हैं।',
          'इसलिए पहले महीने का लक्ष्य राशि नहीं, निरंतरता होनी चाहिए — तय की गई राशि तय तारीख़ को चुक जाए, बस इतना। परिणाम तीसरे महीने से दिखने लगते हैं।',
        ],
        pEn: [
          'Between making a plan and the plan working there is one month, and it is the hardest month because nothing appears to have changed.',
          'The balance stays much the same, the phone calls do not stop, and there is no visible result for the effort. This is exactly where most people give up.',
          'So the goal of the first month should be consistency rather than amount — the agreed sum paid on the agreed date, and nothing more than that. Results start showing from the third month.',
        ],
      },
    ],
  },
  {
    slug: 'upi-safe-habits',
    sections: [
      {
        hHi: 'यूपीआई असल में काम कैसे करता है',
        hEn: 'How UPI actually works',
        pHi: [
          'सुरक्षित रहने का सबसे अच्छा तरीक़ा यह समझना है कि व्यवस्था बनी कैसे है। जो व्यक्ति ढाँचा जानता है, उसे हर नए धोखे के लिए अलग चेतावनी की ज़रूरत नहीं होती।',
          'यूपीआई में आपकी पहचान एक पते के रूप में होती है, और वह पता आपके बैंक खाते से जुड़ा होता है। पता जानने से कोई आपके खाते से पैसा नहीं निकाल सकता — वह केवल आपको पैसा भेजने के काम आता है।',
          'पैसा खाते से बाहर तभी जाता है जब आप अपना पिन डालते हैं। पिन केवल भेजने के लिए है। यह एक वाक्य पूरी सुरक्षा का आधार है और यही सबसे ज़्यादा भुलाया जाता है।',
          'पैसा प्राप्त करने के लिए कभी पिन नहीं डालना पड़ता। कोई क्यूआर कोड स्कैन करने के बाद पिन माँगे और कहे कि इससे पैसा आएगा, तो वह अनुरोध पैसा भेजने का है, पाने का नहीं।',
          'ऐप में एक "संग्रह अनुरोध" की सुविधा भी होती है, जिसमें कोई आपसे पैसे माँग सकता है और वह अनुरोध सूचना के रूप में आता है। उसे स्वीकार करना भी पैसा भेजना ही है।',
          'यह ढाँचा समझ लेने के बाद अधिकांश धोखे अपने-आप पहचान में आ जाते हैं, क्योंकि वे सब एक ही बात पर टिके हैं: आपसे पिन डलवाना।',
        ],
        pEn: [
          'The best way to stay safe is to understand how the system is built. Somebody who knows the structure does not need a separate warning for every new scam.',
          'In UPI your identity takes the form of an address linked to your bank account. Knowing that address does not let anyone take money out — it only lets them send money to you.',
          'Money leaves the account only when you enter your PIN. The PIN is for sending. That one sentence is the foundation of the whole of safety, and it is the thing most often forgotten.',
          'Receiving money never requires a PIN. If scanning a QR code leads to a PIN prompt with the claim that money will arrive, that request is to send money, not to receive it.',
          'Apps also carry a "collect request" facility, through which somebody can ask you for money and the request arrives as a notification. Approving one is also sending money.',
          'Once this structure is understood, most scams identify themselves, because they all rest on the same thing: getting you to enter your PIN.',
        ],
      },
      {
        hHi: 'सबसे आम धोखे और उनका साझा पैटर्न',
        hEn: 'The commonest scams and their shared pattern',
        pHi: [
          'धोखे बदलते रहते हैं पर उनका ढाँचा लगभग हमेशा एक ही रहता है: जल्दबाज़ी, अधिकार का दिखावा, और एक ऐसा क़दम जिसके लिए आपको ख़ुद कुछ करना पड़ता है।',
          'सबसे आम है "ग़लती से पैसे भेज दिए, कृपया लौटा दीजिए"। सूचना नक़ली होती है, पैसा आया ही नहीं होता, और लौटाने के नाम पर आपसे असली पैसा भेजवाया जाता है। पहले बैंक का शेष जाँचिए, ऐप की सूचना पर भरोसा मत कीजिए।',
          'दूसरा, बिक्री-ख़रीद के मंचों पर: ख़रीदार पैसा भेजने के बजाय एक संग्रह अनुरोध भेजता है और कहता है कि स्वीकार करने से पैसा आ जाएगा। स्वीकार करते ही पैसा जाता है।',
          'तीसरा, नक़ली ग्राहक सेवा नंबर। खोज परिणामों में दिखने वाला नंबर अक्सर असली नहीं होता। नंबर हमेशा बैंक की आधिकारिक वेबसाइट या पासबुक से लीजिए।',
          'चौथा, स्क्रीन साझा करने वाले ऐप। कोई भी असली संस्था आपसे स्क्रीन साझा करने या दूरस्थ नियंत्रण वाला ऐप इंस्टॉल करने के लिए नहीं कहती। यह अनुरोध अपने-आप में एक निर्णायक संकेत है।',
          'और पाँचवाँ, नक़ली भुगतान स्क्रीनशॉट — दुकानदारों के साथ सबसे ज़्यादा। भुगतान की पुष्टि हमेशा अपने ऐप या बैंक की सूचना से कीजिए, सामने वाले के फ़ोन की तस्वीर से नहीं।',
        ],
        pEn: [
          'Scams keep changing but their structure stays almost identical: haste, an appearance of authority, and one step that requires you to do something yourself.',
          'The commonest is "money was sent to you by mistake, please return it". The notification is fake, no money arrived, and in the name of returning it you send real money. Check the bank balance first; do not trust an in-app notification.',
          'Second, on buying-and-selling platforms: instead of sending money the "buyer" sends a collect request and says approving it will bring the money in. Approving it sends money out.',
          'Third, fake customer-service numbers. A number appearing in search results is often not genuine. Always take the number from the bank’s official website or your passbook.',
          'Fourth, screen-sharing apps. No genuine institution asks you to share your screen or install a remote-control app. That request is by itself a conclusive signal.',
          'And fifth, fake payment screenshots — most common with shopkeepers. Always confirm a payment from your own app or bank alert, never from a picture on somebody else’s phone.',
        ],
      },
      {
        hHi: 'रोज़ की आदतें जो जोखिम घटाती हैं',
        hEn: 'Daily habits that reduce the risk',
        pHi: [
          'सुरक्षा किसी एक बड़े क़दम से नहीं आती, कुछ छोटी आदतों से आती है जिन्हें एक बार अपनाने के बाद सोचना नहीं पड़ता।',
          'पहली: यूपीआई के लिए एक अलग खाता रखिए जिसमें कम राशि हो। मुख्य बचत खाता यूपीआई से जोड़कर मत रखिए। जो नुक़सान हो सकता है उसकी सीमा इसी एक क़दम से तय हो जाती है।',
          'दूसरी: ऐप में लेन-देन की दैनिक सीमा घटाकर उतनी कर दीजिए जितनी आपको वास्तव में चाहिए। यह सुविधा हर ऐप में उपलब्ध है और इसका उपयोग बहुत कम लोग करते हैं।',
          'तीसरी: पिन कहीं लिखकर मत रखिए, और वह जन्मतिथि या फ़ोन नंबर का हिस्सा न हो। साथ ही, फ़ोन पर स्क्रीन लॉक ज़रूर रखिए — बिना लॉक वाला फ़ोन खो जाए तो पिन अकेला बचाव नहीं रहता।',
          'चौथी: हर लेन-देन की सूचना चालू रखिए, और उन्हें पढ़िए। जो व्यक्ति हर सूचना देखता है, उसे किसी भी अनधिकृत लेन-देन का पता उसी दिन चल जाता है।',
          'पाँचवीं: ऐप केवल आधिकारिक स्टोर से इंस्टॉल कीजिए, और किसी संदेश या लिंक के ज़रिए भेजी गई फ़ाइल से कभी नहीं।',
        ],
        pEn: [
          'Safety does not come from one large step; it comes from a few small habits that need no thought once adopted.',
          'First: keep a separate account for UPI holding a small balance. Do not link the main savings account. This one step sets the ceiling on how much can ever be lost.',
          'Second: reduce the daily transaction limit in the app to what you genuinely need. The facility exists in every app and very few people use it.',
          'Third: do not write the PIN down anywhere, and do not build it from a date of birth or a phone number. Keep a screen lock on the phone as well — with no lock, a lost phone leaves the PIN as the only defence.',
          'Fourth: keep alerts on for every transaction, and read them. Somebody who reads every alert learns about an unauthorised transaction the same day.',
          'Fifth: install apps only from the official store, and never from a file sent through a message or a link.',
        ],
      },
      {
        hHi: 'अगर पैसा चला जाए तो पहले घंटे में क्या करें',
        hEn: 'What to do in the first hour if money is gone',
        pHi: [
          'यहाँ गति सबसे महत्वपूर्ण है। जितनी जल्दी सूचना दी जाती है, राशि रोक पाने की संभावना उतनी ही अधिक होती है, और कुछ घंटों बाद वह संभावना तेज़ी से घटती है।',
          'पहला काम: बैंक के आधिकारिक ग्राहक सेवा नंबर पर फ़ोन कीजिए और लेन-देन की सूचना दीजिए। नंबर अपनी पासबुक, कार्ड के पीछे या बैंक की आधिकारिक वेबसाइट से लीजिए।',
          'दूसरा: ऐप में यूपीआई सेवा या खाता अस्थायी रूप से रोकिए, ताकि आगे कोई और लेन-देन न हो सके।',
          'तीसरा: राष्ट्रीय साइबर अपराध हेल्पलाइन पर शिकायत दर्ज कीजिए। जल्दी दर्ज की गई शिकायत पर राशि रोक पाने की व्यवस्था मौजूद है।',
          'चौथा: सब कुछ लिख लीजिए — समय, राशि, प्राप्तकर्ता का पता या नंबर, और आपने किससे बात की। स्क्रीनशॉट लीजिए, संदेश मत मिटाइए।',
          'और पाँचवाँ: बैंक में लिखित शिकायत दर्ज कीजिए और उसकी पावती लीजिए। मौखिक शिकायत का कोई रिकॉर्ड नहीं रहता, और आगे की हर प्रक्रिया उसी पावती संख्या पर चलती है।',
        ],
        pEn: [
          'Speed matters most here. The sooner it is reported, the greater the chance of the amount being held, and that chance falls sharply after a few hours.',
          'First task: call the bank’s official customer-service number and report the transaction. Take the number from your passbook, the back of your card, or the bank’s official website.',
          'Second: temporarily block the UPI service or the account in the app, so that no further transaction can go through.',
          'Third: register a complaint on the national cybercrime helpline. A mechanism for holding the amount exists for complaints made early.',
          'Fourth: write everything down — the time, the amount, the recipient’s address or number, and who you spoke to. Take screenshots and do not delete the messages.',
          'And fifth: file a written complaint with the bank and take the acknowledgement. A verbal complaint leaves no record, and every step afterwards runs on that acknowledgement number.',
        ],
      },
      {
        hHi: 'परिवार के बाक़ी लोगों के लिए',
        hEn: 'For the rest of the family',
        pHi: [
          'यूपीआई अब हर उम्र के लोग इस्तेमाल करते हैं, और धोखे सबसे ज़्यादा उन्हें निशाना बनाते हैं जिन्होंने हाल ही में इसे अपनाया है।',
          'बुज़ुर्ग सदस्यों के फ़ोन में सीमा कम रखिए, अलग खाता जोड़िए, और सूचना उनके साथ-साथ परिवार के किसी और सदस्य तक भी पहुँचे — यह कई बैंकों में संभव है।',
          'उन्हें नियम याद कराने के बजाय एक वाक्य सिखाइए: "पैसा लेने के लिए पिन कभी नहीं"। यह एक वाक्य लंबी सूची से ज़्यादा काम करता है।',
          'और एक दूसरा वाक्य: "जल्दी करने को कहे तो रुक जाइए"। हर धोखा जल्दबाज़ी पर टिका होता है, इसलिए रुकना अपने-आप में एक बचाव है।',
          'बच्चों और किशोरों के लिए अलग बात है: उन्हें खेल, ऐप और ऑनलाइन ख़रीद के बहाने भेजे गए लिंक निशाना बनाते हैं। उनके फ़ोन में भुगतान की सीमा और भी कम होनी चाहिए।',
          'और सबसे ज़रूरी: घर में यह माहौल रखिए कि धोखा हो जाने पर कोई डाँट नहीं पड़ेगी। जो व्यक्ति डर के कारण घंटों चुप रहता है, वही वह समय गँवा देता है जिसमें पैसा रोका जा सकता था।',
        ],
        pEn: [
          'UPI is now used by people of every age, and scams target most heavily those who have adopted it recently.',
          'On older family members’ phones keep the limit low, link a separate account, and arrange for alerts to reach another family member as well — many banks allow this.',
          'Rather than making them memorise rules, teach one sentence: "never a PIN to receive money". That single sentence does more than a long list.',
          'And a second sentence: "if you are told to hurry, stop". Every scam rests on haste, so stopping is itself a defence.',
          'For children and teenagers it is different: links sent under the cover of games, apps and online purchases are what target them. Payment limits on their phones should be lower still.',
          'And most important: keep a household atmosphere where being scammed brings no scolding. Somebody who stays silent for hours out of fear loses exactly the time in which the money could have been stopped.',
        ],
      },
    ],
  },
  {
    slug: 'upi-safe-habits',
    sections: [
      {
        hHi: 'दुकानदारों के लिए कुछ बातें',
        hEn: 'A few points for shopkeepers',
        pHi: [
          'यूपीआई से सबसे ज़्यादा लेन-देन छोटे दुकानदार करते हैं, और उन पर निशाना भी सबसे ज़्यादा लगता है, क्योंकि वहाँ भीड़, जल्दबाज़ी और भरोसा तीनों एक साथ होते हैं।',
          'सबसे ज़रूरी नियम: भुगतान की पुष्टि हमेशा अपने ऐप या ध्वनि उपकरण से कीजिए। ग्राहक के फ़ोन पर दिखती हुई सफलता की स्क्रीन प्रमाण नहीं है, और नक़ली स्क्रीन बनाना बहुत आसान है।',
          'दूसरा: क्यूआर कोड को समय-समय पर देखिए कि उस पर किसी ने दूसरा स्टिकर तो नहीं चिपका दिया। यह एक पुराना तरीक़ा है और आज भी चलता है।',
          'तीसरा: व्यवसाय का खाता व्यक्तिगत खाते से अलग रखिए। हिसाब साफ़ रहता है, और किसी गड़बड़ी की स्थिति में नुक़सान एक ही जगह तक सीमित रहता है।',
        ],
        pEn: [
          'Small shopkeepers make the largest number of UPI transactions, and they are also targeted most, because crowds, haste and trust all come together at a counter.',
          'The most important rule: always confirm a payment from your own app or sound device. A success screen showing on the customer’s phone is not proof, and a fake screen is very easy to produce.',
          'Second: check the QR code from time to time to make sure nobody has pasted another sticker over it. This is an old method and it still works.',
          'Third: keep the business account separate from the personal one. The bookkeeping stays clean, and if something does go wrong the loss is contained in one place.',
        ],
      },
    ],
  },
  {
    slug: 'digital-payment-fraud-types',
    sections: [
      {
        hHi: 'हर धोखे का एक ही ढाँचा',
        hEn: 'The single structure behind every scam',
        pHi: [
          'धोखे के नाम और तरीक़े हर कुछ महीनों में बदल जाते हैं, इसलिए सूची याद रखना काम नहीं आता। जो काम आता है वह है ढाँचा पहचानना, क्योंकि ढाँचा नहीं बदलता।',
          'हर धोखे में तीन चीज़ें होती हैं। पहली: एक ऐसी कहानी जो भावना जगाए — डर, लालच, सहानुभूति या जल्दबाज़ी। यह आपकी सोचने की क्षमता को कुछ मिनटों के लिए कम कर देती है।',
          'दूसरी: अधिकार का दिखावा। बैंक, पुलिस, बिजली विभाग, कोई सरकारी योजना, या कोई बड़ी कंपनी। असली संस्थाएँ इस तरह संपर्क नहीं करतीं, पर उस क्षण यह याद नहीं रहता।',
          'तीसरी, और यह सबसे महत्वपूर्ण है: एक ऐसा क़दम जो आपको ख़ुद उठाना पड़ता है। पिन डालना, ओटीपी बताना, कोई ऐप इंस्टॉल करना, किसी लिंक पर विवरण भरना।',
          'यह तीसरी बात ही बचाव की कुंजी है। कोई भी आपके खाते से पैसा तब तक नहीं निकाल सकता जब तक आप कोई क़दम न उठाएँ। इसलिए हर बार यही पूछिए: यह मुझसे क्या करवाना चाहता है?',
          'अगर उत्तर है "कुछ भी", तो रुक जाइए। जो अनुरोध रुकने से ख़त्म हो जाए, वह असली नहीं था। कोई भी असली प्रक्रिया दस मिनट रुक जाने से रद्द नहीं होती।',
        ],
        pEn: [
          'The names and methods of scams change every few months, so memorising a list does not help. What helps is recognising the structure, because the structure does not change.',
          'Every scam has three things. The first: a story that provokes emotion — fear, greed, sympathy or urgency. That reduces your capacity to think for a few minutes.',
          'The second: an appearance of authority. A bank, the police, the electricity department, some government scheme, or a large company. Real institutions do not make contact this way, but in that moment it is not remembered.',
          'The third, and the most important: a step that you have to take yourself. Entering a PIN, reading out an OTP, installing an app, filling in details on a link.',
          'That third element is the key to safety. Nobody can take money from your account unless you act. So ask the same question every time: what is this trying to make me do?',
          'If the answer is "anything at all", stop. A request that dies when you pause was never genuine. No real process is cancelled because you waited ten minutes.',
        ],
      },
      {
        hHi: 'ओटीपी और केवाईसी वाले धोखे',
        hEn: 'OTP and KYC scams',
        pHi: [
          'ओटीपी वह अंतिम ताला है जो बैंक ने आपके लिए लगाया है, और लगभग हर बड़ा धोखा उसी ताले की चाबी माँगने पर टिका होता है।',
          'नियम एक पंक्ति का है: ओटीपी किसी को नहीं बताना है। बैंक कर्मचारी को नहीं, पुलिस को नहीं, कूरियर वाले को नहीं, और उस व्यक्ति को भी नहीं जो कहता है कि वह इसे रद्द करने के लिए माँग रहा है।',
          'संदेश में ओटीपी के साथ हमेशा यह भी लिखा होता है कि वह किस काम के लिए है। उसे पढ़िए। अगर संदेश में किसी भुगतान का ज़िक्र है और आपको बताया गया था कि यह "सत्यापन" है, तो वह भुगतान ही है।',
          'केवाईसी वाला धोखा इससे थोड़ा अलग है। संदेश आता है कि खाता आज बंद हो जाएगा अगर केवाईसी अपडेट नहीं हुई, और साथ में एक लिंक या नंबर होता है।',
          'असली केवाईसी कभी लिंक भेजकर, ऐप इंस्टॉल कराकर या फ़ोन पर विवरण पूछकर नहीं होती। वह शाखा में या बैंक के अपने आधिकारिक माध्यम से होती है, और उसके लिए कभी भी इतनी जल्दी नहीं होती।',
          'अगर संदेह हो, तो उस संदेश में दिए नंबर पर मत लौटिए। अपनी पासबुक या कार्ड के पीछे लिखे नंबर पर ख़ुद फ़ोन कीजिए। यह एक अंतर पूरे फ़र्क़ का कारण बनता है।',
        ],
        pEn: [
          'The OTP is the final lock the bank has put in place for you, and nearly every large scam rests on asking for the key to that lock.',
          'The rule is one line: the OTP is told to nobody. Not to a bank employee, not to the police, not to a delivery agent, and not to the person who says they need it in order to cancel something.',
          'The message carrying an OTP always states what it is for. Read it. If it mentions a payment and you were told this was "verification", it is a payment.',
          'The KYC scam works a little differently. A message says the account will be closed today unless KYC is updated, and it carries a link or a number.',
          'Genuine KYC never happens by sending a link, having an app installed, or asking for details over the phone. It happens at a branch or through the bank’s own official channel, and it is never that urgent.',
          'If in doubt, do not call back on the number in the message. Ring the number on your own passbook or the back of your card. That single difference makes all the difference.',
        ],
      },
      {
        hHi: 'नौकरी, निवेश और लॉटरी वाले धोखे',
        hEn: 'Job, investment and lottery scams',
        pHi: [
          'ये वे धोखे हैं जिनमें कोई तकनीकी सेंध नहीं लगती। यहाँ पीड़ित ख़ुद पैसा भेजता है, अपनी इच्छा से, और अक्सर कई किश्तों में।',
          'नौकरी वाला ढाँचा: एक अच्छा प्रस्ताव, फिर पंजीकरण शुल्क, फिर प्रशिक्षण शुल्क, फिर सुरक्षा जमा। असली नियोक्ता नौकरी देने के लिए पैसा नहीं लेता, कभी नहीं।',
          'निवेश वाला ढाँचा: एक ऐसा रिटर्न जो बाज़ार से कहीं ज़्यादा हो और जोखिम-रहित बताया जाए, अक्सर किसी समूह चैट के ज़रिए। शुरुआती छोटी निकासी सफल होती है ताकि भरोसा बने, फिर बड़ी राशि आमंत्रित की जाती है।',
          'यहाँ पहचान का सबसे सरल पैमाना यही है: कोई भी वैध निवेश निश्चित ऊँचे रिटर्न का वादा नहीं करता। जहाँ जोखिम का ज़िक्र नहीं है, वहाँ जोखिम पूरा है।',
          'लॉटरी और उपहार वाला ढाँचा: आपने कुछ जीता है, बस "कर" या "प्रसंस्करण शुल्क" भेज दीजिए। जिस प्रतियोगिता में आपने भाग नहीं लिया, उसे आप जीत नहीं सकते।',
          'तीनों में एक साझा संकेत है: पैसा पाने के लिए पहले पैसा भेजना। यह वाक्य सुनते ही बातचीत ख़त्म कर देनी चाहिए।',
        ],
        pEn: [
          'These are the scams with no technical break-in at all. Here the victim sends the money themselves, willingly, and usually in several instalments.',
          'The job structure: an attractive offer, then a registration fee, then a training fee, then a security deposit. A genuine employer never takes money in order to give you a job.',
          'The investment structure: a return far above the market described as risk-free, usually through a group chat. Small early withdrawals succeed so that trust is built, and then larger sums are invited.',
          'The simplest test here is this: no legitimate investment promises a fixed high return. Where risk is not mentioned, the risk is total.',
          'The lottery and gift structure: you have won something, just send the "tax" or the "processing fee". You cannot win a competition you never entered.',
          'All three share one signal: sending money in order to receive money. The conversation should end the moment that sentence appears.',
        ],
      },
      {
        hHi: 'नक़ली ऐप, लिंक और वेबसाइट',
        hEn: 'Fake apps, links and websites',
        pHi: [
          'यह श्रेणी सबसे तेज़ी से बढ़ रही है, क्योंकि नक़ली चीज़ें अब देखने में असली से लगभग अलग नहीं होतीं।',
          'ऐप केवल आधिकारिक स्टोर से इंस्टॉल कीजिए, और वहाँ भी प्रकाशक का नाम, डाउनलोड की संख्या और समीक्षाएँ देख लीजिए। किसी संदेश में भेजी गई इंस्टॉल फ़ाइल कभी मत खोलिए।',
          'लिंक की जाँच का सबसे सरल तरीक़ा है पता ध्यान से पढ़ना। धोखेबाज़ पते असली नाम के आसपास बने होते हैं — एक अक्षर बदला हुआ, या नाम के बाद कुछ और जुड़ा हुआ।',
          'सबसे सुरक्षित आदत यह है कि किसी भी वित्तीय काम के लिए लिंक पर क्लिक ही न कीजिए। ऐप ख़ुद खोलिए, या पता ख़ुद टाइप कीजिए। इसमें दस सेकंड ज़्यादा लगते हैं और यह लगभग पूरी श्रेणी को हटा देता है।',
          'सार्वजनिक वाई-फ़ाई पर बैंकिंग मत कीजिए, और फ़ोन तथा ऐप के अपडेट टालिए मत। अधिकांश अपडेट में वही सुधार होते हैं जो ज्ञात कमज़ोरियाँ बंद करते हैं।',
          'और अपने फ़ोन में इंस्टॉल किए गए ऐप की सूची साल में एक बार देखिए। जो ऐप आपको याद नहीं कि आपने कब इंस्टॉल किया, उसे हटा देना ही ठीक है।',
        ],
        pEn: [
          'This category is growing fastest, because fakes are now almost indistinguishable from the real thing by sight.',
          'Install apps only from the official store, and even there check the publisher’s name, the download count and the reviews. Never open an install file sent in a message.',
          'The simplest way to check a link is to read the address carefully. Fraudulent addresses are built around the real name — one character changed, or something appended after the name.',
          'The safest habit is not to click a link for any financial task at all. Open the app yourself, or type the address yourself. It costs ten extra seconds and it removes nearly this whole category.',
          'Do not bank on public Wi-Fi, and do not postpone phone and app updates. Most updates contain precisely the fixes that close known weaknesses.',
          'And look through the list of apps installed on your phone once a year. Any app you cannot remember installing is better removed.',
        ],
      },
      {
        hHi: 'शिकायत कहाँ और कैसे',
        hEn: 'Where and how to complain',
        pHi: [
          'धोखा हो जाने पर सबसे ज़्यादा नुक़सान देरी से होता है, और देरी अक्सर इसलिए होती है क्योंकि लोग जानते ही नहीं कि कहाँ जाना है।',
          'पहला: बैंक को तुरंत सूचित कीजिए, आधिकारिक नंबर पर, और लिखित शिकायत की पावती लीजिए। समय की गणना यहीं से शुरू होती है।',
          'दूसरा: राष्ट्रीय साइबर अपराध रिपोर्टिंग व्यवस्था में शिकायत दर्ज कीजिए। शीघ्र दर्ज शिकायत पर राशि रोकने की प्रक्रिया मौजूद है, और वह प्रक्रिया समय पर निर्भर है।',
          'तीसरा: अगर बैंक से संतोषजनक समाधान न मिले, तो पहले उसके शिकायत निवारण अधिकारी को लिखिए और उसके बाद संबंधित लोकपाल के पास जाइए। यह रास्ता निःशुल्क है।',
          'चौथा: सारे प्रमाण सँभालिए — संदेश, स्क्रीनशॉट, कॉल का रिकॉर्ड, लेन-देन का विवरण। शिकायत की ताक़त इन्हीं पर टिकी होती है।',
          'और पाँचवाँ, जो अक्सर छूट जाता है: घटना के बाद अपने बैंकिंग पासवर्ड और यूपीआई पिन बदलिए, और खाते से जुड़े उपकरणों की सूची जाँचकर अनजान उपकरण हटाइए।',
        ],
        pEn: [
          'After a fraud, most of the damage comes from delay, and the delay usually happens because people do not know where to go.',
          'First: inform the bank immediately, on the official number, and take the acknowledgement of a written complaint. The clock starts from there.',
          'Second: register a complaint on the national cybercrime reporting system. A mechanism for holding the amount exists for complaints filed early, and that mechanism depends on time.',
          'Third: if the bank does not resolve it satisfactorily, write to its grievance redressal officer first and then approach the relevant ombudsman. That route is free.',
          'Fourth: preserve all the evidence — messages, screenshots, call records, transaction details. The strength of a complaint rests on these.',
          'And fifth, which is often skipped: after the incident, change your banking passwords and UPI PIN, and check the list of devices linked to the account and remove any you do not recognise.',
        ],
      },
    ],
  },
  {
    slug: 'digital-payment-fraud-types',
    sections: [
      {
        hHi: 'एक बात जो हर सूची से ज़्यादा काम आती है',
        hEn: 'The one thing worth more than any list',
        pHi: [
          'इस लेख की सारी श्रेणियाँ किसी दिन पुरानी पड़ जाएँगी, क्योंकि तरीक़े बदलते रहते हैं। जो नहीं बदलेगा वह यह है कि हर धोखे को आपकी जल्दबाज़ी चाहिए।',
          'इसलिए एक ही आदत बना लीजिए: किसी भी वित्तीय अनुरोध पर, चाहे वह कितना भी तात्कालिक लगे, दस मिनट रुकिए और किसी एक व्यक्ति को बता दीजिए।',
          'यह दोनों काम मिलकर लगभग हर धोखे को रोक देते हैं, क्योंकि कोई भी धोखा दस मिनट की जाँच और एक दूसरे व्यक्ति की राय, दोनों को एक साथ नहीं झेल सकता।',
        ],
        pEn: [
          'Every category in this article will be out of date some day, because the methods keep changing. What will not change is that every scam needs your haste.',
          'So build one habit: on any financial request, however urgent it appears, wait ten minutes and tell one other person about it.',
          'Those two things together stop very nearly every scam, because no scam survives both ten minutes of checking and a second person’s opinion at the same time.',
        ],
      },
    ],
  },
  {
    slug: 'online-shopping-safety',
    sections: [
      {
        hHi: 'भुगतान का तरीक़ा ही सबसे बड़ी सुरक्षा है',
        hEn: 'The payment method is the biggest protection',
        pHi: [
          'ऑनलाइन ख़रीद में सुरक्षा का सबसे बड़ा हिस्सा यह है कि पैसा किस रास्ते से जाता है, क्योंकि हर रास्ते में विवाद की स्थिति में आपके अधिकार अलग होते हैं।',
          'कार्ड से किया गया भुगतान आमतौर पर सबसे अधिक संरक्षण देता है, क्योंकि उसमें विवाद दर्ज कराने और राशि रोकने की एक स्थापित प्रक्रिया होती है।',
          'सीधे किसी व्यक्ति के खाते में पैसा भेजना सबसे कम संरक्षित है। वहाँ लेन-देन पूरा हो जाने के बाद उसे पलटने की कोई सहज व्यवस्था नहीं होती।',
          'यही कारण है कि सामाजिक मंचों पर बिक्री करने वाले अक्सर सीधे खाते में भुगतान माँगते हैं और मंच के अपने भुगतान रास्ते से बचते हैं। यह अनुरोध अपने-आप में एक चेतावनी है।',
          'डिलीवरी पर नक़द भुगतान भी एक विकल्प है, पर वह केवल आधे रास्ते तक बचाता है — पैसा तभी जाता है जब सामान आता है, पर सामान ग़लत निकलने पर वापसी की प्रक्रिया वही रहती है।',
          'और एक व्यावहारिक आदत: ऑनलाइन ख़रीद के लिए एक ही कार्ड इस्तेमाल कीजिए, जिसकी सीमा सीमित हो। कोई गड़बड़ी हो तो उसे रोकना आसान होता है और बाक़ी जीवन रुकता नहीं।',
        ],
        pEn: [
          'The largest part of safety in online shopping is which route the money travels by, because your rights in a dispute differ with every route.',
          'Paying by card usually gives the most protection, because there is an established process for raising a dispute and holding the amount.',
          'Sending money directly to an individual’s account is the least protected. Once that transaction completes there is no easy mechanism to reverse it.',
          'This is exactly why sellers on social platforms often ask for direct bank payment and avoid the platform’s own payment route. That request is a warning in itself.',
          'Cash on delivery is an option too, but it protects only halfway — the money leaves when the goods arrive, but if the goods are wrong the return process is the same as ever.',
          'And one practical habit: use a single card for online shopping, with a limited ceiling. If something goes wrong it is easy to block, and the rest of your life does not stop.',
        ],
      },
      {
        hHi: 'विक्रेता और सौदे की जाँच',
        hEn: 'Checking the seller and the deal',
        pHi: [
          'बड़े मंचों पर भी हर विक्रेता उस मंच का नहीं होता। अधिकांश जगहों पर मंच केवल जगह देता है और बेचने वाला कोई तीसरा पक्ष होता है।',
          'इसलिए ख़रीद से पहले विक्रेता का नाम, उसकी रेटिंग और उसकी समीक्षाओं की संख्या देखिए। नया विक्रेता, बहुत कम समीक्षाएँ, और असामान्य रूप से कम क़ीमत — तीनों एक साथ हों तो रुक जाइए।',
          'समीक्षाएँ पढ़ते समय केवल तारे मत देखिए, तारीख़ें देखिए। एक ही सप्ताह में आई बहुत सारी उत्साही समीक्षाएँ अक्सर वास्तविक नहीं होतीं।',
          'वापसी और धन-वापसी की नीति ख़रीदने से पहले पढ़िए, बाद में नहीं। कई श्रेणियों में वापसी उपलब्ध ही नहीं होती, और यह बात उत्पाद पृष्ठ पर छोटे अक्षरों में लिखी होती है।',
          'क़ीमत की तुलना दो-तीन जगह कर लीजिए। जो सौदा बाज़ार से बहुत सस्ता है, वह आमतौर पर या तो नक़ली उत्पाद है, या पुराना, या मौजूद ही नहीं है।',
          'और सोशल मीडिया के विज्ञापन से सीधे ख़रीदने से बचिए। उत्पाद का नाम अलग से खोजिए और किसी स्थापित मंच पर देखिए कि वह वहाँ किस क़ीमत पर है।',
        ],
        pEn: [
          'Even on large platforms, not every seller belongs to the platform. In most places the platform provides the space and the seller is a third party.',
          'So before buying, look at the seller’s name, rating and the number of reviews. A new seller, very few reviews and an unusually low price — when all three appear together, stop.',
          'While reading reviews, look at the dates rather than only the stars. A large number of enthusiastic reviews arriving in one week is often not genuine.',
          'Read the return and refund policy before buying, not after. In several categories returns are not available at all, and that appears in small type on the product page.',
          'Compare the price in two or three places. A deal far below the market is usually either a counterfeit, or old stock, or does not exist.',
          'And avoid buying directly from a social-media advertisement. Search for the product name separately and see what it costs on an established platform.',
        ],
      },
      {
        hHi: 'ऑर्डर के बाद और डिलीवरी के समय',
        hEn: 'After the order and at delivery',
        pHi: [
          'ऑर्डर करने के बाद का हिस्सा भी उतना ही महत्वपूर्ण है, और यहीं कुछ नए तरह के धोखे काम करते हैं।',
          'सबसे आम है डिलीवरी के नाम पर आने वाला संदेश: "पता अधूरा है, इस लिंक पर सुधार कीजिए" या "पुनः डिलीवरी के लिए छोटा शुल्क दीजिए"। कोई भी असली कूरियर लिंक भेजकर भुगतान नहीं माँगता।',
          'दूसरा: ओटीपी माँगने वाला फ़ोन, जो कहता है कि डिलीवरी की पुष्टि के लिए चाहिए। डिलीवरी का ओटीपी केवल सामान लेते समय, सामने खड़े व्यक्ति को बताया जाता है, फ़ोन पर किसी को नहीं।',
          'पैकेट खोलते समय वीडियो बनाइए, ख़ासकर महँगे सामान का। कई मंचों पर ग़लत या ख़ाली पैकेट की शिकायत में यही सबसे मज़बूत प्रमाण होता है।',
          'सामान मिलते ही जाँच लीजिए और शिकायत की समय-सीमा के भीतर ही उठाइए। अधिकांश नीतियों में यह अवधि छोटी होती है और बीत जाने के बाद विकल्प बहुत कम बचते हैं।',
          'और हर ऑर्डर का पुष्टिकरण, चालान और भुगतान का रिकॉर्ड सँभालकर रखिए। विवाद की स्थिति में यही तीन चीज़ें पूरी बातचीत का आधार बनती हैं।',
        ],
        pEn: [
          'What happens after the order is just as important, and this is where some newer scams operate.',
          'The commonest is a message in the name of delivery: "the address is incomplete, correct it on this link" or "pay a small fee for redelivery". No genuine courier asks for payment through a link.',
          'The second is a call asking for an OTP, claiming it is needed to confirm delivery. A delivery OTP is given only while receiving the goods, to the person standing in front of you, never to anyone on the phone.',
          'Record a video while opening the packet, particularly for expensive items. On many platforms that is the strongest evidence in a complaint about a wrong or empty parcel.',
          'Check the goods as soon as they arrive and raise any complaint within the stated window. Most policies allow only a short period, and once it passes very few options remain.',
          'And keep the confirmation, the invoice and the payment record for every order. In a dispute those three things are the basis of the entire conversation.',
        ],
      },
      {
        hHi: 'सब्सक्रिप्शन और सहेजे गए कार्ड',
        hEn: 'Subscriptions and saved cards',
        pHi: [
          'ऑनलाइन ख़रीद का एक हिस्सा ऐसा है जो एक बार शुरू होकर चलता रहता है, और उसी में सबसे ज़्यादा पैसा चुपचाप जाता है।',
          'मुफ़्त परीक्षण अवधि लगभग हमेशा कार्ड विवरण माँगकर शुरू होती है, और अवधि समाप्त होने पर वह अपने-आप भुगतान में बदल जाती है। परीक्षण शुरू करते ही उसकी समाप्ति तिथि फ़ोन में दर्ज कर लीजिए।',
          'साल में एक बार अपने कार्ड और बैंक विवरण में केवल आवर्ती भुगतान छाँटकर देखिए। लगभग हर व्यक्ति को कम से कम एक ऐसी सेवा मिलती है जो वह अब इस्तेमाल नहीं करता।',
          'सहेजे गए कार्ड के बारे में एक व्यावहारिक बात: जितनी कम जगह आपका कार्ड सहेजा हुआ है, उतनी कम जगहें ऐसी हैं जहाँ से वह विवरण लीक हो सकता है।',
          'जहाँ सुविधा ज़रूरी हो वहाँ टोकन वाली व्यवस्था बेहतर है, जिसमें व्यापारी के पास आपका असली कार्ड नंबर नहीं रहता बल्कि एक विकल्प संख्या रहती है।',
          'और अपने कार्ड की ऑनलाइन लेन-देन की सीमा तथा अंतरराष्ट्रीय लेन-देन की अनुमति ऐप में जाँच लीजिए। जिसे विदेश से ख़रीदना नहीं है, उसे वह सुविधा चालू रखने की कोई ज़रूरत नहीं।',
        ],
        pEn: [
          'One part of online shopping starts once and then keeps running, and that is where money leaves most quietly.',
          'A free trial almost always begins by taking card details, and turns into a payment automatically when the period ends. Put the end date into your phone the moment a trial starts.',
          'Once a year, filter your card and bank statements down to recurring payments alone. Almost everybody finds at least one service they no longer use.',
          'One practical point about saved cards: the fewer places your card is stored, the fewer places those details can leak from.',
          'Where the convenience is necessary, a tokenised arrangement is better — the merchant then holds a substitute number rather than your real card number.',
          'And check your card’s online transaction limit and international transaction permission in the app. Anyone who does not buy from abroad has no reason to leave that facility switched on.',
        ],
      },
      {
        hHi: 'गड़बड़ी होने पर क्या करें',
        hEn: 'What to do when something goes wrong',
        pHi: [
          'सामान न आया, ग़लत आया, या पैसा कट गया और ऑर्डर नहीं बना — इन तीनों स्थितियों में एक ही क्रम काम करता है।',
          'पहला: मंच या विक्रेता के पास शिकायत दर्ज कीजिए और शिकायत संख्या लीजिए। मौखिक या चैट में की गई बातचीत का कोई मूल्य नहीं रहता अगर उसका कोई संदर्भ संख्या न हो।',
          'दूसरा: अगर पैसा कट गया पर ऑर्डर नहीं बना, तो आमतौर पर वह राशि कुछ कार्य-दिवसों में अपने-आप लौट आती है। उतना इंतज़ार कीजिए, फिर बैंक में विवाद दर्ज कीजिए।',
          'तीसरा: कार्ड से किए गए भुगतान में, अगर विक्रेता जवाब न दे, तो कार्ड जारी करने वाले बैंक के पास विवाद दर्ज कराया जा सकता है। इसकी भी एक समय-सीमा होती है, इसलिए देर मत कीजिए।',
          'चौथा: उपभोक्ता शिकायत के लिए राष्ट्रीय हेल्पलाइन और ऑनलाइन व्यवस्था मौजूद है, और वह निःशुल्क है। कई मामले वहीं सुलझ जाते हैं।',
          'और हर चरण में वही चीज़ काम आती है: ऑर्डर की पुष्टि, भुगतान का प्रमाण, पैकेट खोलने का वीडियो, और तारीख़ के क्रम में रखा गया पत्राचार।',
        ],
        pEn: [
          'The goods did not arrive, the wrong item arrived, or money left the account and no order was created — the same sequence works in all three situations.',
          'First: register a complaint with the platform or seller and get a complaint number. A conversation held verbally or in chat carries no weight without a reference number.',
          'Second: if money was debited but no order was created, that amount usually returns by itself within a few working days. Wait that long, then raise a dispute with the bank.',
          'Third: on a card payment, if the seller does not respond, a dispute can be raised with the card-issuing bank. That too has a time limit, so do not delay.',
          'Fourth: a national consumer helpline and an online system exist for consumer complaints, and they are free. A good many cases are resolved there.',
          'And at every stage the same things do the work: the order confirmation, proof of payment, the unboxing video, and correspondence kept in date order.',
        ],
      },
    ],
  },
  {
    slug: 'online-shopping-safety',
    sections: [
      {
        hHi: 'त्योहार और सेल के दिनों की सावधानी',
        hEn: 'Care during festivals and sale days',
        pHi: [
          'साल के कुछ हफ़्ते ऐसे होते हैं जब ऑनलाइन ख़रीद कई गुना बढ़ जाती है, और ठीक उन्हीं हफ़्तों में धोखे भी सबसे ज़्यादा होते हैं। कारण सरल है: उन दिनों जल्दबाज़ी सामान्य लगती है।',
          '"केवल आज", "स्टॉक ख़त्म होने वाला है", "पहले पचास ग्राहकों के लिए" — ये वाक्य बिक्री बढ़ाने के लिए भी इस्तेमाल होते हैं और धोखे के लिए भी, और दोनों में अंतर करना उस क्षण मुश्किल होता है।',
          'सबसे उपयोगी आदत यह है कि सेल शुरू होने से पहले एक सूची बना लीजिए कि क्या ख़रीदना है और अधिकतम कितने का। सूची से बाहर की हर चीज़ पर चौबीस घंटे रुकिए।',
          'क़ीमत की तुलना करते समय यह भी देख लीजिए कि "पहले की क़ीमत" वास्तव में क्या थी। कई जगह सेल से कुछ दिन पहले क़ीमत बढ़ाकर फिर छूट दिखाई जाती है।',
          'इन दिनों नक़ली वेबसाइट और नक़ली विज्ञापन भी सबसे ज़्यादा होते हैं, क्योंकि लोग खोज परिणामों और विज्ञापनों पर सामान्य से ज़्यादा भरोसा कर रहे होते हैं।',
          'और एक व्यावहारिक बात: सेल के दिनों में डिलीवरी में देरी सामान्य है। इसका फ़ायदा उठाकर नक़ली "डिलीवरी अपडेट" संदेश भेजे जाते हैं। ऑर्डर की स्थिति हमेशा मंच के ऐप में ही देखिए।',
        ],
        pEn: [
          'There are a few weeks in the year when online shopping multiplies, and those are exactly the weeks when fraud peaks as well. The reason is simple: in those days haste looks normal.',
          '"Today only", "stock running out", "for the first fifty customers" — these lines are used to increase genuine sales and to run scams alike, and telling them apart in the moment is hard.',
          'The most useful habit is to make a list before the sale starts of what you intend to buy and the maximum you will pay. On anything outside that list, wait twenty-four hours.',
          'While comparing prices, check what the "previous price" actually was. In many places the price is raised a few days before a sale so that a discount can be shown.',
          'Fake websites and fake advertisements also peak in these days, because people are trusting search results and advertisements more than they usually would.',
          'And one practical point: delivery delays are normal during sales. Fake "delivery update" messages exploit exactly that. Always check the status of an order inside the platform’s own app.',
        ],
      },
    ],
  },
  {
    slug: 'mobile-banking-security',
    sections: [
      {
        hHi: 'फ़ोन ही असली तिजोरी है',
        hEn: 'The phone itself is the real safe',
        pHi: [
          'मोबाइल बैंकिंग की सुरक्षा पर बात करते समय ध्यान आमतौर पर ऐप और पासवर्ड पर जाता है। असल में सबसे कमज़ोर कड़ी अक्सर फ़ोन ख़ुद होता है।',
          'फ़ोन में आपका बैंकिंग ऐप है, आपका ईमेल है, और सबसे महत्वपूर्ण — वह सिम है जिस पर ओटीपी आता है। जिसके पास खुला हुआ फ़ोन है, उसके पास तीनों हैं।',
          'इसलिए पहला और सबसे बुनियादी क़दम स्क्रीन लॉक है — पिन, पैटर्न या बायोमेट्रिक। बिना लॉक वाले फ़ोन में सबसे मज़बूत बैंकिंग पासवर्ड भी बेकार है।',
          'दूसरा: सिम पर भी एक अलग पिन लगाया जा सकता है, ताकि सिम निकालकर दूसरे फ़ोन में डालने पर वह काम न करे। यह सुविधा हर फ़ोन में है और लगभग कोई इस्तेमाल नहीं करता।',
          'तीसरा: फ़ोन खोजने और दूरस्थ रूप से मिटाने की सुविधा पहले से चालू रखिए। खो जाने के बाद उसे चालू नहीं किया जा सकता।',
          'और चौथा: फ़ोन की सूचनाएँ लॉक स्क्रीन पर पूरा पाठ न दिखाएँ, यह सेटिंग बदल दीजिए। बंद फ़ोन की स्क्रीन पर दिखता ओटीपी किसी लॉक से नहीं बचता।',
        ],
        pEn: [
          'When mobile-banking security is discussed, attention usually goes to the app and the password. In reality the weakest link is very often the phone itself.',
          'The phone holds your banking app, your email, and most importantly the SIM that receives the OTP. Whoever has an unlocked phone has all three.',
          'So the first and most basic step is a screen lock — PIN, pattern or biometric. On a phone with no lock, even the strongest banking password is useless.',
          'Second: a separate PIN can be set on the SIM itself, so that removing it and putting it in another phone does not work. Every phone offers this and almost nobody uses it.',
          'Third: turn on find-my-device and remote wipe in advance. They cannot be enabled after the phone is lost.',
          'And fourth: change the setting so notifications do not show their full text on the lock screen. An OTP visible on a locked screen is not protected by any lock.',
        ],
      },
      {
        hHi: 'सिम बदलने वाला धोखा',
        hEn: 'The SIM-swap scam',
        pHi: [
          'यह सबसे गंभीर धोखों में से एक है क्योंकि इसमें आपका फ़ोन चोरी नहीं होता, फिर भी आपके सारे ओटीपी किसी और के पास पहुँचने लगते हैं।',
          'तरीक़ा यह है कि धोखेबाज़ आपके नाम से डुप्लिकेट सिम जारी करा लेता है, आमतौर पर चुराई गई पहचान की जानकारी के आधार पर। जैसे ही नया सिम चालू होता है, आपका सिम बंद हो जाता है।',
          'इसका सबसे स्पष्ट संकेत यही है: आपका फ़ोन अचानक बिना नेटवर्क का हो जाए और कुछ घंटों तक वैसा ही रहे, जबकि आसपास सबके फ़ोन चल रहे हों।',
          'ऐसा होने पर इंतज़ार मत कीजिए। किसी दूसरे फ़ोन से तुरंत अपने दूरसंचार प्रदाता को फ़ोन कीजिए और पूछिए कि क्या कोई नया सिम जारी हुआ है, और साथ ही बैंक को सूचित कीजिए।',
          'बचाव के तौर पर, बैंक में दर्ज ईमेल भी सक्रिय रखिए और सूचनाएँ ईमेल पर भी चालू रखिए। अगर सिम बंद हो जाए तो ईमेल ही वह दूसरा रास्ता है जिससे आपको पता चलेगा।',
          'और जहाँ उपलब्ध हो वहाँ ऐप-आधारित सत्यापन का उपयोग कीजिए, क्योंकि वह सिम पर निर्भर नहीं करता और इसलिए सिम बदलने से प्रभावित नहीं होता।',
        ],
        pEn: [
          'This is among the most serious scams, because your phone is not stolen and yet all your OTPs start reaching somebody else.',
          'The method is that a fraudster gets a duplicate SIM issued in your name, usually using stolen identity information. The moment the new SIM activates, yours goes dead.',
          'The clearest signal is exactly that: your phone suddenly loses network and stays that way for hours, while everybody else’s phone around you is working.',
          'If that happens, do not wait. Call your telecom provider from another phone immediately and ask whether a new SIM has been issued, and inform the bank at the same time.',
          'As a precaution, keep the email registered with the bank active and keep alerts switched on for email as well. If the SIM goes dead, email is the other route by which you will find out.',
          'And where it is available, use app-based verification, because it does not depend on the SIM and is therefore unaffected by a SIM swap.',
        ],
      },
      {
        hHi: 'ऐप की अनुमतियाँ और अनजान इंस्टॉल',
        hEn: 'App permissions and unknown installs',
        pHi: [
          'बैंकिंग को नुक़सान पहुँचाने वाले अधिकांश ऐप बैंकिंग ऐप जैसे नहीं दिखते। वे टॉर्च, गेम, फ़ोटो एडिटर या किसी सरकारी योजना के नाम पर आते हैं।',
          'सबसे ख़तरनाक अनुमतियाँ तीन हैं: संदेश पढ़ने की, स्क्रीन देखने या नियंत्रित करने की, और अन्य ऐप के ऊपर दिखने की। इन तीनों में से कोई भी माँगने वाला साधारण ऐप संदेह के योग्य है।',
          'संदेश पढ़ने की अनुमति सीधे ओटीपी तक पहुँच देती है। स्क्रीन नियंत्रण की अनुमति किसी को आपके फ़ोन पर वही करने देती है जो आप कर सकते हैं।',
          'साल में एक बार सेटिंग में जाकर देखिए कि किन ऐप के पास ये अनुमतियाँ हैं। जो ऐप याद न हो, या जिसकी अनुमति उसके काम से मेल न खाती हो, उसे हटा दीजिए।',
          'और "अज्ञात स्रोत से इंस्टॉल" वाली सेटिंग बंद रखिए। लगभग हर बड़ा मोबाइल धोखा किसी न किसी बिंदु पर इसी सेटिंग को चालू कराने के लिए कहता है।',
          'बैंकिंग ऐप हमेशा आधिकारिक स्टोर से ही अपडेट कीजिए, और कभी भी किसी कॉल पर, किसी के कहने से, कोई ऐप इंस्टॉल मत कीजिए — चाहे वह कितना भी आधिकारिक लगे।',
        ],
        pEn: [
          'Most apps that damage banking do not look like banking apps. They arrive as a torch, a game, a photo editor, or something named after a government scheme.',
          'The three most dangerous permissions are: reading messages, viewing or controlling the screen, and drawing over other apps. An ordinary app asking for any of these deserves suspicion.',
          'Permission to read messages gives direct access to OTPs. Permission to control the screen lets somebody do on your phone exactly what you can do.',
          'Once a year, open the settings and see which apps hold these permissions. Remove any app you do not remember, or whose permissions do not match what it does.',
          'And keep "install from unknown sources" switched off. Nearly every large mobile fraud asks, at some point, for exactly that setting to be turned on.',
          'Always update the banking app from the official store, and never install an app because somebody on a call told you to — however official they sound.',
        ],
      },
      {
        hHi: 'फ़ोन खोने या बदलने पर',
        hEn: 'When a phone is lost or changed',
        pHi: [
          'फ़ोन खो जाना केवल एक उपकरण का नुक़सान नहीं है। जब तक कुछ क़दम न उठाए जाएँ, वह आपके बैंक खाते तक खुला रास्ता बना रहता है।',
          'पहला क़दम: सिम ब्लॉक कराइए। यह सबसे ज़रूरी है, क्योंकि सिम बंद होते ही ओटीपी का रास्ता बंद हो जाता है।',
          'दूसरा: बैंक को सूचित करके मोबाइल बैंकिंग और यूपीआई अस्थायी रूप से रोकिए। तीसरा: दूरस्थ रूप से फ़ोन का डेटा मिटाइए, अगर वह सुविधा पहले से चालू थी।',
          'चौथा: ईमेल का पासवर्ड बदलिए और उस खाते से जुड़े सभी सक्रिय सत्र समाप्त कीजिए। ईमेल अक्सर बाक़ी सब कुछ वापस पाने की चाबी होता है, इसलिए वह सबसे पहले सुरक्षित होना चाहिए।',
          'फ़ोन बदलते समय भी एक सूची चलनी चाहिए: पुराने फ़ोन से बैंकिंग ऐप और यूपीआई पंजीकरण हटाइए, फिर फ़ैक्ट्री रीसेट कीजिए, और उसके बाद ही उसे बेचिए या किसी को दीजिए।',
          'केवल ऐप हटा देना पर्याप्त नहीं है। पुराने फ़ोन में सहेजे गए ईमेल, तस्वीरें और दस्तावेज़ अक्सर वही जानकारी रखते हैं जिससे पहचान की चोरी शुरू होती है।',
        ],
        pEn: [
          'Losing a phone is not merely losing a device. Until certain steps are taken, it remains an open road to your bank account.',
          'First step: get the SIM blocked. This matters most, because once the SIM is dead the route for OTPs is closed.',
          'Second: inform the bank and have mobile banking and UPI suspended temporarily. Third: wipe the phone remotely, if that facility was enabled beforehand.',
          'Fourth: change the email password and end every active session on that account. Email is often the key to recovering everything else, so it should be secured first.',
          'A checklist belongs to changing a phone as well: remove the banking apps and the UPI registration from the old phone, then do a factory reset, and only then sell it or pass it on.',
          'Deleting the apps alone is not enough. Emails, photographs and documents saved on an old phone frequently hold exactly the information that identity theft starts from.',
        ],
      },
      {
        hHi: 'एक छोटी मासिक जाँच',
        hEn: 'A short monthly check',
        pHi: [
          'सुरक्षा को एक बार का काम मानने के बजाय एक छोटी मासिक आदत मान लेना कहीं ज़्यादा असरदार है। इसमें पाँच मिनट लगते हैं।',
          'पहला: पिछले महीने के सारे लेन-देन देखिए और किसी भी अपरिचित प्रविष्टि को चिह्नित कीजिए, चाहे राशि छोटी हो। छोटी राशि अक्सर परीक्षण होती है।',
          'दूसरा: बैंक ऐप में उन उपकरणों की सूची देखिए जिनसे लॉगिन हुआ है, और जो पहचान में न आए उसे हटाइए।',
          'तीसरा: देखिए कि दर्ज मोबाइल नंबर और ईमेल वही हैं जो आप इस्तेमाल करते हैं। बदला हुआ संपर्क विवरण किसी गड़बड़ी का सबसे पहला संकेत होता है।',
          'चौथा: फ़ोन के अपडेट लंबित हैं या नहीं, यह देख लीजिए और उन्हें लगा दीजिए।',
          'और पाँचवाँ: साल में एक बार पासवर्ड बदलिए, और हर जगह अलग पासवर्ड रखिए। जिसे याद रखना कठिन लगे, वह एक भरोसेमंद पासवर्ड प्रबंधक का उपयोग कर सकता है।',
        ],
        pEn: [
          'Treating security as a small monthly habit rather than a one-time task is far more effective. It takes five minutes.',
          'First: look through the last month’s transactions and mark anything unfamiliar, however small. A small amount is often a test.',
          'Second: check the list of devices logged into the banking app and remove any you do not recognise.',
          'Third: confirm that the registered mobile number and email are still the ones you use. Changed contact details are the earliest signal that something is wrong.',
          'Fourth: see whether phone updates are pending and install them.',
          'And fifth: change passwords once a year, and keep a different one everywhere. Anyone who finds that hard to remember can use a trustworthy password manager.',
        ],
      },
    ],
  },
  {
    slug: 'mobile-banking-security',
    sections: [
      {
        hHi: 'सार्वजनिक जगहों पर बैंकिंग',
        hEn: 'Banking in public places',
        pHi: [
          'सुरक्षा की चर्चा आमतौर पर तकनीक तक सीमित रहती है, जबकि नुक़सान का एक अच्छा हिस्सा बहुत साधारण परिस्थितियों में होता है — बस, ट्रेन, दुकान की क़तार, या किसी दफ़्तर की प्रतीक्षा।',
          'सबसे पुराना तरीक़ा आज भी सबसे कारगर है: पीछे खड़े होकर पिन देख लेना। स्क्रीन को ढककर पिन डालने की आदत उतनी ही ज़रूरी है जितनी एटीएम पर होती है।',
          'सार्वजनिक वाई-फ़ाई पर बैंकिंग न करने की सलाह अक्सर दी जाती है और उसका कारण सरल है: उस नेटवर्क पर कौन और क्या देख रहा है, यह आप नहीं जानते। मोबाइल डेटा कहीं सुरक्षित है।',
          'किसी और के फ़ोन या कंप्यूटर से बैंकिंग करने से बचिए। अगर करना ही पड़े, तो निजी विंडो का उपयोग कीजिए, और काम पूरा होने पर लॉगआउट करके सारा इतिहास हटा दीजिए।',
          'सार्वजनिक चार्जिंग पॉइंट पर केवल चार्जिंग वाली केबल का उपयोग कीजिए, या अपना पावर बैंक साथ रखिए। डेटा वाली केबल से जुड़ा फ़ोन केवल बिजली नहीं लेता।',
          'और एक साधारण आदत जो सबसे कम अपनाई जाती है: लेन-देन पूरा होते ही ऐप बंद कीजिए, उसे पृष्ठभूमि में खुला मत छोड़िए।',
        ],
        pEn: [
          'Discussion of security usually stays with technology, while a good share of the losses happen in very ordinary circumstances — a bus, a train, a queue in a shop, or waiting in an office.',
          'The oldest method still works best of all: standing behind somebody and watching the PIN. The habit of shielding the screen while entering a PIN matters as much here as at an ATM.',
          'The advice not to bank on public Wi-Fi is given often and the reason is simple: you do not know who is on that network or what they can see. Mobile data is considerably safer.',
          'Avoid banking from somebody else’s phone or computer. If it cannot be avoided, use a private window, and log out and clear the history once the task is done.',
          'At a public charging point, use a charge-only cable or carry your own power bank. A phone connected by a data cable is not only taking electricity.',
          'And one simple habit that is adopted least of all: close the app as soon as the transaction finishes, rather than leaving it open in the background.',
        ],
      },
    ],
  },
  {
    slug: 'riders-in-insurance',
    sections: [
      {
        hHi: 'राइडर होता क्या है',
        hEn: 'What a rider actually is',
        pHi: [
          'राइडर मूल बीमा पॉलिसी के साथ जोड़ा जाने वाला एक अतिरिक्त कवर है। वह अपने-आप में अलग पॉलिसी नहीं है; वह मूल पॉलिसी पर टिका होता है और उसी के साथ चलता है।',
          'इसका एक महत्वपूर्ण परिणाम है: अगर मूल पॉलिसी समाप्त हो जाए, तो राइडर भी समाप्त हो जाता है, चाहे उसका प्रीमियम अलग से गिना जाता हो।',
          'राइडर आमतौर पर अलग से ख़रीदी गई पूरी पॉलिसी से सस्ते होते हैं, क्योंकि उनमें अलग से जारी करने और सत्यापन की लागत नहीं आती।',
          'दूसरी ओर, उनका कवर आमतौर पर सीमित होता है और शर्तें अधिक कड़ी होती हैं। सस्ता होने का यही कारण है, और यही उनकी सीमा भी है।',
          'राइडर अक्सर पॉलिसी लेते समय ही जोड़े जा सकते हैं, और कुछ को नवीनीकरण के समय भी जोड़ा जा सकता है। बाद में जोड़ने पर उम्र और स्वास्थ्य की स्थिति के हिसाब से शर्तें बदल सकती हैं।',
          'सबसे ज़रूरी बात: राइडर का काम मूल कवर की कमी पूरी करना नहीं है। अगर मूल कवर ही कम है, तो पहले उसे बढ़ाइए — कोई भी राइडर अपर्याप्त मूल राशि की भरपाई नहीं कर सकता।',
        ],
        pEn: [
          'A rider is an additional cover attached to a main insurance policy. It is not a separate policy in itself; it rests on the main policy and runs with it.',
          'That has an important consequence: if the main policy lapses, the rider lapses too, even though its premium is counted separately.',
          'Riders are usually cheaper than a full policy bought separately, because they do not carry the cost of separate issue and underwriting.',
          'On the other side, their cover is usually limited and their conditions are stricter. That is why they are cheap, and it is also their limitation.',
          'Riders can generally be added at the time of taking the policy, and some can be added at renewal. Adding one later can change the terms according to age and health.',
          'The most important point: a rider is not there to make up for a shortfall in the main cover. If the main cover is inadequate, raise that first — no rider can compensate for an insufficient base amount.',
        ],
      },
      {
        hHi: 'सबसे आम राइडर और वे किसके लिए हैं',
        hEn: 'The commonest riders and who they are for',
        pHi: [
          'दुर्घटना से जुड़ा राइडर आमतौर पर सबसे सस्ता होता है और उन लोगों के लिए सार्थक है जिनका काम सड़क पर बहुत समय बिताने वाला है, या जो जोखिम वाले वातावरण में काम करते हैं।',
          'गंभीर बीमारी वाला राइडर एक निश्चित सूची की बीमारियों के निदान पर एकमुश्त राशि देता है। यह उपचार के बिल की भरपाई नहीं है; यह उस अवधि की आय की भरपाई है जब काम करना संभव नहीं होता।',
          'प्रीमियम माफ़ी वाला राइडर उन परिस्थितियों में आगे के प्रीमियम माफ़ कर देता है जो पॉलिसी में तय हैं, और पॉलिसी चलती रहती है। यह उन परिवारों के लिए उपयोगी है जहाँ आय एक ही व्यक्ति पर निर्भर है।',
          'आय लाभ वाला राइडर एकमुश्त राशि के बजाय कुछ वर्षों तक मासिक भुगतान देता है। जिन परिवारों में एकमुश्त बड़ी राशि सँभालना कठिन हो, वहाँ यह ढाँचा बेहतर काम करता है।',
          'स्वास्थ्य पॉलिसी में कमरे के किराए की सीमा हटाने वाला या सह-भुगतान घटाने वाला विकल्प भी उपलब्ध होता है। यह दिखने में छोटा है पर अस्पताल के अंतिम बिल पर इसका असर सबसे सीधा होता है।',
          'हर राइडर के लिए वही एक सवाल पूछिए: यह किस विशिष्ट परिस्थिति में भुगतान करेगा, और वह परिस्थिति मेरे जीवन में कितनी संभावित है? जिसका उत्तर स्पष्ट न हो, वह राइडर आपके लिए नहीं है।',
        ],
        pEn: [
          'The accident rider is usually the cheapest and makes sense for people whose work keeps them on the road, or who work in hazardous surroundings.',
          'The critical-illness rider pays a lump sum on the diagnosis of a listed condition. It is not reimbursement of treatment bills; it replaces income during the period when working is not possible.',
          'The waiver-of-premium rider waives future premiums in the circumstances set out in the policy, and the policy continues. It is useful in families where income depends on one person.',
          'The income-benefit rider pays a monthly amount for some years instead of a lump sum. Where handling a single large amount would be difficult, that structure works better.',
          'In health policies there are options to remove the room-rent limit or reduce the co-payment. These look small and have the most direct effect of all on the final hospital bill.',
          'Ask the same question of every rider: in what specific situation will this pay, and how likely is that situation in my life? Any rider without a clear answer is not for you.',
        ],
      },
      {
        hHi: 'परिभाषाएँ ही असली शर्तें हैं',
        hEn: 'The definitions are the real conditions',
        pHi: [
          'राइडर के मामले में सबसे ज़्यादा विवाद इस बात पर होता है कि कोई स्थिति उस राइडर की परिभाषा में आती है या नहीं। इसीलिए परिभाषाएँ ही असली शर्तें हैं, नाम नहीं।',
          'गंभीर बीमारी वाले राइडर में हर बीमारी की एक विस्तृत चिकित्सीय परिभाषा होती है — किस चरण पर, किन जाँचों से पुष्टि होने पर, और कितने दिन जीवित रहने के बाद भुगतान होगा।',
          'इसी तरह दुर्घटना वाले राइडर में "स्थायी" और "आंशिक" अपंगता की परिभाषाएँ बहुत विशिष्ट होती हैं, और वे रोज़मर्रा की भाषा से मेल नहीं खातीं।',
          'इसलिए ख़रीदने से पहले वह सूची पढ़िए जिसमें ये परिभाषाएँ हैं, केवल विवरणिका का सारांश नहीं। सारांश बेचने के लिए बनता है; परिभाषा भुगतान के लिए।',
          'प्रतीक्षा अवधि पर भी ध्यान दीजिए। कई राइडर पॉलिसी शुरू होने के कुछ महीनों बाद ही प्रभावी होते हैं, और उससे पहले हुई कोई घटना कवर नहीं होती।',
          'और अपवादों की सूची ज़रूर देखिए। पहले से मौजूद स्थितियाँ, कुछ विशिष्ट गतिविधियाँ, और कुछ परिस्थितियाँ आमतौर पर बाहर रखी जाती हैं, और यह सब लिखा हुआ होता है।',
        ],
        pEn: [
          'With riders, most disputes turn on whether a condition falls inside the rider’s definition. That is why the definitions are the real conditions, not the names.',
          'In a critical-illness rider every condition carries a detailed medical definition — at what stage, confirmed by which tests, and after surviving how many days the payment is made.',
          'Similarly, in an accident rider the definitions of "permanent" and "partial" disability are highly specific, and they do not correspond to everyday language.',
          'So before buying, read the schedule containing these definitions, not merely the summary in the brochure. The summary is written to sell; the definition is written to pay.',
          'Pay attention to waiting periods too. Many riders take effect only some months after the policy starts, and anything occurring before that is not covered.',
          'And do read the exclusions. Pre-existing conditions, certain specific activities and certain circumstances are usually placed outside the cover, and all of it is written down.',
        ],
      },
      {
        hHi: 'राइडर या अलग पॉलिसी',
        hEn: 'Rider or separate policy',
        pHi: [
          'यह वह फ़ैसला है जिस पर सबसे कम सोचा जाता है और जिसका असर सबसे लंबा होता है। दोनों रास्ते वैध हैं और दोनों की अपनी जगह है।',
          'राइडर तब बेहतर है जब राशि छोटी हो, आवश्यकता सहायक हो, और आप मूल पॉलिसी को लंबे समय तक चलाने वाले हों। तब कम लागत में अतिरिक्त सुरक्षा मिल जाती है।',
          'अलग पॉलिसी तब बेहतर है जब वह ज़रूरत अपने-आप में बड़ी हो — जैसे पर्याप्त स्वास्थ्य कवर या पर्याप्त गंभीर बीमारी कवर। वहाँ राइडर की सीमित राशि काम नहीं आती।',
          'एक व्यावहारिक अंतर यह भी है: अलग पॉलिसी स्वतंत्र होती है। मूल पॉलिसी बंद करने या बदलने पर वह चलती रहती है, जबकि राइडर उसी के साथ समाप्त हो जाता है।',
          'लचीलापन भी अलग है। अलग पॉलिसी को नवीनीकरण पर बदला, बढ़ाया या दूसरी कंपनी में पोर्ट किया जा सकता है; राइडर इस मामले में मूल पॉलिसी से बँधा रहता है।',
          'सामान्य मार्गदर्शन यह है: बुनियादी ज़रूरतें अलग और पर्याप्त पॉलिसियों से पूरी कीजिए, और राइडर का उपयोग उन छोटी, विशिष्ट कमियों के लिए कीजिए जो उसके बाद बचती हैं।',
        ],
        pEn: [
          'This is the decision least thought about and longest in its effect. Both routes are valid and each has its place.',
          'A rider is better when the amount is small, the need is supplementary, and you intend to keep the main policy running for a long time. It adds protection at low cost.',
          'A separate policy is better when the need is large in itself — adequate health cover, or adequate critical-illness cover. A rider’s limited amount does not serve there.',
          'There is a practical difference too: a separate policy stands on its own. It continues if the main policy is closed or changed, whereas a rider ends with it.',
          'Flexibility differs as well. A separate policy can be changed, increased or ported to another insurer at renewal; a rider stays tied to the main policy in this respect.',
          'The general guidance is this: meet the basic needs with separate, adequate policies, and use riders for the small, specific gaps that remain after that.',
        ],
      },
      {
        hHi: 'पहले से चल रहे राइडर की समीक्षा',
        hEn: 'Reviewing riders you already have',
        pHi: [
          'बहुत से लोगों की पॉलिसियों में ऐसे राइडर चल रहे हैं जिनके बारे में उन्हें याद भी नहीं, क्योंकि वे बिक्री के समय जोड़ दिए गए थे और प्रीमियम में मिल गए थे।',
          'इसलिए साल में एक बार अपनी पॉलिसी की अनुसूची निकालिए और देखिए कि उसमें कौन-कौन से राइडर हैं और हर एक का प्रीमियम अलग से कितना है।',
          'फिर हर राइडर पर वही सवाल दोहराइए: क्या यह ज़रूरत अब भी मौजूद है? जीवन बदलता है, और जो राइडर दस साल पहले उपयुक्त था वह आज बेमानी हो सकता है, या इसका उल्टा भी सच हो सकता है।',
          'देखिए कि कहीं वही कवर आपको दो जगह से तो नहीं मिल रहा — नियोक्ता की योजना से और राइडर से। दोहरा प्रीमियम देकर दोहरा भुगतान नहीं मिलता, ख़ासकर प्रतिपूर्ति वाले कवर में।',
          'जो राइडर अब उपयोगी नहीं, उसे नवीनीकरण पर हटाया जा सकता है। और जो कमी अब दिख रही है, उसके लिए राइडर जोड़ने या अलग पॉलिसी लेने का फ़ैसला उसी समय लिया जा सकता है।',
          'यह पूरी समीक्षा साल में एक बार, आधे घंटे में हो जाती है, और यह उन कुछ कामों में है जिनका सीधा असर परिवार की सुरक्षा पर पड़ता है।',
        ],
        pEn: [
          'Many people are running riders on their policies that they do not even remember, because they were added at the time of sale and merged into the premium.',
          'So once a year, take out the policy schedule and see which riders are attached and what each one costs separately.',
          'Then repeat the same question for each: does this need still exist? Life changes, and a rider that suited ten years ago may be pointless today — or the reverse may be true.',
          'Check whether the same cover is reaching you from two places — an employer scheme and a rider. Paying twice does not produce a double payout, particularly in reimbursement-based cover.',
          'A rider that is no longer useful can be removed at renewal. And where a gap is now visible, the decision to add a rider or take a separate policy can be made at the same time.',
          'This whole review takes half an hour once a year, and it is among the few tasks with a direct effect on the family’s security.',
        ],
      },
    ],
  },
  {
    slug: 'riders-in-insurance',
    sections: [
      {
        hHi: 'दावा करते समय राइडर का अलग रास्ता',
        hEn: 'A rider follows its own path at claim time',
        pHi: [
          'एक बात जो अक्सर देर से पता चलती है: राइडर का दावा मूल पॉलिसी के दावे के साथ अपने-आप नहीं हो जाता। उसे अलग से माँगना पड़ता है।',
          'कई परिवार मूल पॉलिसी का भुगतान पाकर मान लेते हैं कि सब कुछ मिल गया, जबकि गंभीर बीमारी या दुर्घटना वाले राइडर का दावा अलग दस्तावेज़ों के साथ अलग दर्ज होता है।',
          'दस्तावेज़ भी अलग हो सकते हैं — जैसे निदान की पुष्टि करने वाली विशिष्ट जाँच रिपोर्ट, जो सामान्य अस्पताल के बिल में नहीं आती।',
          'इसलिए दावा करते समय पॉलिसी की अनुसूची निकालिए और उसमें दर्ज हर राइडर के लिए अलग से पूछिए कि उसके लिए क्या चाहिए और उसकी समय-सीमा क्या है।',
        ],
        pEn: [
          'One point often discovered late: a rider claim does not happen automatically along with the main policy claim. It has to be asked for separately.',
          'Many families receive the main policy payment and assume everything has come, while the critical-illness or accident rider claim is filed separately with separate documents.',
          'The documents can differ too — a specific diagnostic report confirming the condition, for instance, which does not appear in an ordinary hospital bill.',
          'So when claiming, take out the policy schedule and ask separately, for every rider listed on it, what is required and what the time limit is.',
        ],
      },
    ],
  },
  {
    slug: 'emergency-fund-where-to-keep',
    sections: [
      {
        hHi: 'तीन कसौटियाँ, इसी क्रम में',
        hEn: 'Three tests, in this order',
        pHi: [
          'आपातकालीन कोष कहाँ रखा जाए, इसका फ़ैसला तीन कसौटियों से होता है, और उनका क्रम बदलना ही सबसे आम ग़लती है।',
          'पहली कसौटी: पहुँच। पैसा उसी दिन या अगले दिन हाथ में आना चाहिए, बिना किसी की मंज़ूरी के और बिना किसी जुर्माने के। जिस जगह से पैसा तीन दिन में आता है, वह आपात के लिए नहीं है।',
          'दूसरी कसौटी: स्थिरता। जिस दिन ज़रूरत पड़े उस दिन राशि उतनी ही होनी चाहिए जितनी आपने रखी थी। जो जगह उतार-चढ़ाव वाली है, वह इस काम के लिए उपयुक्त नहीं, चाहे उसका औसत रिटर्न कितना भी अच्छा हो।',
          'तीसरी कसौटी, और केवल तीसरी: रिटर्न। पहली दो शर्तें पूरी होने के बाद ही यह देखा जाना चाहिए कि कहाँ थोड़ा ज़्यादा ब्याज मिल सकता है।',
          'अधिकांश ग़लतियाँ इसी क्रम को उलटने से होती हैं। ऊँचा रिटर्न देखकर चुनी गई जगह अक्सर पहली या दूसरी कसौटी पर विफल हो जाती है, और यह विफलता ठीक उसी दिन सामने आती है जिस दिन सबसे कम सहनीय होती है।',
          'यह भी याद रखिए कि आपातकालीन कोष का उद्देश्य पैसा बढ़ाना नहीं है। उसका उद्देश्य यह है कि मुश्किल के दिन आपको क़र्ज़ न लेना पड़े। उस उद्देश्य के सामने कुछ प्रतिशत का अंतर बहुत छोटा है।',
        ],
        pEn: [
          'Where to keep an emergency fund is decided by three tests, and reversing their order is the commonest mistake of all.',
          'First test: access. The money should reach your hands the same day or the next, without anyone’s approval and without a penalty. Anywhere that takes three days is not for emergencies.',
          'Second test: stability. On the day it is needed the amount should be what you put in. A place that fluctuates is unsuitable for this job, however good its average return.',
          'Third test, and only third: return. Only after the first two are satisfied should you look at where a little more interest is available.',
          'Most mistakes come from inverting that order. A place chosen for a high return often fails the first or the second test, and that failure surfaces on exactly the day it is least bearable.',
          'Remember too that the purpose of an emergency fund is not to grow money. Its purpose is that you do not have to borrow on a difficult day. Against that purpose, a difference of a few per cent is very small.',
        ],
      },
      {
        hHi: 'एक जगह नहीं, परतों में रखिए',
        hEn: 'Not one place — keep it in layers',
        pHi: [
          'सबसे व्यावहारिक ढाँचा यह है कि पूरा कोष एक ही जगह न रखा जाए, बल्कि तीन परतों में बाँटा जाए, हर परत की पहुँच अलग।',
          'पहली परत: थोड़ी नक़दी घर में और कुछ राशि उस बचत खाते में जिसका कार्ड आपके पास है। यह वह हिस्सा है जो घंटों में चाहिए — किसी को अस्पताल ले जाना, कोई तत्काल यात्रा।',
          'दूसरी परत: बचत खाते या ऐसी जमा में जो तुरंत तोड़ी जा सके। यह वह हिस्सा है जो एक-दो दिन में चाहिए और जो कुल कोष का सबसे बड़ा हिस्सा होता है।',
          'तीसरी परत: थोड़ी लंबी अवधि वाली जमा, जिन्हें सीढ़ी की तरह अलग-अलग तारीख़ों पर परिपक्व होने के लिए बनाया गया हो। यह वह हिस्सा है जो लंबी मुश्किल में काम आता है।',
          'सीढ़ी बनाने का तरीक़ा सरल है: पूरी राशि एक जमा में डालने के बजाय उसे चार हिस्सों में बाँटिए और हर तीन महीने के अंतर पर परिपक्व होने वाली चार जमा बनाइए।',
          'इससे हर तिमाही में एक हिस्सा बिना किसी जुर्माने के उपलब्ध हो जाता है, और अगर ज़रूरत न पड़े तो वह अपने-आप आगे बढ़ जाता है। यह पहुँच और ब्याज, दोनों को एक साथ साधता है।',
        ],
        pEn: [
          'The most practical structure is not to keep the whole fund in one place but to divide it into three layers, each with different access.',
          'First layer: a little cash at home and some money in the savings account whose card you carry. This is the part needed within hours — taking somebody to hospital, an immediate journey.',
          'Second layer: a savings account or a deposit that can be broken at once. This is the part needed within a day or two, and it is usually the largest share of the fund.',
          'Third layer: somewhat longer deposits, built as a ladder maturing on different dates. This is the part that carries you through a long difficulty.',
          'Building the ladder is simple: instead of putting the whole amount into one deposit, divide it into four and create four deposits maturing three months apart.',
          'That way one portion becomes available every quarter with no penalty, and if it is not needed it rolls forward by itself. It serves access and interest at the same time.',
        ],
      },
      {
        hHi: 'कितना बड़ा होना चाहिए',
        hEn: 'How large it should be',
        pHi: [
          'तीन से छह महीने के ख़र्च वाला नियम बहुत सुना जाता है, पर वह एक शुरुआती बिंदु है, अंतिम उत्तर नहीं। सही आकार आपकी आय की स्थिरता पर निर्भर करता है।',
          'यहाँ "ख़र्च" का अर्थ आय नहीं है। इसका अर्थ है वह राशि जो मुश्किल के महीने में भी देनी ही पड़ेगी — किराया या किश्त, राशन, बिजली, दवा, स्कूल की फ़ीस, आने-जाने का ख़र्च।',
          'स्थिर सरकारी नौकरी और दो कमाने वाले सदस्यों वाले परिवार के लिए तीन महीने पर्याप्त हो सकते हैं। एक ही कमाने वाले, ठेके पर काम करने वाले, या अपना काम करने वाले परिवार के लिए छह से नौ महीने अधिक उपयुक्त हैं।',
          'कुछ परिस्थितियाँ इस आकार को और बढ़ाती हैं: घर में कोई बुज़ुर्ग या बीमार सदस्य, कोई बड़ी चल रही किश्त, या ऐसा पेशा जिसमें दोबारा काम मिलने में समय लगता है।',
          'पूरा आकार एक साथ बनाना ज़रूरी नहीं। पहले एक महीने का लक्ष्य रखिए, फिर तीन, फिर छह। पहला महीना सबसे कठिन होता है और सबसे ज़्यादा फ़र्क़ लाता है।',
          'और आकार को साल में एक बार दोबारा देखिए। ख़र्च बढ़ते हैं, और जो कोष तीन साल पहले छह महीने का था वह आज शायद चार महीने का ही रह गया हो।',
        ],
        pEn: [
          'The rule of three to six months of expenses is heard often, but it is a starting point rather than a final answer. The right size depends on how steady your income is.',
          '"Expenses" here does not mean income. It means the amount that has to be paid even in a difficult month — rent or an instalment, food, electricity, medicines, school fees, travel to work.',
          'For a family with a secure job and two earners, three months may be enough. For a single earner, a contract worker, or a self-employed household, six to nine months is more appropriate.',
          'Some circumstances push the size further up: an elderly or unwell member at home, a large running instalment, or a profession where finding work again takes time.',
          'The full size does not have to be built at once. Aim for one month first, then three, then six. The first month is the hardest and makes the most difference.',
          'And revisit the size once a year. Expenses rise, and a fund that covered six months three years ago may now cover only four.',
        ],
      },
      {
        hHi: 'कब इस्तेमाल करना है और कब नहीं',
        hEn: 'When to use it and when not to',
        pHi: [
          'आपातकालीन कोष का सबसे कठिन हिस्सा उसे बनाना नहीं, उसे छूने से रोकना है। इसके लिए एक स्पष्ट परिभाषा चाहिए, जो पहले से लिखी हो।',
          'एक अच्छा नियम यह है: आपात वह है जो अप्रत्याशित हो, ज़रूरी हो, और तत्काल हो। तीनों शर्तें एक साथ पूरी होनी चाहिए, कोई दो नहीं।',
          'नौकरी जाना तीनों पूरी करता है। अचानक चिकित्सा ख़र्च भी। गाड़ी का ऐसा नुक़सान जिसके बिना काम पर जाना संभव न हो, भी।',
          'त्योहार तीनों में से एक भी पूरा नहीं करता — वह हर साल उसी तारीख़ पर आता है, इसलिए वह अप्रत्याशित नहीं। उसके लिए सिंकिंग फ़ंड है, आपातकालीन कोष नहीं।',
          'इसी तरह कोई अच्छा निवेश अवसर, या भारी छूट वाली कोई ख़रीद, आपात नहीं है — चाहे वह कितनी भी आकर्षक हो और कितनी भी तात्कालिक लगे।',
          'और जब कोष इस्तेमाल हो ही जाए, तो अगला काम स्पष्ट है: उसे दोबारा भरना, उसी अनुशासन से जिससे वह पहली बार बना था। उपयोग करना विफलता नहीं है; उसे दोबारा न भरना विफलता है।',
        ],
        pEn: [
          'The hardest part of an emergency fund is not building it but not touching it. That needs a clear definition, written down in advance.',
          'A good rule is this: an emergency is something unexpected, necessary and urgent. All three conditions have to be met together, not any two.',
          'Losing a job meets all three. A sudden medical expense does too. Damage to the vehicle you cannot get to work without does as well.',
          'A festival meets none of them — it arrives on the same date every year, so it is not unexpected. A sinking fund exists for that, not the emergency fund.',
          'Equally, a good investment opportunity or a heavily discounted purchase is not an emergency, however attractive and however urgent it appears.',
          'And when the fund does get used, the next task is plain: refill it, with the same discipline that built it the first time. Using it is not a failure; not refilling it is.',
        ],
      },
      {
        hHi: 'कहाँ नहीं रखना चाहिए',
        hEn: 'Where not to keep it',
        pHi: [
          'कुछ जगहें ऐसी हैं जो आकर्षक लगती हैं और इस काम के लिए उपयुक्त नहीं हैं, और इनका उल्लेख उतना ही ज़रूरी है जितना सही जगहों का।',
          'ऐसी जगहें जहाँ राशि रोज़ बदलती है, इस काम के लिए नहीं हैं — भले ही उनका लंबी अवधि का इतिहास अच्छा हो। आपात कभी पूछकर नहीं आता कि आज बाज़ार कैसा है।',
          'लंबी अवधि की लॉक-इन वाली योजनाएँ भी उपयुक्त नहीं। वहाँ पैसा सुरक्षित हो सकता है, पर वह उस दिन नहीं मिलेगा जिस दिन चाहिए, और यही पूरी बात है।',
          'क्रेडिट कार्ड की सीमा को आपातकालीन कोष मान लेना सबसे आम और सबसे महँगी ग़लती है। वह पैसा आपका नहीं है, वह उधार है, और आपात की स्थिति में उधार लेना ठीक वही चीज़ है जिससे बचना था।',
          'सोना एक मध्यवर्ती मामला है। वह मूल्यवान है और अनौपचारिक रूप से पहुँच में भी, पर उसकी क़ीमत बदलती रहती है और ज़रूरत के समय उसे बेचना या गिरवी रखना जल्दबाज़ी में होता है, जो अच्छी क़ीमत नहीं देता।',
          'और अंत में, किसी और के खाते में रखा गया पैसा — चाहे वह कितना ही भरोसेमंद व्यक्ति हो — आपातकालीन कोष नहीं है, क्योंकि उस तक पहुँच आपकी अपनी नहीं रहती।',
        ],
        pEn: [
          'Some places look attractive and are unsuitable for this job, and naming them matters as much as naming the right ones.',
          'Anywhere the amount changes daily is not for this purpose — however good its long-term record. An emergency never asks first how the market is doing today.',
          'Schemes with a long lock-in are unsuitable too. The money may be safe there, but it will not arrive on the day it is needed, and that is the whole point.',
          'Treating a credit-card limit as an emergency fund is the commonest and costliest mistake. That money is not yours, it is borrowing, and borrowing in an emergency is exactly what was to be avoided.',
          'Gold is an intermediate case. It is valuable and informally accessible, but its price moves, and selling or pledging it in a hurry is what happens at the moment of need — which does not fetch a good price.',
          'And finally, money kept in somebody else’s account — however trustworthy the person — is not an emergency fund, because access to it is no longer your own.',
        ],
      },
    ],
  },
  {
    slug: 'advance-tax-basics',
    sections: [
      {
        hHi: 'यह किन पर लागू होता है',
        hEn: 'Who this applies to',
        pHi: [
          'अग्रिम कर का सामान्य सिद्धांत यह है कि कर उसी वर्ष चुकाया जाए जिस वर्ष आय हुई, न कि वर्ष बीत जाने के बाद एक साथ। इसे "जैसे कमाओ, वैसे चुकाओ" कहा जाता है।',
          'वेतनभोगी व्यक्ति के लिए यह काम आमतौर पर अपने-आप हो जाता है, क्योंकि नियोक्ता हर महीने वेतन से कर काटता है। इसलिए बहुत से लोगों को कभी अग्रिम कर की चिंता नहीं करनी पड़ती।',
          'पर वही वेतनभोगी व्यक्ति जब वेतन के अलावा कोई और आय कमाता है — ब्याज, किराया, लाभांश, प्रतिभूतियों की बिक्री, या कोई अतिरिक्त काम — तो उस आय पर कर अपने-आप नहीं कटता।',
          'यहीं अधिकांश लोग चूक जाते हैं। उन्हें लगता है कि नियोक्ता ने कर काट दिया इसलिए मामला पूरा है, जबकि अन्य आय पर देय कर उनकी अपनी ज़िम्मेदारी बना रहता है।',
          'स्वरोज़गार करने वालों, पेशेवरों, दुकानदारों और स्वतंत्र रूप से काम करने वालों के लिए तो यह पूरी व्यवस्था ही अग्रिम कर पर चलती है, क्योंकि वहाँ कोई नियोक्ता काटने वाला नहीं है।',
          'नियमों में कुछ श्रेणियों के लिए छूट भी है — जैसे कुछ शर्तों के साथ वरिष्ठ नागरिक। अपनी स्थिति पर क्या लागू होता है, यह एक बार किसी योग्य कर सलाहकार से जाँच लेना उचित है।',
        ],
        pEn: [
          'The general principle of advance tax is that tax should be paid in the same year the income arises, rather than all at once after the year has ended. It is described as "pay as you earn".',
          'For a salaried person this usually happens automatically, because the employer deducts tax from salary every month. So a great many people never have to think about advance tax at all.',
          'But when that same salaried person earns something besides salary — interest, rent, dividends, the sale of securities, or additional work — no tax is deducted from that income automatically.',
          'This is where most people slip. They feel the matter is closed because the employer deducted tax, while the tax due on other income remains their own responsibility.',
          'For the self-employed, professionals, shopkeepers and freelancers the whole arrangement runs on advance tax, because there is no employer to deduct anything.',
          'The rules also provide relief for certain categories — senior citizens under certain conditions, for instance. What applies to your own position is worth checking once with a qualified tax adviser.',
        ],
      },
      {
        hHi: 'चार तारीख़ों वाला ढाँचा',
        hEn: 'The structure of four dates',
        pHi: [
          'अग्रिम कर एक साथ नहीं, वर्ष भर में कुछ किस्तों में चुकाया जाता है। हर किस्त पर अनुमानित वार्षिक कर का एक निर्धारित प्रतिशत तक जमा हो जाना चाहिए।',
          'यह ढाँचा संचयी है, यानी हर किस्त पर यह देखा जाता है कि उस तारीख़ तक कुल कितना जमा हुआ, न कि उस किस्त में अकेले कितना दिया।',
          'इसका एक व्यावहारिक लाभ है: अगर किसी किस्त में कम जमा हुआ, तो अगली किस्त में अतिरिक्त डालकर उसे काफ़ी हद तक ठीक किया जा सकता है, हालाँकि बीच की अवधि का ब्याज लग जाता है।',
          'गणना का आधार अनुमान है, और अनुमान ग़लत होना सामान्य है। इसलिए हर किस्त पर पूरे वर्ष का अनुमान दोबारा बनाना चाहिए, न कि साल की शुरुआत वाला अनुमान दोहराते रहना चाहिए।',
          'अंतिम किस्त के बाद भी, वर्ष समाप्त होने से पहले एक बार पूरा हिसाब देख लेना उपयोगी है। साल के आख़िरी हफ़्तों में हुई कोई बड़ी आय पूरे अनुमान को बदल देती है।',
          'भुगतान ऑनलाइन किया जा सकता है और उसकी रसीद तुरंत मिल जाती है। उस रसीद को सँभालकर रखिए और यह जाँचिए कि वह राशि आपके फ़ॉर्म 26AS में दिख रही है।',
        ],
        pEn: [
          'Advance tax is not paid in one go but in a few instalments across the year. By each instalment date a prescribed percentage of the estimated annual tax should have been deposited.',
          'The structure is cumulative — at each date what matters is how much has been deposited in total by then, not how much was paid in that instalment alone.',
          'That carries a practical benefit: if one instalment fell short, putting extra into the next largely corrects it, though interest applies for the intervening period.',
          'The calculation rests on an estimate, and estimates being wrong is normal. So the whole year should be re-estimated at every instalment rather than repeating the estimate made at the start of the year.',
          'Even after the last instalment, it is useful to review the full position before the year ends. A large income arriving in the final weeks changes the entire estimate.',
          'Payment can be made online and a receipt is issued immediately. Keep that receipt and check that the amount appears in your Form 26AS.',
        ],
      },
      {
        hHi: 'चूकने पर क्या होता है',
        hEn: 'What happens if you miss it',
        pHi: [
          'अग्रिम कर न चुकाने या कम चुकाने पर जुर्माना नहीं, ब्याज लगता है। यह अंतर महत्वपूर्ण है, क्योंकि ब्याज एक गणना है, दंड नहीं, और वह समय के साथ बढ़ता है।',
          'ब्याज दो तरह से लग सकता है: एक, किस्तों में कम जमा करने पर; और दूसरा, वर्ष समाप्त होने के बाद बचे हुए कर पर, भुगतान की तारीख़ तक।',
          'यही कारण है कि देर से चुकाने पर भी जल्दी चुकाना बेहतर है। ब्याज महीनों के हिसाब से गिना जाता है, इसलिए हर बीता महीना उसे बढ़ाता है।',
          'यहाँ एक आम स्थिति है: कोई बड़ी बिक्री या एकमुश्त आय साल के बीच में आ जाती है, और व्यक्ति यह मानकर चलता है कि रिटर्न भरते समय कर चुका देगा। तब तक कई महीनों का ब्याज जुड़ चुका होता है।',
          'दूसरी आम स्थिति: वेतन से कर तो कट रहा है, पर बचत खाते और जमा के ब्याज पर नहीं। साल के अंत में वह छोटी-छोटी राशि मिलकर एक ऐसा अंतर बना देती है जिस पर ब्याज लगता है।',
          'इससे बचने का सबसे सरल तरीक़ा यह है कि नियोक्ता को अन्य आय की घोषणा कर दी जाए, जहाँ यह संभव हो। तब कर वेतन से ही कट जाता है और अलग से कुछ करना नहीं पड़ता।',
        ],
        pEn: [
          'Not paying advance tax, or paying too little, attracts interest rather than a penalty. The distinction matters, because interest is a calculation rather than a punishment, and it grows with time.',
          'Interest can arise in two ways: on paying less than required in the instalments, and on the tax still outstanding after the year ends, up to the date it is paid.',
          'This is why paying late is still better than paying later. Interest is counted by months, so every month that passes adds to it.',
          'One common situation: a large sale or a lump-sum income arrives mid-year and the person assumes the tax can be settled while filing the return. By then several months of interest have accumulated.',
          'Another common situation: tax is being deducted from salary but not on interest from savings accounts and deposits. At year end those small amounts add up into a gap that carries interest.',
          'The simplest way to avoid this is to declare other income to the employer, where that is possible. The tax is then deducted from salary and nothing separate has to be done.',
        ],
      },
      {
        hHi: 'अनुमान लगाने का व्यावहारिक तरीक़ा',
        hEn: 'A practical way to estimate',
        pHi: [
          'अनुमान लगाना कठिन लगता है और असल में एक पन्ने का काम है, ख़ासकर अगर उसे हर तिमाही दोहराया जाए।',
          'ऊपर सारी अपेक्षित आय की सूची बनाइए: वेतन, ब्याज, किराया, लाभांश, कोई पेशेवर आय, और परिसंपत्तियों की बिक्री से होने वाला लाभ।',
          'फिर लागू कटौतियाँ घटाइए — वही जो आप वास्तव में लेने वाले हैं, वे नहीं जो लेने का इरादा है। इरादे पर बनाया गया अनुमान लगभग हमेशा कम कर दिखाता है।',
          'बची हुई राशि पर अपनी व्यवस्था के अनुसार कर निकालिए, फिर उसमें से वह कर घटाइए जो पहले ही स्रोत पर कट चुका है या कटने वाला है। जो बचता है, वही अग्रिम कर के रूप में देना है।',
          'इस पन्ने को हर तिमाही दोबारा भरिए, पुराना मिटाकर नहीं बल्कि उसके नीचे। तीन-चार बार करने के बाद यह पाँच मिनट का काम रह जाता है, और अनुमान काफ़ी सटीक होने लगते हैं।',
          'जिनकी आय जटिल है — व्यवसाय, कई स्रोत, बड़ी पूँजीगत बिक्री — उनके लिए यह हिसाब एक बार किसी योग्य कर सलाहकार के साथ बैठकर बनाना बेहतर है, और उसके बाद वही ढाँचा दोहराया जा सकता है।',
        ],
        pEn: [
          'Estimating sounds difficult and is in fact one page of work, particularly if it is repeated each quarter.',
          'List all the expected income at the top: salary, interest, rent, dividends, any professional income, and gains from selling assets.',
          'Then subtract the deductions that apply — the ones you will actually take, not the ones you intend to. An estimate built on intentions almost always shows too little tax.',
          'Work out the tax on what remains under your chosen regime, then subtract the tax already deducted at source or due to be deducted. What is left is what has to go as advance tax.',
          'Fill this page in afresh each quarter, below the old one rather than over it. After three or four rounds it becomes a five-minute task and the estimates get considerably closer.',
          'For anyone whose income is complicated — a business, several sources, a large capital sale — it is better to build this working once with a qualified tax adviser and then repeat the same structure.',
        ],
      },
      {
        hHi: 'एक साल का सरल कैलेंडर',
        hEn: 'A simple calendar for the year',
        pHi: [
          'अग्रिम कर को याद रखने का सबसे अच्छा तरीक़ा उसे कैलेंडर में डाल देना है, ताकि वह हर बार एक नई खोज न बने।',
          'हर किस्त की तारीख़ से एक हफ़्ता पहले का अनुस्मारक लगाइए, तारीख़ का नहीं। एक हफ़्ता इसलिए, क्योंकि अनुमान बनाने और भुगतान करने में समय लगता है।',
          'उसी दिन तीन काम कीजिए: अनुमान दोबारा बनाइए, आवश्यक राशि जमा कीजिए, और रसीद उसी वित्तीय वर्ष के फ़ोल्डर में रख दीजिए।',
          'साल में एक बार, आमतौर पर रिटर्न भरने के समय, यह भी मिला लीजिए कि जो अग्रिम कर आपने जमा किया वह फ़ॉर्म 26AS में सही दिख रहा है। कभी-कभी भुगतान ग़लत मूल्यांकन वर्ष में दर्ज हो जाता है।',
          'अगर ऐसा हुआ हो, तो उसे सुधारा जा सकता है, पर उसके लिए भुगतान की रसीद चाहिए — यही वह जगह है जहाँ फ़ोल्डर वाली आदत काम आती है।',
          'और अंत में एक बात जो पूरे विषय पर लागू होती है: नियम, दरें और सीमाएँ समय-समय पर बदलती हैं। हर साल की शुरुआत में यह देख लेना कि इस वर्ष क्या लागू है, पिछले वर्ष का ढाँचा दोहराने से कहीं बेहतर है।',
        ],
        pEn: [
          'The best way to remember advance tax is to put it in the calendar, so that it is not a fresh discovery every time.',
          'Set the reminder for a week before each instalment date, not for the date itself. A week, because building the estimate and making the payment take time.',
          'Do three things on that day: rebuild the estimate, deposit what is required, and put the receipt into that financial year’s folder.',
          'Once a year, usually while filing the return, also check that the advance tax you paid appears correctly in Form 26AS. A payment sometimes gets recorded against the wrong assessment year.',
          'If that has happened it can be corrected, but the payment receipt is needed for it — which is exactly where the folder habit pays off.',
          'And finally, a point that applies to the whole subject: rules, rates and thresholds change from time to time. Checking at the start of each year what applies this year is far better than repeating last year’s structure.',
        ],
      },
    ],
  },
  {
    slug: 'why-not-endowment-plans',
    sections: [
      {
        hHi: 'एक प्रीमियम, तीन हिस्से',
        hEn: 'One premium, three parts',
        pHi: [
          'इन योजनाओं को समझने का सबसे साफ़ तरीक़ा यह देखना है कि आपका प्रीमियम असल में किन हिस्सों में बँटता है, क्योंकि वह पूरा का पूरा निवेश नहीं होता।',
          'पहला हिस्सा मृत्यु कवर की लागत है — यानी वह राशि जो बीमा जोखिम के लिए ली जाती है। यह उचित है और हर बीमा में होती है।',
          'दूसरा हिस्सा वितरण और प्रशासन की लागत है — कमीशन, दस्तावेज़ीकरण, और चलाने का ख़र्च। पहले कुछ वर्षों में यह हिस्सा सबसे बड़ा होता है।',
          'तीसरा हिस्सा वह है जो वास्तव में निवेश किया जाता है। यही वह हिस्सा है जिस पर आपका अंतिम भुगतान टिका है, और यह प्रीमियम से काफ़ी छोटा होता है।',
          'यही कारण है कि इन योजनाओं का प्रभावी रिटर्न आमतौर पर उससे कम बैठता है जितना विवरणिका में दिखाई देने वाली कुल राशि से लगता है। कुल राशि बड़ी दिखती है क्योंकि अवधि लंबी है।',
          'तुलना करने का सही तरीक़ा कुल भुगतान देखना नहीं, बल्कि यह पूछना है कि इतने वर्षों में इतना प्रीमियम देकर इतना पाने का वार्षिक प्रतिफल क्या बैठता है। यह संख्या माँगिए, और लिखित माँगिए।',
        ],
        pEn: [
          'The clearest way to understand these plans is to see how your premium actually divides, because not all of it is invested.',
          'The first part is the cost of the death cover — the amount charged for the insurance risk. That is fair and exists in every insurance policy.',
          'The second part is distribution and administration — commission, documentation and the cost of running the policy. In the first few years this part is the largest.',
          'The third part is what is actually invested. That is what your final payout rests on, and it is considerably smaller than the premium.',
          'This is why the effective return on these plans usually works out lower than the total figure in the brochure suggests. The total looks large because the term is long.',
          'The right way to compare is not to look at the total payout but to ask what annual return it represents, given that premium over those years. Ask for that number, and ask for it in writing.',
        ],
      },
      {
        hHi: 'कवर की राशि, जो सबसे कम बात की जाती है',
        hEn: 'The cover amount, which is discussed least',
        pHi: [
          'इन योजनाओं की सबसे बड़ी कमी रिटर्न नहीं है। सबसे बड़ी कमी यह है कि उनमें मिलने वाला जीवन कवर लगभग हमेशा ज़रूरत से बहुत कम होता है।',
          'कारण सीधा है: कवर जितना बड़ा होगा, प्रीमियम उतना ही बड़ा होगा। चूँकि यहाँ प्रीमियम पहले से ऊँचा है, इसलिए अधिकांश लोग एक ऐसी कवर राशि पर रुक जाते हैं जो वे वहन कर सकें।',
          'नतीजा यह होता है कि परिवार को उस स्थिति में, जिसके लिए बीमा लिया ही गया था, बहुत कम राशि मिलती है — जबकि प्रीमियम कई गुना ज़्यादा चुकाया गया।',
          'सादे सुरक्षा-बीमा में यह समस्या नहीं आती, क्योंकि वहाँ पूरा प्रीमियम केवल जोखिम की क़ीमत होता है। इसीलिए उसी बजट में कई गुना बड़ा कवर मिल जाता है।',
          'यहाँ अक्सर यह तर्क दिया जाता है कि सुरक्षा-बीमा में "कुछ वापस नहीं मिलता"। यह सच है, और यह उसकी कमी नहीं बल्कि उसकी परिभाषा है — ठीक वैसे ही जैसे गाड़ी के बीमे में।',
          'सही तुलना यह है: उतना ही कवर सादे बीमे से लीजिए, और जो प्रीमियम बचा उसे अलग से निवेश कीजिए। फिर दोनों रास्तों के अंतिम परिणाम की तुलना कीजिए। यह गणना करने लायक़ है।',
        ],
        pEn: [
          'The biggest shortcoming of these plans is not the return. It is that the life cover inside them is almost always far below what is needed.',
          'The reason is straightforward: the larger the cover, the larger the premium. Since the premium is already high here, most people settle at a cover amount they can afford.',
          'The result is that in exactly the situation the insurance was taken for, the family receives a small amount — while several times as much premium was paid.',
          'Plain protection cover does not have this problem, because there the entire premium is the price of the risk alone. That is why the same budget buys many times the cover.',
          'The argument is often made that with protection cover "you get nothing back". That is true, and it is not a shortcoming but its definition — exactly as with motor insurance.',
          'The correct comparison is this: take the same cover through plain insurance, and invest the premium saved separately. Then compare where the two routes end up. That calculation is worth doing.',
        ],
      },
      {
        hHi: 'लंबी प्रतिबद्धता और बीच में छोड़ने की क़ीमत',
        hEn: 'The long commitment and the cost of stopping',
        pHi: [
          'इन योजनाओं की अवधि आमतौर पर पंद्रह से पच्चीस वर्ष होती है, और हर वर्ष प्रीमियम देना पड़ता है। यह प्रतिबद्धता उस समय छोटी लगती है जब आय स्थिर होती है।',
          'पर बीस साल एक लंबा समय है। उसमें नौकरी बदलती है, शहर बदलता है, ज़िम्मेदारियाँ बदलती हैं, और आय हमेशा एक दिशा में नहीं जाती।',
          'यही कारण है कि इन योजनाओं का एक बड़ा हिस्सा पूरा नहीं होता। शुरुआती वर्षों में छोड़ने पर आमतौर पर बहुत कम या कुछ भी वापस नहीं मिलता।',
          'यह शर्त दस्तावेज़ में लिखी होती है और बिक्री के समय शायद ही कभी उतनी स्पष्टता से बताई जाती है जितनी अंतिम राशि बताई जाती है।',
          'इसलिए ख़रीदने से पहले एक सीधा सवाल पूछिए: "अगर मैं तीन साल बाद बंद कर दूँ तो मुझे कितना मिलेगा? और सात साल बाद?" ये दोनों संख्याएँ लिखित में माँगिए।',
          'अगर उत्तर में हिचक हो, तो वही हिचक अपने-आप में एक उत्तर है। कोई भी योजना जिसकी शर्तें सीधे शब्दों में न बताई जा सकें, बीस साल की प्रतिबद्धता के लायक़ नहीं है।',
        ],
        pEn: [
          'The term of these plans is usually fifteen to twenty-five years, with a premium due every year. That commitment looks small at a moment when income is steady.',
          'But twenty years is a long time. Jobs change, cities change, responsibilities change, and income does not always move in one direction.',
          'This is why a large share of these plans is never completed. Stopping in the early years typically returns very little, or nothing at all.',
          'That condition is written in the document and is rarely explained at the point of sale with the clarity given to the final amount.',
          'So ask one direct question before buying: "If I stop after three years, how much do I get? And after seven?" Ask for both numbers in writing.',
          'If the answer comes with hesitation, that hesitation is itself an answer. Any plan whose terms cannot be stated in plain words is not worth a twenty-year commitment.',
        ],
      },
      {
        hHi: 'जहाँ ये योजनाएँ उपयुक्त हो सकती हैं',
        hEn: 'Where these plans can be suitable',
        pHi: [
          'ईमानदारी की माँग है कि दूसरा पक्ष भी रखा जाए। ये योजनाएँ हर किसी के लिए ग़लत नहीं हैं, और कुछ परिस्थितियों में उनका स्थान है।',
          'पहली स्थिति: वह व्यक्ति जो किसी भी हाल में निवेश नहीं करेगा। जिसके लिए विकल्प "यह योजना" और "कुछ नहीं" के बीच है, उसके लिए यह योजना बेहतर है। अनुशासन का अपना मूल्य है।',
          'दूसरी: वह व्यक्ति जो बाज़ार के उतार-चढ़ाव को बिल्कुल सहन नहीं कर सकता और गिरावट देखकर निवेश बंद कर देगा। उसके लिए स्थिरता का मूल्य कुछ प्रतिशत के रिटर्न से बड़ा हो सकता है।',
          'तीसरी: वह जिसके पास पहले से पर्याप्त सुरक्षा-बीमा, पर्याप्त स्वास्थ्य बीमा और पर्याप्त आपातकालीन कोष है, और जो अपने कुल पोर्टफ़ोलियो के एक छोटे हिस्से को एक निश्चित, अनुमानित रास्ते पर रखना चाहता है।',
          'ध्यान दीजिए कि तीनों स्थितियों में एक बात समान है: यह योजना बुनियादी सुरक्षा की जगह नहीं ले रही, बल्कि उसके बाद आ रही है। यही क्रम पूरे लेख का सार है।',
          'और जो व्यक्ति यह योजना ले रहा है, उसे पूरी जानकारी के साथ लेनी चाहिए — कवर की राशि, प्रभावी वार्षिक प्रतिफल, प्रतिबद्धता की अवधि, और बीच में छोड़ने पर मिलने वाली राशि, चारों लिखित में।',
        ],
        pEn: [
          'Honesty requires the other side to be stated too. These plans are not wrong for everybody, and in some circumstances they have a place.',
          'The first case: somebody who will not invest under any circumstances. Where the real choice is between "this plan" and "nothing", this plan is better. Discipline has a value of its own.',
          'The second: somebody who cannot tolerate market movement at all and would stop investing at the first fall. For them the value of steadiness can exceed a few per cent of return.',
          'The third: somebody who already has adequate protection cover, adequate health cover and an adequate emergency fund, and who wants a small part of the overall portfolio on a fixed, predictable path.',
          'Notice that all three cases share one thing: the plan is not replacing basic protection, it is coming after it. That order is the point of this whole article.',
          'And anyone taking such a plan should take it fully informed — the cover amount, the effective annual return, the length of the commitment, and the amount payable on stopping midway, all four in writing.',
        ],
      },
      {
        hHi: 'पहले से चल रही योजना के विकल्प',
        hEn: 'Options for a plan already running',
        pHi: [
          'यह लेख पढ़ने के बाद सबसे स्वाभाविक सवाल यही है, और इसका उत्तर हर किसी के लिए एक जैसा नहीं है। यह इस बात पर निर्भर करता है कि योजना कितनी चल चुकी है।',
          'अगर एक-दो साल ही हुए हैं, तो जमा राशि आमतौर पर बहुत कम वापस मिलती है। यहाँ नुक़सान पहले ही हो चुका है, और आगे की प्रतिबद्धता से बचना अक्सर बेहतर होता है।',
          'अगर योजना आधी से ज़्यादा पूरी हो चुकी है, तो उसे चलाकर पूरा कर लेना अक्सर बेहतर होता है, क्योंकि तब सबसे महँगे शुरुआती वर्ष बीत चुके होते हैं।',
          'बीच की स्थिति में तीसरा विकल्प देखिए: योजना को "पेड-अप" करना। आगे प्रीमियम नहीं देना पड़ता, कवर घट जाता है, पर जो जमा है वह ख़त्म नहीं होता और परिपक्वता पर एक घटी हुई राशि मिलती है।',
          'जो भी रास्ता चुनें, क्रम वही रहेगा: पहले पर्याप्त सुरक्षा-बीमा और स्वास्थ्य बीमा चालू कीजिए, उनकी पुष्टि हो जाने के बाद ही पुरानी योजना पर कोई क़दम उठाइए।',
          'और यह फ़ैसला संख्याओं पर लीजिए, पछतावे पर नहीं। जो प्रीमियम पहले चुकाया जा चुका है वह किसी भी फ़ैसले से वापस नहीं आता; सवाल केवल यह है कि आज से आगे कौन सा रास्ता बेहतर है।',
        ],
        pEn: [
          'This is the most natural question after reading such an article, and the answer is not the same for everyone. It depends on how far the plan has run.',
          'If only a year or two has passed, very little of what was paid comes back. The loss has already occurred there, and avoiding further commitment is often the better course.',
          'If the plan is more than half complete, running it to the end is often better, because the most expensive early years are already behind.',
          'In the middle case, look at a third option: making the plan "paid-up". No further premium is due, the cover falls, but what has accumulated is not lost and a reduced amount is paid at maturity.',
          'Whichever route is chosen, the order stays the same: put adequate protection cover and health cover in place first, and only after confirming they are active take any step on the old plan.',
          'And take the decision on the numbers rather than on regret. The premium already paid does not return under any decision; the only question is which route is better from today onwards.',
        ],
      },
    ],
  },
];

export function applyExpansions(all: Article[]): void {
  const bySlug = new Map(all.map((a) => [a.slug, a]));
  for (const exp of EXPANSIONS) {
    const article = bySlug.get(exp.slug);
    if (!article) continue;
    article.sections.push(...exp.sections);
  }
}
